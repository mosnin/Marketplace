...
export default async function AgencyInvitesPage({
  ...
  try {
    ...
    console.error('[settings/agency-setup] Failed to fetch user', err);
    return (
      <div className="flex min-h-[40vh] items-center justify-center">
        <div className="text-center space-y-3 p-8">
          ...
          <a href={`/s/${slug}/settings/agency-setup`} className={PRIMARY_PILL}>
            Try again
          </a>
        </div>
      </div>
  ...
  try {
    const { data, error } = await supabase
      .from('Invitation')
      .select('id, email, roleToAssign, token, status, expiresAt, createdAt, Agency(id, name)')
    ...
    console.error('[settings/agency-setup] Failed to fetch invitations', err);
  }

  const roleLabel = (role: string) => (role === 'agency_admin' ? 'Admin' : 'Member');
  ...
  const narration =
    invitations.length === 0
      ...
      : invitations.length === 1
        ? '1 agency invite waiting.'
        : `${invitations.length} agency invites waiting.`;
  ...
  return (
    <div className={`${SECTION_RHYTHM} ${READING_MAX} pb-12`}>
      ...
      {invitations.length === 0 ? (
        ...
        <div>
          {invitations.map((inv) => {
            const agencyName = Array.isArray(inv.Agency)
              ? (inv.Agency as Array<{ name?: string }>)[0]?.name
              : inv.Agency?.name;
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className={`${BODY} font-medium truncate`}>{agencyName ?? 'Unknown agency'}</p>
