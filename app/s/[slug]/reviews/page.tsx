...
// app/agency/reviews/page.tsx). Types mirror the GET list API response.
export default async function ProviderReviewsPage({ params }: PageProps) {
  const { slug } = await params;

  const { userId: clerkId } = await auth();
  if (!clerkId) redirect('/login/provider');
  ...
  // Non-agency space: there are no reviews possible. Render an empty list
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto pb-12">
        <header className="space-y-1.5">
          ...
          <p className="text-sm text-muted-foreground">No agency connected yet.</p>
        </header>
  ...
  // 2. Parallel joins: deals + comment counts. Mirror the agency page shape.
