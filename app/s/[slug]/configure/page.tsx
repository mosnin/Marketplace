...
export default async function ConfigurePage({
  ...
  const initialData = {
    ...
    providerPhotoUrl: (dbUser?.space?.settings as any)?.providerPhotoUrl ?? '',
  ...
  // Check agency status for the agency section
  let existingAgencyName: string | null = null;
  try {
    const agencyCtx = await getAgencyContext();
    existingAgencyName = agencyCtx?.agency.name ?? null;
  } catch {
    // non-blocking
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      ...
      {/* Agency section — links to dedicated page */}
      <div>
        <div className="mb-3">
          <p className="text-sm font-semibold">Agency</p>
          <p className="text-xs text-muted-foreground mt-0.5">
            {existingAgencyName
              ? 'Manage your agency or view the agency dashboard.'
              : 'Create your own agency or join one with an invite code.'}
          </p>
        </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold truncate">{existingAgencyName}</p>
