...
/**
 * /s/[slug]/calendar — the provider's calendar, here.
 *
 * Koala doesn't own a calendar. The provider already lives in Google
 ...
 * localStorage so the provider's preferred lens sticks.
 *
...
/* Chip enter (optimistic, just-created by the provider): scale 0.95 → 1 +
 * fade. The slight pop tells the provider "your event landed" without
...
interface CalendarEventOut {
  ...
  /**
   * Client-only marker — set true when the provider just created this event
...
function viewStorageKey(slug: string) {
  return `koala:calendar:view:${slug}`;
}
...
export function CalendarView({
  ...
  useEffect(() => {
    try {
      ...
      // provider explicitly taps Month/Week later, that intent sticks.
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      >
        <header className="space-y-1.5">
          ...
          <p className={BODY_MUTED}>
            {connected
              ...
              : 'Connect your calendar so I can see your day and put appointments on it.'}
          </p>
        </header>
        ...
        {connected && !loading && !errorMessage && (
          // Cross-fade the body when the provider flips Month/Week/Day/Agenda,
      ...
      <div className="flex items-center gap-2 sm:gap-3 sm:ml-auto flex-wrap">
        ...
        {showNav && (
          <div className="flex items-center gap-1 min-w-0">
             ...
             * provider feels the calendar step instead of just seeing it. */}
            <motion.button
              type="button"
              onClick={onPrev}
              whileTap={{ rotate: -10 }}
            ...
            {days.map((day, i) => {
              ...
              return (
                ...
                >
                   ...
                   * provider lands on the month and today already has a
...
function ChipInCell({
  ...
  /** `short` = "9a Appointment" (Month — time prefix matters). `title` = "Appointment"
 ...
 * same fields a provider would want to search. Renders as an Agenda-style
      ...
      <CardContent className="p-6 space-y-5">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1.5">
            <p className={BODY}>
              Connect your calendar and I&apos;ll put appointments on it, watch for
...
function AddEventModal({
  ...
  // Mobile: slide up from below (220ms). The provider's thumb just tapped
  ...
  // same custom service, so cascade order isn't reliable.
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Appointment @ 456 Oak"
              autoFocus
              required
            />
          </div>
