...
export default async function KoalaPage({
  ...
  // `prefill` populates the composer but does NOT auto-send — the provider
  // finishes the sentence themselves. Used by "or just tell Koala →" shortcuts
  ...
  try {
    const { data: convData } = await supabase
      ...
      .select('*')
      .eq('spaceId', space.id)
      // Provider surface NEVER shows agency-Koala or team chat. These live in
    ...
    if (targetConvId) {
      // Verify the requested conversation belongs to THIS provider space and is
      // a provider conversation BEFORE loading any messages. Without this guard
      // an arbitrary conversationId in the URL (a agency conversation, or
      ...
      const { data: convRow } = await supabase
        ...
        .eq('id', targetConvId)
        .maybeSingle();
      const isOwnProviderConversation = isProviderConversation(
        convRow as { spaceId: string; title: string } | null,
        space.id,
      );

      if (isOwnProviderConversation) {
      ...
      // Foreign / agency / unknown conversation id → fall through to the
  ...
  // morning when the provider has zero active integrations AND Composio is
