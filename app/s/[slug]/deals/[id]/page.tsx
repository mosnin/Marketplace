...
import { DealServicePicker } from '@/components/deals/deal-service-picker';
...
export default async function DealDetailPage({
  ...
  // provider moves between People and Deals all day; the chrome — and the URL
  ...
  // Without this, a logged-in provider could read another provider's deal data
  ...
  let linkedService: Service | null = null;
  ...
  try {
    ...
    const [stagesResult, dcResult, activityResult, checklistResult, docsResult, ownerResult] = await Promise.all([
      ...
      // The space's owner IS the assigned provider for everything inside the
    ...
    // Load the linked Service, if any. Kept as a small separate fetch so we
    // don't join here — Service rows can be referenced from multiple deals.
    const linkedServiceId = (dealRow.serviceId as string | null | undefined) ?? null;
    if (linkedServiceId) {
  ...
  // Is DocuSign connected for this provider? Drives whether the documents tab
  ...
  // are how providers actually talk about deals — "the Maple Ave place", not
  ...
  return (
    <div className={cn(PAGE_MAX, 'mx-auto space-y-8 pb-12')}>
      ...
      <header className="space-y-5">
        <div className="space-y-2">
          ...
          {/* When the address IS the headline, show the deal title beneath as
              a secondary identifier. The provider named the deal for a reason
      ...
      {/* Sidebar + main grid. Card wraps both because the provider reads the
          page as one object, not two columns. */}
      <div className="rounded-xl border border-border/70 bg-card overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-[300px_minmax(0,1fr)]">

          {/* LEFT SIDEBAR — hero (assigned provider + linked contacts) on top,
          ...
          <aside className="border-b lg:border-b-0 lg:border-r border-border/60 p-5 space-y-5">
            ...
            {/* Hero — the assigned provider on top, contacts beneath. Avatars
            ...
            <div className="space-y-4">
              ...
              <DealServicePicker dealId={id} slug={slug} initial={linkedService} />
            </div>
          </aside>

          {/* RIGHT MAIN — URL-driven tabs that mirror the People detail
              page. Same `?tab=` contract, same border-b-2 active underline,
              same default (`activity`). The provider switches between People
            ...
            {activeTab === 'activity' && (
              <div className="space-y-5">
                {/* Deal metadata — title / address / notes used to live in
                    ...
                    not "activity", but the provider wants them visible
...
function buildStatusSentence({
  ...
  // Lost — short, with last-activity recency so the provider remembers when
