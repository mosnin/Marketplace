...
export default async function ClientDetailPage({
  ...
  // Skipping this lets any logged-in provider see any other provider's contacts.
  ...
  let contact: (Contact & { dealContacts: { deal: { id: string; title: string; address: string | null; value: number | null; status: string; priority: string; stage: { name: string; color: string } } }[]; appointments: { id: string; startsAt: string; endsAt: string; status: string; serviceAddress: string | null }[] }) | null = null;
  ...
  try {
    ...
    if (contactError && contactError.code === 'PGRST116') {
      ...
      const { data: appointmentRows } = await supabase.from('Appointment').select('id, guestName, startsAt, endsAt, status, serviceAddress').eq('contactId', id).eq('spaceId', space.id).order('startsAt', { ascending: false }).limit(10);
  ...
  // E-signature — let the provider send a document from this person's linked
  ...
  return (
    <ContactDetailFrame className="max-w-4xl mx-auto space-y-8 pb-12">
      {/* Headline — name is the page. The next four lines tell the provider
        ...
        <section className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm border-t border-border/60 pt-4">
          ...
          {contact.applicationRef && contact.statusPortalToken && (
            <CopyApplicantPortalLink
              url={`${process.env.NEXT_PUBLIC_APP_URL ?? 'https://my.usekoala.com'}/apply/${slug}/status?ref=${encodeURIComponent(contact.applicationRef)}&token=${encodeURIComponent(contact.statusPortalToken)}`}
            />
          )}
        </section>
      )}
      ...
      {/* Score — open when scored, since "why is this person hot" is often
          the provider's next question. Closed when no score yet. */}
      ...
      {/* Send a document to sign — pull a document from this person's deals,
          send it to them on the provider's connected DocuSign, watch status.
      ...
      {/* Application — open when received/null (needs review), closed once
          processed. The provider's eye goes where the work is. */}
      {app && (
          ...
          <div className="mt-4 space-y-4">
            ...
            {formSnapshot ? (
              ...
              <div className="divide-y divide-border/40">
                {(app.serviceAddress || app.unitType || app.targetMoveInDate || app.monthlyRent != null || app.leaseTermPreference || app.numberOfOccupants != null) && (
                  <CollapsibleSection title="Service" defaultOpen>
                    <DetailGrid>
                      {app.serviceAddress && <Detail label="Address" value={app.serviceAddress} />}
      ...
      {/* Pipeline — closed by default; not where the provider's eye goes. */}
      <details className="group border-t border-border/60 pt-4">
        <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-semibold text-foreground hover:text-foreground/80 transition-colors">
      ...
      {/* Follow-up + lifecycle — at the bottom because they're edits-on-this-
          record, not the provider's daily task. */}
      <details className="group border-t border-border/60 pt-4">
        <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-semibold text-foreground hover:text-foreground/80 transition-colors">
        ...
        <div className="mt-4 space-y-4">
          ...
          <FollowUpSuggestions
            ...
            hasAppointments={contact.appointments.length > 0}
...
function derivePersonState(
  ...
  // past today reads as "archived for action-pill purposes" — the provider
 ...
 * thing the provider already cared enough to write down.
 */
