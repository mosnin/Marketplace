...
interface ServiceProfilesProps {
  slug: string;
  profiles: ServiceProfile[];
  onUpdate: (profiles: ServiceProfile[]) => void;
}
...
export function ServiceProfiles({ slug, profiles: initialProfiles, onUpdate }: ServiceProfilesProps) {
  const [profiles, setProfiles] = useState<ServiceProfile[]>(initialProfiles);
  ...
  function startEdit(p: ServiceProfile) {
    setEditingId(p.id);
    setFormName(p.name);
    setFormAddress(p.address || '');
    setFormDuration(p.appointmentDuration);
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/appointments/services/${editingId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: formName.trim(),
            address: formAddress.trim() || null,
            appointmentDuration: formDuration,
            startHour: formStart,
            endHour: formEnd,
  ...
  async function toggleActive(id: string, isActive: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormError('Failed to update service status');
      }
    } catch {
      setFormError("Couldn't reach the server — usually temporary.");
    } finally {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Each service can have its own appointment hours, duration, and booking link.
          </p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Buffer Between Appointments (min)</label>
      ...
      {profiles.length === 0 && !showForm ? (
        <div className="text-center py-10 text-muted-foreground space-y-2">
          ...
          <p className="text-sm">No service profiles yet</p>
          <p className="text-xs">Add services to give each listing its own appointment schedule and booking link.</p>
        </div>
      ) : (
