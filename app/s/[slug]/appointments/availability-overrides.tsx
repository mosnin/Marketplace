...
export function AvailabilityOverrides({ slug, serviceProfiles = [] }: AvailabilityOverridesProps) {
  ...
  const loadOverrides = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/appointments/overrides?slug=${encodeURIComponent(slug)}`);
      if (res.ok) {
  ...
  async function handleSave() {
    ...
    setFormError(null);
    try {
      const res = await fetch('/api/appointments/overrides', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          serviceProfileId: formServiceId || null,
        }),
      });
      if (!res.ok) {
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/appointments/overrides/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const profileMap = new Map(serviceProfiles.map((p) => [p.id, p]));
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          {serviceProfiles.length > 0 && (
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Service (optional)</label>
              <select
                value={formServiceId}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {overrides.map((o) => {
            const profile = o.serviceProfileId ? profileMap.get(o.serviceProfileId) : null;
            return (
              <div
                key={o.id}
                className={cn(
