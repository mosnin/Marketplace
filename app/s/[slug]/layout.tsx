...
export default async function DashboardLayout({
  ...
  try {
    const [leadResult, followUpResult, draftResult, serviceResult] = await Promise.all([
      supabase
        .from('Contact')
  ...
  // Check agency context and agency memberships for sidebar
  let isAgency = false;
  let agencyName: string | null = null;
  let agencyRole: string | null = null;
  let agencyMemberships: { id: string; name: string; role: string }[] = [];
  try {
    const { data: memberships } = await supabase
      .from('AgencyMembership')
      .select('agencyId, role, Agency(id, name)')
      .eq('userId', dbUser.id);

    agencyMemberships = (memberships ?? []).map((m: any) => ({
      id: Array.isArray(m.Agency) ? m.Agency[0]?.id : m.Agency?.id,
      name: Array.isArray(m.Agency) ? m.Agency[0]?.name : m.Agency?.name,
      role: m.role,
    })).filter(m => m.id && m.name);

    if (agencyMemberships.length > 0) {
      isAgency = agencyMemberships.some(m => m.role === 'agency_owner' || m.role === 'agency_admin');
      agencyName = agencyMemberships[0].name;
      agencyRole = agencyMemberships[0].role;
    }
  } catch {
    isAgency = false;
  }
  ...
  return (
    <div className="app-theme flex h-screen overflow-hidden bg-background text-foreground">
      {/* First-paint splash — greets the provider by name (varied each open),
      ...
      />
      {/* Detects ?embed=1 from the Koala RightPanel iframe and strips
      ...
      <SidebarCollapseProvider>
        <Sidebar slug={slug} spaceName={space.name} unreadLeadCount={unreadLeadCount} pendingDraftCount={pendingDraftCount ?? 0} overdueFollowUpCount={overdueFollowUpCount} activeServiceCount={activeServiceCount} isAgency={isAgency} agencyName={agencyName} agencyRole={agencyRole} agencyMemberships={agencyMemberships} isPlatformAdmin={dbUser.isPlatformAdmin} />
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <PlatformBanner />
          <Header slug={slug} spaceId={space.id} spaceName={space.name} title={space.name} isAgency={isAgency} agencyName={agencyName} isPlatformAdmin={dbUser.isPlatformAdmin} />
          <LayoutShell slug={slug} liveNotifications={<LiveNotifications spaceId={space.id} slug={slug} />}>
            {children}
          </LayoutShell>
        </div>
      </SidebarCollapseProvider>
      <MobileNav slug={slug} isAgency={isAgency} />
      <KoalaBar slug={slug} />
