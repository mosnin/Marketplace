...
type FormConfigSource = 'custom' | 'agency' | 'legacy';
...
export default function IntakeCustomizePage() {
  ...
  // appear on the provider's keystroke (CSS-driven services) instead of
  // waiting for Save.
  ...
  // services (accent, dark mode, font) update without a full reload.
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (!target) return;
      target.postMessage(
        { type: 'koala:preview-update', customization: config },
  ...
  const handleReset = useCallback(async () => {
    ...
    if (
      !confirm(
        `Reset the ${label} form to the standard Koala default? Your custom changes will be removed.`,
      )
    )
      return;
    try {
  ...
  const isAgency = configSource === 'agency';
  ...
  return (
    ...
    // sections / service inspector) — stacking the live-preview beside
    ...
    // is the escape hatch when the provider wants a dedicated window.
