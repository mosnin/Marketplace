 ...
 *
 * Queue a post: an image, a caption, the social platforms the provider has
 * connected through Composio, and a time. Creates a StudioPost row. The
 * connected-platform list is whatever the provider connected on the
...
export function SchedulePanel({
  ...
  // post image so the provider never has to re-upload.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ignore — the provider can still upload manually
      }
    })();
    return () => {
      cancelled = true;
