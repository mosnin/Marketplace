...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select('*')
    .eq('spaceId', space.id)
    .is('agencyId', null); // Exclude agency leads — those show on /agency/leads
...
export async function POST(req: NextRequest) {
  ...
  const { slug, name, email, phone, budget, preferences, services, address, notes, type, tags } = body;
  ...
  // hand back the existing record than make the provider merge later.
  ...
  const propsVal = Array.isArray(services)
    ? services
  ...
  const VALID_TYPES = ['QUALIFICATION', 'APPOINTMENT', 'APPLICATION'] as const;
