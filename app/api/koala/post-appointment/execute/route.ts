/**
 * POST /api/koala/post-appointment/execute
 *
...
import {
  POST_APPOINTMENT_TOOL_ALLOWLIST,
  POST_APPOINTMENT_INTEGRATION_SLUG_ALLOWLIST,
  doneVerbForToolkit,
...
interface ResultOut {
  ...
  summary: string;
  error?: string;
  /** Provider-voice verb for the post-execute toast. Only set on success. */
  doneVerb?: string;
}
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await checkRateLimit(`koala:post-appointment-exec:${userId}`, 30, 60);
  ...
  // Sanitize the input. Native tools must be on the post-appointment allowlist;
  // integration tools must reference a toolkit the provider has actually
  // connected AND their slug must be on the integration-slug allowlist.
  // This route is a post-appointment orchestrator, not a generic Composio
  // passthrough — `GMAIL_TRASH_EMAIL` and friends have no business here.
  const nativeAllow = new Set(POST_APPOINTMENT_TOOL_ALLOWLIST as readonly string[]);
  const integrationSlugAllow = new Set(
    POST_APPOINTMENT_INTEGRATION_SLUG_ALLOWLIST as readonly string[],
  );
  ...
  for (const p of body.proposals) {
    ...
    if (nativeAllow.has(tool)) {
      ...
      // Integration proposal — accept only if (1) the provider still has
    ...
    try {
      ...
      if (p.integrationToolkit) {
        ...
        if (resp.successful) {
          ...
          // create through Composio. The provider's calendar is the
      ...
      logger.error('[koala/post-appointment/execute] tool threw', { tool: p.tool }, err);
      results.push({
        tool: p.tool,
        ok: false,
 ...
 * (which the provider approved) + the response (which gives us the
 * Google event id). Never throws — the provider's calendar already has
...
async function logCalendarMirrorBestEffort(args: {
  ...
  try {
    ...
    logger.warn(
      '[koala/post-appointment/execute] mirror insert failed',
      { spaceId: args.spaceId },
      err,
    );
  }
