...
import { isValidListingStatus, isValidServiceType } from '@/lib/services';
...
function sanitiseBody(body: Record<string, unknown>, mode: 'create' | 'update') {
  ...
  if ('serviceType' in body) {
    if (body.serviceType === null || body.serviceType === '') out.serviceType = null;
    else if (isValidServiceType(body.serviceType)) out.serviceType = body.serviceType;
    else errors.push('Invalid serviceType');
  }
...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from('Service')
    .select('*')
    // The provider's own services PLUS any agency-pool service assigned
  ...
  if (error) {
    logger.error('[services/GET] query failed', { spaceId: space.id }, error);
...
export async function POST(req: NextRequest) {
  ...
  if (error) {
    ...
    logger.error('[services/POST] insert failed', { spaceId: space.id }, error);
...
export { sanitiseBody as _sanitiseServiceBody };
