 ...
 * Contact/Deal/Service DELETE handlers — the inline path is the fast
...
const PREFIX_SPECS: PrefixSpec[] = [
  ...
  {
    prefix: STORAGE_PREFIXES.servicePhotos,
    label: 'service-photos',
    referencedKeys: async (candidates) => {
      // Service.photos is a JSONB array of public URLs; we map every
      // candidate KEY back through getPublicUrl-shape and check if any
      // listed service contains the URL. Easier: pull every Service
      ...
      const { data } = await supabase.from('Service').select('photos').limit(5000);
  ...
  },
  {
    ...
    // keeps the object until the next upload so the provider can
  ...
  {
    // Branding assets uploaded via /api/upload — logo, provider photo,
    ...
    // the provider abandons onboarding or replaces the file before
    ...
    referencedKeys: async (candidates) => {
      const { data } = await supabase
        .from('SpaceSetting')
        .select('logoUrl, providerPhotoUrl, intakeFaviconUrl')
        .limit(5000);
      const referenced = new Set<string>();
      for (const row of (data ?? []) as {
        ...
        for (const url of [row.logoUrl, row.providerPhotoUrl, row.intakeFaviconUrl]) {
          if (!url) continue;
          const key = publicUrlToKey(url);
          if (key) referenced.add(key);
        }
