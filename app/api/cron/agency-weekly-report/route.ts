...
/**
 * GET /api/cron/agency-weekly-report
 *
 * Runs every Monday at 9 AM. Compiles a weekly activity report for each
 * active agency and emails it to the agency owner.
 *
 ...
 * fire, cold-start race). Without it, every agency gets the report twice.
 */
async function handler(req: NextRequest) {
  const cronSecret = process.env.CRON_SECRET;
  if (!cronSecret) {
    console.error('[cron/agency-weekly-report] CRON_SECRET env var is not set — rejecting request');
  ...
  const lockKey = `cron-lock:agency-weekly-report:${today}`;
  ...
  try {
    ...
    const { data: agencies, error: brokErr } = await supabase
      .from('Agency')
    ...
    if (!agencies?.length) {
      return NextResponse.json({ sent: 0, message: 'No active agencies' });
    }
    ...
    for (const agency of agencies) {
      try {
        ...
        const [newLeadsRes, contactedLeadsRes, newDealsRes, wonDealsRes] = await Promise.all([
          // New leads assigned (contacts created in the past 7 days with assigned-by-agency tag)
          supabase
            .from('Contact')
            .select('id, spaceId')
            .in('spaceId', spaceIds)
        ...
        for (const m of rawMembers) {
          ...
          const row: AgentRow = {
            ...
            email: user?.email ?? '',
            role: m.role === 'agency_owner' ? 'Owner' : m.role === 'agency_admin' ? 'Admin' : 'Provider',
        ...
        console.error('[cron/agency-weekly-report] Failed for agency', {
          agencyId: agency.id,
          error: err,
        });
      }
    ...
    console.error('[cron/agency-weekly-report] Unhandled error', error);
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
        ...
        <tr><td style="background:#0f172a;padding:20px 28px">
          <p style="margin:0;color:#94a3b8;font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.05em">${esc(agencyName)}</p>
        ...
        <tr><td style="padding:16px 28px;border-top:1px solid #f1f5f9">
          <p style="margin:0;font-size:11px;color:#9ca3af;text-align:center">
            This weekly report is automatically generated for <strong>${esc(agencyName)}</strong>. Manage your notification preferences in agency settings.
          </p>
        </td></tr>
      </table>
    </td></tr>
