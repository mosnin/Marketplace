/**
 * GET   /api/profile-page — the provider's public-page config (defaults if unset).
...
// surface the provider owns — the public page, the application, the booking
// page all read these. The PATCH below threads them through.
const SETTINGS_SELECT = 'isVerified, socialLinks';

// The picker only needs to render — full service pages live elsewhere.
// We cap the list at 50 so the editor stays snappy; if a provider truly has
// more, the picker isn't the right tool for the long tail.
const AVAILABLE_SERVICES_CAP = 50;

// Cap the provider's curated set at 12 — enough for a carousel without
...
export async function GET() {
  ...
  const [{ data }, { data: settingsRow }, { data: serviceRows }] = await Promise.all([
    ...
    // beyond that the provider isn't picking from a list any more, they're
  ...
  // Service photos are stored as a JSONB array of URLs (see the Service
  // migration comment). For the picker we only need the first one — the
  // public page's carousel uses the same shape.
  const availableServices: AvailableService[] = ((serviceRows ?? []) as Array<{
    id: string;
    address: string;
    city: string | null;
    stateRegion: string | null;
  ...
  merged.availableServices = availableServices;
...
export async function PATCH(req: NextRequest) {
  ...
  for (const key of ['enabled', 'showIntake', 'showAppointments', 'showServices'] as const) {
  ...
  // featuredServiceIds: validate each id belongs to the space and is an
  // active listing. Unknown / stale ids are dropped silently rather than
  // 400-ing the whole save — the editor's set can drift when a service
  ...
  if (Array.isArray(body.featuredServiceIds)) {
    const raw = (body.featuredServiceIds as unknown[])
      .filter((id): id is string => typeof id === 'string' && id.length > 0)
      .slice(0, FEATURED_CAP);

    if (raw.length === 0) {
      patch.featuredServiceIds = [];
    } else {
      ...
      // Preserve the provider's submitted order — `.in()` returns rows in
      // whatever order Postgres feels like.
      patch.featuredServiceIds = raw.filter((id) => valid.has(id));
    }
  }
  ...
  // every public surface the provider owns reads from there). Build a
