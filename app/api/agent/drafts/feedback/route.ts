 ...
 *
 * This endpoint exists for the third case: the provider sees a draft, taps
 * "Hold for later", and the draft stays pending. We still want the signal
 * — "this provider wasn't ready to act on this" is data — but the draft's
...
export async function POST(req: NextRequest) {
  ...
  if (existing.status !== 'pending') {
    // Idempotent no-op — the provider held something we already terminated.
