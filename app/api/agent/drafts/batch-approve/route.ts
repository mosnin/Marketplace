 ...
 * the provider batch-approves 5, four succeed, one returns its error, and the
 ...
 *
 * Server-side scoping: every draftId is verified to belong to the provider's
 ...
 * batch 30-50 drafts; 100 covers the heaviest provider.
 */
  ...
  for (const draftId of uniqueIds) {
    try {
      ...
      if (updateError) {
        ...
        // provider but mark the result as failed so they re-check.
        results.push({
          draftId,
          ok: false,
