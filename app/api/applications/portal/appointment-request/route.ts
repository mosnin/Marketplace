 ...
 *
 * Side effects:
 *   1. Creates an ApplicationMessage with a structured appointment-request body
 *      so the existing message thread reflects the ask.
 *   2. Creates an AgentQuestion scoped to the provider's space + this
 *      contact, which surfaces in /koala's focus card / questions panel
 *      so the provider sees it as the next thing that needs them.
 *   3. (TODO follow-up) emails the provider via the existing notify path.
 *
 * Why a structured request and not "just a chat message" — the provider's
...
export async function POST(req: NextRequest) {
  ...
  const { applicationRef, token, preferredTimes, serviceAddress, notes } = body;
  ...
  const trimmedAddress = serviceAddress?.trim().slice(0, 300) ?? '';
  const trimmedNotes = notes?.trim().slice(0, 1000) ?? '';

  // Rate limit. Appointment requests are heavier than chat — tighter cap.
  const ip = getClientIp(req);
  const { allowed: ipAllowed } = await checkRateLimit(`portal:appointment:ip:${ip}`, 10, 3600);
  if (!ipAllowed) {
    return NextResponse.json(
  ...
  const { allowed } = await checkRateLimit(`portal:appointment:${tokenKey}`, 5, 3600);
  if (!allowed) {
    return NextResponse.json(
      { error: 'Too many requests. Try again in a bit.' },
      { status: 429, headers: { 'Retry-After': '3600' } },
  ...
  if (contactError) {
    console.error('[portal/appointment-request] Contact lookup error:', contactError);
  ...
  // Compose the appointment-request body for the message thread.
  const messageBody = [
    '🏠 Appointment requested',
    safeAddress ? `Service: ${safeAddress}` : null,
    `Available: ${safeTimes}`,
  ...
  // Compose the AgentQuestion that surfaces in the provider's Koala focus
  ...
  const questionInsert = supabase
    ...
    .insert({
      ...
      context: [
        ...
        .join('\n'),
      status: 'pending',
      priority: 50, // mid-high — applicant action; provider should see it today
      contactId: contact.id,
    })
    .select('id')
    .single();
  ...
  if (messageRes.error) {
    console.error('[portal/appointment-request] Message insert error:', messageRes.error);
    return NextResponse.json({ error: 'Failed to send appointment request' }, { status: 500 });
  }
  if (questionRes.error) {
    // Soft-fail the AgentQuestion — the message landed, the provider sees it.
    // Log for observability but don't 500 the user.
    console.error('[portal/appointment-request] Question insert error:', questionRes.error);
  }

  // Notify provider (fire and forget — same pattern as message endpoint).
  void notifyProviderOfAppointmentRequest(
    contact.spaceId,
    contact.name,
    safeTimes,
    safeAddress,
    ...
    console.error('[portal/appointment-request] Provider notification failed:', err),
  );
...
/**
 * Email the provider about a new appointment request. Same template family as
 * portal/message's notification, with a clearer "appointment requested" subject
 * and a structured body.
 */
async function notifyProviderOfAppointmentRequest(
  spaceId: string,
  ...
  const FROM = process.env.RESEND_FROM_EMAIL?.includes('@')
    ...
    : `notifications@${process.env.RESEND_FROM_EMAIL ?? 'alerts.usekoala.com'}`;
  ...
  await resend.emails.send({
    from: FROM,
    to: owner.email,
    subject: `Appointment requested by ${safeName}`,
    html: `
<!DOCTYPE html>
<html>
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
        ...
        <tr><td style="padding:16px 28px;border-top:1px solid #f1f5f9">
          <p style="margin:0;font-size:11px;color:#9ca3af">This appears in your Koala focus card so you can schedule it inline.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
