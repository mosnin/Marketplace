...
function buildServer(spaceId: string): McpServer {
  ...
  server.tool(
    ...
    'List contacts in your CRM. Optionally filter by type, lead type, or search query.',
    {
      ...
      type: z
        .enum(['QUALIFICATION', 'APPOINTMENT', 'APPLICATION'])
        .optional()
  ...
  server.tool(
    ...
    async ({ status, limit }) => {
      let q = supabase
        .from('Appointment')
        .select(
          'id, guestName, guestEmail, guestPhone, serviceAddress, startsAt, endsAt, status, createdAt',
        )
        .eq('spaceId', spaceId)
  ...
  server.tool(
    'dashboard_summary',
    'Get a high-level summary: lead count, deal pipeline value, upcoming appointments, overdue follow-ups.',
    {},
    async () => {
      const now = new Date().toISOString();
      const [contactCount, dealAgg, appointmentCount, followUpCount, buyerLeadCount] = await Promise.all([
        supabase
          .from('Contact')
