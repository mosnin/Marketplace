 ...
 * Export all leads across the agency as a CSV download.
...
export async function GET() {
  ...
  if (memberUserIds.length === 0) {
    const csv = 'Name,Email,Phone,Lead Type,Budget,Score,Score Label,Status,Service Address,Notes,Move-in Date,Employment,Income,Assigned To,Created At\n';
  ...
  if (error) {
    console.error('[agency/leads/export] query error', error);
  ...
  const filename = `${agency.name.replace(/[^a-zA-Z0-9]/g, '_')}_leads_export_${new Date().toISOString().split('T')[0]}.csv`;
