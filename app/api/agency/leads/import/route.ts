...
/**
 * POST /api/agency/leads/import
 * Import leads from a CSV file upload.
 * Auth: agency_owner / agency_admin only.
 */
export async function POST(req: NextRequest) {
  ...
  // ── Rate limit ──────────────────────────────────────────────────────────
  const { allowed } = await checkRateLimit(`agency-import:${agency.id}`, 5, 3600);
  if (!allowed) {
  ...
  const colIdx = {
    ...
    address: findIndex(headers, ['service address', 'address', 'serviceaddress']),
  ...
  // Default space: the agency owner's space
  const defaultSpaceId = spaceByOwner[agency.ownerId] ?? null;
  if (!defaultSpaceId) {
    return NextResponse.json(
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (assignTo) {
      ...
      if (!targetUserId) {
        errors.push(`Row ${rowNum}: Member "${assignTo}" not found in agency. Using default.`);
      } else {
    ...
    contactsToInsert.push({
      ...
      notes: notes || null,
      type: 'QUALIFICATION',
      tags: ['new-lead', 'imported', 'agency-lead'],
  ...
  if (contactsToInsert.length > 0) {
    ...
    for (let i = 0; i < contactsToInsert.length; i += BATCH_SIZE) {
      ...
      if (insertError) {
        console.error('[agency/leads/import] insert error', insertError);
