...
/**
 * GET /api/agency/join-code
 * Returns the current join code for the authenticated agency's agency.
 */
export async function GET() {
  ...
  return NextResponse.json({ joinCode: ctx.agency.joinCode ?? null });
}
...
export async function POST() {
  ...
  // Rate limit: 5 regenerations per hour per agency
  const { allowed } = await checkRateLimit(`agency-join-code:${ctx.agency.id}`, 5, 3600);
  if (!allowed) {
  ...
  if (error) {
    console.error('[agency/join-code] update failed', error);
    return NextResponse.json({ error: 'Failed to save join code' }, { status: 500 });
  }

  void audit({ actorClerkId: clerkId ?? null, action: 'UPDATE', resource: 'Agency', resourceId: ctx.agency.id, metadata: { field: 'joinCode' } });
