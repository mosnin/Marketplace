...
/**
 * PATCH /api/agency/commissions/ledger/[id]
 * Update a commission-ledger row. Only agency_owner / agency_admin may call.
 *
 * Body (any subset):
 *   { status?, payoutAt?, agentRate?, agencyRate?, referralRate?,
 *     referralUserId?, notes? }
 *
 * Amounts (agentAmount / agencyAmount / referralAmount) are recomputed from
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const hasAgencyRate = Object.prototype.hasOwnProperty.call(body, 'agencyRate');
  ...
  let agencyRateVal: number | undefined;
  if (hasAgencyRate) {
    if (!isValidRate(body.agencyRate)) {
      return NextResponse.json({ error: 'Invalid agencyRate (must be 0-100)' }, { status: 400 });
    }
    agencyRateVal = body.agencyRate;
  }
  ...
  // Load the ledger row; must belong to this agency.
  ...
  if (fetchErr) {
    console.error('[agency/commissions/ledger/PATCH] fetch failed', fetchErr);
  ...
  if (referralUserIdVal !== undefined && referralUserIdVal !== null) {
    const { data: userRow, error: userErr } = await supabase
      ...
      .maybeSingle();
    if (userErr) {
      console.error('[agency/commissions/ledger/PATCH] user lookup failed', userErr);
  ...
  const rateChanged = hasAgentRate || hasAgencyRate || hasReferralRate;
  const effectiveAgentRate = agentRateVal ?? row.agentRate;
  const effectiveAgencyRate = agencyRateVal ?? row.agencyRate;
  ...
  const rateSum =
    ...
    (effectiveAgencyRate ?? 0) +
    (effectiveReferralRate ?? 0);
  if (rateSum > 100) {
    return NextResponse.json(
      {
        error: `Rates sum to ${rateSum}% — agent + agency + referral cannot exceed 100% of the deal value.`,
      },
      { status: 400 },
    );
  }
  ...
  if (agencyRateVal !== undefined) updates.agencyRate = agencyRateVal;
  ...
  if (rateChanged) {
    ...
    updates.agencyAmount = (dealValue * effectiveAgencyRate) / 100;
    updates.referralAmount = (dealValue * effectiveReferralRate) / 100;
  }

  // Scope the update to agencyId to guard against TOCTOU races.
  ...
  if (updateErr) {
    console.error('[agency/commissions/ledger/PATCH] update failed', updateErr);
