...
/**
 * GET /api/agency/commissions/export
...
export async function GET(req: NextRequest) {
  ...
  // amount. Letting provider_member export the whole agency's ledger is
  ...
  if (error) {
    console.error('[agency/commissions/export] query failed', error);
  ...
  for (const r of rows) {
    lines.push(
      [
        ...
        escapeCsv(r.agencyRate),
        escapeCsv(r.agencyAmount),
  ...
  // Agency has no `slug` field in the current schema; use the id as a
  // stable, URL-safe identifier.
  const slug = ctx.agency.id;
