 ...
 * Send multiple agency invitations from a list of emails.
 * Max 50 per request.
 */
export async function POST(req: Request) {
  ...
  const seatCheck = await checkSeatCapacity(agency.id, entries.length);
  if (!seatCheck.ok) {
  ...
  for (const entry of entries) {
    ...
    const roleToAssign = entry.role === 'agency_admin' ? 'agency_admin' : 'provider_member';
    ...
    try {
      ...
      console.error('[agency/invite/bulk] email failed', err);
    }
  ...
  void audit({
    ...
    action: 'CREATE',
    resource: 'Invitation',
    metadata: { bulk: true, count: sentCount, agencyId: agency.id },
  });
