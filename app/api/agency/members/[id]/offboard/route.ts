...
/**
 * POST /api/agency/members/[id]/offboard
 * Transfer a member's contacts, deals, and open appointments to a destination member
 * and offboard them from the agency.
 *
 * Only agency_owner can offboard a member. The owner cannot be offboarded,
 * the caller cannot offboard themselves, and the destination must be a
 * different, active user in the same agency.
 *
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // Only the agency owner can offboard members.
  ...
  // Load the target membership, scoped to this agency.
  ...
  // Load the destination membership, scoped to this agency.
  ...
  if (target.role === 'agency_owner') {
    return NextResponse.json({ error: 'Cannot offboard the agency owner' }, { status: 403 });
  }
  ...
  const { data: rpcData, error: rpcError } = await supabase.rpc('offboard_agency_member', {
  ...
  if (rpcError) {
    logger.error('[agency/members/offboard] rpc failed', {
  ...
  if (dryRun) {
    ...
    return NextResponse.json({
      ...
      openAppointmentCount: Number(payload.open_appointment_count ?? payload.openAppointmentCount ?? 0),
  ...
  return NextResponse.json({
    ...
    appointmentsMoved: Number(payload.appointments_moved ?? payload.appointmentsMoved ?? 0),
  });
}
