 ...
 * is still useful, and the agency can re-publish to retry.
 *
 * Restricted to agency_owner / agency_admin.
 */
export async function POST(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  // 1. Load the source template scoped to the caller's agency.
  const { data: tmpl, error: loadErr } = await supabase
    .from('AgencyTemplate')
    .select(TEMPLATE_COLUMNS)
    .eq('id', templateId)
    .eq('agencyId', ctx.agency.id)
    .maybeSingle<AgencyTemplateRow>();

  if (loadErr) {
    logger.error(
      '[agency/templates/publish] load failed',
      { templateId, agencyId: ctx.agency.id },
      loadErr,
    );
  ...
  // 2. Enumerate provider_member userIds for this agency. Agencies don't
  ...
  if (memberErr) {
    logger.error(
      '[agency/templates/publish] member fetch failed',
      { templateId, agencyId: ctx.agency.id },
      memberErr,
    );
  ...
  if (agentUserIds.length > 0) {
    ...
    //    agency. A provider can be a member of this agency AND also
    //    own a Space in another agency (dual-membership); without the
    //    agencyId filter, publishing a template from Agency A would
    //    fan out into Spaces in Agency B — a cross-tenant data leak.
    ...
    if (spaceErr) {
      logger.error(
        '[agency/templates/publish] space fetch failed',
        { templateId, agencyId: ctx.agency.id },
        spaceErr,
      );
    ...
    // Agents with no Space — count one skip per such agent so the agency
    // can see something didn't land. No write to do for them.
    for (const userId of agentUserIds) {
      if (!spacesByOwner.has(userId)) {
        skipped += 1;
        logger.warn('[agency/templates/publish] agent has no space; skipped', {
          templateId,
          userId,
        });
      }
    ...
    if (targetSpaceIds.length > 0) {
      ...
      if (existingErr) {
        logger.error(
          '[agency/templates/publish] existing fetch failed',
          { templateId, agencyId: ctx.agency.id },
          existingErr,
        );
        return NextResponse.json(
      ...
      try {
        ...
        if (existing.length === 0) {
          ...
          if (insertErr) {
            skipped += 1;
            logger.error(
              '[agency/templates/publish] insert failed',
        ...
        for (const row of existing) {
          if (row.sourceVersion === null) {
            skipped += 1;
            logger.info(
              '[agency/templates/publish] skipped (locally edited)',
          ...
          if (updateErr) {
            skipped += 1;
            logger.error(
              '[agency/templates/publish] update failed',
              { templateId, messageTemplateId: row.id, spaceId: space.id },
              updateErr,
            );
          } else {
        ...
        skipped += 1;
        logger.error(
          '[agency/templates/publish] unexpected error for space',
          { templateId, spaceId: space.id },
          err,
        );
      }
  ...
  if (stampErr) {
    logger.error(
      '[agency/templates/publish] stamp failed',
      { templateId, agencyId: ctx.agency.id, pushed, skipped },
      stampErr,
    );
