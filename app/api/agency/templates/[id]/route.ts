...
const patchSchema = z
  .object({
    ...
    category: z.enum(['follow-up', 'intro', 'closing', 'appointment-invite']).optional(),
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  // Confirm the template is in the caller's agency before touching it.
  // A missing row is a 404 regardless of whether it exists for another
  // agency — we don't want to leak cross-agency existence.
  ...
  if (loadErr) {
    logger.error(
      '[agency/templates/PATCH] load failed',
  ...
  const patch: Partial<AgencyTemplateRow> & { updatedAt: string } = {
  ...
  if (updateErr) {
    logger.error(
      '[agency/templates/PATCH] update failed',
      { templateId, agencyId: ctx.agency.id },
      updateErr,
    );
 ...
 *
 * Removes the AgencyTemplate row. Agent-local copies (MessageTemplate rows
...
export async function DELETE(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  if (deleteErr) {
    logger.error(
      '[agency/templates/DELETE] delete failed',
      { templateId, agencyId: ctx.agency.id },
      deleteErr,
    );
