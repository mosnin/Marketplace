/**
 * GET  /api/agency/profile — the calling agency's OWN member profile.
 * PATCH /api/agency/profile — update the calling agency's OWN member profile.
 *
 ...
 *
 * Profile fields (displayName, title, bio, photoUrl, phone) mirror the provider
 * profile on SpaceSetting but live per-member on AgencyMembership — each
 * admin has their own profile within the agency.
 */
...
export async function PATCH(req: Request) {
  ...
  // from the resolved agency context, never from the client.
  ...
  if (updateErr) {
    console.error('[agency/profile] update failed', updateErr);
