...
// ── GET — any agency member may read their agency's rules ──────────────────

/**
 * GET /api/agency/routing-rules
 *
 * Returns the agency's routing rules ordered by priority ASC then
 * createdAt ASC — the same order the engine evaluates them in. Any
 * agency member can read (helpful for debugging / transparency into
...
export async function GET(): Promise<NextResponse> {
  ...
  if (error) {
    ...
    logger.error(
      '[agency/routing-rules/GET] list failed',
...
// ── POST — create a new rule (agency_owner / agency_admin only) ───────────────

/**
 * POST /api/agency/routing-rules
 *
 * Creates a rule for the current agency. See the migration header for
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // agency — otherwise we'd accept cross-tenant IDs. We don't require
  // the target to be a provider_member (owners can legitimately want a
  // VIP rule that points at themselves) but we DO require agency
  // membership.
  if (data.destinationUserId) {
    ...
    if (memErr) {
      logger.error(
        '[agency/routing-rules/POST] destination membership check failed',
        { agencyId: ctx.agency.id, destinationUserId: data.destinationUserId },
        memErr,
      );
    ...
    if (!membership) {
      return NextResponse.json(
        { error: 'destinationUserId is not a member of this agency' },
        { status: 400 },
      );
    }
  }
  ...
  if (insertErr || !inserted) {
    logger.error(
      '[agency/routing-rules/POST] insert failed',
      { agencyId: ctx.agency.id },
      insertErr,
    );
