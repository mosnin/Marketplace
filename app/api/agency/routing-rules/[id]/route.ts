...
/**
 * PATCH /api/agency/routing-rules/[id]
 *
 * Partial update restricted to agency_owner / agency_admin. The route
 * re-verifies the row belongs to the caller's agency before writing,
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  if (loadErr) {
    logger.error(
      '[agency/routing-rules/PATCH] load failed',
      { ruleId, agencyId: ctx.agency.id },
      loadErr,
    );
  ...
  // new target is still a member of THIS agency.
  if (
    ...
    if (memErr) {
      logger.error(
        '[agency/routing-rules/PATCH] destination membership check failed',
        { ruleId, agencyId: ctx.agency.id, newUserId },
        memErr,
      );
  ...
  if (updateErr) {
    logger.error(
      '[agency/routing-rules/PATCH] update failed',
      { ruleId, agencyId: ctx.agency.id },
      updateErr,
    );
...
/**
 * DELETE /api/agency/routing-rules/[id]
 *
 * Removes a rule. Restricted to agency_owner / agency_admin.
 */
export async function DELETE(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  if (deleteErr) {
    logger.error(
      '[agency/routing-rules/DELETE] delete failed',
      { ruleId, agencyId: ctx.agency.id },
      deleteErr,
    );
