/**
 * GET /api/agency/morning
 *
 * The agency's morning narration — ONE prioritized action the agency should
 ...
 * question the agency can answer with a tap.
 *
 * Priority ladder (descending urgency):
 *   1. stuck-deals      — highest (days × value) score across the team.
 *   2. unassigned-leads — agency-tagged Contacts sitting in the queue.
 *   3. overloaded       — a provider whose open work (active deals + open
 *                         leads) exceeds 15 — the canonical team_health
 *                         threshold from agent/tools/agency/team.py.
 *   4. underused        — a provider with open work below 3 who's still
 *                         active in the last 30 days (canonical team_health
 *                         underused signal). Newly-invited dormant members
 *                         don't surface — only currently-active providers
 *                         going quiet.
 ...
 *     stats: { providers, activeDeals, gciMtd } }
 *
 * `suggestedPrompt` is non-null on branches 1-4 (the "want me to ..."
 * affordance); the component routes the agency into /agency/koala with the
 * prompt pre-filled. Branches 5 and 6 have no action, so the headline
 * renders non-interactive.
 *
 * The thresholds and definitions mirror agent/tools/agency/ exactly — the
 * Python tools are the canonical math for "what's happening on the team".
 * If they drift, Koala's narrated answer and this headline will disagree,
 * which is the worst kind of inconsistency.
 *
 * Auth: agency_owner or agency_admin. Provider-only members get 403 — they
...
export interface AgencyMorningResponse {
  /**
   * The single sentence the agency reads first thing. Formed as
   * "<fact> — <call-to-action>." so the component can render the
   * call-to-action in slightly stronger weight without parsing HTML.
   */
  headline: string;
  /**
   * Text to prefill into /agency/koala when the agency taps the headline.
...
// agent/tools/agency/pipeline.py:42 so the row pill and Koala answer the
// same question with the same math.
const STUCK_MIN_DAYS = 7;

// team_health canonical thresholds — see agent/tools/agency/team.py:55-57.
// Keep these in lockstep with the Python tool; the agency hears one number
// across the surface.
...
// the providers page and team_health use to define "responding".
const RESPONSE_OUTBOUND_TYPES = ['call', 'email', 'meeting'] as const;

// GCI milestones the agency actually wants to hear about. Crossed-this-month
...
/** First name only — agency speaks about providers the way they call them. */
...
interface ProviderLoad {
  ...
  openLoad: number;
  /** True when the provider has any outbound activity in the last 30 days. */
...
export async function GET() {
  ...
  const memberRows: MemberRow[] = members
    .map((m) => ({
      userId: m.userId,
      name: m.User?.name ?? m.User?.email ?? 'Provider',
      spaceId: m.Space?.id ?? '',
    }))
  ...
  // Six queries; nothing fancier than what /agency/providers already does.
  // Each is agency-scoped through `spaceId IN (...)` or
  // `agencyId = ...` so RLS doesn't leak across agencies even if
  // someone bypasses requireAgency().
  const [
    allActiveDealsRes,
    stuckCandidatesRes,
    openLeadsRes,
    ...
    // Every active deal — just spaceId for the per-provider active-deals count
    ...
    // Open leads per provider — Contact WHERE type='QUALIFICATION'. This is
    // the same definition team_health uses for "open_leads". The agency's
    ...
    // Unassigned agency leads — same predicates as
    // app/agency/leads/page.tsx and find_unassigned_leads:
    // agencyId match, tag includes 'agency-lead', NOT 'assigned'.
    supabase
      .from('Contact')
      .select('id, tags')
  ...
  // ── Stuck deals: rank by (days × value), pick the top provider ───────────
  ...
  for (const [spaceId, rows] of stuckBySpace) {
    ...
    // applied per provider instead of per deal. Floor of $1 on value so a
  ...
  // ── Per-provider open load (active deals + open leads) ───────────────────
  ...
  const loads: ProviderLoad[] = memberRows.map((m) => {
  ...
  // Overloaded = highest open_load AND open_load > 15. Single-provider team
  // never trips this — the agency can't redistribute to themselves.
  let overloaded: ProviderLoad | null = null;
  if (loads.length >= 2) {
  ...
  // copy: we don't call out a provider who's already on vacation or hasn't
  // started. Same guard as team_health._load_signal.
  let underused: ProviderLoad | null = null;
  if (loads.length >= 2) {
  ...
  // 2. Unassigned leads.
    ...
    headline = `${n} ${leadWord} sitting unassigned — want me to route ${n === 1 ? 'it' : 'them'} to your fewest-loaded provider?`;
    suggestedPrompt =
      n === 1
        ? 'Route the unassigned lead to the provider with the lightest load.'
        : `Route the ${n} unassigned leads to the provider with the lightest load.`;
  }
  // 3. Overloaded provider.
  else if (overloaded) {
    const fn = firstName(overloaded.name);
    // Fact framing — never "Bob is slow," always "Bob's at 18 active vs
    // threshold 15." Mirrors the row-pill voice from /agency/providers.
  ...
  const response: AgencyMorningResponse = {
    headline,
    suggestedPrompt,
    stats: {
      providers: memberRows.length,
