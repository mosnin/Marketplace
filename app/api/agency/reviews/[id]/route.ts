...
import { getAgencyMemberContext, requireAgency } from '@/lib/permissions';
...
/**
 * GET /api/agency/reviews/[id]
 * Returns a single review (scoped to the caller's agency) plus its
 * comments in chronological order. Any agency member role may read.
 */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  if (error) {
    logger.error(
      '[agency/reviews/GET] load failed',
      { reviewId, agencyId: ctx.agency.id },
      error,
    );
  ...
  try {
    ...
    logger.error('[agency/reviews/GET] shape failed', { reviewId }, err);
    return NextResponse.json({ error: 'Failed to load review' }, { status: 500 });
  }
}
 ...
 * Only agency_owner/agency_admin may resolve. Already-resolved rows return 409.
 */
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  // Role gate — requireAgency already restricts to agency_owner/agency_admin,
  // but we double-check defensively so this route's contract doesn't silently
  // widen if requireAgency's behavior ever changes.
  ...
  if (loadErr) {
    logger.error(
      '[agency/reviews/PATCH] load failed',
      { reviewId, agencyId: ctx.agency.id },
      loadErr,
    );
    return NextResponse.json({ error: 'Failed to load review' }, { status: 500 });
  ...
  if (updateErr) {
    logger.error('[agency/reviews/PATCH] update failed', { reviewId }, updateErr);
  ...
  try {
    ...
    logger.error('[agency/reviews/PATCH] shape failed', { reviewId }, err);
