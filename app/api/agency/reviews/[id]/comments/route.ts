 ...
 *   - the caller is any agency member of the review's agency, OR
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // Load the review first — we need its agencyId and requestingUserId
  ...
  const { data: review, error: loadErr } = await supabase
    ...
    .select('id, agencyId, requestingUserId, status')
    .eq('id', reviewId)
    .maybeSingle<ReviewRow>();

  if (loadErr) {
    logger.error('[agency/reviews/comments/POST] load failed', { reviewId }, loadErr);
  ...
  // the agency had resolved the review (the UI was going through stale
  ...
  if (userErr) {
    logger.error('[agency/reviews/comments/POST] user lookup failed', { clerkId }, userErr);
  ...
  // Authorization: agency member of this agency OR the requesting agent.
  const agencyCtx = await getAgencyMemberContext();
  const isAgencyMember = Boolean(agencyCtx && agencyCtx.agency.id === review.agencyId);
  const isRequester = review.requestingUserId === dbUser.id;
  if (!isAgencyMember && !isRequester) {
  ...
  if (insertErr) {
    logger.error('[agency/reviews/comments/POST] insert failed', { reviewId, commentId }, insertErr);
