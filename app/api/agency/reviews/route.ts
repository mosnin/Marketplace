...
/**
 * GET /api/agency/reviews?status=open|approved|closed|all
 * Default status = 'open'. Returns up to 200 rows belonging to this agency.
...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select(
      'id, dealId, status, reason, createdAt, resolvedAt, resolvedNote, requestingUserId, agencyId',
    )
  ...
  if (rowsErr) {
    logger.error('[agency/reviews/GET] list failed', { agencyId: ctx.agency.id }, rowsErr);
  ...
  if (usersRes.error) {
    logger.error('[agency/reviews/GET] user fetch failed', { agencyId: ctx.agency.id }, usersRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (dealsRes.error) {
    logger.error('[agency/reviews/GET] deal fetch failed', { agencyId: ctx.agency.id }, dealsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (commentsRes.error) {
    logger.error('[agency/reviews/GET] comment count failed', { agencyId: ctx.agency.id }, commentsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  ...
  if (spacesRes.error) {
    logger.error('[agency/reviews/GET] space fetch failed', { agencyId: ctx.agency.id }, spacesRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
