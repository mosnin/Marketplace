 ...
 *
 * Returns contacts scoped to ALL member spaces in the agency's agency,
 * each annotated with `providerName` (the owning provider's display name).
 *
 * Query params:
 *   search  — optional; case-insensitive substring match on name/email/phone
 *   type    — optional; QUALIFICATION | APPOINTMENT | APPLICATION | ALL (default ALL)
...
export async function GET(req: NextRequest) {
  ...
  // ── Build spaceId → provider display-name map ──────────────────────────────
  const spaceToProvider = new Map<string, string>();
  for (const m of allMembers) {
    const sid = m.Space?.id;
    if (!sid) continue;
    spaceToProvider.set(sid, m.User?.name ?? m.User?.email ?? 'Unknown provider');
  }
  ...
  if (error) {
    console.error('[agency/contacts/GET] query error:', error);
    return NextResponse.json({ error: 'Failed to fetch contacts' }, { status: 500 });
  }

  // Annotate each row with providerName
  const annotated = (data ?? []).map((c: any) => ({
    ...c,
    providerName: spaceToProvider.get(c.spaceId) ?? 'Unknown provider',
  }));
