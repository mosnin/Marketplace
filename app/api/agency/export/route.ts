...
export async function GET() {
  ...
  const rows = members.map((m) => {
    ...
    const role = m.role === 'agency_owner' ? 'Owner' : m.role === 'agency_admin' ? 'Admin' : 'Provider';
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${agency.name.replace(/[^a-zA-Z0-9]/g, '_')}_team_export_${new Date().toISOString().split('T')[0]}.csv"`,
    },
  });
}
