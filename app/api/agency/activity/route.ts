 ...
 *
 * Returns the agency-scoped slice of the AuditLog table for compliance
 * review. Any agency-level member (agency_owner, agency_admin) can read.
 *
 * Query params:
 ...
 *
 * Agency scoping:
 *   - Resolve spaceIds[] via Space.agencyId = ctx.agency.id
 *   - Query A: AuditLog WHERE spaceId IN spaceIds
 *   - Query B: AuditLog WHERE spaceId IS NULL
 *              AND metadata->>'agencyId' = ctx.agency.id
 *   - Merge + sort + limit. This avoids the easy-to-write leak where
 *     `.in('spaceId', ids).or('spaceId.is.null')` returns every agency's
...
export async function GET(req: NextRequest) {
  ...
  // ── Resolve agency spaces ──────────────────────────────────────────────
  const { data: spaceRows, error: spaceErr } = await supabase
    .from('Space')
    .select('id, slug')
    .eq('agencyId', ctx.agency.id);
  if (spaceErr) {
    console.error('[agency/activity] space lookup failed', spaceErr);
  ...
  // If the agency owns zero spaces we skip query A entirely — `.in('spaceId',
  ...
  if (spaceIds.length > 0) {
    ...
    if (error) {
      console.error('[agency/activity] space query failed', error);
      return NextResponse.json({ error: 'Failed to load activity' }, { status: 500 });
    }
    spaceRowsResult = (data ?? []) as AuditLogRow[];
  }

  // ── Query B: agency-wide null-space rows ───────────────────────────────
  // Match via metadata->>'agencyId' — safe because it's a strict equality
  // filter tied to the caller's agency id. Rows that lack this metadata
  ...
  let nullSpaceResult: AuditLogRow[] = [];
  {
    let q = supabase
      ...
      .select('id, clerkId, ipAddress, action, resource, resourceId, spaceId, metadata, createdAt')
      .is('spaceId', null)
      .eq('metadata->>agencyId', ctx.agency.id)
      .gte('createdAt', sinceIso)
      .order('createdAt', { ascending: false })
      .order('id', { ascending: false })
      .limit(pageSize);
    ...
    if (error) {
      ...
      console.error('[agency/activity] null-space query failed', error);
    } else {
