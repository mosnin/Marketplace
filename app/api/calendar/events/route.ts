 ...
 *
 * GET: returns the next 30 days of events from the provider's connected
 ...
 *
 * POST: provider-initiated manual event creation. Validates ownership,
 * connection presence, and the payload shape; pushes to the provider
 * via `writeEventThrough` (same helper appointment booking uses) so the event
 ...
 *
 * The Koala calendar surface is a thin read view; the provider's
...
/** In-process memo. 60s TTL absorbs a provider flipping between days
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Don't 500 — the provider sees an empty list with the calm empty
...
/**
 * Manual event creation. Validates the provider owns the space, has an
 * active calendar connection, and that the payload makes sense; then
 * fires `writeEventThrough` (same helper appointment booking uses) so the event
...
export async function POST(req: NextRequest) {
  ...
  if (!result.externalOk) {
    // The mirror row landed (forensics) but the provider's calendar
