 ...
 * Koala agent (Modal/Python). Authed by AGENT_INTERNAL_SECRET.
 *
 ...
 * recoverable errors and surfaces `requestId` to the provider if they
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // the provider gets the request_id for Composio support.
    const e = err as {
      message?: string;
      code?: string;
      statusCode?: number;
  ...
  // and KEEP the requestId so the provider (and Koala) can mention it if
  ...
  // Slack `channel_not_found` happens when the provider names a channel the
  ...
  // mean" so the provider (or Koala) can pick the real one explicitly.
  ...
  if (!ok && slug === 'SLACK_SEND_MESSAGE' && errMsg && /channel[_\s]?not[_\s]?found/i.test(errMsg)) {
    ...
    try {
      ...
      if (names.length > 0) {
        ...
        // the same provider-readable outcome on 99% of typos.
