/**
 * POST /api/internal/integrations/search — internal endpoint the Koala
 * agent's `find_integration_tool` calls when it needs to discover what
 * Composio actions are available for the provider's connected toolkits.
 * Authed by AGENT_INTERNAL_SECRET.
 *
 * Why a search endpoint instead of front-loading every tool: a provider
...
export async function POST(req: NextRequest) {
  ...
  // the provider has already triggered the chat turn, so latency here is
