...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('spaceId', space.id)
      // Reserved agency/team prefixes are sourced from
      // lib/chat/conversation-access so the provider exclusion set lives in one
      // place. The provider surface never serves agency-Koala or team chats.
