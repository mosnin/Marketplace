 ...
 *
 * Every chat turn proxies to the Modal Python sandbox running Koala via the
 ...
 *
 * Set KOALA_CHAT_RUNTIME=ts to fall back to the in-process TypeScript runtime
...
interface PostBody {
   ...
   *               search over the provider's data. No tools, no agent loop, so
...
function proxyModalStream({
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Koala actually did (tool calls, plan cards), not just a flat reply.
...
export async function POST(req: NextRequest) {
  ...
  // Agency pool and their own Space status stays 'inactive' by design — so
  // reading only the Space let a lapsed agency keep premium AI on every seat.
  try {
    ...
    if (isSubscriptionDelinquent(subStatus)) {
      ...
      if (userRow?.platformRole !== 'admin') {
        return NextResponse.json(
          {
            error:
              'Your subscription needs attention — update your payment method in billing to keep using Koala. Your workspace and data stay available.',
          },
          { status: 402 },
        );
      }
  ...
  try {
    ...
    if (err instanceof CreditsExhaustedError) {
      return NextResponse.json(
        { error: 'Out of credits. Buy a top-up or upgrade your plan to keep chatting with Koala.' },
        { status: 402 },
      );
    }
    throw err;
  ...
  void (async () => {
    try {
      if (await hasEmittedTelemetry(ctx.space.id, 'koala_first_message')) return;
      const signupAt = await getFirstEmittedAt(ctx.space.id, 'signup_completed');
      await emitTelemetry({
        event: 'koala_first_message',
        spaceId: ctx.space.id,
        userId: ctx.userId,
        payload: {
          conversationId,
  ...
  //   2. KOALA_CHAT_RUNTIME=modal proxies the WHOLE agent turn to the sandbox
  ...
  if (route === 'direct') {
    ...
    return streamDirectTurn({
      ...
      // misrouted action committed the toolless deflection — the provider
  ...
  //   1. KOALA_CHAT_RUNTIME=modal forces ALL agent turns through the sandbox.
  ...
  //   2. The provider picked Agent mode for THIS message. Prefer Modal, but if
  ...
  // App-wide LLM client (OpenRouter-first), the provider's workspace model,
  // full Koala tool set, approval gates, rate limits, tool-call logging,
...
/**
 * Resolve the chat model the provider's workspace is configured for. The
...
async function callModalAgent(input: CallModalAgentInput): Promise<Response> {
  ...
  const payload = {
    ...
    // this, Modal can't know which provider's Gmail / Slack / etc. to
    // load — the agent ends up with zero integration tools regardless
    // of how many the provider has connected. The Python side reads
