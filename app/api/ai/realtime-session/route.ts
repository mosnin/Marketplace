...
export async function POST(req: Request) {
  ...
  const [{ data: contacts }, { data: deals }, { data: notes }, { data: appointments }, calResult] = await Promise.all([
    supabase
      .from('Contact')
    ...
    supabase
      .from('Appointment')
      .select('guestName, serviceAddress, startsAt, status')
      .eq('spaceId', space.id)
  ...
  const appointmentCtx = (appointments ?? []).map((t: any) =>
    `- ${t.guestName} | ${t.serviceAddress ?? 'No address'} | ${new Date(t.startsAt).toLocaleDateString()} | ${t.status}`
  ).join('\n');
  ...
  const instructions = [
    `You are Koala, the provider's agentic workspace assistant for "${space.name}".`,
    `Today is ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}.`,
    `You help the agent manage their rental and buyer leads, deals, appointments, notes, calendar, and follow-ups through natural conversation.`,
    `Buyer stages: Lead → Pre-Approved → Showings → Offer → Under Contract → Closing. Rental stages: Qualification → Appointment → Application.`,
    ...
    ``,
    appointmentCtx ? `Upcoming Appointments:\n${appointmentCtx}` : '',
    followUpCtx ? `\nFollow-ups Due:\n${followUpCtx}` : '',
    noteCtx ? `\nNotes:\n${noteCtx}` : '',
    calCtx ? `\nCalendar Events:\n${calCtx}` : '',
  ].filter(Boolean).join('\n');
