 ...
 *
 * Initiate an OAuth connection for the calling provider + the given
 * toolkit (e.g. `gmail`, `slack`, `notion`). Returns the URL the
 * provider's browser should redirect to so Composio can run the auth
 * flow. After the provider approves, Composio sends them back to our
 ...
 * entirely, the provider still sees the row.
 */
...
export async function POST(
  ...
  // first — reconnect is the provider explicitly choosing a fresh auth.
  ...
  try {
    ...
    // Persist the row NOW, before the provider leaves for OAuth. The
    ...
    // load tools for it (a half-finished connection 401s and the provider
    // reads it as "Koala lost my integrations"). Promotion to 'active'
    ...
    // exists for …") to the provider verbatim — they tell the provider what
...
/**
 * The URL Composio sends the provider to after OAuth completes.
 *
 ...
 * provider completes OAuth at the provider but never lands back in the app.
 * Logs loud on the server so this misconfiguration is detectable in
 * production logs without the provider having to report a confused
