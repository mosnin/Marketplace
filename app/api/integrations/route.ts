 ...
 *
 * List the provider's connections — one row per (toolkit, status) the
...
export async function GET() {
  ...
  // providers who completed OAuth on the previous codebase where the row
  ...
  // Drop revoked rows — they're audit-only, not provider-facing.
  ...
  // but provider paused). Drives the "Koala is watching" / "Paused"
  ...
  // callback URL is undefined, the provider completes OAuth at the provider,
