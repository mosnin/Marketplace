  ...
  try {
    ...
    const { data: dealRow, error: updateError } = await supabase
      ...
      .update({
        ...
        // serviceId: null unlinks; otherwise validated as string referencing a
        // Service in this space. We don't load the row here — the FK will
        // reject a mismatched id; validating at edit time adds a round-trip
        // without additional safety.
        ...(body.serviceId !== undefined && {
          serviceId: body.serviceId ? String(body.serviceId).slice(0, 64) : null,
        }),
