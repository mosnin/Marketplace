 ...
 *
 * Agent flags one of their deals for agency review. Creates a DealReviewRequest
 ...
 *
 * Auth: caller must own the deal's space (or manage its agency, which is
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const { data: dealRow, error: dealErr } = await supabase
    .from('Deal')
    .select('id, title, spaceId, Space(id, slug, agencyId)')
    .eq('id', dealId)
  ...
  if (!dealRow.Space.agencyId) {
    return NextResponse.json(
      { error: 'Deal is in a non-agency workspace; no review possible.' },
      { status: 409 },
    );
  }

  // Resolve the requesting User row (DB id, not clerk id).
  // Include `name` in the select — notifyAgency wants it in the metadata so
  // the agency's notification renders "Alice flagged ..." without a second
  // lookup downstream.
  ...
  const { data: inserted, error: insertErr } = await supabase
    ...
    .insert({
      ...
      agencyId: dealRow.Space.agencyId,
      status: 'open',
      reason,
      createdAt: nowIso,
    })
  ...
  if (insertErr) {
    ...
    logger.error(
      ...
      { dealId, agencyId: dealRow.Space.agencyId },
      insertErr,
    );
  ...
  void audit({
    ...
    spaceId: space.id,
    req,
    metadata: { dealId, agencyId: dealRow.Space.agencyId },
  });
