 ...
 *
 * Detects the provider's connected CRM(s) and pulls records via Composio.
 *
 * Unified contract:
...
function mapToRecords(toolkit: LiveCrmToolkit, raw: unknown[]): SyncRecord[] {
  return raw
    ...
    .map((item, idx) => {
      ...
      // HubSpot: { id, services: { firstname, lastname, email, phone, hs_lead_status, lastmodifieddate } }
      if (toolkit === 'hubspot') {
        const props = (obj.services ?? obj) as Record<string, unknown>;
