...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'create_space') {
      const {
        slug, intakePageTitle, intakePageIntro, businessName, logoUrl, providerPhotoUrl,
        ...
        phone, websiteUrl, mlsId, agencyAffiliation,
      ...
      if (agencyAffiliation) userUpdates.agencyAffiliation = String(agencyAffiliation).slice(0, 200);
          ...
          .upsert(
            {
              ...
              ...(providerPhotoUrl !== undefined && { providerPhotoUrl }),
    ...
    if (action === 'complete') {
      ...
      if (user.onboard) {
        ...
        if (accountType && ['provider', 'agency_only', 'both'].includes(accountType)) {
      ...
      const accountType = (body as { accountType?: string }).accountType;
      const isAgencyOnly = accountType === 'agency_only';

      // Agency-only users don't need a workspace
      if (!space && !isAgencyOnly) {
        return NextResponse.json(
      ...
      void emitTelemetry({
        ...
        payload: {
          ...
          accountType: (updatePayload.accountType as string | undefined) ?? user.accountType ?? 'provider',
          isAgencyOnly,
        },
      });
    ...
    if (action === 'save_provider_profile') {
      // Persist the provider's onboarding answers into AIUserProfile. Requires
      ...
      const ALLOWED_ROLES = ['solo', 'team_lead', 'agency_owner'];
