 ...
 * the agency pool for Team/Team Plus (resolveBillingAccount handles that).
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Which balance does this space fund? Team/Team Plus pool at the agency.
    const { account } = await resolveBillingAccount(space.id);

    // Use the FUNDING account's Stripe customer, not the space's. For a
    // Team/Team Plus space the balance pools at the agency, whose customer
    // lives on Agency.stripeCustomerId — the space's own customer is usually
    ...
    const customerTable = account.type === 'agency' ? 'Agency' : 'Space';
