...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const type = formData.get('type') as string; // 'logo' | 'photo' | 'favicon' | 'link-thumb' | 'service-photo'

    if (!file) return NextResponse.json({ error: 'No file provided' }, { status: 400 });
    if (!['logo', 'photo', 'favicon', 'link-thumb', 'service-photo'].includes(type)) {
    ...
    // Service photos can be a hair larger than profile assets (5 MB) so the
    // provider doesn't have to compress every MLS-quality JPEG before upload.
    // Other types stay capped at 2 MB.
    const sizeCap = type === 'service-photo' ? 5 * 1024 * 1024 : 2 * 1024 * 1024;
    if (file.size > sizeCap) {
      return NextResponse.json(
        { error: type === 'service-photo' ? 'File must be under 5MB' : 'File must be under 2MB' },
        { status: 400 },
      );
    }
    ...
    // Service photos go under the `service-photos/` prefix the storage-gc
    // sweeper already knows how to scan (it intersects Service.photos URLs
    ...
    // /apply, the service packet share page, and the public profile.
    const key = type === 'service-photo'
      ? buildKey(
          'servicePhotos',
          space.id,
