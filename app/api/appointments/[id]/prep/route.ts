...
/**
 * GET — Generate AI appointment prep notes for a specific appointment.
 * Combines the guest's contact info, application data, score, and appointment details
...
export async function GET(
  ...
  const { data: appointment } = await supabase.from('Appointment').select('*').eq('id', id).maybeSingle();
  if (!appointment) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const space = await getSpaceForUser(userId);
  if (!space || appointment.spaceId !== space.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  ...
  const prep: {
    ...
    appointmentDate: new Date(appointment.startsAt).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', timeZone: timezone }),
    appointmentTime: new Date(appointment.startsAt).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: timezone }),
  ...
  if (appointment.contactId) {
    const { data: contact } = await supabase.from('Contact').select('*').eq('id', appointment.contactId).maybeSingle();
    ...
    const { count } = await supabase
      ...
      .eq('contactId', appointment.contactId)
      .in('status', ['completed', 'confirmed', 'scheduled'])
      .neq('id', id);
    prep.previousAppointments = count ?? 0;
  }

  // Generate talking points based on available data
  if (appointment.serviceAddress) {
    prep.talkingPoints.push(`Confirm the guest is looking at ${appointment.serviceAddress}`);
  }
  ...
  if (prep.previousAppointments > 0) {
    prep.talkingPoints.push(`This guest has ${prep.previousAppointments} previous appointment${prep.previousAppointments > 1 ? 's' : ''} — ask what they liked/disliked`);
  }
