...
async function resolveProfile(userId: string, profileId: string) {
  const { data: row } = await supabase.from('AppointmentServiceProfile').select('*').eq('id', profileId).maybeSingle();
  if (!row) return null;
...
export async function PATCH(
  ...
  if (body.appointmentDuration !== undefined) update.appointmentDuration = body.appointmentDuration;
