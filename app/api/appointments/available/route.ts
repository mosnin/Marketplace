...
export async function GET(req: NextRequest) {
  ...
  // provider's GCal quota (~1k free-tier ops/day burnt in minutes by 100
  ...
  const { data: settings } = await supabase
    .from('SpaceSetting')
    .select('appointmentDuration, appointmentStartHour, appointmentEndHour, appointmentDaysAvailable, timezone, appointmentBufferMinutes, appointmentBlockedDates')
    .eq('spaceId', space.id)
    .maybeSingle();

  // If a service profile is specified, use its settings instead of defaults
  let duration = settings?.appointmentDuration ?? 30;
  let startHour = settings?.appointmentStartHour ?? 7;
  let endHour = settings?.appointmentEndHour ?? 17;
  let daysAvailable: number[] = settings?.appointmentDaysAvailable ?? [1, 2, 3, 4, 5];
  let bufferMinutes = settings?.appointmentBufferMinutes ?? 0;
  const timezone = settings?.timezone ?? 'America/New_York';
  const blockedDates: string[] = settings?.appointmentBlockedDates ?? [];
  ...
  // Fetch existing appointments in range (filter by service if specified)
  ...
  if (serviceId) {
    appointmentsQuery = appointmentsQuery.eq('serviceProfileId', serviceId);
  }
  const { data: existingAppointments } = await appointmentsQuery;

  const bookedSlots = (existingAppointments ?? []).map((t: any) => ({
  ...
  // Fetch overrides (single-date and recurring) scoped to this service or global
  let overridesQuery = supabase
    .from('AppointmentAvailabilityOverride')
    .select('date, isBlocked, startHour, endHour, recurrence, endDate, serviceProfileId')
    .eq('spaceId', space.id);
  ...
  for (const o of overridesRaw ?? []) {
    // Filter by service: use override if it's global (null) or matches the requested service
    if (serviceId && o.serviceProfileId && o.serviceProfileId !== serviceId) continue;
    if (!serviceId && o.serviceProfileId) continue;
  ...
  // Also fetch all active service profiles for this space (so the booking page can show them)
  const { data: profiles } = await supabase
    .from('AppointmentServiceProfile')
    .select('id, name, address, appointmentDuration, isActive')
    .eq('spaceId', space.id)
    .eq('isActive', true)
  ...
  return NextResponse.json({
    ...
    duration,
    timezone,
    serviceProfileId: serviceId ?? null,
