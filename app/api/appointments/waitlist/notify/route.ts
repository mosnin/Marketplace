...
export async function POST(req: NextRequest) {
  ...
  // Use the appointment-emails module for sending
  try {
    const { sendEmail } = await import('@/lib/appointment-waitlist-email');
    await sendEmail(entry.guestEmail, `A spot opened up — ${businessName}`, `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        ...
        <p>A appointment slot just opened up for <strong>${new Date(entry.preferredDate + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</strong> with ${businessName}.</p>
