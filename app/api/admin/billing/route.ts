 ...
 * requirePlatformAdmin(). The account is { space | agency, id }; credit ops
...
function parseAccount(type: unknown, id: unknown): BillingAccount | null {
  if ((type === 'space' || type === 'agency') && typeof id === 'string' && id) {
    return { type, id };
  }
  return null;
}
...
async function lookupPlan(acc: BillingAccount): Promise<string | null> {
  const table = acc.type === 'space' ? 'Space' : 'Agency';
...
export async function GET(req: Request) {
  ...
  if (!account) {
    return NextResponse.json({ error: 'accountType (space|agency) and accountId are required' }, { status: 400 });
  }
  try {
