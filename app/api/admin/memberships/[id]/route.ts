...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  // Unlink space from agency (best-effort)
  const { data: space } = await supabase
    ...
    .maybeSingle();
  if (space) {
    await supabase.from('Space').update({ agencyId: null }).eq('id', space.id);
  }

  const { error } = await supabase.from('AgencyMembership').delete().eq('id', id);
  if (error) {
