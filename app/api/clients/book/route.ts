...
/**
 * POST /api/clients/book — portal appointment booking. A thin proxy over the existing
 * public /api/appointments/book endpoint: it forces guestEmail/guestName/guestPhone
 * from the verified session (so a client can only book under their own email)
 * and restricts the provider to one the client is already engaged with. The
...
export async function POST(req: NextRequest) {
  ...
  // Only let the client book with a provider they already have a relationship
  // with (an application or appointment on their verified email).
  const { applications, appointments } = await getClientPortalData(user.email);
  const engagedSlugs = new Set(
    [...applications.map((a) => a.providerSlug), ...appointments.map((t) => t.providerSlug)].filter(
  ...
  const res = await fetch(`${origin}/api/appointments/book`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      serviceAddress: body.serviceAddress?.slice(0, 500) || undefined,
