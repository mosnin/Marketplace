 ...
 * Marks the provider's messages as read on fetch.
 */
export async function GET(req: NextRequest) {
  ...
  // Mark provider → client messages read now that the client has loaded them.
  await supabase
    .from('ClientMessage')
    .update({ readAt: new Date().toISOString() })
    .eq('contactId', contactId)
    .eq('senderType', 'provider')
    .is('readAt', null);
 ...
 * Optionally notifies the provider by email (best-effort).
 */
export async function POST(req: NextRequest) {
  ...
  // Resolve the contact's space (needed for the row + provider lookup).
  ...
  // Best-effort provider notification. Resolve the owner's email via User.
