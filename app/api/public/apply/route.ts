 ...
 *   3. Agency dual: Agency.[agency[Rental|Buyer]FormConfig]
 *   4. Agency legacy: Agency.agencyFormConfig (if leadType matches)
...
async function fetchFormConfigForLeadType(
  ...
  try {
    ...
    // Fall back to agency-level config
    if (agencyId) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      const rawConfig = await fetchFormConfigForLeadType(space.id, space.agencyId, resolvedLeadType);
      if (rawConfig) {
    ...
    // Send provider notification + applicant confirmation email in parallel
    const businessName = spaceBusinessName || space.name;

    const providerNotification = notifyNewLead({
      spaceId: space.id,
      contactId: contact.id,
      name: contactName,
      ...
      logger.error('[apply] provider notification failed', { contactId: contact.id }, notifyErr);
    });
    ...
    await Promise.all([providerNotification, applicantConfirmation]);
    ...
    // time (drafts a follow-up, scores against the provider's criteria, etc.)
