...
import { notificationForNewAgencyLead } from '@/lib/notification-voice';
...
/**
 * Agency-specific intake schema.
 * Same as the regular application schema but uses `agencyId` instead of `slug`.
 */
const agencyApplicationSchema = publicApplicationSchema
  .omit({ slug: true })
  .extend({
 ...
 *   2. Agency legacy: agencyFormConfig (if leadType matches)
 ...
 */
async function fetchAgencyFormConfig(
  agencyId: string,
  spaceId: string,
  leadType: 'rental' | 'buyer',
): Promise<IntakeFormConfig | null> {
  try {
    // First try agency-level configs directly
    ...
    if (agency) {
      ...
      const dualRaw = leadType === 'buyer'
        ? agency.agencyBuyerFormConfig
        : agency.agencyRentalFormConfig;
      if (dualRaw) {
      ...
      if (agency.agencyFormConfig) {
        const parsed = formConfigSchema.safeParse(agency.agencyFormConfig);
        if (parsed.success) {
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await checkRateLimit(`apply-agency:rl:${ip}`, 10, 3600);
  if (!allowed) {
  ...
  // Extract agencyId and leadType from raw body before validation
  const rawBody = typeof requestBody === 'object' && requestBody !== null
    ? (requestBody as Record<string, unknown>)
    : {};
  const rawAgencyId = rawBody.agencyId;
  if (!rawAgencyId || typeof rawAgencyId !== 'string') {
  ...
  try {
    ...
    // ── Find the agency-linked Space owned by the agency owner ──────────
    let space: { id: string; slug: string; name: string; ownerId: string; agencyId: string | null } | null = null;

    const { data: linkedSpace, error: spaceError } = await supabase
      .from('Space')
      .select('id, slug, name, ownerId, agencyId')
    ...
    try {
      formConfig = await fetchAgencyFormConfig(agency.id, space.id, resolvedLeadType);
      if (formConfig) {
    ...
    if (formConfig) {
      ...
      contactPreferences = typeof data.serviceAddress === 'string' ? data.serviceAddress : null;
      ...
      contactPreferences = payload.serviceAddress ?? null;
      ...
      if (payload.serviceAddress) noteParts.push(`Service: ${payload.serviceAddress}`);
    ...
    const fingerprint = applicationFingerprintKey({
      ...
      phone: contactPhone,
      email: contactEmail,
      slug: `agency:${agency.id}`,
    });
    const idempotencyKey = `apply-agency:idempotency:${fingerprint}`;
    ...
    // exists, insert into their space; otherwise fall back to the agency
    // owner's space (current behaviour). `routeAgencyLead` never throws
    ...
    // Send agency dashboard notification + applicant confirmation email in parallel
    const businessName = spaceBusinessName || agency.name || space.name;

    const agencyLeadCopy = notificationForNewAgencyLead(contactName, {
      phone: contactPhone,
      email: contactEmail,
    });
    const agencyNotification = notifyAgency({
      agencyId: agency.id,
      type: 'lead_hot',
      title: agencyLeadCopy.title,
      body: agencyLeadCopy.description,
      metadata: {
        contactId: contact.id,
        leadScore: scoring.leadScore,
        scoreLabel: scoring.scoreLabel,
