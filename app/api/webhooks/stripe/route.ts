 ...
 *
 * Returns one of:
 *   'ok'       — safe to write (either customers match, or agency has none)
 *   'missing'  — agency row doesn't exist (orphaned subscription)
 *   'mismatch' — customer IDs don't match; treat as handled but DO NOT write
 *
 * Every handler that writes to Agency based on subscription.metadata
 ...
 */
async function verifyAgencyOwnsSubscription(
  agencyId: string,
  ...
  if (!existing) {
    logger.warn('[stripe-webhook] subscription references missing agency — ignoring', {
      agencyId,
  ...
  if (
    ...
    logger.error(
      '[stripe-webhook] agencyId metadata mismatch — agency belongs to different customer',
      {
        agencyId,
        agencyCustomer: existing.stripeCustomerId,
        webhookCustomer,
        subscriptionId: subscription.id,
      },
    );
...
async function updateAgencyFromSubscription(
  agencyId: string,
  subscription: Stripe.Subscription,
  opts: { customerId?: string | null; includePlanFromMetadata?: boolean } = {},
): Promise<boolean> {
  const guard = await verifyAgencyOwnsSubscription(
    agencyId,
    subscription,
    opts.customerId,
  );
  ...
  if (error) {
    logger.error('[stripe-webhook] failed to update Agency', {
      agencyId,
      subscriptionId: subscription.id,
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (topupId && topupId in TOPUPS) {
          ...
          if (acctId && (acctType === 'space' || acctType === 'agency')) {
            ...
            const acctTable = acctType === 'space' ? 'Space' : 'Agency';
        ...
        // Agency path: metadata.agencyId may live on the session or the subscription
        const agencyId =
          session.metadata?.agencyId ?? subscription.metadata?.agencyId;
        if (agencyId) {
          await updateAgencyFromSubscription(agencyId, subscription, {
      ...
      case 'customer.subscription.deleted': {
        ...
        // Agency path: mark canceled but preserve subscription id + seatLimit
        ...
        // can set metadata.agencyId on their OWN subscription could cancel
        // a victim agency simply by deleting their sub. (Audit-driven fix.)
        const agencyId = subscription.metadata?.agencyId;
        if (agencyId) {
          const guard = await verifyAgencyOwnsSubscription(agencyId, subscription);
          ...
          const { error } = await supabase
            ...
            .eq('id', agencyId);
          if (error) {
            logger.error('[stripe-webhook] failed to mark agency canceled', {
              agencyId,
              subscriptionId: subscription.id,
              dbError: error.message,
            });
      ...
      case 'invoice.payment_succeeded': {
        ...
        // Agency path
        const agencyId = paidSub.metadata?.agencyId;
        if (agencyId) {
          await updateAgencyFromSubscription(agencyId, paidSub, {
            includePlanFromMetadata: true,
          });
          ...
          if (grantableInvoice) {
            try {
              await grantPlanMonthly({ type: 'agency', id: agencyId }, livePlan ?? paidSub.metadata?.plan ?? '', invoice.id);
            } catch (e) {
              logger.error('[stripe-webhook] agency monthly grant failed', { agencyId }, e);
            }
          }
          break;
        }
      ...
      case 'customer.subscription.trial_will_end': {
        ...
        // Agency subscriptions don't email via the Space-owner notifier;
        // skip notification for agency-scoped trials (owners see dashboard state).
        if (trialSub.metadata?.agencyId) break;
      ...
      case 'invoice.payment_failed': {
        ...
        // Fetch live subscription to branch on metadata.agencyId
        const failedSub = await stripe.subscriptions.retrieve(subId);
        const agencyId = failedSub.metadata?.agencyId;
        if (agencyId) {
          // Same metadata-poisoning guard as subscription.deleted.
          const guard = await verifyAgencyOwnsSubscription(agencyId, failedSub);
          ...
          const { error } = await supabase
            ...
            .eq('id', agencyId);
          if (error) {
            logger.error('[stripe-webhook] failed to mark agency past_due', {
              agencyId,
