 ...
 *
 * The provider's email, channel-pure (no WhatsApp). Mirrors the calendar
 ...
 * a screen, small enough that the provider isn't waiting on a 200-row
...
export interface EmailListItem {
  ...
  fromName: string;
  fromAddress: string;
  /** Provider's own send: shown in the Sent filter; renders "to <recipient>" */
  toName: string;
  toAddress: string;
  subject: string | null;
  snippet: string;
