...
const roleLabel = (role: string) =>
  role === 'agency_admin' ? 'Agency Admin' : 'Provider';
...
export default async function AcceptInvitationPage({ params }: Params) {
  ...
  try {
    ...
    if (error) {
      ...
      const agency = data.Agency as unknown as { name: string; logoUrl: string | null } | null;
      inv = {
        id: data.id,
        status: data.status,
        email: data.email,
          ...
          <div className="bg-foreground px-6 py-5">
            <div className="flex items-center gap-2.5">
              ...
              <p className="text-background font-semibold text-base">
                {inv?.agencyName ?? 'Koala'}
              </p>
            </div>
            <p className="mt-1 text-background/60 text-sm">Agency invitation</p>
          </div>
          ...
          <div className="px-6 py-6 space-y-4">
            {fetchError ? (
              ...
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400">
                <AlertTriangle size={16} className="flex-shrink-0 mt-0.5" />
                <p className="text-sm">This invitation is no longer valid. Ask your agency to send a new one.</p>
              </div>
            ) : inv ? (
              <>
                ...
                <div>
                  <p className="text-sm text-foreground leading-relaxed">
                    ...
                    <span className="font-semibold">{inv.agencyName}</span> as a{' '}
                    <span className="font-semibold">{roleLabel(inv.roleToAssign)}</span>.
                  </p>
                  <p className="text-xs text-muted-foreground mt-2">
                    {inv.roleToAssign === 'agency_admin'
                      ? 'You\'ll get access to the agency dashboard to help manage the team. No subscription required.'
