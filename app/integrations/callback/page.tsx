/**
 * /integrations/callback — where Composio redirects the provider after a
 ...
 *   3. Redirect the provider back into the app — settings, ideally onto
 *      the integrations panel — with a success/error flag in the URL.
 *
 * Server component so the persistence happens before the provider ever
 * sees a UI flash.
 *
 ...
 * step failed when a provider reports "I connected at the provider but the
...
export default async function IntegrationsCallback({
  ...
  // Resolve the provider's space — we won't ship without it.
  ...
  // account fetch itself failed, the provider still landed back here via the
  ...
  // (the upsert above already wrote that) and the provider lands back on
  ...
  // creates failed IntegrationTrigger rows the provider would have to
...
/**
 * Pull a provider-friendly label from Composio's account payload — usually
