...
import { ProvidersClient, type ProviderRow } from './providers-client';

export const metadata: Metadata = { title: 'Providers — Teams' };

export default async function AgencyProvidersPage() {
  const ctx = await getAgencyContext();
  if (!ctx) redirect('/');

  const { agency } = ctx;

  const members = await getAgencyMembers(agency.id, { includeOnboard: true, includeSpaceName: true });

  const spaceIds = members.map((m) => m.Space?.id).filter(Boolean) as string[];

  // 7-day response-time window. Mirrors agent/tools/agency/performance.py
  // (which uses 30 days for the deeper Koala audit); the inline row pill
  ...
  // Pull the full health picture per provider in one parallel volley:
  ...
  //   - unworkedRows      → agency-assigned leads with no first contact
  //   - slaNudgedRows     → leads that have hit the SLA nudge threshold
  //   - slaEscalatedRows  → leads that have escalated to the agency
  ...
  const [
    ...
    spaceIds.length > 0
      ? supabase
          ...
          .then((r) => r.data ?? [])
      : Promise.resolve([]),
    // Un-worked agency-assigned leads: assigned but lastContactedAt IS NULL
    spaceIds.length > 0
      ? supabase
          .from('Contact')
          .select('spaceId')
  ...
  // ContactActivity instead of N per-space queries — the agency page already
  ...
  // Team median across ALL response samples — same definition as the Koala
  ...
  // Band multipliers — match agent/tools/agency/performance.py:48-49 so
  // Koala's narrative answer ("Alice is slow") and the row pill agree.
  const FAST_MULTIPLIER = 0.5;
  const SLOW_MULTIPLIER = 2.0;
  function bandFor(avgHours: number | null): ProviderRow['responseBand'] {
  ...
  const providers: ProviderRow[] = members.map((m) => {
  ...
  const subtitle = (() => {
    if (providers.length === 0) {
      return 'No providers yet. Send the first invite.';
    }
    const active = providers.filter((r) => r.onboard);
    if (active.length === 0) {
      return `${providers.length} invited. Nobody onboard yet.`;
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-56 md:pb-24">
      <header className="space-y-1.5">
        <p className={cn(BODY_MUTED)}>Providers.</p>
      ...
      {members.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
          <p className="text-sm text-foreground">No providers yet.</p>
