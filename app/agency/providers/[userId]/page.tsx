...
export const metadata: Metadata = { title: 'Provider Detail — Teams' };

type Params = { params: Promise<{ userId: string }> };

export default async function ProviderDrilldownPage({ params }: Params) {
  ...
  if (!membership) redirect('/agency/providers');
  ...
  if (!user) redirect('/agency/providers');
  ...
  const roleLabel = membership.role === 'agency_owner' ? 'Owner' : membership.role === 'agency_admin' ? 'Admin' : 'Provider';
      ...
      {!space ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            This provider hasn&apos;t set up their workspace.
          </p>
        </div>
      ) : (
