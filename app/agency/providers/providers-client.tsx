...
export interface ProviderRow {
  ...
  openLeads: number;
  /** Agency-assigned leads with no first contact (un-worked). */
  unworked: number;
  ...
  /** Band relative to team median (mirrors agent/tools/agency/performance.py). */
...
// Health sort order — needs-attention surfaces first so the agency acts.
const HEALTH_ORDER: Record<ProviderRow['health'], number> = {
  'needs-attention': 0,
  'on-track':        1,
  'top-performer':   2,
  'pending':         3,
...
const roleLabel = (role: string) =>
  role === 'agency_owner' ? 'Owner' : role === 'agency_admin' ? 'Admin' : 'Provider';
...
function HealthChip({ health }: { health: ProviderRow['health'] }) {
  const base =
...
function HealthNumbers({ r }: { r: ProviderRow }) {
...
// ── Coach prompt ─────────────────────────────────────────────────────────────
// One-line health summary for the Koala prefill. Calm, factual, no em dashes.
function coachHealthSummary(r: ProviderRow): string {
...
function buildCoachHref(r: ProviderRow): string {
  ...
  return `/agency?prompt=${encodeURIComponent(prompt)}`;
}
          ...
          {sorted.map((r) => {
            ...
            return (
              ...
              >
                ...
                <Link
                  href={`/agency/providers/${r.userId}`}
                ...
                <Link
                  ...
                  aria-label={`Coach ${displayName} with Koala`}
