...
export function PipelineClient({ deals, stages, providers, summary }: Props) {
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter((d) => {
      ...
      if (providerFilter !== 'all' && d.agentUserId !== providerFilter) return false;
      return true;
    });
  }, [deals, statusFilter, providerFilter]);
  ...
  return (
    <div className="space-y-10">
      ...
      {/* the agency home commission grid: gap-px on a border-bg makes  */}
      ...
      <section className="space-y-4">
        ...
        {agentGroups.length === 0 ? (
          <DashedEmpty>No providers yet.</DashedEmpty>
        ) : (
          <ul className="divide-y divide-border/60">
            {agentGroups.map((agent) => {
              const isExpanded = expandedProviders.has(agent.userId);
              ...
              return (
                <li key={agent.userId}>
                  <button
                    onClick={() => toggleProvider(agent.userId)}
...
/**
 * Canonical dashed empty container — matches the agency home placeholder
 * vocabulary. Keep the copy first-person so Koala stays the narrator.
 */
