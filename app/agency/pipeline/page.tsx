...
export default async function AgencyPipelinePage() {
  ...
  // Get all provider members with their spaces
  const allMembers = await getAgencyMembers(agency.id, { includeSpaceName: true });
  const members = allMembers.filter((m) => m.role === 'provider_member');

  const spaceIds = members.map((m) => m.Space?.id).filter(Boolean) as string[];

  // Build space -> provider lookup
  const spaceToProvider = new Map<string, { userId: string; name: string }>();
  const providers: ProviderInfo[] = [];
  for (const m of members) {
    const name = m.User?.name ?? m.User?.email ?? 'Unknown';
    if (m.Space?.id) {
      spaceToProvider.set(m.Space?.id, { userId: m.userId, name });
    }
    providers.push({ userId: m.userId, name });
  }

  // Fetch all deals across provider spaces. The health classifier needs
  ...
  // Fetch all deal stages across provider spaces
  ...
  // deals" at a glance without the agency pivoting through every card.
  ...
  for (const d of (dealsRaw ?? []) as DealRow[]) {
    const provider = spaceToProvider.get(d.spaceId);
    ...
    const deal: PipelineDeal = {
      ...
      agentName: provider?.name ?? 'Unknown',
      agentUserId: provider?.userId ?? '',
    ...
    if (health.state === 'at-risk' || health.state === 'stuck') {
      ...
      const key = provider?.userId ?? 'unknown';
