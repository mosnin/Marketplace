...
export interface ResponseShape {
  ...
  agency: { totals: ProviderRollup['totals']; providerCount: number };
}
...
export function AgentActivityClient({ initial, agencyName }: Props) {
  void agencyName;
  ...
  function selectWindow(next: Window) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`/api/agency/agent-activity?days=${next}`, { cache: 'no-store' });
  ...
  const headlineNumber = data.agency.totals.all;
  const providerCount = data.agency.providerCount;
  ...
  // provider's side. Routes collapse into one bucket so we don't overweight
  // agency-only signal in the headline strip.
  const headlineCells = useMemo(() => {
    const t = data.agency.totals;
    return [
      { label: 'Drafts', value: t.drafts, icon: FilePen },
      { label: 'Appointments', value: t.appointments, icon: CalendarCheck },
    ...
    <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              {headlineNumber === 0
                ? <>Nothing yet {periodCopy(windowDays)}. Koala takes a few days to warm up on a new team.</>
                : <>actions {periodCopy(windowDays)} across {providerCount} {providerCount === 1 ? 'provider' : 'providers'}.</>
              }
            </p>
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3 pb-3 border-b border-border/60">
          ...
          {providerCount > 0 && (
            <span className="text-[11px] text-muted-foreground tabular-nums">{providerCount}</span>
          )}
          {pending && (
        ...
        {data.providers.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invite a provider and Koala&apos;s work will start landing here.
            </p>
          </div>
        ) : (
          <AnimatePresence mode="wait" initial={false}>
            <StaggerList key={`list-${windowDays}`} className="divide-y divide-border/60">
              {data.providers.map((r) => (
                <StaggerItem key={r.spaceId}>
                  <ProviderRow row={r} maxAll={data.providers[0]?.totals.all ?? 0} />
                </StaggerItem>
              ))}
            </StaggerList>
          </AnimatePresence>
...
function ProviderRow({ row, maxAll }: { row: ProviderRollup; maxAll: number }) {
  const name = row.name ?? row.email ?? 'Unnamed';
  const t = row.totals;
  // Visual weight: a thin horizontal bar inside the row whose length is
  // proportional to total activity vs. the busiest provider in the window.
  ...
  return (
    <div className="group/row flex items-start gap-3 py-3">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        {/* Activity bar — proportional to the busiest provider */}
