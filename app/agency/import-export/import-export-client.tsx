...
export default function ImportExportClient({ totalLeads }: { totalLeads: number }) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/agency/leads/import', {
        method: 'POST',
        body: formData,
      });
  ...
  const subtitle = totalLeads
    ? `${totalLeads.toLocaleString()} ${totalLeads === 1 ? 'lead' : 'leads'} across the agency.`
...
{`Name,Email,Phone,Lead Type,Budget,Service Address,Notes,Move-in Date,Assign To
