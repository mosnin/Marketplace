...
import { RemoveMemberButton } from '@/components/agency/remove-member-button';
import { ChangeRoleButton } from '@/components/agency/change-role-button';
import { OffboardMemberDialog } from '@/components/agency/offboard-member-dialog';
...
export function MembersClient({
  ...
  // One-sentence status read of the team. Loud fact wins. Agency name
  // anchors the empty case so the page still reads as "your team."
  const subtitle = (() => {
    if (members.length === 0) return `Nobody on the team at ${agencyName} yet.`;
    if (pendingCount > 0) {
              ...
              {filtered.map((m) => {
                const canManage =
                  ...
                  (currentUserRole === 'agency_owner' ||
                    (currentUserRole === 'agency_admin' && m.role === 'provider_member'));
