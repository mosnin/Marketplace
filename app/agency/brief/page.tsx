...
import { SECTION_LABEL, TITLE_FONT, KOALA_PILL, CAPTION, META, STAT_NUMBER_COMPACT } from '@/lib/typography';
import { cn } from '@/lib/utils';
import { dealHealth } from '@/lib/deals/health';
import { TeamActivityFeed } from '@/components/agency/team-activity-feed';
import { AgencyMorningStory } from '@/components/agency/agency-morning-story';
import { DraftImpactCard } from '@/components/agency/draft-impact-card';
import {
  aggregateDraftStats,
...
// ── Revenue forecast helpers (mirrors app/agency/forecast/page.tsx) ──────────
//
...
export default async function AgencyBriefPage() {
  const ctx = await getAgencyMemberContext();
  if (!ctx) redirect('/');

  // provider_member sees their own work surface, not the swarm.
  if (ctx.membership.role === 'provider_member') {
    return <MemberDashboard ctx={ctx} />;
  }

  const { agency } = ctx;
  const members = await getAgencyMembers(ctx.agency.id, {
    includeOnboard: true,
  ...
  // Make sure the agency owner's space is included — agency leads land
  ...
  const [
    ...
    draftStatsRows,
      ...
      // Same shape `aggregateDraftStats` expects; agency-wide rollup.
      spaceIds.length > 0
        ? supabase
            .from('AgentDraft')
  ...
  // updatedAt falls in the window. Mirrors `agent/tools/agency/revenue.py`
  ...
  // Top provider MTD — owner name keyed off the spaceId → space.ownerId map
  // from the `members` rows we already fetched. We only label the cell when
  // someone actually earned non-zero GCI this month.
  let topProviderMtd: { name: string; amount: number } | null = null;
  ...
  if (topSpaceId) {
    ...
    const name = owner?.User?.name?.split(' ')[0] ?? owner?.User?.name ?? 'Provider';
    topProviderMtd = { name, amount: topAmount };
  }

  // ── Revenue forecast (brief read) ─────────────────────────────────────────
  // Reuses the forecast model from app/agency/forecast/page.tsx.
  // Won GCI: same MTD window + defaultAgencyRate fallback already computed above.
  // Active GCI: weighted by closingProbability × dealHealth — one extra query.

  const b = agency as unknown as { defaultAgencyRate?: number | null };
  const defaultAgencyRate =
    typeof b.defaultAgencyRate === 'number' ? b.defaultAgencyRate : 0.02;
  ...
  for (const d of wonYtdRows) {
    const updatedMs = new Date(d.updatedAt).getTime();
    if (updatedMs >= mtdStartMs) {
      const rate = d.commissionRate != null ? d.commissionRate / 100 : defaultAgencyRate;
  ...
  // Two numbers: leads currently breaching (needs action) + leads Koala
  // escalated to the agency today (proof of work).
  let slaEnabled = false;
  let needsResponse = 0;
  let escalatedToday = 0;
  if (agency.slaEnabled && spaceIds.length > 0) {
    slaEnabled = true;
    const todayStart = new Date(
      Date.UTC(nowDate.getUTCFullYear(), nowDate.getUTCMonth(), nowDate.getUTCDate()),
    ).toISOString();
    const firstThreshold = new Date(
      Date.now() - agency.slaFirstResponseMinutes * 60000,
    ).toISOString();
    ...
    const [needsRes, escalatedRes] = await Promise.all([
      ...
      // Escalated today: AgencyNotification rows Koala created today for this agency.
      supabase
  ...
  const hasSettings = !!(agency.name && (agency.logoUrl || agency.websiteUrl));

  // Sort members by current activity load (active drafts + new leads + active deals)
  // so the agency scans the busiest providers first. Owner row stays pinned to top.
  const memberRows = members
    ...
    .sort((a, b) => {
      ...
      if (a.member.role === 'agency_owner' && b.member.role !== 'agency_owner') return -1;
      if (b.member.role === 'agency_owner' && a.member.role !== 'agency_owner') return 1;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-12">
      {/* Header — canonical three-line pattern. Muted greeting (with the
          team name as the calm anchor), serif Times h1 from AgencyMorningStory
          carrying Koala's chief-of-staff sentence, and the small hairline
      ...
      {/* Koala — the focal entry. The agency's chief of staff is the home of
          this page, not its fourth section. A calm, paper-flat doorway: serif
          name, one line of what it does, a quiet pill into /agency/koala.
          No orange wash, no shadow — the surface stays out of the way and the
          invitation is the loud note. */}
      <Link
        href="/agency"
        className="group/koala block rounded-xl border border-border/70 bg-card px-5 py-5 hover:bg-muted/30 transition-colors"
      >
        <div className="flex items-center gap-4">
          ...
          <span className={cn(KOALA_PILL, 'flex-shrink-0')}>
            Open
      ...
      {/* First-run nudge — only when the team hasn't been set up yet.
          Neutral icon: this is a settings affordance, not Koala speaking,
      ...
      {/* Snapshot — three numbers the agency actually cares about.
      ...
      {/* Commission — money is the loudest signal. Hairline-divider grid
          ...
          drills into /agency/commissions for the per-deal breakdown. */}
      <section>
        <Link
        ...
        <div className="grid grid-cols-4 gap-px bg-border/70 rounded-xl overflow-hidden border border-border/70 mt-4">
          ...
          <div className="bg-background p-4">
            <p className={SECTION_LABEL}>Top provider (MTD)</p>
            <p
              className="text-2xl tracking-tight mt-1.5 text-foreground truncate"
              style={TITLE_FONT}
            >
              {topProviderMtd ? topProviderMtd.name : <span className="text-muted-foreground">—</span>}
            </p>
            {topProviderMtd && (
              <p className="text-[11px] text-muted-foreground tabular-nums mt-1">
                {formatCompact(topProviderMtd.amount)}
              </p>
            )}
          </div>
          <div className="bg-background p-4">
      ...
      {/* Revenue — projected GCI for the month at a glance.
          ...
          deal to watch. Computation mirrors app/agency/forecast/page.tsx. */}
      <section>
        <Link
          href="/agency/forecast"
              ...
              <>
                ...
                {escalatedToday > 0 && (
                  <span className="text-muted-foreground">
                    {' '}Koala escalated {escalatedToday} {escalatedToday === 1 ? 'lead' : 'leads'} to you today.
                  </span>
                )}
              </>
            ) : escalatedToday > 0 ? (
              <span className="text-muted-foreground">
                Every lead has been answered. Koala escalated {escalatedToday} {escalatedToday === 1 ? 'lead' : 'leads'} to you today.
              </span>
            ) : (
        ...
        <section>
          ...
          <ul className="divide-y divide-border/60">
            {pendingInvitations.map((inv) => {
              const role =
                inv.roleToAssign === 'agency_owner'
                  ? 'Owner'
                  : inv.roleToAssign === 'agency_admin'
                    ? 'Admin'
                    : 'Provider';
              return (
        ...
        {memberRows.length === 0 ? (
          <div className="mt-4 rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invite your first provider and their work will land in this view.
            </p>
          </div>
        ) : (
          <ul className="divide-y divide-border/60">
            {memberRows.map(({ member, leads, deals, drafts, apps }) => {
              ...
              const href = member.userId ? `/agency/providers/${member.userId}` : '#';
              const role =
      ...
      {/* Draft impact — how Koala's drafts have actually been landing across
          the team over the last 30 days. Sits below the team list so the
          provider row remains the focal element of the page. */}
