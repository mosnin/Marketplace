...
export const metadata: Metadata = { title: 'Invitations — Agency Dashboard' };
...
const roleLabel = (role: string) =>
  role === 'agency_admin' ? 'Admin' : 'Provider';

// Role pill — small caps, muted bg. Matches the Members + Providers rows so
...
export default async function AgencyInvitationsPage() {
  ...
  const subtitle = (() => {
    if (invs.length === 0)
      return `Nobody invited to ${ctx.agency.name} yet.`;
    if (pendingCount > 0) {
  ...
  return (
    <div className="space-y-6 max-w-3xl pb-56 md:pb-24">
      ...
      <InviteCodeCard isOwner={ctx.membership.role === 'agency_owner'} />

      <Card>
        <CardContent className="px-5 py-4 space-y-3">
          <p className="text-sm font-medium">Send an email invite</p>
          <InviteForm
            isOwner={ctx.membership.role === 'agency_owner'}
            seatUsage={seatUsage}
          />
        </CardContent>
      </Card>
