...
type AgencyContact = {
  ...
  email: string | null;
  phone: string | null;
  type: 'QUALIFICATION' | 'APPOINTMENT' | 'APPLICATION';
...
/**
 * Lead-score → tier. Mirrors contact-table.tsx and agency-leads-client.tsx.
...
export function AgencyPeopleTable() {
  ...
  const fetchContacts = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/agency/contacts?${params}`);
      if (!res.ok) {
        setError(true);
        return;
      }
      ...
      console.error('[agency-people-table] fetch failed:', err);
      setError(true);
    } finally {
      setLoading(false);
    }
  ...
  const subtitle = (() => {
    ...
    if (newThisWeekCount > 0) {
      return `${contacts.length} ${noun} across the agency. ${newThisWeekCount} new this week.`;
    }
    return `${contacts.length} ${noun} across the agency.`;
  })();
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-1.5">
        ...
        <h1 className={H1} style={TITLE_FONT}>
          Everyone your agency is working with
        </h1>
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44">
                {(['ALL', 'QUALIFICATION', 'APPOINTMENT', 'APPLICATION'] as const).map((key) => (
                  <DropdownMenuItem
                    key={key}
        ...
        if (!search && !hasAnyFilter && contacts.length === 0) {
          return (
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
              <p className="text-base text-foreground">No contacts across the agency yet.</p>
              <p className={cn(BODY_MUTED, 'mt-1.5')}>
                Contacts will appear here as your providers add them.
              </p>
            </div>
          );
        }
...
// agency-only addition: a Provider byline under the contact's name/email line.
// Row click drills into /agency?prompt=... (Koala prefilled about this lead).

function AgencyContactRow({
  contact,
  idx,
}: {
  ...
  const drillPrompt = encodeURIComponent(
    `Tell me about ${contact.name} (${contact.providerName}'s lead). Where are they in the funnel and what's the next move?`
  );
  ...
  return (
    ...
    >
      <Link href={`/agency?prompt=${drillPrompt}`} className={rowClassName}>
        {/* Avatar */}
        ...
        {/* Name + stage pill + email/phone + provider byline */}
        <div className="min-w-0 flex-1">
          ...
          {/* Provider byline — the agency-only addition */}
