...
/**
 * AnalyticsClient — interactive funnel + table breakdown for the agency
 * analytics page.
 *
  ...
  return (
    <section className="rounded-xl border border-border/70 bg-background px-4 py-4 space-y-3">
      ...
      <div className="space-y-1.5 pt-1">
        ...
        <StepRate pct={agent.leadToAppointment} />
        <FunnelBar label="Appointments" value={agent.appointments} maxValue={max} />
        <StepRate pct={agent.appointmentToApp} />
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-border/60">
        <div className="text-center">
          <p className="text-xs tabular-nums font-medium text-foreground">{agent.leadToAppointment}%</p>
          <p className={cn(CAPTION, 'mt-0.5')}>Lead to appointment</p>
        </div>
        <div className="text-center">
          <p className="text-xs tabular-nums font-medium text-foreground">{agent.appointmentToApp}%</p>
          <p className={cn(CAPTION, 'mt-0.5')}>Appointment to app</p>
        </div>
        <div className="text-center">
...
export function AnalyticsClient({ agents }: Props) {
  ...
  const teamTotals = useMemo(() => {
    const t = agents.reduce(
      ...
      { totalLeads: 0, appointments: 0, applications: 0, totalDeals: 0, wonDeals: 0, wonValue: 0 },
    );
    return {
      ...t,
      leadToAppointment: t.totalLeads > 0 ? Math.round((t.appointments / t.totalLeads) * 100) : 0,
      appointmentToApp: t.appointments > 0 ? Math.round((t.applications / t.appointments) * 100) : 0,
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border/70 bg-background px-4 sm:px-5 py-5 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <StepRate pct={teamTotals.leadToAppointment} />
          <FunnelBar label="Appointments" value={teamTotals.appointments} maxValue={teamTotals.totalLeads} />
          <StepRate pct={teamTotals.appointmentToApp} />
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {sorted.map((a) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{a.appointments}</td>
                  ...
                  {/* Rate columns */}
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{a.leadToAppointment}%</td>
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground hidden md:table-cell">{a.appointmentToApp}%</td>
              ...
              {sorted.length > 0 && (
                <tr className="bg-foreground/[0.04]">
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{teamTotals.appointments}</td>
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{teamTotals.leadToAppointment}%</td>
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground hidden md:table-cell">{teamTotals.appointmentToApp}%</td>
