/**
 * /agency/analytics -- Agency conversion funnel + per-agent breakdown.
 *
 * Shows the rental-funnel metrics aggregated across all member spaces.
 * Gated with resolveAgencyContext() (agency_owner + agency_admin only).
 *
 * Structure:
...
export const metadata: Metadata = { title: 'Analytics -- Agency' };
...
export default async function AgencyAnalyticsPage() {
  // Gate: agency_owner + agency_admin only.
  ...
  for (const c of contacts) {
    ...
    else if (t === 'APPOINTMENT') s.appointment++;
  ...
  const agentData: AgentFunnelData[] = members
    ...
    .map((m) => {
      ...
      return {
        ...
        totalDeals,
        wonValue: s.wonValue,
        leadToAppointment: s.totalLeads > 0 ? Math.round((s.appointment / s.totalLeads) * 100) : 0,
        appointmentToApp: s.appointment > 0 ? Math.round((s.application / s.appointment) * 100) : 0,
