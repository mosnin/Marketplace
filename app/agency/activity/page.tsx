...
// agency, then hand off to the client for filter/pagination. Mirrors the
// pattern used by app/agency/reviews/page.tsx — use getAgencyContext (not
// requireAgency) so non-agencies get a clean redirect instead of a 500.
export default async function AgencyActivityPage() {
  const ctx = await getAgencyContext();
  if (!ctx) redirect('/');

  // 1. Resolve agency spaces.
  ...
  //    agency-tagged null-space rows, merged and trimmed. See the route
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <header className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          Every action across your agency, newest first.
        </p>
      </header>
      <ActivityClient
