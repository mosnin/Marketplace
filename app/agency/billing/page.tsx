...
export const metadata: Metadata = { title: 'Billing — Teams — Koala' };
...
export default async function AgencyBillingPage() {
  ...
  // If user has no personal space, find the agency owner's space
  ...
  const usingAgencyEntity = Boolean(agencyStripe?.stripeSubscriptionId);

  if (!spaceRow && !usingAgencyEntity) {
    return (
  ...
  // Only the agency_owner can manage billing. Others see a read-only message.
  const isAgencyOwner = ctx.membership.role === 'agency_owner';
  if (!isAgencyOwner) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto pb-12">
        <BillingHeader status="The agency owner manages billing." />
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          <p className="text-sm text-foreground">Billing lives with the owner.</p>
          <p className="text-xs text-muted-foreground mt-1">
            Ask the agency owner to make changes to the subscription.
          </p>
        </div>
      </div>
    );
  ...
  const billingCustomerId = usingAgencyEntity
    ? ((agencyStripe!.stripeCustomerId as string | null) ?? null)
    : ((spaceRow?.stripeCustomerId as string | null) ?? null);

  const slug = (spaceRow?.slug as string | undefined) ?? '';
  let subscriptionStatus: 'active' | 'trialing' | 'past_due' | 'canceled' | 'inactive' = usingAgencyEntity
    ? mapDbStatus(agencyStripe?.stripeSubscriptionStatus)
  ...
  if (billingSubscriptionId) {
    try {
      ...
      console.error('[agency/billing] Stripe fetch failed:', err);
      stripeError = true;
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      ...
      <BillingPage
        slug={slug}
        plan={(((ctx.agency as { plan?: string }).plan) ?? 'free') as PlanId}
