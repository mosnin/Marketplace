...
export default function RulesClient({
  ...
  const memberById = useMemo(() => {
    const m = new Map<string, AgencyMember>();
  ...
  const reloadList = useCallback(async () => {
    try {
      const res = await fetch('/api/agency/routing-rules', { cache: 'no-store' });
      if (res.ok) {
  ...
  const handleToggleEnabled = useCallback(
    async (rule: DealRoutingRuleRow) => {
      ...
      try {
        const res = await fetch(`/api/agency/routing-rules/${rule.id}`, {
          method: 'PATCH',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/agency/routing-rules/${mode.rule.id}` : '/api/agency/routing-rules',
        {
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <div>
                  ...
                  <p className={CAPTION}>
                    Uses the provider pool. Tag narrowing is ignored until the membership tags column ships.
                  </p>
                </div>
              </label>
            </div>
            ...
            {form.destinationKind === 'agent' ? (
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {members.length === 0 ? (
                      <div className={cn(CAPTION, 'px-3 py-2')}>No agency members yet.</div>
                    ) : (
                      members.map((m) => (
                        <SelectItem key={m.userId} value={m.userId}>
                          {m.name ?? m.email}
                          {m.role !== 'provider_member' ? ` · ${roleLabel(m.role)}` : ''}
                        </SelectItem>
                      ))
                    )}
                  </SelectContent>
...
function roleLabel(role: string): string {
  ...
  if (role === 'provider_member') return 'Provider';
  return role;
}
...
function summarizeRule(
  ...
  memberById: Map<string, AgencyMember>,
): string {
  const bits: string[] = [];
