...
export default function AgencyFormBuilderPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/agency/form-config', {
        method: 'PUT',
      ...
      toast.success(`Agency ${activeLeadType === 'rental' ? 'rental' : 'buyer'} form saved.`);
    } catch (err) {
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Agency ${label} form reset to the standard Koala default.`);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Something went wrong.');
    }
  }, [activeLeadType]);
  ...
  const memberLabel = memberCount != null
    ...
    : 'all member providers';

  const handlePushToMembers = useCallback(async () => {
    setPushing(true);
    try {
      const res = await fetch('/api/agency/form-config/push', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div className="space-y-6 max-w-3xl mx-auto pb-56 md:pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <header className="space-y-1.5">
            ...
            <p className={BODY_MUTED}>
              {hasChanges
                ? 'Unsaved changes. Save to update your agency standard.'
                : hasSavedConfig
                  ? 'Saved as your agency standard.'
        ...
        <div className="space-y-1.5 border-b border-border/60 pb-4">
          ...
          {!hasSavedConfig && (
            <p className="text-xs text-muted-foreground">
              No custom agency {activeLeadType === 'rental' ? 'rental' : 'buyer'} form yet. Customize the fields below and save to create your agency standard.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2 border-b border-border/60 pb-6">
        ...
        {!eitherHasSavedConfig && !anyHasChanges && (
          <p className="text-xs text-muted-foreground">
            Save a custom agency form first before pushing to members.
          </p>
        )}
      </div>
      ...
      <AlertDialog open={resetConfirmOpen} onOpenChange={setResetConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This restores the standard Koala default and removes your custom agency {activeLeadType === 'rental' ? 'rental' : 'buyer'} form.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={pushConfirmOpen} onOpenChange={setPushConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This overrides both the rental and buyer form settings for {memberLabel} with your agency forms. Their form source switches to the agency standard.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
