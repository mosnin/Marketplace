...
// Agency DEFAULT rates are stored as DECIMALS (0.03 = 3%); convert to
...
function buildEditDefaults(row: LedgerRow): EditFields {
  return {
    ...
    agencyRate: formatPct(row.agencyRate),
...
function EditRowDialog({ row, onClose, onSaved }: EditDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const newAgency = parsePct(fields.agencyRate);
    if (newAgency === null) {
      toast.error('Agency rate must be between 0 and 100');
      return;
    }
    if (Math.abs(newAgency - row.agencyRate) > 1e-9) patch.agencyRate = newAgency;
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/agency/commissions/ledger/${row.id}`, {
        method: 'PATCH',
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="agency-rate">Agency %</Label>
              <Input
                id="agency-rate"
                type="number"
                step="0.01"
                min={0}
                max={100}
                value={fields.agencyRate}
                onChange={(e) => setFields((p) => ({ ...p, agencyRate: e.target.value }))}
              />
            </div>
            <div className="space-y-1.5">
...
function normalizeRow(apiResponse: unknown, prev: LedgerRow): LedgerRow {
  ...
  return {
    ...
    agencyRate: pick('agencyRate', prev.agencyRate),
    ...
    agencyAmount: pick('agencyAmount', prev.agencyAmount),
...
export function CommissionsClient({ ledger: initialLedger, defaultAgentRate, defaultAgencyRate }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      totalCommissions += r.agencyAmount + r.agentAmount + r.referralAmount;
      if (r.status === 'paid') paid += r.agencyAmount;
      else if (r.status === 'pending') pending += r.agencyAmount;
    }
    return {
      totalCommissions,
      totalValue,
  ...
  const byAgent = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      if (existing) {
        ...
        existing.agencyCommission += r.agencyAmount;
        ...
        map.set(r.agentUserId, {
          ...
          agencyCommission: r.agencyAmount,
          rows: [r],
        });
      }
    }
  ...
  function handleExport() {
    ...
    const url = `/api/agency/commissions/export?${params.toString()}`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="text-xs text-muted-foreground pb-2">
            Defaults: agent {decimalToPct(defaultAgentRate)}% &middot; agency {decimalToPct(defaultAgencyRate)}%
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-px rounded-xl overflow-hidden border border-border/60 bg-border/60">
        ...
        <div className="bg-background px-4 py-4">
          ...
          <p className={cn(CAPTION, 'mt-1')}>Pending (agency)</p>
        </div>
        <div className="bg-background px-4 py-4">
          <p className={cn(STAT_NUMBER)}>{formatCurrency(summary.paidThisMonth)}</p>
          <p className={cn(CAPTION, 'mt-1')}>Paid (agency)</p>
        </div>
        <div className="bg-background px-4 py-4">
              ...
              {byAgent.map((a) => (
                <tr key={a.agentUserId} className="hover:bg-foreground/[0.04] transition-colors">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums text-xs font-medium">
                    {formatCurrency(a.agencyCommission)}
                  </td>
                </tr>
              ))}
              {byAgent.length === 0 && (
            ...
            <tbody className="divide-y divide-border/60 bg-card">
              {filtered.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-right tabular-nums text-xs">{formatPct(row.agencyRate)}%</td>
                    <td className="px-3 py-3 text-right tabular-nums text-xs font-medium">
                      {formatCurrency(row.agencyAmount)}
                    </td>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        in effect at that moment; changes to agency defaults only affect future
        won deals.
      </p>
