 ...
 * joins in memory.
 */
async function fetchLedgerRows(agencyId: string): Promise<LedgerRow[]> {
...
export default async function AgencyCommissionsPage() {
  ...
  const ledger = await fetchLedgerRows(agency.id);
  ...
  const defaultAgencyRate = typeof b.defaultAgencyRate === 'number' ? b.defaultAgencyRate : 0.02;
  ...
  return (
    <div className={`max-w-5xl mx-auto ${SECTION_RHYTHM} w-full pb-12`}>
      ...
      <CommissionsClient
        ...
        defaultAgencyRate={defaultAgencyRate}
      />
    </div>
  );
}
