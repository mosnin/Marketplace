...
export default async function LeaderboardPage() {
    ...
    return (
      <div className="space-y-8 max-w-5xl mx-auto pb-56 md:pb-24">
        <header className="space-y-1.5">
          ...
          <p className={cn(BODY_MUTED)}>No providers to rank yet.</p>
        </header>
  ...
  for (const member of members) {
    try {
      ...
      const [totalLeadsRes, dealsClosedRes, activeDealsRes, appointmentsCompletedRes] = await Promise.all([
        // Count total leads
        supabase
          .from('Contact')
  ...
  if (stats.length > 0) {
    ...
    // Hot Streak — most appointments completed
    const maxAppointments = Math.max(...stats.map((s) => s.appointmentsCompleted));
    if (maxAppointments >= 3) {
      stats.filter((s) => s.appointmentsCompleted === maxAppointments).forEach((s) => s.badges.push('Hot Streak'));
    }
  }
  ...
  // Status sentence — calm fact about the team, narrated by Koala.
  const ranked = stats.length;
  const closed = stats.reduce((sum, s) => sum + s.dealsClosed, 0);
  const statusSentence = (() => {
    if (ranked === 0) return 'No providers to rank yet.';
    if (closed === 0) return `Ranking ${ranked} ${ranked === 1 ? 'provider' : 'providers'} — no deals closed yet.`;
    return `Ranking ${ranked} ${ranked === 1 ? 'provider' : 'providers'} · ${closed} ${closed === 1 ? 'deal' : 'deals'} closed.`;
  })();
