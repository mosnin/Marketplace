...
type EditorMode = { kind: 'create' } | { kind: 'edit'; template: AgencyTemplate };
...
const CATEGORY_ORDER: TemplateCategory[] = ['follow-up', 'intro', 'closing', 'appointment-invite'];
...
function publishStatus(t: AgencyTemplate): PublishStatus {
...
function formFromTemplate(t: AgencyTemplate): EditorForm {
  return {
    name: t.name,
    category: t.category,
    channel: t.channel,
...
export default function TemplatesClient() {
  ...
  const [publishTarget, setPublishTarget] = useState<AgencyTemplate | null>(null);
  const [publishing, setPublishing] = useState(false);

  const [deleteTarget, setDeleteTarget] = useState<AgencyTemplate | null>(null);
  const [deleting, setDeleting] = useState(false);

  // Separate "initial load failed" from the templates list itself so the UI
  // can offer a retry button instead of leaving the agency staring at an
  ...
  const loadTemplates = useCallback(async () => {
    setLoadError(null);
    try {
      const res = await fetch('/api/agency/templates', { cache: 'no-store' });
      if (!res.ok) throw new Error('Failed to load templates');
      const data = (await res.json()) as AgencyTemplate[];
      setTemplates(data);
    } catch (err) {
  ...
  function openEdit(t: AgencyTemplate) {
  ...
  async function saveOnly(): Promise<AgencyTemplate | null> {
    ...
    const url =
      editorMode.kind === 'edit'
        ? `/api/agency/templates/${editorMode.template.id}`
    ...
    const saved = (await res.json()) as AgencyTemplate;
    setTemplates((prev) => {
  ...
  async function confirmPublish() {
    ...
    setPublishing(true);
    try {
      const res = await fetch(`/api/agency/templates/${publishTarget.id}/publish`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  async function handleDuplicate(t: AgencyTemplate) {
    try {
      ...
      const copy = (await res.json()) as AgencyTemplate;
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/agency/templates/${deleteTarget.id}`, { method: 'DELETE' });
  ...
  const grouped = useMemo(() => {
    const byCat: Record<TemplateCategory, AgencyTemplate[]> = {
      'follow-up': [],
      intro: [],
      closing: [],
      'appointment-invite': [],
        ...
        <div className="space-y-4">
          ...
          {form.channel === 'email' && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Following up on your appointment"
              />
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-muted-foreground">
              Use {'{{name}}'}, {'{{service}}'}, {'{{budget}}'} as placeholders.
            </p>
          </div>
        </div>
