...
export async function MemberDashboard({ ctx }: MemberDashboardProps) {
  ...
  const userName = userRow?.name ?? userRow?.email ?? 'Provider';
      ...
      <div className="space-y-6 w-full">
        <header className="space-y-1.5">
          ...
          <p className={cn(BODY_MUTED)}>
            {`${agency.name} · finish your workspace to start tracking leads.`}
          </p>
        </header>
  ...
  // Resolve the agency's admin/owner spaces FIRST so the announcement
  ...
  // rows over the wire.
  const agencyMembers = await getAgencyMembers(agency.id);
  const agencySpaceIds = Array.from(
    new Set(
      agencyMembers
        .filter((m) => m.role === 'agency_owner' || m.role === 'agency_admin')
        .map((m) => m.Space?.id)
  ...
  const [
    ...
    // Count leads assigned by agency
    spaceId
      ? supabase
          .from('Contact')
  ...
  // Announcements are already server-scoped to agencySpaceIds and limit(3).
  ...
  const statusSentence = (() => {
    ...
    if (parts.length === 0) {
      return `${agency.name} · quiet day — nothing in flight.`;
    }
    return `${agency.name} · ${parts.join(' · ')}.`;
  })();
