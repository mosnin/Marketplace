...
export interface AssignedLeadProgress {
  ...
  currentStage: 'QUALIFICATION' | 'APPOINTMENT' | 'APPLICATION';
...
function stagePill(stage: AssignedLeadProgress['currentStage']) {
  const styles: Record<string, string> = {
    ...
    APPOINTMENT: 'text-violet-700 bg-violet-50 dark:text-violet-400 dark:bg-violet-500/15',
    APPLICATION:
...
function ProviderPicker({
  ...
  providers: ProviderOption[];
  onSelect: (r: ProviderOption) => void;
  disabled?: boolean;
}) {
...
function LeadNotes({ contactId }: { contactId: string }) {
  ...
  const loadNotes = useCallback(async () => {
    if (loaded) return;
    try {
      const res = await fetch(`/api/agency/lead-note?contactId=${contactId}`);
      if (res.ok) {
  ...
  async function handleAddNote(noteText: string) {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/agency/lead-note', {
        method: 'POST',
...
function UnassignedRow({
  ...
  lead: LeadRow;
  providers: ProviderOption[];
  onAssigned: (leadId: string, provider: ProviderOption) => void;
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const res = await fetch(`/api/agency/leads/${lead.id}`, { method: 'DELETE' });
  ...
  async function handleAssign(provider: ProviderOption) {
    setAssigning(true);
    try {
      const res = await fetch('/api/agency/assign-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contactId: lead.id, providerUserId: provider.userId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "I couldn't route that lead.");
      }
      toast.success(`Routed to ${provider.name ?? provider.email}.`);
      onAssigned(lead.id, provider);
    } catch (err: unknown) {
    ...
    <li>
      <div className="group/row flex items-center gap-3 py-3 px-2 -mx-2 rounded-md hover:bg-muted/30 transition-colors">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <ProviderPicker providers={providers} onSelect={handleAssign} disabled={assigning} />
          {confirmDelete ? (
...
function AssignedRow({
  ...
  onUnassigned?: (leadId: string, providerName: string) => void;
}) {
  ...
  const providerName = progress?.providerName ?? lead.assignedTo ?? 'this provider';

  async function handleUnassign() {
    setUnassigning(true);
    try {
      const res = await fetch('/api/agency/unassign-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(`Pulled back from ${providerName}.`);
      setConfirmOpen(false);
      onUnassigned?.(lead.id, providerName);
    } catch (err: unknown) {
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {lead.name || 'This lead'} comes off {providerName}&rsquo;s plate and lands back in the unassigned queue.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
          ...
          <div className="rounded-lg bg-muted/30 border border-border/60 p-3 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                ...
                <p className="mt-0.5 font-medium text-foreground">{progress.providerName}</p>
              </div>
              <div>
...
export function AgencyLeadsClient({
  unassignedLeads,
  assignedLeads,
  providers,
  ...
  // Resolve the provider a routed lead sits with — same rule AssignedRow uses
  // (live progress wins, falls back to the assignment label).
  const providerOf = useCallback(
    (lead: LeadRow) => assignedLeadProgress[lead.id]?.providerName ?? lead.assignedTo ?? null,
    [assignedLeadProgress],
  );

  // Providers who actually have routed leads, plus any team member — so the
  // menu mirrors the team even before everyone has a lead. Sorted, deduped.
  const providerNames = useMemo(() => {
    const set = new Set<string>();
    for (const lead of assigned) {
      const name = providerOf(lead);
  ...
  const filteredUnassigned = useMemo(() => {
    // A specific provider has no claim on unassigned leads — hide them.
    if (providerFilter !== 'all') return [];
    return applySearch(unassigned);
  }, [unassigned, applySearch, providerFilter]);

  const filteredAssigned = useMemo(() => {
    const byProvider =
      providerFilter === 'all'
        ? assigned
        : assigned.filter((l) => providerOf(l) === providerFilter);
    return applySearch(byProvider);
  }, [assigned, applySearch, providerFilter, providerOf]);

  function handleAssigned(leadId: string, provider: ProviderOption) {
    ...
    setAssigned((prev) => [
      {
        ...lead,
        assignedTo: provider.name ?? provider.email,
  ...
  return (
    <div className="space-y-8 pb-56 md:pb-24">
      {/* Search + by-provider filter — the only chrome. View toggles, sort
          dropdown, lead type filter, score pills, and "Add Lead" button all
          cut. Agencies don't manually add leads; intake forms do. */}
      {hasAnyLeads && (
      ...
      {/* Fresh-agency empty state — no leads at all. Canonical dashed card,
          one sentence, first-person Koala voice. */}
      {!hasAnyLeads && (
      ...
      {/* Unassigned — the one demanding routing. Shown first so it's the
          first thing the agency reaches for. Hidden when filtering to a
          single provider: unassigned leads belong to no one, so an empty
          queue under a provider filter is noise. */}
      {hasAnyLeads && providerFilter === 'all' && (
        <section className="space-y-2">
          <SectionHeader
            label="Unassigned"
            count={filteredUnassigned.length}
            hint={filteredUnassigned.length > 0 ? 'Route to a provider' : undefined}
          />
            ...
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
              <p className="text-sm text-foreground">
                {isSearching
                  ...
                  : providerFilter !== 'all'
                    ? `Nothing on ${providerFilter}’s plate yet.`
                    : "Nothing on a provider's plate yet."}
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-border/60">
