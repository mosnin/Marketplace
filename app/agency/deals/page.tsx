...
import type { AgencyKanbanColumn } from './agency-kanban-board';
import type { Metadata } from 'next';

export const metadata: Metadata = { title: 'Deals — Agency' };
 ...
 *
 * Because each provider can have custom stage names, we build columns from
 ...
 *     stage?" when providers name them the same.
 ...
 *
 * The result: the agency sees a kanban whose column order mirrors the
 * provider pipeline direction even across mixed stage names.
 */
function buildColumns(
  stages: StageRow[],
  dealsWithProvider: Array<AgencyDealItem & { stageId: string }>,
): AgencyKanbanColumn[] {
  ...
  const columnMap = new Map<string, AgencyKanbanColumn>();
  ...
  const unmapped: Array<AgencyDealItem & { stageId: string }> = [];
  for (const deal of dealsWithProvider) {
...
export default async function AgencyDealsPage() {
  const ctx = await resolveAgencyContext();
  if (!ctx) redirect('/');

  const { agency } = ctx;

  // Resolve ALL agency members — same pattern as the previous page.
  ...
  // Build spaceId to provider name lookup
  ...
  // Won/lost/on-hold deals are closed chapter; the agency oversight
  ...
  // ── Enrich deals with provider names ──────────────────────────────────────

  const allDeals: Array<AgencyDealItem & { stageId: string }> = [];
  ...
  for (const d of (dealsRaw ?? []) as DealRow[]) {
    ...
    allDeals.push({
      ...
      nextActionDueAt,
      providerName,
      // Pass stageId for column slotting; not in AgencyDealItem interface
      stageId: d.stageId,
    });
  }
  ...
  const statusSentence = (() => {
    if (activeCount === 0) return 'No active deals across your agency yet.';
    const parts: string[] = [];
    parts.push(
  ...
  return (
    <div className={cn('max-w-[1500px] mx-auto pb-12', SECTION_RHYTHM)}>
      ...
      <AgencyKanbanBoard columns={columns} />
    </div>
  );
}
