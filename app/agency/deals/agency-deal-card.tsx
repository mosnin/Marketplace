 ...
 * Agency kanban card.
 *
 * Mirrors the provider DealCard visual vocabulary exactly — same layout,
 * same serif value, same health dot, same days-in-stage line — with two
 * additions: a provider byline below the address, and no drag handle (the
 * agency is read-only, no restaging). The card is intentionally a replica,
...
export interface AgencyDealItem {
  id: string;
  title: string;
  address: string | null;
  value: number | null;
...
export function AgencyDealCard({
  deal,
  entranceIndex = null,
  onClick,
}: AgencyDealCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Focal value — serif Times, matches provider card */}
        {deal.value != null && (
          <p
        ...
        {/* Days in stage — agency-specific signal */}
        ...
        {/* Provider byline — the agency oversight addition */}
