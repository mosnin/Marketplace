...
export const metadata: Metadata = { title: 'Revenue Forecast — Agency' };
...
// set; otherwise fall back to the agency's defaultAgencyRate (same proxy
...
//   1. deal.commissionRate (set per-deal by the provider)
//   2. agency.defaultAgencyRate (agency-wide default, ~ 2%)
...
export default async function AgencyForecastPage() {
  ...
  const b = agency as unknown as {
    defaultAgentRate?: number | null;
    defaultAgencyRate?: number | null;
  };
  ...
  const allMembers = await getAgencyMembers(agency.id, {
  ...
  // Space to provider name lookup
  const spaceToProvider = new Map<string, string>();
  for (const m of allMembers) {
    const sid = m.Space?.id;
    if (!sid) continue;
    spaceToProvider.set(sid, m.User?.name ?? m.User?.email ?? 'Unknown');
  }
  ...
  if (spaceIds.length === 0) {
    return <EmptyPage agencyName={agency.name} monthName={monthName} />;
  }
  ...
  // Fetch closed deals (won + lost, all time) for per-provider conversion and
  ...
  // the per-provider rollup sees the full pipeline, not just the closed tail.
  ...
  let wonGci = 0;
  let wonCount = 0;
  // Realized GCI this month, per provider space — feeds the per-provider rollup.
  ...
  for (const d of (wonRaw ?? []) as Array<{
    ...
    const rate =
      d.commissionRate != null ? d.commissionRate / 100 : defaultAgencyRate;
  ...
  for (const d of (activeRaw ?? []) as DealRow[]) {
    ...
    const providerName = spaceToProvider.get(d.spaceId) ?? 'Unknown';
  ...
  // ── Per-provider performance context ────────────────────────────────────────
  // For each provider space we fold together: their active deals (from
  ...
  type ProviderMetricRow = DealMetricRow & { spaceId: string };

  const metricRowsBySpace = new Map<string, ProviderMetricRow[]>();
  const pushRow = (row: ProviderMetricRow) => {
  ...
  const providerSpaceIds = new Set<string>([
    ...metricRowsBySpace.keys(),
    ...realizedBySpace.keys(),
  ]);

  const providerRollups: ProviderRollup[] = Array.from(providerSpaceIds)
    .map((sid) => {
      const rows = metricRowsBySpace.get(sid) ?? [];
      const activeForSpace = rows.filter((r) => r.status === 'active').length;
      return {
        spaceId: sid,
        name: spaceToProvider.get(sid) ?? 'Unknown',
    ...
    // Show providers with anything in flight or realized this month first; drop
  ...
  // Run across the agency's active deals. Stages come from the same set we
  ...
  return (
    <div className={cn('max-w-5xl mx-auto pb-12', SECTION_RHYTHM)}>
      ...
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>{agency.name}.</p>
      ...
      {!hasAnything ? (
        ...
        <>
          ...
          <section>
            ...
            {swingPool.length === 0 ? (
              ...
              <ul className="divide-y divide-border/60">
                {swingPool.map((deal) => {
                  const koalaPrompt = `What can we do to close "${deal.title}" (${deal.providerName}) this month?`;
                  const koalaHref = `/agency?prompt=${encodeURIComponent(koalaPrompt)}`;
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex items-center flex-wrap gap-x-3 gap-y-0.5">
                            <span className={META}>{deal.providerName}</span>
                        ...
                        <div className="flex-shrink-0 text-right space-y-1.5">
                          ...
                          <Link
                            href={koalaHref}
                            className={cn(KOALA_PILL, 'h-7 px-3 text-[11px]')}
                          >
                            Ask Koala
          ...
          <section>
            ...
            {providerRollups.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center mt-4">
                <p className="text-sm text-foreground">No provider activity yet.</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Each provider&rsquo;s forecast and pace will appear here as deals move.
                </p>
              </div>
            ) : (
              <ul className="divide-y divide-border/60">
                {providerRollups.map((r) => {
                  const forecast = r.projected + r.realized;
                  return (
                    <li key={r.spaceId} className="py-4">
                      <div className="flex items-start justify-between gap-4">
                        {/* Provider identity + pace */}
                        <div className="flex-1 min-w-0 space-y-1">
                          <p className="text-sm font-medium text-foreground truncate">
                            {r.name}
                          </p>
...
// ── Empty state when agency has no members/spaces ─────────────────────────
...
function EmptyPage({
  ...
  return (
    <div className={cn('max-w-5xl mx-auto pb-12', SECTION_RHYTHM)}>
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>{agencyName}.</p>
        <h1 className={H1} style={TITLE_FONT}>
          {monthName} revenue
        </h1>
      ...
      <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
        <p className="text-sm text-foreground">Nothing in the pipeline yet.</p>
        <p className="text-xs text-muted-foreground mt-1">
          Invite your first provider and their deals will appear here.
        </p>
      </div>
    </div>
  );
