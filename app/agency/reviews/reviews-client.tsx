...
export function ReviewsClient({ initialReviews, initialOpenCount, role, agencyName }: Props) {
  ...
  // somewhere in this agency. Best-effort: derived from the cached
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/agency/reviews?status=${tab}`, {
          cache: 'no-store',
        });
