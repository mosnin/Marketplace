...
//
// Use getAgencyContext (not requireAgency) so non-agencies get a clean
// redirect instead of an uncaught throw — matches app/agency/members/page.tsx.
export default async function AgencyReviewsPage() {
  const ctx = await getAgencyContext();
  if (!ctx) redirect('/');

  // 1. Pull open review requests for this agency, newest-first. Other tabs
  //    fetch on demand via /api/agency/reviews?status=X.
  ...
  // Headline status — calm, factual. Mirrors the agency overview pattern
