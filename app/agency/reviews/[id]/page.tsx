...
export default async function AgencyReviewDetailPage({ params }: PageProps) {
  ...
  //    different agency — we must not leak existence across agencies.
  const { data: reviewRow } = await supabase
    .from('DealReviewRequest')
    .select(
      'id, dealId, agencyId, status, reason, createdAt, resolvedAt, resolvedNote, resolvedByUserId, requestingUserId'
    )
    .eq('id', id)
    .maybeSingle();

  if (!reviewRow || reviewRow.agencyId !== ctx.agency.id) {
    notFound();
  }
