...
interface BookingFormProps {
   ...
   *  provider's public page so the applicant has somewhere to go after
...
type Step = 'service' | 'date' | 'details' | 'confirmed';
...
export function BookingForm({ slug, duration: defaultDuration, businessName, timezone, accentColor = '#ff964f', profileHref }: BookingFormProps) {
  ...
  async function loadSlots(propId?: string | null, currentStep?: Step) {
    ...
    try {
      let url = `/api/appointments/available?slug=${encodeURIComponent(slug)}`;
      if (propId) url += `&serviceId=${encodeURIComponent(propId)}`;
      const res = await fetch(url);
      if (res.ok) {
        const data = await res.json();
        setSlots(data.slots ?? []);
        setEffectiveDuration(data.duration ?? defaultDuration);
        // If services exist and we haven't shown the service picker yet
        // Use the passed-in step to avoid stale closure on the state variable
        const stepAtLoad = currentStep ?? step;
        if (data.serviceProfiles?.length > 0 && !propId && stepAtLoad === 'date') {
          setServices(data.serviceProfiles);
          setStep('service');
        }
      } else {
  ...
  function selectService(id: string | null) {
    ...
    if (id) {
      const prop = services.find((p) => p.id === id);
      if (prop?.address) setServiceAddress(prop.address);
    }
    setStep('date');
    loadSlots(id, 'date');
  }
      ...
      <>
        ...
        >
          ...
          <p className="text-base text-muted-foreground max-w-sm mx-auto">
            Your appointment is set for <span className="font-medium text-foreground">{dateLabel}</span>{' '}
  ...
  // - 'service' lock: only the service section is interactive
  // - 'date' lock: service is summarized + collapsed; date+time is open
  // - 'details' lock: above are summarized; details form is open
  const showService = services.length > 0;
  const serviceChosen = step === 'date' || step === 'details';
  const showDateSection = step === 'date' || step === 'details';
  const showDetailsSection = step === 'details';

  const selectedService = selectedServiceId
    ? services.find((p) => p.id === selectedServiceId)
    : null;
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              {serviceChosen && (
                <button
                  type="button"
                  onClick={() => { setStep('service'); setSelectedServiceId(null); }}
            ...
            {step === 'service' ? (
              <div className="space-y-2">
                {services.map((p) => {
                  const active = selectedServiceId === p.id;
                  return (
                    <button
                      key={p.id}
                      type="button"
                      onClick={() => selectService(p.id)}
                      className={cn(
                ...
                {selectedService ? (
                  <>
                    ...
                    {selectedService.address && (
                      <span className="text-muted-foreground"> · {selectedService.address}</span>
                    )}
                  </>
                ) : (
        ...
        {submitting && (
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-foreground">Booking your appointment…</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
