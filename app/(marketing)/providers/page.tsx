/**
 * `/providers` — Koala for the solo provider who's boots-on-the-ground.
 *
 * The story: a provider flying solo spends the day in the field — at the
 * showing, in the car, between doors. Koala is the extra teammate who works
 * alongside them, reachable on the phone, the installed web app, or the office
 * desktop. It reads the inbox, drafts in their voice, books the appointment, scores
 ...
 * tabs, and the provider stays in the driver's seat on every send.
 *
 ...
 *
 * Auth-aware like the homepage: signed-in providers bounce to their workspace.
 */
...
import { ProvidersHero } from '@/components/marketing/providers/providers-hero';
import { FieldSurfaces } from '@/components/marketing/providers/field-surfaces';
import { FeatureRow } from '@/components/marketing/providers/feature-row';
import {
  OnePlace,
  TailorIt,
} from '@/components/marketing/providers/consolidation-tailor';
import { ClosingCTA } from '@/components/marketing/providers/closing-cta';
import {
...
export default async function ProvidersPage() {
  ...
  if (userId) {
    redirect('/auth/redirect?intent=provider');
  }
    ...
    <div className="bg-muted text-foreground">
      ...
      <div className="relative mx-auto max-w-7xl space-y-24 px-6 py-12 md:space-y-32 md:px-8 md:py-16">
        ...
        <FeatureRow
          ...
          sub="Koala reads the thread, writes the reply the way you actually write, and stops. you read it, you press send. or you don't."
          points={[
        ...
        <FeatureRow
          ...
          sub="emails, appointments, notes, drafts, replies, assembled chronologically per contact. you scroll the relationship; you don't reconstruct it from memory."
          points={[
            'appointments and meetings logged as they happen.',
            'Koala-authored actions tagged on the timeline.',
            'ask Koala to recall the history and it does.',
          ]}
      ...
      >
        <Reveal>
          ...
          <p className="mx-auto mt-5 max-w-xl text-lg leading-relaxed text-foreground/55">
            Koala does the work; you make the call. approval-first by default,
