 ...
 * surfaces a provider depends on, each carrying a single dot.
 *
 ...
 * The per-user, per-workspace health (a provider's own Gmail/Calendar
...
export const metadata = { title: 'Status · Koala' };
...
/**
 * The agent (Koala) needs two things to do real work: an LLM provider key
 ...
 * health (this provider's Gmail token) is auth-gated and not shown here.
 */
...
export default async function StatusPage() {
  ...
  const subsystems: Subsystem[] = [
    { label: 'Agent (Koala)', health: agent },
  ...
  return (
    <>
      <MarketingHero
        eyebrow="STATUS"
        title={title}
        sub="Live status of Koala’s agent, integrations, and dashboard."
      />
