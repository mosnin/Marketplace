...
export function FormAnalyticsClient({
  ...
  topAgencies: AgencySubmissionRow[];
  ...
  const filteredAgencies = useMemo(() => {
    ...
    if (!q) return topAgencies;
    return topAgencies.filter((b) =>
      (b.agencyName ?? b.agencyId).toLowerCase().includes(q),
    );
  }, [filter, topAgencies]);
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative w-full sm:w-[280px]">
            ...
            />
            <Input
              placeholder="Filter agencies or spaces…"
              value={filter}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          {filteredAgencies.length === 0 ? (
            <Card className="rounded-xl border bg-card">
              <EmptyState icon={Building2} title="No agency submissions yet." size="sm" />
            </Card>
          ) : (
            <Card className="rounded-xl border bg-card">
              <div className="divide-y divide-border">
                {filteredAgencies.map((b) => (
                  <Link
                    key={b.agencyId}
                    href={`/admin/agencies/${b.agencyId}`}
