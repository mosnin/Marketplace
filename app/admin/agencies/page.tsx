...
export default async function AdminAgenciesPage() {
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="text-center space-y-2 p-8">
          <p className="text-sm text-muted-foreground">Couldn&apos;t load agencies.</p>
          <a href="/admin/agencies" className="text-xs text-primary hover:underline">Retry</a>
        </div>
      </div>
    );
  }

  const allIds = (agencies ?? []).map((b) => b.id);
  const { data: memberships } = allIds.length > 0
    ? await supabase.from('AgencyMembership').select('agencyId').in('agencyId', allIds)
    : { data: [] };
  ...
  const active    = (agencies ?? []).filter((b) => b.status === 'active').length;
  const suspended = (agencies ?? []).filter((b) => b.status === 'suspended').length;
    ...
    <div className="space-y-8 pb-12">
      <header className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          {agencies?.length ?? 0} total · {active} active · {suspended} suspended.
        </p>
      </header>
