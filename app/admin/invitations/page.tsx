...
export default async function AdminInvitationsPage() {
  ...
  const { data: invitations, error } = await supabase
    .from('Invitation')
    .select('id, email, roleToAssign, status, expiresAt, createdAt, agencyId, Agency(name)')
  ...
  const roleLabel = (r: string) => r === 'agency_admin' ? 'Admin' : 'Provider';
    ...
    <div className="space-y-8 pb-12 max-w-5xl mx-auto">
      <header className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          {invs.length} invitation{invs.length !== 1 ? 's' : ''} across all agencies.
        </p>
      </header>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {inv.Agency?.name ?? '—'} · {roleLabel(inv.roleToAssign)} · Sent {sentAt}
