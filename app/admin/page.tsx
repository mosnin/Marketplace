...
export default async function AdminOverviewPage() {
  ...
  try {
    const [
      ...
      supabase.from('Agency').select('*', { count: 'exact', head: true }),
      supabase
        .from('Agency')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'active'),
      ...
      supabase
        ...
        .order('createdAt', { ascending: false })
        .limit(3),
      // Recent agencies for activity feed
      supabase
        .from('Agency')
    ...
    activeAgencies = activeAgencyRes.count ?? 0;
    ...
    for (const b of (recentAgencies.data ?? []) as any[]) {
      activities.push({
        type: 'agency',
        label: b.name,
        detail: 'Agency created',
        time: b.createdAt,
      });
    }
    ...
    // ── Feature usage metrics ─────────────────────────────────────────
    try {
      const [contactSpaces, dealSpaces, appointmentSpaces, appointmentsCount, followUpsCount] =
        await Promise.all([
          supabase.from('Contact').select('spaceId').limit(1000),
          supabase.from('Deal').select('spaceId').limit(1000),
          supabase.from('Appointment').select('spaceId').limit(1000),
      ...
      spacesWithAppointments = new Set(
        (appointmentSpaces.data ?? []).map((r: any) => r.spaceId).filter(Boolean)
      ).size;
          ...
          <div className="rounded-xl border border-border/70 bg-card px-5 py-5">
            ...
            <div className="space-y-4">
              {[
                ...
                { label: 'Spaces with appointments', value: spacesWithAppointments, total: totalSpaces, icon: Calendar },
                { label: 'Total appointments booked', value: totalAppointments, total: null, icon: Calendar },
