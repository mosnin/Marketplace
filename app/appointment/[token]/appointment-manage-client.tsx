...
import { PostAppointmentFeedback } from '@/components/appointments/post-appointment-feedback';
import {
  BODY,
  BODY_MUTED,
  CAPTION,
...
interface AppointmentManageClientProps {
  appointment: AppointmentData;
  token: string;
  businessName: string;
  bookingSlug: string;
  /** Link back to the provider's public page — calm secondary affordance
...
export function AppointmentManageClient({ appointment, token, businessName, bookingSlug, profileHref }: AppointmentManageClientProps) {
      ...
      <div className="w-full max-w-md">
        <div className="rounded-xl bg-background border border-border/70 p-6 text-center space-y-4">
          ...
          <p className={cn(BODY_MUTED, 'max-w-sm mx-auto')}>
            Your appointment with {businessName} has been cancelled. You can book a new
            time below.
          </p>
          {canRebook && (
            <div className="pt-2">
  ...
  return (
    <div className="w-full max-w-md">
      <div className="rounded-xl bg-background border border-border/70 p-6">
        ...
        {/* ─── Appointment summary — hairline-divided rows ──────────────── */}
        <div className="border-t border-border/60 mt-6 pt-2 divide-y divide-border/60">
          <SummaryRow label="Guest" value={appointment.guestName} />
          <SummaryRow label="Date" value={dateLabel} />
          <SummaryRow label="Time" value={`${timeLabel} (${duration} min)`} />
          {appointment.serviceAddress && (
            <SummaryRow label="Service" value={appointment.serviceAddress} />
          )}
        </div>

        {/* ─── Post-appointment feedback (only on completed) ────────────── */}
        {isCompleted && (
          <div className="mt-8">
