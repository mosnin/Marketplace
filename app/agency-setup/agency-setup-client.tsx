...
interface AgencySetupClientProps {
  spaceSlug: string;
...
function CreateForm() {
  ...
  return (
    <form onSubmit={handleCreate} className="space-y-4">
      <div>
        <label className={cn(SECTION_LABEL, 'block mb-1.5')}>Agency name</label>
        <input
          type="text"
          required
    ...
    <form onSubmit={handleJoin} className="space-y-4">
      <div>
        ...
        <p className={cn(CAPTION, 'mt-1.5')}>
          Ask your agency for the 8-character code from their dashboard.
        </p>
      </div>
      {error && <p className="text-xs text-destructive">{error}</p>}
...
export function AgencySetupClient({
  spaceSlug,
  existingAgencyName,
    ...
    return (
      <div className="app-theme min-h-screen bg-background flex flex-col">
        <header className="px-6 py-5 border-b border-border/70">
          <Link href="/" aria-label="Koala home">
            <BrandLogo className="h-6 w-auto" alt="Koala" />
          </Link>
        </header>
        ...
        <main className="flex-1 flex items-center justify-center px-6 py-16">
          <div className="w-full max-w-md text-center">
            ...
            <p className={cn(BODY_MUTED, 'mt-3')}>
              You&apos;re a member of{' '}
              <span className="font-medium text-foreground">{existingAgencyName}</span>.
            </p>
      ...
      <main className="flex-1 flex items-start justify-center px-6 py-16">
        <div className="w-full max-w-4xl">
          ...
          <header className="mb-10 space-y-1.5">
            <p className={cn(SECTION_LABEL)}>Agency</p>
            <h1 className={cn(H1)} style={TITLE_FONT}>
              Set up your agency
            </h1>
            <p className={cn(BODY_MUTED, 'max-w-xl')}>
              Create your own agency to manage a team of providers, or join one
              with a code from your agency.
            </p>
          </header>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="space-y-2 mb-6">
                  {[
                    { icon: Users, label: 'Invite providers via email or share code' },
                    { icon: BarChart3, label: 'Team-wide lead and pipeline analytics' },
                    { icon: Mail, label: 'Manage agency invitations' },
                  ].map((f) => (
              ...
              >
                ...
                <div className="space-y-2 mb-6">
                  {[
                    ...
                    { icon: CheckCircle2, label: 'Your agency gets team-level visibility' },
