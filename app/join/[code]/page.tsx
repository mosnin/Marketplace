...
export default async function JoinWithCodePage({ params }: Params) {
  ...
  if (!userId) {
    redirect(`/login/provider?redirect_url=/join/${code}`);
  }
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="bg-foreground px-6 py-5">
            <div className="flex items-center gap-2.5">
              {agency?.logoUrl ? (
                <img src={agency.logoUrl} alt="" className="h-6 max-w-[80px] object-contain rounded" />
              ) : (
            ...
            <p className="mt-1 text-background/60 text-sm">Agency invite code</p>
          </div>
          ...
          <div className="px-6 py-6 space-y-4">
            {isInvalid ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-destructive/10 text-destructive">
                ...
                <p className="text-sm">This invite code is invalid or has been revoked. Ask your agency for a new one.</p>
              </div>
            ) : isSuspended ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400">
                <AlertTriangle size={16} className="flex-shrink-0 mt-0.5" />
                <p className="text-sm">This agency is currently suspended and not accepting new members.</p>
              </div>
            ) : (
              <>
                <div>
                  <p className="text-sm text-foreground leading-relaxed">
                    ...
                    <span className="font-semibold">{agency.name}</span> as a{' '}
                    <span className="font-semibold">Provider Member</span>.
                  </p>
