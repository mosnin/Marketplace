...
export default async function DashboardPage() {
  ...
  const total = applications.length + appointments.length;
  const statusSentence =
    total === 0
      ? 'nothing in flight yet — it lands here the moment you apply or book.'
      : `${applications.length} application${applications.length === 1 ? '' : 's'} and ${appointments.length} appointment${
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-12 px-4 py-10 pb-16 sm:px-6">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">
                      {app.providerName ?? 'Your application'}
                    </p>
        ...
        <section className="space-y-4">
          ...
          <ul className="divide-y divide-border/60 overflow-hidden rounded-xl border border-border/70 bg-card">
            {appointments.map((appointment) => (
              <li key={appointment.id} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[11px] text-muted-foreground">
                    {appointment.serviceAddress ?? 'Address to be shared'}
      ...
      {/* Book another appointment — quiet entry point for engaged clients */}
      {applications.length > 0 && (
        <div>
          <Link
            href="/clients/book"
