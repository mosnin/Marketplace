...
Operating manual for AI coding agents working in the Koala repository.
...
| 14 | Agency permission helpers | `lib/permissions.ts` (`requireAgency`, `getAgencyContext`, `getAgencyMemberContext`, role predicates) and `lib/api-auth.ts` (`requireAuth`, `requireSpaceOwner`, `requireContactAccess`). Never bypass these with raw `auth()` or ad-hoc role checks. |
