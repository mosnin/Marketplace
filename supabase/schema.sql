-- Supabase schema for Koala — Professional-Services Marketplace & Agentic OS
...
CREATE TABLE IF NOT EXISTS "User" (
  ...
  "accountType"           text NOT NULL DEFAULT 'provider' CHECK ("accountType" IN ('provider', 'agency_only', 'both'))
);
...
CREATE TABLE IF NOT EXISTS "Agency" (
  ...
  "agencyFormConfig" jsonb DEFAULT NULL,
  "agencyRentalFormConfig" jsonb DEFAULT NULL,
  "agencyBuyerFormConfig" jsonb DEFAULT NULL,
  "agencyRentalScoringModel" jsonb DEFAULT NULL,
  "agencyBuyerScoringModel" jsonb DEFAULT NULL,
  "createdAt"   timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS "Space" (
  ...
  "agencyId" text REFERENCES "Agency"(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS "SpaceSetting" (
  ...
  "notifyAppointmentBookings" boolean NOT NULL DEFAULT true,
  ...
  "appointmentStartHour"        integer NOT NULL DEFAULT 9,
  "appointmentEndHour"          integer NOT NULL DEFAULT 17,
  "appointmentDaysAvailable"    integer[] NOT NULL DEFAULT '{1,2,3,4,5}',
  "appointmentBookingPageTitle" text,
  "appointmentBookingPageIntro" text,
  "appointmentBufferMinutes"    integer NOT NULL DEFAULT 0,
  "appointmentBlockedDates"     text[] NOT NULL DEFAULT '{}',
  ...
  "formConfigSource"     text NOT NULL DEFAULT 'legacy'
    CHECK ("formConfigSource" IN ('custom', 'agency', 'legacy')),
...
-- AppointmentServiceProfile must be defined before Appointment (Appointment.serviceProfileId FK)
...
-- Appointment must be defined before Deal (Deal.sourceAppointmentId FK)
...
-- Agency Koala conversations live in their OWN tables, keyed by agencyId
-- (NOT spaceId), so agency-private chat is structurally isolated from the
-- provider "Conversation"/"Message" tables. See migration
-- 20260616000000_agency_chat_separate_storage.sql.
...
CREATE TABLE IF NOT EXISTS "AgencyMembership" (
  ...
  "createdAt"     timestamptz NOT NULL DEFAULT now(),
  -- Per-member agency profile (20260615000000). Owner/admin customize their
  -- own profile within the agency; mirror of the provider profile fields
...
-- Agency-level integrations via Composio (20260615000000). The agency
-- analogue of "IntegrationConnection": each owner/admin connects their OWN
-- third-party accounts at the agency level. Keyed on
-- (agencyId, userId, toolkit). Owner/admin only — provider_member is blocked
-- in the API layer (requireAgency / canEditSettings). Composio holds the OAuth
...
CREATE TABLE IF NOT EXISTS "AgencyNotification" (
  id              text PRIMARY KEY DEFAULT gen_random_uuid()::text,
  "agencyId"   text NOT NULL REFERENCES "Agency"(id) ON DELETE CASCADE,
  type            text NOT NULL,
  title           text NOT NULL,
...
CREATE TABLE IF NOT EXISTS "AppointmentAvailabilityOverride" (
  id                  text PRIMARY KEY DEFAULT gen_random_uuid()::text,
  "spaceId"           text NOT NULL REFERENCES "Space"(id) ON DELETE CASCADE,
  "serviceProfileId" text REFERENCES "AppointmentServiceProfile"(id) ON DELETE CASCADE,
  date                date NOT NULL,
...
CREATE TABLE IF NOT EXISTS "ApplicationMessage" (
  ...
  "contactId" text NOT NULL REFERENCES "Contact"(id) ON DELETE CASCADE,
  "spaceId"   text NOT NULL REFERENCES "Space"(id) ON DELETE CASCADE,
  "senderType" text NOT NULL CHECK ("senderType" IN ('applicant', 'provider')),
...
ALTER TABLE "Appointment" ADD COLUMN IF NOT EXISTS "serviceProfileId" text REFERENCES "AppointmentServiceProfile"(id) ON DELETE SET NULL;

ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "accountType" text NOT NULL DEFAULT 'provider' CHECK ("accountType" IN ('provider', 'agency_only', 'both'));

ALTER TABLE "Space" ADD COLUMN IF NOT EXISTS "agencyId" text REFERENCES "Agency"(id) ON DELETE SET NULL;

ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "joinCode"   text UNIQUE;
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "logoUrl"    text;
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "websiteUrl" text;

ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentDuration"         integer NOT NULL DEFAULT 30;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentStartHour"        integer NOT NULL DEFAULT 9;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentEndHour"          integer NOT NULL DEFAULT 17;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentDaysAvailable"    integer[] NOT NULL DEFAULT '{1,2,3,4,5}';
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentBookingPageTitle" text;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentBookingPageIntro" text;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentBufferMinutes"    integer NOT NULL DEFAULT 0;
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "appointmentBlockedDates"     text[] NOT NULL DEFAULT '{}';
...
ALTER TABLE "SpaceSetting" ADD COLUMN IF NOT EXISTS "notifyAppointmentBookings" boolean NOT NULL DEFAULT true;
...
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "privacyPolicyHtml"    text;
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "agencyFormConfig"  jsonb DEFAULT NULL;
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "agencyRentalFormConfig" jsonb DEFAULT NULL;
ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "agencyBuyerFormConfig"  jsonb DEFAULT NULL;
...
CREATE INDEX IF NOT EXISTS idx_agency_form_config
  ON "Agency" USING gin("agencyFormConfig") WHERE "agencyFormConfig" IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_appointment_space_starts      ON "Appointment"("spaceId", "startsAt" DESC);
CREATE INDEX IF NOT EXISTS idx_appointment_contact           ON "Appointment"("contactId");
CREATE INDEX IF NOT EXISTS idx_appointment_status            ON "Appointment"(status);
CREATE INDEX IF NOT EXISTS idx_appointment_manage_token      ON "Appointment"("manageToken");
CREATE INDEX IF NOT EXISTS idx_appointment_service_profile  ON "Appointment"("serviceProfileId");

CREATE UNIQUE INDEX IF NOT EXISTS idx_override_space_date
  ON "AppointmentAvailabilityOverride"("spaceId", date);

CREATE INDEX IF NOT EXISTS idx_agency_notif_agency
  ON "AgencyNotification"("agencyId", "createdAt" DESC);
CREATE INDEX IF NOT EXISTS idx_agency_notif_unread
  ON "AgencyNotification"("agencyId", read) WHERE read = false;
...
ALTER TABLE "AgencyNotification"      ENABLE ROW LEVEL SECURITY;
