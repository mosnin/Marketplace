-- ============================================================================
-- One pending invitation per (agency, email) — close a seat-cap race.
--
...
-- per agency+email) so the index can build on a live table.
--
...
WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY "agencyId", lower(email)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_invitation_pending_email
  ON "Invitation" ("agencyId", lower(email))
  WHERE status = 'pending';
