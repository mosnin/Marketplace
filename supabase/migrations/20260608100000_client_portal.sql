-- Client Portal — end-user (applicant / appointment-booker) accounts, fully separate
-- from the provider Clerk auth. A client signs up with email + password; once
-- their email is verified, the portal aggregates every Contact (application)
-- and Appointment that shares their email across ALL spaces — "one page by email".
--
-- These tables never touch the provider auth path. Messaging, info-requests and
-- documents hang off the existing Contact so the provider sees them in their
-- normal contact view.
...
-- ── Provider → client "please send X" requests ────────────────────────────────
