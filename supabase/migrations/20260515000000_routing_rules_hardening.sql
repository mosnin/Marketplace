...
--      read rules from every agency. The app already uses the service
--      role key everywhere, but defence-in-depth says enable RLS with no
--      policies (same pattern the rest of the agency tables use — only
...
--      a user who's referenced as a routing target until the agency
...
-- agency can recreate it when they know who the replacement should be.
