-- Call log — a provider clicks "Call" on a contact; Koala places the call via
...
--
-- spaceId links the call to the provider's workspace; contactId is nullable so a
-- provider can dial a raw number that isn't yet a contact. telnyxCallId is the
