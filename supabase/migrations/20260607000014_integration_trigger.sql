-- Composio trigger subscriptions per provider connection.
--
-- Each row tracks ONE registered Composio trigger for ONE
-- IntegrationConnection. When a provider connects Gmail we register the
...
--             kept as a breadcrumb so the provider isn't silently missing
