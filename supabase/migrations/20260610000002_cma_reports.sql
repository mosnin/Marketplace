-- CMA reports — a provider generates a Comparative Market Analysis from a
-- subject service + comparable Service rows already in their workspace, then
-- shares a public link with a seller.
--
-- Fully in-house: comps are selected from the provider's own Service table (no
...
-- page renders a stable report even if the underlying Service rows later
-- change or get deleted. shareToken gates the public /cma/[token] route the way
-- ServicePacket.token gates /packet/[token].
--
-- RLS is enabled with no policies: every read/write goes through the server
-- with the service-role key (same posture as SupportTicket + ServicePacket),
...
-- The provider's "my CMAs" list reads newest-first within a space.
