-- ============================================================================
-- Model-aware credit metering for every agent/chat turn (provider + agency).
--
...
--
-- ONE trigger on ChatUsage covers EVERY path: provider direct (direct-stream),
-- provider agent in-process (sdk-chat-stream), agency direct (agency-direct), and
-- provider+agency via Modal (agent/ledger.py) — they all INSERT one ChatUsage row
...
-- the agency pool only when the agency is on a team plan AND the space
...
DECLARE
  ...
  v_member text;
BEGIN
  SELECT id, plan, "agencyId", "ownerId" INTO v_space FROM "Space" WHERE id = p_space_id;
  IF NOT FOUND THEN RETURN; END IF;

  IF v_space."agencyId" IS NOT NULL THEN
    SELECT id, plan INTO v_brk FROM "Agency" WHERE id = v_space."agencyId";
    IF FOUND AND v_brk.plan IN ('team', 'team_plus') THEN
      SELECT "userId" INTO v_member FROM "AgencyMembership"
        WHERE "agencyId" = v_space."agencyId" AND "userId" = v_space."ownerId" LIMIT 1;
      IF v_member IS NOT NULL THEN
        account_type := 'agency'; account_id := v_brk.id; RETURN NEXT; RETURN;
      END IF;
    END IF;
  END IF;
