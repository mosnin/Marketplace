...
-- Contact/Deal/DealStage/Appointment (policied in 20260607000011). None of the
...
-- ── AgencyIntegrationConnection — scope by agency-level membership ───────
-- AgencyMembership.userId is the INTERNAL User.id (REFERENCES "User"(id)),
-- and roles are 'agency_owner' | 'agency_admin' | 'provider_member'. Only
-- owners/admins manage agency integrations, so restrict to those two.
DROP POLICY IF EXISTS "agency_integration_connection_authenticated_select" ON "AgencyIntegrationConnection";
CREATE POLICY "agency_integration_connection_authenticated_select"
  ON "AgencyIntegrationConnection"
  FOR SELECT TO authenticated
  USING (
    "agencyId" IN (
      SELECT m."agencyId" FROM "AgencyMembership" m
      JOIN "User" u ON u.id = m."userId"
      WHERE u."clerkId" = auth.jwt() ->> 'sub'
        AND m."role" IN ('agency_owner', 'agency_admin')
    )
  );
REVOKE ALL ON "AgencyIntegrationConnection" FROM anon, authenticated;
GRANT SELECT ON "AgencyIntegrationConnection" TO authenticated;
