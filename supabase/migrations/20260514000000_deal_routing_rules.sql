-- ============================================================================
-- Agency Deal Routing Rules: Schema (Phase BP7d — rules layer)
-- ============================================================================
-- The BP7a/b lead-routing engine picks a provider_member via a single global
-- choice on the Agency row: manual, round_robin, or score_based. That
...
--   1. Load DealRoutingRule rows WHERE agencyId=? AND enabled=true
...
--      the agent is still eligible (active member of this agency with
...
--      Agency.assignmentMethod behaviour (round_robin / score_based /
...
CREATE TABLE IF NOT EXISTS "DealRoutingRule" (
  ...
  -- that agent is no longer eligible (offboarded, left the agency, no
  ...
  -- the tag is NULL the pool is the full provider_member set.
  --
  -- NOTE: as of this migration, neither User nor AgencyMembership has
  ...
  -- provider pool"). A follow-up migration can add
  -- AgencyMembership.tags text[] and the engine will start honouring
