...
--   SpaceSetting (per-agent custom forms), Agency (agency-level templates),
...
-- Where the form config came from: 'custom' (agent built it), 'agency'
-- (inherited from agency template), or 'legacy' (hardcoded form)
ALTER TABLE "SpaceSetting"
  ADD COLUMN IF NOT EXISTS "formConfigSource" text NOT NULL DEFAULT 'legacy'
    CHECK ("formConfigSource" IN ('custom', 'agency', 'legacy'));

-- ============================================================
-- Agency: agency-level form template
-- ============================================================

-- Agencies can define a template form that member agents inherit
ALTER TABLE "Agency"
  ADD COLUMN IF NOT EXISTS "agencyFormConfig" jsonb DEFAULT NULL;
...
-- GIN index on agencyFormConfig for JSONB queries
