-- Brief — the daily 7am snapshot Koala generates per provider.
--
-- One row per Space per UTC day. The cron at /api/cron/daily-briefing
-- compiles a Brief from the provider's pipeline + leads + calendar
...
--   'seen'       — provider opened it; tracked for momentum-line analytics
...
-- regenerating before the provider sees it would UPSERT not INSERT.
