-- ============================================================================
-- Deal Review Requests: Agency Sign-Off On High-Stakes Deals
-- ============================================================================
-- WHY: Today agencies can't be pinged for sign-off on a specific deal. Agents
-- run high-stakes transactions (unusual commission splits, aggressive price
-- concessions, atypical contingencies) past their agency over Slack or in
-- person, leaving no audit trail and no structured queue on the agency side.
-- High-stakes deals effectively fly blind — the agency only finds out when
...
-- can't spam the agency queue with duplicate flags on the same deal.
-- ============================================================================

-- 1. DealReviewRequest — one row per "flag this deal for agency review".
...
-- 2. DealReviewComment — threaded back-and-forth between agency and agent.
...
--    (a) Agency's queue: filter by agency + status, usually status='open'.
...
--    (agency membership, agent-owns-deal, agency-owns-agency) at the
