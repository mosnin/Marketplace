-- ============================================================================
-- Agency Billing: Per-Agency Seat-Based Subscriptions
-- ============================================================================
-- WHY: Previously, billing was tied to the agency_owner's personal Space. This
...
--
-- Moving the subscription, plan, and seatLimit onto Agency itself closes
-- all three gaps: seats are counted against the agency row, usage is
-- directly queryable, and the agency_owner's personal Space billing becomes
...
-- 3. Bootstrap existing agencies' billing from their agency_owner's personal
--    Space subscription — but ONLY for rows that don't already have a
--    subscription on the Agency side. One-shot, idempotent by the guard.
--    Space.stripe* columns are intentionally left intact; solo providers (users
--    with no Agency) still bill through their Space.
UPDATE "Agency" b
