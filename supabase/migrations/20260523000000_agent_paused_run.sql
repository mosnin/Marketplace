...
-- checkpoint: the provider asked Koala to do something, a tool needs approval,
-- the run is suspended until they decide. Drafts are "Koala composed
...
-- when the provider decides. Schema-versioned by the SDK itself
...
--
-- The approvals JSONB is the provider-facing prompt list — `ApprovalPrompt[]`
...
CREATE TABLE IF NOT EXISTS "AgentPausedRun" (
  ...
  -- Clerk userId of the provider whose chat is paused. Not a Space-scoped
...
-- Per-user filter for sessions where multiple providers share a Space.
