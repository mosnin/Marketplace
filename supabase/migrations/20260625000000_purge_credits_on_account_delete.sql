...
--   rows and leaves another account's rows untouched; same for Agency.
-- ============================================================================

CREATE OR REPLACE FUNCTION purge_credit_rows_for_account() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  -- TG_ARGV[0] is the accountType this trigger guards ('space' | 'agency').
...
CREATE OR REPLACE TRIGGER trg_purge_credits_on_agency_delete
  AFTER DELETE ON "Agency"
  FOR EACH ROW EXECUTE FUNCTION purge_credit_rows_for_account('agency');
