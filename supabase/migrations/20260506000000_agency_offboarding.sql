...
--   2. Defines `offboard_agency_member(...)`, a SECURITY DEFINER function
--      that performs (or previews, via dry-run) the re-parenting of every
--      agency-scoped row in a single transaction. If any statement fails,
...
-- is responsible for authorization (only agency_owner / agency_manager may
...
-----------------------------------------------------------------------
-- 2. offboard_agency_member — atomic transfer + state update
-----------------------------------------------------------------------
...
BEGIN
  ...
  IF p_dry_run THEN
        ...
        AND d.id IN (
          SELECT dc."dealId"
            ...
            WHERE c."spaceId" = v_leaving_space_id
              AND c."agencyId" = p_agency_id
        );

    SELECT COUNT(*) INTO v_open_appointment_count
      FROM "Appointment" t
    ...
    RETURN json_build_object(
      ...
      'openAppointmentCount', v_open_appointment_count
    );
  END IF;
  ...
  -- (a) Move agency-scoped contacts; capture their ids.
  ...
  -- (f) Move Appointments that are in the leaving space AND for a moved contact.
  WITH moved AS (
    UPDATE "Appointment"
       SET "spaceId" = v_destination_space_id
     WHERE "spaceId" = v_leaving_space_id
  ...
  -- (h) Remove the agency membership row — access-block agent also keys
  --     off User.status, but dropping the membership makes the severance
  --     explicit and causes any agency-scoped queries to fail closed.
