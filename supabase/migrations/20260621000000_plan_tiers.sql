...
--
-- Adds the Space-level plan tier (free|solo|pro) and extends the Agency plan
...
--
-- ✓ VALIDATED on PostgreSQL 16 (applies clean against Space/Agency; CHECK
...
-- ── Agency: extend plan enum with team|team_plus ─────────────────────────
-- Drop whatever CHECK currently constrains Agency.plan (name may vary), then
...
BEGIN
  SELECT conname INTO c
    FROM pg_constraint
   WHERE conrelid = '"Agency"'::regclass
     AND contype = 'c'
     AND pg_get_constraintdef(oid) ILIKE '%plan%';
  IF c IS NOT NULL THEN
    EXECUTE format('ALTER TABLE "Agency" DROP CONSTRAINT %I', c);
  END IF;
END $$;

ALTER TABLE "Agency" ADD COLUMN IF NOT EXISTS "planActivatedAt" timestamptz;
ALTER TABLE "Agency"
  ADD CONSTRAINT "Agency_plan_check"
