-- Knowledge base for Koala — app documentation that Koala can look up
-- on demand when providers ask "how do I..." or "why isn't X working".
...
Each contact has a lead type: buyer (purchasing a service), rental (looking to lease), or seller (listing a service for sale).
...
To view all contacts, click People in the sidebar. To add a new contact, tell Koala "add a contact named [name]" or click the + button on the People page.
Contacts move through pipeline stages: QUALIFICATION → APPOINTMENT → APPLICATION.'),
...
The intake form is customizable — ask Koala to "show me my intake form" or go to Intake form in the More section of the sidebar.
Koala can edit your intake form questions directly: ask it to "add a question about pets to the rental form".'),
...
To create a deal, tell Koala "create a deal for [contact name]" or click New Deal on the Deals page.
Deals can have a linked service, value, commission rate, close date, and contacts attached.
...
('services', 'How services work',
'Services are real estate listings linked to your deals. They store address, list price, beds, baths, square footage, and listing status.
To add a service, create a new deal — services are created as part of the deal flow. Click Add service in the sidebar under Services, which takes you to the new deal form.
To view all services, click Services in the sidebar.
Commission reports and year-to-date earnings are under Services → Commissions in the sidebar.'),
...
'The Calendar page shows all scheduled appointments and follow-up dates.
Appointments are automatically added to the calendar when booked via Koala or the appointment booking flow.
If you have Google Calendar connected (via Settings → Integrations), appointments sync to your Google Calendar automatically.
To connect Google Calendar, go to Settings → Integrations in the sidebar.'),

('koala', 'What Koala can do',
...
Koala never sends messages — it creates drafts for your review.'),

('koala', 'Koala does not create contacts unless you ask it to',
'A common mistake: telling Koala about a new lead and expecting it to save automatically. You must explicitly ask Koala to create the contact.
Correct: "Add a new lead named Orlando, rental, $3500-4500/mo, looking for 2BR in Edgewater, end of June"
Incorrect: "I have a new lead named Orlando" — Koala will acknowledge but not create the record unless you say "add them" or "create a contact".
If Koala says it has "noted" something, that means it stored the info in memory only, not in your CRM. Always say "add" or "create" to get a real contact record.'),

('settings', 'Settings and integrations',
'Settings are in the sidebar. Key sections:
- Profile: your name, business focus, communication preferences for Koala
- Integrations: connect Google Calendar for appointment sync
- Intake form: customize your lead intake form questions
- Agents: configure AI agent behavior and personalization
The Agents page lets you set Koala''s tone and focus areas.'),

('troubleshooting', 'Contact or deal not showing up after Koala created it',
'If Koala says it created a contact or deal but you do not see it, check:
1. Refresh the page — real-time updates sometimes need a manual refresh.
2. Check that Koala actually called the tool — look for a confirmation like "Created contact Orlando" in the response.
3. If Koala said "I have noted..." or "I have added to memory..." without confirming a tool call, it did NOT create the record. Ask again: "Please create a contact for Orlando."
