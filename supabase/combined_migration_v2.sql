...
-- ── 3. Agency offboarding (20260506000000) ─────────────────────────────────
ALTER TABLE "User"
...
DECLARE
  ...
  v_open_appointment_count      integer := 0;
BEGIN
  ...
  IF p_dry_run THEN
    SELECT COUNT(*) INTO v_contact_count FROM "Contact"
      WHERE "spaceId" = v_leaving_space_id AND "agencyId" = p_agency_id;
    SELECT COUNT(DISTINCT d.id) INTO v_deal_count FROM "Deal" d
      WHERE d."spaceId" = v_leaving_space_id
        AND d.id IN (SELECT dc."dealId" FROM "DealContact" dc
          ...
          WHERE c."spaceId" = v_leaving_space_id AND c."agencyId" = p_agency_id);
    SELECT COUNT(*) INTO v_open_appointment_count FROM "Appointment" t
      JOIN "Contact" c ON c.id = t."contactId"
      WHERE t."spaceId" = v_leaving_space_id AND c."spaceId" = v_leaving_space_id
        AND c."agencyId" = p_agency_id AND t."startsAt" >= now();
    RETURN json_build_object('dryRun', true, 'contactCount', v_contact_count,
      'dealCount', v_deal_count, 'openAppointmentCount', v_open_appointment_count);
  END IF;
  ...
  UPDATE "DealChecklistItem" SET "spaceId" = v_destination_space_id
    WHERE "dealId" IN (SELECT id FROM _moved_deals);
  WITH moved AS (
    UPDATE "Appointment" SET "spaceId" = v_destination_space_id
    WHERE "spaceId" = v_leaving_space_id
      AND "contactId" IN (SELECT id FROM _moved_contacts)
    RETURNING 1)
  SELECT COUNT(*) INTO v_appointment_count FROM moved;
  ...
  DELETE FROM "AgencyMembership"
    WHERE "userId" = p_leaving_user_id AND "agencyId" = p_agency_id;
  RETURN json_build_object('dryRun', false, 'contactsMoved', v_contact_count,
    'dealsMoved', v_deal_count, 'appointmentsMoved', v_appointment_count);
END;
$$;
...
DECLARE
  v_agency_id text; v_owner_id text;
  v_agent_rate numeric(5,2); v_agency_rate numeric(5,2);
  v_deal_value numeric(12,2);
BEGIN
  SELECT s."agencyId", s."ownerId" INTO v_agency_id, v_owner_id
    FROM "Space" s WHERE s.id = NEW."spaceId";
  IF v_agency_id IS NULL THEN RETURN NEW; END IF;
  SELECT b."defaultAgentRate", b."defaultAgencyRate" INTO v_agent_rate, v_agency_rate
    FROM "Agency" b WHERE b.id = v_agency_id;
  IF v_agent_rate IS NULL OR v_agency_rate IS NULL THEN RETURN NEW; END IF;
  v_deal_value := COALESCE(NEW.value, 0);
  INSERT INTO "CommissionLedger" ("agencyId","agentUserId","dealId","closedAt",
    "dealValue","agentRate","agencyRate","referralRate","agentAmount","agencyAmount",
    "referralAmount",status)
  VALUES (v_agency_id, v_owner_id, NEW.id, now(), v_deal_value, v_agent_rate,
    v_agency_rate, 0,
...
INSERT INTO "CommissionLedger" ("agencyId","agentUserId","dealId","closedAt",
  "dealValue","agentRate","agencyRate","referralRate","agentAmount","agencyAmount",
  "referralAmount",status)
SELECT s."agencyId", s."ownerId", d.id, COALESCE(d."updatedAt", now()),
  COALESCE(d.value, 0), b."defaultAgentRate", b."defaultAgencyRate", 0,
...
WHERE d.status = 'won' AND s."agencyId" IS NOT NULL
...
DECLARE
  ...
  v_appointment_count integer := 0; v_open_appointment_count integer := 0;
  v_dest_status text;
BEGIN
  SELECT id INTO v_leaving_space_id
    FROM "Space" WHERE "ownerId" = p_leaving_user_id FOR UPDATE;
...
-- ── 6. Agency billing (20260509000000) ─────────────────────────────────────
ALTER TABLE "Agency"
...
UPDATE "Agency" SET "seatLimit" = 5  WHERE plan = 'starter' AND "seatLimit" IS NULL;
UPDATE "Agency" SET "seatLimit" = 15 WHERE plan = 'team'    AND "seatLimit" IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_agency_stripe_customer
  ON "Agency"("stripeCustomerId")
...
CREATE INDEX IF NOT EXISTS idx_dealreview_agency_status
  ON "DealReviewRequest"("agencyId", status);
...
-- ── 8. Agency templates (20260511000000) ───────────────────────────────────
...
-- Data migration from legacy '[AGENCY_TEMPLATES]' Note rows (idempotent)
DO $$
DECLARE
  note_rec record; tmpl jsonb;
  v_agency_id text; v_name text; v_category text; v_body text;
...
BEGIN
  ...
  LOOP
    IF note_rec.agency_id IS NULL THEN CONTINUE; END IF;
    ...
    LOOP
      ...
      IF v_category NOT IN ('follow-up','intro','closing','appointment-invite') THEN CONTINUE; END IF;
      v_created_by := NULL;
      IF v_created_by_raw IS NOT NULL THEN
        ...
        END IF;
      END IF;
      INSERT INTO "AgencyTemplate" ("agencyId",name,category,channel,subject,body,
        version,"publishedAt","publishedCount","createdByUserId")
      SELECT v_agency_id,v_name,v_category,'note',NULL,v_body,1,NULL,0,v_created_by
      WHERE NOT EXISTS (SELECT 1 FROM "AgencyTemplate" bt
        WHERE bt."agencyId" = v_agency_id AND bt.name = v_name);
    END LOOP;
  END LOOP;
END $$;
...
-- ── 10. Agency routing (20260513000000) ────────────────────────────────────
ALTER TABLE "Agency"
...
CREATE INDEX IF NOT EXISTS idx_deal_routing_rule_agency_priority
  ON "DealRoutingRule" ("agencyId", priority ASC, enabled);
CREATE INDEX IF NOT EXISTS idx_deal_routing_rule_agency_enabled
  ON "DealRoutingRule" ("agencyId", enabled);
...
CREATE TABLE IF NOT EXISTS "AgentGoal" (
  ...
  "goalType"     VARCHAR(50) NOT NULL CHECK ("goalType" IN ('follow_up_sequence','appointment_booking','offer_progress','deal_close','reengagement','custom')),
...
CREATE TABLE IF NOT EXISTS "Artifact" (
  ...
  "artifactType"     text        NOT NULL CHECK ("artifactType" IN (
                                   ...
                                   'appointment_booking','goal_plan','report','raw_output')),
  "title"            text        NOT NULL,
...
CREATE TABLE IF NOT EXISTS "ArtifactVersion" (
  ...
  "metadata"       jsonb       DEFAULT '{}',
  "createdByAgent" text        NOT NULL DEFAULT 'koala',
  "createdAt"      timestamptz NOT NULL DEFAULT now()
);
