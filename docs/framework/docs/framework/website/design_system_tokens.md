...
Use CSS custom services for colors (enables runtime light/dark switching):
```css
...
Map these tokens to Tailwind CSS theme extensions in `tailwind.config.ts`. Use CSS custom services for runtime theme switching (light/dark mode). The `prefers-color-scheme` media query handles automatic theme detection; a manual toggle overrides it.
