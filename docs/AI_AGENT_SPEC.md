...
> The core agent runtime that turns a provider's natural-language request into a
...
> reload and agency-review.

A provider types `"email Jane about the appointment Friday"`; the server opens an SSE
stream, the model plans a `send_email` call, the client shows an approval
card, the provider approves, the email ships, and a `ToolCallBlock` lands in
...
```
KoalaWorkspace (components/koala/koala-workspace.tsx)
...
| `get_contact` | auto | none | Single contact + linked deals + recent appointments |
...
| `schedule_appointment` | required | **30/hr** (schedule-appointment.ts:70) | Accepts contactId OR walk-in guest fields |
