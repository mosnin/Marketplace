...
### Edge Case: Agency-only user tries to access /dashboard
Scenario: User with accountType='agency_only' navigates to /dashboard.
Expected system behavior: Redirect to /agency. Do not show workspace creation form.
...
### Edge Case: Appointment double-booking
Scenario: Two prospects try to book the same time slot simultaneously.
Expected system behavior: First booking succeeds. Second receives "slot no longer available" error. Buffer minutes between appointments enforced. Availability check is authoritative at booking time.
Relevant page or module: /book/[slug], appointment API

### Edge Case: Delete contact linked to deals
Scenario: Provider tries to delete a contact that is linked to one or more deals via DealContact.
...
Scenario: Provider's Google Calendar OAuth token expires.
Expected system behavior: Appointment creation works without calendar sync. Calendar sync fails silently. Prompt to reconnect in appointment settings.
Relevant page or module: /s/[slug]/appointments, appointment settings
...
Scenario: Provider clicks an invitation link after the 7-day expiry.
Expected system behavior: Show "Invitation expired" message with prompt to request a new invitation from the agency. Do not auto-accept or create membership.
