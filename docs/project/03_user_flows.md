...
2. Sees "Welcome to Koala" with Clerk sign-in component
...
3. If no workspace → multi-step onboarding flow:
   - Step 1: Account type selection (provider / agency)
...
Branch: If agency-only account type selected → redirect to `/agency` after agency creation.
Branch: If user already has a space → redirect directly to `/s/[slug]` via `/dashboard`.

## Flow: Onboarding To First Value

1. Provider lands on workspace dashboard `/s/[slug]`
2. Dashboard shows intake link card with "Live" badge
3. Provider copies intake link and shares it (bio, DMs, ads, email)
...
7. Provider sees new application in leads view with "New" badge and score
8. **First value event achieved** — structured lead with AI context in CRM

## Flow: Repeat Usage (Daily Workflow)

1. Provider logs in → `/dashboard` redirects to `/s/[slug]`
2. Reviews dashboard: new applications count, follow-ups due, upcoming appointments
...
6. Schedules appointments via appointments view or shares booking link
...
## Flow: Appointment Booking (Prospect)

1. Prospect receives appointment booking link `/book/[slug]`
2. Selects service profile (if multiple)
3. Views available dates and time slots
4. Fills in guest name, email, phone, notes
5. Confirms booking → Appointment created with status='scheduled'
6. Confirmation email sent to prospect via Resend
7. Provider sees appointment in appointments view and dashboard widget
...
## Flow: Agency Onboarding

1. Agency signs up and selects "agency" account type
2. Creates agency (name, optional logo, website)
3. Receives join code for self-service provider joining
4. Sends email invitations to providers via `/agency/invitations`
5. Invited provider clicks invite link `/invite/[token]` → accepts invitation
6. Provider's Space linked to Agency via agencyId
7. Agency sees provider in `/agency/providers` list

Branch: Provider can also join via join code at `/join/[code]`.

## Flow: Billing Upgrade

1. Provider navigates to `/s/[slug]/billing`
...
5. `/admin/agencies` → view all agencies, click into detail with members
