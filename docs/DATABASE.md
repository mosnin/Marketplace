...
> [docs/AGENCY_SPEC.md](AGENCY_SPEC.md)) which explain WHAT tables
> exist and WHY.
...
- `20260511000000_agency_templates.sql`
...
renamed to `20260319000007_appointments_applications_pro.sql` so it sorts *after* the
migration that creates `Appointment` (it referenced `Appointment` before it existed, which
...
- Prefer `CHECK` constraints for enum-ish text columns over a trigger
  (see `20260511000000_agency_templates.sql` line 31 for the
  canonical pattern:
  `CHECK (category IN ('follow-up', 'intro', 'closing', 'appointment-invite'))`).
...
- Use `plpgsql` `DO $$ ... $$` blocks for one-shot data migrations —
  ...
  [`supabase/migrations/20260511000000_agency_templates.sql`](../supabase/migrations/20260511000000_agency_templates.sql)
...
| Backfill data | `DO $$ ... $$` block with `WHERE NOT EXISTS` guard; log skipped rows via `RAISE NOTICE` (pattern: `20260511000000_agency_templates.sql`) |
...
| Enable RLS on a new table | `ALTER TABLE "Foo" ENABLE ROW LEVEL SECURITY;` with no policies — defence-in-depth, matches the rest of the agency tables |
