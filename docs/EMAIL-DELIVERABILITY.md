...
Transactional mail Koala sends from its **own** domain (follow-up digests,
agency weekly reports, system notifications via Resend) needs DNS
...
> the sending domain (wherever `usekoala.com` DNS is hosted), **not** in this
...
1. **User-connected Gmail / Outlook sending (no action needed).**
   When a provider connects their own Gmail/Outlook (via the integrations layer)
...
2. **Transactional mail from the app's own domain (records REQUIRED).**
   ...
   `RESEND_FROM_EMAIL`, default sender `notifications@alerts.usekoala.com`) —
   comes from a Koala-owned domain. That domain needs SPF, DKIM, and DMARC, or
...
Assume the sending subdomain is **`alerts.usekoala.com`** (matches the Resend
default `notifications@alerts.usekoala.com`). Adjust the host to whatever
...
```
Type:  TXT
Host:  alerts.usekoala.com        (or "send.usekoala.com" per Resend)
...
Host:  <selector>._domainkey.alerts.usekoala.com
...
```
Type:  TXT
Host:  _dmarc.usekoala.com
Value: v=DMARC1; p=none; rua=mailto:dmarc@usekoala.com; fo=1; adkim=s; aspf=s
```
...
2. **Command line spot-check:**
   ```
   dig +short TXT alerts.usekoala.com          # SPF present
   dig +short CNAME <selector>._domainkey.alerts.usekoala.com   # DKIM present
   dig +short TXT _dmarc.usekoala.com          # DMARC present
   ```
