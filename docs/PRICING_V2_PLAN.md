...
Implementation plan for the **Koala V2 three-layer pricing model** (see the
product spec: Free / Solo / Pro Performer / Team / Team Plus + agency
...
| Billing columns (Agency) | same + `plan (starter\|team\|enterprise)`, `seatLimit (5\|15\|null)` | `supabase/migrations/20260509000000_agency_billing.sql` |
...
| Access gating | `status IN (active,trialing)` in both layouts | `app/s/[slug]/layout.tsx`, `app/agency/layout.tsx` |
| Seat counting | members + non-expired pending invites; `checkSeatCapacity()` | `lib/agency-seats.ts` |
...
- ❌ No per-account **plan tier** on `Space` (only `Agency.plan`). No Free tier.
...
- **Layer 2 — Agency expansion:** auto-expanding per-agent price bands (10–24 → $69/mo, 25–49 → $59, 50–99 → $49, 100–199 → $39, 200+ custom; annual ≈ 18–20% less).
...
- **Credits** are spent per premium workflow: audit 50, follow-up sequence 40, qualification 25, appointment 15, briefing 10, call prep 3, score update 1. Paid-plan credits **roll over 30 days**; top-ups (1k/$29, 3k/$69, 8k/$149) are one-time, also 30-day rollover.
...
6. **Migration of current subscribers — DECIDED.** `STRIPE_PRICE_ID` (current Solo, already **$97**) → new **Solo** (clean 1:1, just start the 1,500/mo grant). Agency `starter`→**Team**, `team`→**Team Plus**, `enterprise`→Layer-2 custom. Grandfather any price delta for one billing cycle; backfill `plan` + an initial credit lot on deploy.
...
| Appointment booking | 15 | $0.28–0.98 | ~$0.05–0.15 | healthy |
...
Introduce a single concept: a **billing account** = the entity that owns a plan + credit balance. It is either a `Space` (Free/Solo/Pro) or a `Agency` (Team/Team Plus). One helper `lib/billing/account.ts → resolveBillingAccount(spaceId) → { type, id, plan }` so every debit/grant/gate goes through one resolver. This avoids scattering `space vs agency` branching across the metering call sites.
...
```
- DB: add `plan text NOT NULL DEFAULT 'free'` to `Space` (CHECK `free|solo|pro`); extend `Agency.plan` CHECK to add `team|team_plus` (+ the expansion is Layer 2). Replace `seatLimit` semantics with `includedUsers` from `PLANS`.
- `lib/env.ts`: add `STRIPE_PRICE_{SOLO,PRO,TEAM,TEAM_PLUS}` + the top-up + annual price IDs, with boot validation (today even the agency IDs are read raw from `process.env`).
...
CREATE TABLE "CreditLot" (              -- a grant or top-up that can expire
  ...
  "accountType" text NOT NULL CHECK ("accountType" IN ('space','agency')),
...
| Appointment booking | 15 | ✅ exists | `lib/ai-tools/tools/schedule-appointment.ts` |
...
### 4.5 Layer 2 — agency auto-expand
- Move agency subscription to **`quantity = active agent count`** (the count already exists in `lib/agency-seats.ts`) with a Stripe **graduated/tiered price** encoding the $69→$59→$49… bands. Adjust quantity (with proration) on member add/remove; reconcile in the webhook. Replaces the hard `seatLimit` caps with metered seats.
...
- **Phase 0 — Tiers + billing account.** `lib/plans.ts`, `Space.plan` migration, `Agency.plan` extension, env price IDs + validation, checkout/webhook tier routing, Free-tier gating. *(Stripe products must exist first.)*
...
- Products + monthly **and** annual recurring prices: Solo, Pro, Team (+ per-seat add-on), Team Plus (+ add-on), agency graduated seat price (bands), Layer-3 base + metered-usage price.
