...
The global search is triggered by **Cmd+K** (or Ctrl+K) from anywhere in the app. It searches **contacts**, **deals**, and **appointments** in parallel and shows results in a modal overlay.
...
User types in search modal (Cmd+K)
  ...
  → 3 parallel Supabase queries (contacts, deals, appointments)
  → JSON response: { contacts, deals, appointments }
...
| **Appointment** | `guestName`, `guestEmail`, `serviceAddress` | Guest name, service/date, status |

## Page-level search

Both the **Deals** and **Appointments** pages have inline search bars for filtering large datasets client-side.

| Page | File | Fields searched |
|------|------|----------------|
| Deals (Pipeline) | `components/deals/kanban-board.tsx` | title, address, description, contact names |
| Appointments | `app/s/[slug]/appointments/appointments-client.tsx` | guest name, email, phone, service address, notes, linked contact |
...
- The route logs specific errors for each entity: `[search] contacts error:`, `[search] deals error:`, `[search] appointments error:`
...
### 3. Appointments don't appear in results

**Check:**
- Does the `Appointment` table exist? Created by migration `20260319000001_appointment_booking.sql`
- Are the column names correct? The query uses: `guestName`, `guestEmail`, `serviceAddress`, `startsAt`, `status`
- Appointment search failures are non-fatal — contacts and deals will still return even if the Appointment query fails
