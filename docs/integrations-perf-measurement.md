...
A provider with Gmail + HubSpot + Slack + Google Calendar connected
...
- `agent/integrations.py` — `integration_tools_loaded` on agent build
  ...
  `total`, `connected_toolkits`. Tells us the realistic per-provider
  tool count.
...
- **Curated wins by <0.5s** → the model isn't really using the fast
  ...
  a curated tool fits. Likely a koala.py prompt fix.
...
The decision threshold is **net provider-perceived latency** —
...
number. The interesting number is whether the provider's chats — which
