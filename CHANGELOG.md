...
Chronological record of features, fixes, and changes in Koala. Most recent first.
...
- **Docs: Agency feature spec** — Landed `docs/AGENCY_SPEC.md` in four chunks covering concepts + permissions + data model, feature map for BP1–BP7 and linear steps 1–3, and the full route inventory + audit-logging notes (`b680f91`, `02e2b5b`, `14cedb5`, `491a103`).
...
Agency-side features delivered in seven phases. See `docs/AGENCY_SPEC.md` for the full feature map.

- **BP1 — agent offboarding** — Atomic RPC transfers contacts/deals/appointments when removing a provider; dialog wired + vitest coverage (`6db1e83`, `345e46d`).
...
- **BP4 — deal-at-risk dashboard** — Risk view on the agency pipeline with colourblind-safe dots and a healthy-state strip (`b721de9`, `3417eb1`).
- **BP5 — review requests** — Schema + flag UI + first two APIs, then review detail APIs + agency queue UI, API coverage, clickable "Review pending" chip, and offboarding gate + notify enrichment (`a72e108`, `c1507c0`, `1463ed4`, `cd6c99b`, `6d05915`).
- **BP6 — template versioning** — Agency templates schema + magic-Note extraction, GET/POST rewrite, `[id]` routes + tests, UI rewrite onto the new API, and security/correctness fixes (`46612ab`, `3246091`, `3e9196b`, `2571397`, `e274236`).
- **BP7 — lead routing** — Auto-routing schema on `Agency`, routing engine + settings wiring + tests, plus a no-mutate fix on Supabase responses (`d3176d4`, `511adb5`, `380fd83`).

### Linear steps 1–3

- **Step 1 — Provider reviews** — New provider-side "My reviews" page surfacing the agency's review requests (`b524072`).
- **Step 2 — Audit-log viewer** — Agency audit-log viewer that preserves the team-activity feed (`733f4a5`).
...
- **Feature: Resend email notifications for appointments** — All appointment emails (confirmation, reminder, follow-up, agent notification) migrated from SMTP/Nodemailer to Resend, matching the design language of lead notification emails (dark header, detail boxes, CTA buttons).
- **Feature: Telnyx SMS notifications** — New SMS integration (`lib/sms.ts`) for real-time text notifications:
  ...
  - Appointment bookings → guest confirmation SMS + agent alert SMS
...
- **Feature: Unified notification dispatcher** (`lib/notify.ts`) — Single entry point for all notifications. Checks both channel preferences (email on/off, SMS on/off) and per-event preferences before dispatching. Functions: `notifyNewLead`, `notifyNewAppointment`, `notifyNewDeal`, `notifyNewContact`.
- **Feature: Per-event notification toggles** — Users can independently enable/disable notifications for each event type: new leads, appointment bookings, new deals, follow-up reminders. These work as a second layer alongside the channel toggles.
...
- **Enhancement: Dashboard redesign** — Aligned agency dashboard with provider dashboard design patterns:
  ...
  - Max-width 1120px constraint matching provider dashboard
- **Feature: Agency onboarding checklist** — 4-step guided setup (invite providers, configure settings, review performance, export report) with progress bar and localStorage-persisted dismissal.
- **Feature: Team activity feed** — New `TeamActivityFeed` component + `/api/agency/activity` API route showing recent leads, deals, and appointments across all team members in a colored-dot timeline.
- **Feature: Search on providers page** — Client-side search input filtering providers by name or email.
...
- **Feature: Global search for agency users** — `GlobalSearch` and `NotificationCenter` now visible for all users with a workspace slug, including agency users.
...
- `notifyAppointmentBookings` boolean (default true)
...
- `components/agency/onboarding-checklist.tsx` — Agency onboarding checklist
- `components/agency/team-activity-feed.tsx` — Team activity timeline
- `components/agency/members-search.tsx` — Members page search wrapper
- `app/api/agency/activity/route.ts` — Agency team activity API
...
- Rewrote `README.md` from multi-tenant template to Koala-specific documentation covering features, tech stack, project structure, and setup
