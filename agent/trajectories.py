...
Why a dedicated table instead of joining at query time:
  ...
  - Cross-run queries ("every run where schedule_appointment fired after
    appointment_completed and the draft was approved") become a single SELECT
...
# table. The provider-facing live stream truncates at the same boundaries
