"""Contract + permission-gate tests for the 15 agency tools.

Phase 5 of Koala-for-Agencies — defends three invariants:

  1. Every tool calls `require_agency_role(ctx)` first.
  2. Cross-agency scope is enforced where applicable.
...
disabled so guard refusals re-raise as `AgencyPermissionError` instead of
being silently converted into model-visible strings.

Run from `agent/` with:

    NEXT_PUBLIC_SUPABASE_URL=stub SUPABASE_SERVICE_ROLE_KEY=stub \\
      python -m pytest tests/test_agency_tools.py -v
"""
...
# Make `import db`, `import security.context`, `import tools.agency.*` work
...
from tools.agency import AGENCY_TOOLS  # noqa: E402
from tools.agency._guards import AgencyPermissionError  # noqa: E402
from tools.agency import actions as actions_mod  # noqa: E402
from tools.agency import performance as performance_mod  # noqa: E402
from tools.agency import pipeline as pipeline_mod  # noqa: E402
from tools.agency import revenue as revenue_mod  # noqa: E402
from tools.agency import team as team_mod  # noqa: E402
...
def fake_db(monkeypatch: pytest.MonkeyPatch) -> _FakeSupabase:
    """Replace every `supabase()` import with a fake. Each agency tool module
...
def _ctx(
    *,
    agency_role: str = "agency_owner",
    agency_id: str = "bk_a",
    user_id: str = "u_agency",
) -> AgentContext:
    return AgentContext(
        space_id="s_agency",
        space_name="Agency Workspace",
...
def _tool_by_name(name: str) -> Any:
    for t in AGENCY_TOOLS:
        if t.name == name:
            return t
    raise LookupError(f"No agency tool named {name!r}")
...
_ALL_TOOL_INPUTS: list[tuple[str, dict[str, Any]]] = [
    ("team_health", {}),
    ("provider_performance", {"provider_id": "u_provider"}),
    ("read_provider_morning_story", {"provider_id": "u_provider"}),
    ("find_stuck_deals", {}),
    ...
    ("find_at_risk_agents", {}),
    ("reassign_lead", {"lead_id": "c_lead", "to_provider_id": "u_provider"}),
    ("flag_deal_for_agency_review", {"deal_id": "d1", "reason": "needs a look here please"}),
    ("send_team_announcement", {"message": "stand-up at 9am"}),
    ("change_member_role", {"member_id": "m1", "new_role": "agency_admin"}),
...
@pytest.mark.asyncio
async def test_non_agency_ctx_is_refused(tool_name: str, args: dict[str, Any]) -> None:
    """Every agency tool refuses a non-agency caller before doing any work.
    ...
    ctx = _ctx(agency_role="provider_member")
...
async def test_empty_role_is_refused(tool_name: str, args: dict[str, Any]) -> None:
    """An empty agency_role (autonomous run / missing context) is refused."""
    tool = _tool_by_name(tool_name)
    ctx = _ctx(agency_role="")
    with pytest.raises(AgencyPermissionError):
        await _invoke(tool, ctx, args)


# ── 2. Cross-agency scope ────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_provider_performance_refuses_cross_agency(fake_db: _FakeSupabase) -> None:
    """`provider_performance` runs against a provider whose Space lives in a
    DIFFERENT agency — must refuse with AgencyPermissionError."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(
            data={"id": "s_other", "ownerId": "u_provider", "agencyId": "bk_OTHER", "name": "Other Agency"}
        ),
    )
    tool = _tool_by_name("provider_performance")
    ctx = _ctx(agency_id="bk_a")
    with pytest.raises(AgencyPermissionError):
        await _invoke(tool, ctx, {"provider_id": "u_provider"})


@pytest.mark.asyncio
async def test_read_provider_morning_story_refuses_cross_agency(fake_db: _FakeSupabase) -> None:
    """Same cross-agency refusal on `read_provider_morning_story`."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_other", "ownerId": "u_provider", "agencyId": "bk_OTHER"}),
    )
    tool = _tool_by_name("read_provider_morning_story")
    ctx = _ctx(agency_id="bk_a")
    with pytest.raises(AgencyPermissionError):
        await _invoke(tool, ctx, {"provider_id": "u_provider"})
...
@pytest.mark.asyncio
async def test_reassign_lead_refuses_cross_agency_contact(fake_db: _FakeSupabase) -> None:
    """`reassign_lead` returns ok=false when the contact is missing for this
    agency. The TS path scopes contact lookup to agency_id, and falls
    back to the agency owner's space — both lookups should miss for a
    cross-agency lead. We mock contact lookup as None and ensure the
    Agency row in `_resolve_agency_space` also returns no useful space.
    """
    # First contact lookup (by agencyId): miss.
    fake_db.set_for("Contact", "select", _FakeResult(data=None))
    # `_resolve_agency_space` then queries Agency; return a row but with
    # no matching space the second query won't help.
    fake_db.set_for(
        "Agency",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_agency", "name": "B"}),
    )
    # The agency-owner's Space lookup yields a space (s_agency).
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_agency", "ownerId": "u_agency", "name": "Agency Space", "agencyId": "bk_a"}),
    )
    # Second contact lookup (by agency_space.space_id): also miss.
    fake_db.set_for("Contact", "select", _FakeResult(data=None), )

    tool = _tool_by_name("reassign_lead")
    ctx = _ctx(agency_id="bk_a")
    res = await _invoke(tool, ctx, {"lead_id": "c_other", "to_provider_id": "u_provider"})
    assert res["ok"] is False
    assert "agency" in res["summary"].lower() or "not found" in res["summary"].lower()
...
async def test_change_member_role_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        ...
        "select",
        _FakeResult(
            data={"id": "m1", "userId": "u_provider", "role": "provider_member", "agencyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_provider", "name": "Alice", "email": "alice@x.com"}),
    )

    tool = _tool_by_name("change_member_role")
    ctx = _ctx(agency_role="agency_owner")
    res = await _invoke(tool, ctx, {"member_id": "m1", "new_role": "agency_admin"})
    assert res["ok"] is False
...
async def test_change_member_role_confirmed_executes(fake_db: _FakeSupabase) -> None:
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m1", "new_role": "agency_admin", "confirmed": True},
    )
    assert res["ok"] is True
    assert res["newRole"] == "agency_admin"
    assert any(c["table"] == "AgencyMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_offboard_member_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_provider", "name": "Alice", "email": "alice@x.com", "status": "active"}),
    )
...
async def test_offboard_member_confirmed_calls_rpc(fake_db: _FakeSupabase) -> None:
    """Confirmed call fires the offboard_agency_member RPC and audits."""
    # Target membership.
    fake_db.set_for(
        "AgencyMembership",
        "select",
        ...
        _FakeResult(
            data={"id": "m2", "userId": "u_dest", "role": "provider_member", "agencyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    fake_db.set_for(
        "rpc:offboard_agency_member",
        "rpc",
        _FakeResult(data=[{"contacts_moved": 4, "deals_moved": 2, "appointments_moved": 1}]),
    )
    ...
    assert res["appointmentsMoved"] == 1
    rpc_calls = [c for c in fake_db.log if c["op"] == "rpc"]
    assert len(rpc_calls) == 1
    assert rpc_calls[0]["payload"]["fn"] == "offboard_agency_member"
    assert any(c["table"] == "AuditLog" and c["op"] == "insert" for c in fake_db.log)
...
async def test_reassign_lead_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    # Contact lookup by agencyId succeeds.
    fake_db.set_for(
        "Contact",
        "select",
        _FakeResult(
    ...
    # `_resolve_agency_space` chain.
    fake_db.set_for(
        "Agency",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_agency", "name": "B"}),
    )
    # Space lookups: agency's space, target provider's space.
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_agency", "ownerId": "u_agency", "name": "Agency", "agencyId": "bk_a"}),
        _FakeResult(data={"id": "s_provider", "ownerId": "u_provider", "name": "Provider", "agencyId": "bk_a"}),
    )
    # Membership for target provider.
    fake_db.set_for(
        "AgencyMembership",
        "select",
        _FakeResult(
            data={"id": "m_provider", "userId": "u_provider", "role": "provider_member"}
        ),
    )
    # Target user.
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_provider", "name": "Jordan", "email": "j@x.com"}),
    )
    ...
    res = await _invoke(
        tool,
        ctx,
        {"lead_id": "c1", "to_provider_id": "u_provider", "reason": "lighter load"},
    )
    assert res["ok"] is True
    audit_inserts = [
...
async def test_flag_deal_writes_audit(fake_db: _FakeSupabase) -> None:
    fake_db.set_for(
        "Deal",
        "select",
        _FakeResult(data={"id": "d1", "title": "Sarah Chen — 123 Main", "spaceId": "s_provider", "status": "active"}),
    )
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_provider", "ownerId": "u_provider", "agencyId": "bk_a"}),
    )

    tool = _tool_by_name("flag_deal_for_agency_review")
    ctx = _ctx(agency_role="agency_owner")
    res = await _invoke(
        tool,
        ctx,
...
async def test_send_team_announcement_writes_audit(fake_db: _FakeSupabase) -> None:
    fake_db.set_for(
        "Agency",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_agency", "name": "Apex"}),
    )
    fake_db.set_for(
        "Space",
        "select",
    ...
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_agency", "name": "Sam", "email": "sam@x.com"}),
    )
    fake_db.set_for(
        "AgencyMembership",
        "select",
        _FakeResult(
            data=[
                {"id": "m1", "userId": "u_agency", "role": "agency_owner"},
                {"id": "m2", "userId": "u_provider", "role": "provider_member"},
            ]
        ),
    )
...
async def test_set_routing_rule_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    audits = [c for c in fake_db.log if c["table"] == "AuditLog" and c["op"] == "insert"]
    assert len(audits) == 1
    assert audits[0]["payload"]["resource"] == "Agency"
    ...
    updates = [c for c in fake_db.log if c["table"] == "Agency" and c["op"] == "update"]
...
# ── 6. agency_admin can only move provider_member ⇄ provider_member ───────────
...
async def test_change_member_role_admin_cannot_change_other_admins(
    ...
    """A agency_admin caller cannot demote another agency_admin to
    provider_member — that move is owner-only."""
    fake_db.set_for(
        "AgencyMembership",
        "select",
        _FakeResult(
            data={"id": "m_other_admin", "userId": "u_admin2", "role": "agency_admin", "agencyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    tool = _tool_by_name("change_member_role")
    ctx = _ctx(agency_role="agency_admin")
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_other_admin", "new_role": "provider_member", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
    # No UPDATE happened — admin attempted owner-only move.
    assert not any(c["table"] == "AgencyMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_change_member_role_admin_cannot_promote_to_admin(
    ...
    """A agency_admin cannot promote a provider_member to agency_admin
    either — promotion-to-admin is owner-only."""
    fake_db.set_for(
        "AgencyMembership",
        "select",
        _FakeResult(
            data={"id": "m_provider", "userId": "u_provider", "role": "provider_member", "agencyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_provider", "new_role": "agency_admin", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
...
async def test_offboard_member_admin_refused(fake_db: _FakeSupabase) -> None:
    """offboard_member is agency_owner-only — agency_admin gets refused."""
    tool = _tool_by_name("offboard_member")
    ctx = _ctx(agency_role="agency_admin")
    res = await _invoke(tool, ctx, {"member_id": "m1"})
    assert res["ok"] is False
...
async def test_set_routing_rule_rejects_invalid_strategy(fake_db: _FakeSupabase) -> None:
    ...
    # No UPDATE attempted.
    assert not any(c["table"] == "Agency" and c["op"] == "update" for c in fake_db.log)
...
# ── Sanity: AGENCY_TOOLS has all 15 tools registered ────────────────────────


def test_agency_tools_registry_has_fifteen() -> None:
    assert len(AGENCY_TOOLS) == 15
    names = {t.name for t in AGENCY_TOOLS}
    expected = {
        "team_health",
        "provider_performance",
        "read_provider_morning_story",
        "find_stuck_deals",
        "find_unassigned_leads",
        "find_breached_leads",
        "commission_report",
