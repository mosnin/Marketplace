...
sent without the provider's approval — that draft-only boundary is the
...
# one opaque step, so a sweep that failed at the 4th tool gave the agency
...
async def pop_triggers(space_id: str) -> list[dict]:
    ...
    try:
        ...
        # never process the queued lead/appointment/stage events. Log it.
...
async def requeue_triggers(
    ...
    A crashed run would otherwise drop the provider's events silently. When
...
def _build_opening_prompt(
    ...
    """Frame the autonomous run for Koala.
    ...
    if instruction:
        triggers_block = (
            "ROUTINE — a standing instruction the provider saved for Koala "
...
async def run_agent_for_space(
    ...
    trigger_source:
        ...
        inbox UI can render the "Koala noticed because…" breadcrumb.
        None for chat / routine / sweep paths.
    """
    # Respect the on/off switch. The provider can pause Koala from the
    ...
    # drafts in the provider's inbox — and would both pass the daily-budget
...
async def _run_locked(
    ...
    # Solo providers: this is them. Agencies: it's the agency_owner.
    ...
    # the provider's intake URL where it's useful.
    ...
    # the provider's activity feed and the agency dashboard see what Koala
    ...
    # paths share the same _translate_tool_event payload so the provider's
    ...
    try:
        ...
        # Re-queue so a crashed run doesn't silently drop the provider's
        ...
        await publish_event(ctx, "error", f"Agent error: {exc}", agent_type="koala")
        # Make the failure visible. A swallowed error leaves the provider's
        ...
        # Drain the fire-and-forget publish tasks so the provider's activity
