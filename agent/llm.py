...
def _patch_openai_event_schemas() -> None:
    ...
    for cls, field_defaults in event_classes_and_fields:
        if getattr(cls, "_koala_patched", False):
            continue
...
# Allowlist of provider-selectable models — mirrors CHAT_MODELS in lib/llm.ts.
...
# bar is intentionally narrow — these are phrases that imply the provider
...
def decide_reasoning_effort(user_message: str | None = None) -> str:
    ...
    The env var KOALA_REASONING_EFFORT, when set to "low"/"medium"/"high",
    ...
    """
    override = (os.environ.get("KOALA_REASONING_EFFORT") or "").strip().lower()
