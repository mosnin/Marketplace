...
Secrets: a single Modal secret named "koala-secrets" containing all env
vars listed in config.py.

Sandbox economics — why we stay on Modal (audit, 2026-Q2):
  Koala uses @app.function + @modal.fastapi_endpoint — Modal's STANDARD
  ...
  wins), Modal raising standard pricing >50%, or Koala adding hostile-
  code execution (which would push us onto sandbox tier and change the
  math). Detailed model + traffic shapes for 10/100/1000 provider scales
...
image = (
    ...
    .pip_install(
        ...
        "python-docx>=1.1.0,<2",
        "openpyxl>=3.1.0,<4",
        # Composio integrations — load provider's connected toolkits
...
app = modal.App("koala-agent", image=image)

secrets = [modal.Secret.from_name("koala-secrets")]
...
async def run_space(space_id: str) -> None:
    """Run Koala for one space. Useful for local testing / cron drains."""
    import sys
...
async def run_now_webhook(item: dict) -> dict:
    """HTTP webhook that runs Koala autonomously for a space.
...
# ---------------------------------------------------------------------------
# Runs Koala inline in this Modal function and streams SDK events back as
...
async def chat_turn(item: dict):
    """Run one chat turn for the provider and stream SDK events as SSE."""
    import json
    import os
    import sys
    import time
    ...
    space_id = (item.get("space_id") or "").strip()
    # Clerk userId of the provider sending this message. Required for
    ...
    # ── Mode dispatch — provider (default) vs. agency ────────────────────
    # The Next.js layer sends `mode: 'provider' | 'agency'` so this function
    # picks the right agent (different system prompt + different tool set)
    # without forking the SSE plumbing. Provider stays the historical
    # default — older Next.js deploys that don't send `mode` keep working.
    #
    # Agency mode also carries `agency_id` + `agency_role` from the
    # API-gate's `resolveAgencyContext()`. These flow into AgentContext
    # so the per-tool guard (`tools/agency/_guards.py:require_agency_role`,
    # defense layer 3) can refuse a call whose context isn't agency-shaped.
    raw_mode = item.get("mode")
    mode = (raw_mode or "provider").strip().lower() if isinstance(raw_mode, str) else "provider"
    if mode not in ("provider", "agency"):
        mode = "provider"
    agency_id = (item.get("agency_id") or "").strip() if isinstance(item.get("agency_id"), str) else ""
    agency_role = (item.get("agency_role") or "").strip().lower() if isinstance(item.get("agency_role"), str) else ""
    # Agency mode must arrive with both context fields. The Next.js API
    ...
    from koala_agency import make_agency_agent
    ...
    # Load this provider's connected-toolkit tools (Gmail, Slack, HubSpot,
    ...
    #
    # Agency mode skips this entirely — the agency chat surface is the
    # chief-of-staff agent and does not draft on a provider's behalf from
    # the agency's chat. Cross-provider outreach is mediated by dedicated
    # agency tools (Phase 3), not by Composio-loaded provider connections.
    integration_tools: list = []
    connected_toolkits: list[str] = []
    if mode == "provider" and user_id:
        try:
    ...
    # Workspace info — gives the model the provider's intake URL up front
    ...
    # Tell the model which integrations the provider actually has connected
    ...
    # Agency mode gets agency context, NOT the provider block above — the
    # agency agent was previously handed the OWNER's personal workspace info
    # (their intake link + "include it in contact-facing drafts"), which is
    # provider-persona instruction injected into the agency chief-of-staff.
    if mode == "agency":
        workspace_info = (
            "# Agency context\n"
            f"- Agency id: {agency_id}\n"
            f"- Operator role: {agency_role}\n"
            "- You are the agency chief-of-staff. Answer about the TEAM —"
            " providers, routing, performance, revenue — through the agency"
    ...
    async def event_stream():
        try:
            # ── Tool registry selection — Phase 2/3 plug into AGENCY_TOOLS ──
            # Phase 2 (read tools) and Phase 3 (write tools) for the agency
            # variant only need to append entries to
            # `agent/tools/agency.AGENCY_TOOLS` — `make_agency_agent` reads
            # the list at agent-build time. No edit to this dispatch needed
            # when Phase 2 lands.
            if mode == "agency":
                koala = make_agency_agent(
        ...
        for attempt, model in enumerate(models):
            ...
            try:
                ...
                # delay the provider's last token.
                logger.info(
                    "chat_turn_finished",
                    space_id=space_id,
                    model=model,
