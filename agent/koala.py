...
This agent serves both surfaces:
  - chat (the provider talks to Koala via /api/ai/task → Modal chat_turn)
...
Tool surface (36 tools):
  ...
  - book_appointment
  - route_lead
  - add_service / send_service_packet
...
from tools.services import add_service, send_service_packet
from tools.questions import ask_provider
...
- CHAT — provider sent a message. Identify the job, run tools, answer.
...
what the provider has connected. When the provider names a connected
...
every connected toolkit. NEVER tell the provider you lack access to a
...
- After draft_message: quote or paraphrase the `nextStep` string. The
  provider must know it's drafted, awaiting approval, and whether a
...
provider cares about (ambiguous recipient, real subject for serious
outreach, body of a follow-up with no context). One sentence, one
question, no menu. Don't re-confirm what the provider explicitly asked.

# Boundaries
- Never reveal internal IDs, API keys, or raw row metadata. Use names.
- Never claim a write you didn't execute.
- Never change deal status, value, or title from chat — provider's call.
...
async def load_ai_profile(space_id: str, db) -> str | None:
    ...
    try:
        ...
        if profile.get("displayName"):
            parts.append(f"The provider's preferred name: {profile['displayName']}")
...
def make_koala_agent(
    ...
    provider (Gmail, Slack, HubSpot, etc. via Composio). Native CRM tools
    ...
    workspace name) so the model can drop the provider's intake link into
    ...
    universally-stable text first and per-space content after:
      1. KOALA_INSTRUCTIONS — identical across every space + every run.
         Caches once per OpenAI organization at runtime; every provider's
      ...
      3. ai_profile_text — per-space, slowly changing (provider edits their
