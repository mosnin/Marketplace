...
because it's two LLM hops + two HTTP round-trips. A provider with Gmail +
...
Curation rules:
- 4-8 slugs per toolkit, chosen by what a provider *actually does* daily.
...
- Read-shaped actions (LIST, FETCH, GET) over write-shaped where both fit
  the provider's verb. Writes still go through the approval-gated draft
...
# Order within each list is the provider's likely frequency-of-use,
...
CURATED_ACTIONS: dict[str, list[str]] = {
    ...
    # because providers often want Koala to PREP a Gmail draft they'll then
    ...
    # ── Messaging (team) ─────────────────────────────────────────────────
    # Slack is internal team comms — agency pings, deal updates, listing
    ...
    # ── CRM ──────────────────────────────────────────────────────────────
    # HubSpot is the only CRM most providers run alongside Koala. The
    # CREATE_*/UPDATE_* writes here ARE approval-relevant (they touch the
    # provider's external CRM), but Composio doesn't have a per-action
    # approval flag; trust comes from Koala's instruction prompt + the
    ...
    # ── Social ───────────────────────────────────────────────────────────
    # LinkedIn for providers is mostly: read my profile, post a listing
    ...
    # Instagram lead-gen for providers: DMs from listing-post engagement,
    ...
    # ── Productivity ─────────────────────────────────────────────────────
    # Notion is where some providers keep their per-deal note pages and
    # transaction checklists. Search + read + write the three actions
    # they'll hit; database row insert is for providers who treat Notion
