"""Question tool — agents use this to ask the provider for guidance when uncertain."""
...
async def ask_provider(
    ...
    """Queue a question for the provider; returns immediately, answer is async."""
    ...
    await publish_event(
        ctx.context,
        "info",
        f"Question queued for provider: {question[:80]}",
    ...
    return {
        ...
        "status": "pending",
        "message": "Question queued for provider review",
    }
