...
lands in the provider's approval inbox as an AgentDraft, and the provider
...
the provider uses imperative verbs ("send", "fire off", "shoot them"),
...
The provider is in control — they asked, we ship.
...
returned instead of a new one. Prevents the agent from burying the provider
...
async def draft_message(
    ...
    """Draft a person-facing message for provider approval; canonical outreach path."""
    ...
    # reasoning is shown to provider. priority 0-100 orders the inbox.
    ...
    # phone in this space; auto-create a stub if missing so the provider's
    ...
    if contact_id:
        ...
        if found and found.data:
            ...
            # part ("jane.doe" → "Jane Doe") so the provider sees something
            # readable in their inbox. The provider can edit later.
    ...
    # the "Koala noticed because…" breadcrumb so the provider sees WHY a
...
# explicit-intent paths the agent reaches for only when the provider used
...
#   - publish_event so the provider sees the send land in the activity feed.
#   - persist_log with action_type='message_sent', outcome='completed'
#     so the agency rollup sees a real send, not a draft.
...
async def send_email_now(
    ...
    # ONLY when the provider used an imperative verb ("send", "fire off", "ship it").
...
async def send_sms_now(
    ...
    # ONLY when the provider used an imperative verb. Routines must use draft_message.
