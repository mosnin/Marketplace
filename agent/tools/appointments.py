...
calendar. After booking the Appointment row, this tool writes through to the
...
calendar is connected, the Appointment is still booked; the calendar surface
will teach the provider to connect.
...
async def book_appointment(
    ...
    # Validate contact + pull canonical name/email/phone for the Appointment row.
    check = await (
        db.table("Contact")
    ...
    try:
        result = await db.table("Appointment").insert(appointment_row).execute()
    except Exception as exc:  # surface DB error to the agent
        return {"error": f"appointment insert failed: {exc}"}

    # Best-effort through-write to the provider's external calendar +
    # CalendarEventMirror backup row. The Appointment is committed; this seam
    # is what lands the event on Google Calendar so the provider's day
    # view (their actual calendar) reflects it. If no calendar is
    # connected we skip cleanly — the booking still lives in Appointment, and
    # the /calendar surface teaches them to connect.
    try:
        await _write_appointment_through_to_external_calendar(
            space_id=space_id,
        ...
        log.warning(
            "appointment_calendar_write_through_outer_failed",
            space_id=space_id,
            appointment_id=appointment_id,
            error=str(exc)[:300],
        )
    ...
    summary_addr = f" at {service_address}" if service_address else ""
    await db.table("ContactActivity").insert({
        ...
        "type": "note",
        "content": (
            f"[Agent] Appointment booked for "
    ...
    await publish_event(
        ctx.context,
        "action",
        f"Appointment booked for {guest_name} — {starts.strftime('%b %d, %I:%M%p')}",
        agent_type=ctx.context.current_agent_type,
        metadata={"contactId": contact_id, "appointmentId": appointment_id},
    )

    # Audit trail for agency rollup
    try:
        await persist_log(
            ctx.context,
            action_type="appointment_booked",
            outcome="completed",
            reasoning=f"Appointment at {service_address or 'TBD'} on {starts.isoformat()}",
            contact_id=contact_id,
        )
    except Exception:
        pass

    created = result.data[0] if result.data else appointment_row
    return {
        "ok": True,
        "appointmentId": created.get("id", appointment_id),
        "startsAt": starts.isoformat(),
        "endsAt": ends.isoformat(),
        "contactId": contact_id,
    }
...
async def _write_appointment_through_to_external_calendar(
    *,
    space_id: str,
    appointment_id: str,
    guest_name: str,
    ...
    """Write the appointment through to the provider's connected external calendar
    via Composio, then log a CalendarEventMirror row.

    Two-stage best-effort:
      1. Find an active calendar connection (googlecalendar /
         outlook_calendar). No connection → skip everything; the Appointment row
         alone is enough.
    ...
    try:
        ...
        log.warning(
            ...
            space_id=space_id, appointment_id=appointment_id, error=str(exc)[:300],
        )
        return
    ...
    if service_address:
        description_parts.append(f"Service: {service_address}")
    if guest_email:
        ...
        if (
            ...
            if resp.status_code < 400:
                ...
                log.warning(
                    ...
                    space_id=space_id, appointment_id=appointment_id, status=resp.status_code,
                )
    except Exception as exc:  # noqa: BLE001
        log.warning(
    ...
    try:
        await (
            ...
            .insert({
                ...
                "title": f"Appointment: {guest_name}",
