"""Routines tool — create, list, update, delete the provider's standing
instructions for Koala.

A routine is a sentence and a schedule: the provider saves an instruction
...
async def manage_routines(
    ...
    """Manage provider routines (standing scheduled instructions that DRAFT, never send)."""
    ...
    if action == "create":
        ...
        if len(text) < MIN_INSTRUCTION:
            return {"error": "instruction is required — a full sentence describing what Koala should do"}
