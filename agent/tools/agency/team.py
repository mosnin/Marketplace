"""Agency read tools — team-wide capacity, per-provider performance, morning mirror.

Three tools live here because they all answer the agency's question "what's
happening on the team?" from three zoom levels:

  team_health()                 — every provider at a glance, with a load_signal
                                  classification per person and the team median
                                  response speed as a context benchmark.
  provider_performance(id)       — drill into one provider: deals by stage, GCI,
                                  conversion percentage, response speed, last
                                  activity. Cross-agency drill is refused.
  read_provider_morning_story(id) — mirror what the provider sees on their own
                                  /koala home, scoped to that provider's space.
                                  PII-safe: the agency sees the provider's
                                  surface, not their private notes/threads.

All three call `require_agency_role(ctx)` first (defense layer 3) and scope
every read through `Space WHERE agencyId = ctx.agency_id`. Cross-
agency queries return `AgencyPermissionError`, not empty data — a silent
empty is indistinguishable from a permissions bug.

The team-median + load_signal heuristics are intentionally simple:
  - one number per provider (open_load = active_deals + open_leads)
  - one number per provider (avg_response_hours_7d)
  - team median is the seven-day agency median, used as a "vs benchmark"
    framing in the agency's chat so Koala reports facts, not judgments.
"""
...
from ._guards import AgencyPermissionError, require_agency_role


# ── Shared constants ────────────────────────────────────────────────────────

# Activity types that count as outbound contact with a human. Matches the
# convention in `app/api/agency/morning/route.ts`:
...
async def _team_spaces(agency_id: str) -> list[dict[str, Any]]:
    """Resolve the agency's provider spaces in one query.

    Returns rows shaped {id, ownerId, name}. Empty list if the agency has
    no spaces yet. The caller pairs ownerId → User to get provider names; we
...
async def _response_hours_for_space(
    ...
    The provider side never sees raw values — they roll up into a median.
    """
    db = await supabase()
    ...
    # how fast did the provider get back to the lead after it arrived?
    contacts_res = await (
        db.table("Contact")
        .select("id,createdAt")
        .eq("spaceId", space_id)
...
def _load_signal(open_load: int, avg_response_hours: float | None, last_active: datetime | None, now: datetime) -> str:
    """Classify a provider's load. See docstring on team_health for thresholds."""
...
async def team_health(ctx: RunContextWrapper[AgentContext]) -> dict[str, Any]:
    """Read every provider's current capacity, load, and 7-day response speed."""
    # Per-provider: active_deals, open_leads, avg_response_hours_7d, load_signal.
    ...
    spaces = await _team_spaces(agency_id)
    if not spaces:
        return {
            "ok": True,
            "providers": [],
    ...
    # Per-provider stats as a coroutine so the roster runs in PARALLEL —
    # the sequential version paid 5 DB round-trips per provider in series
    ...
    async def _stats(space: dict[str, Any]) -> tuple[dict[str, Any], list[float]]:
        ...
        name = (user or {}).get("name") or (user or {}).get("email") or "Provider"
        ...
        # Response hours for this provider.
    ...
    providers: list[dict[str, Any]] = [entry for entry, _ in gathered]
...
async def provider_performance(
    ...
    """Deep-dive on one provider: deals by stage/status, conversion, YTD GCI, response speed."""
    # Cross-agency drill raises AgencyPermissionError.
    ...
    # Cross-agency guard — resolve the provider's space and confirm its
    # agencyId matches the caller's. Empty data = treat as forbidden:
    # we don't want a leak signal that "this user exists but isn't in your
    # agency."
    space_res = await (
        db.table("Space")
        .select("id,ownerId,agencyId,name")
        .eq("ownerId", provider_id)
        .maybe_single()
        .execute()
    )
    if not space_res.data or space_res.data.get("agencyId") != agency_id:
        raise AgencyPermissionError(
            "That provider isn't in your agency."
        )
    ...
    if not user_res.data:
        ...
        return {
            "ok": True,
            "provider": {"id": provider_id, "name": "Provider", "joinedAt": None},
            "deals_by_stage": {},
            "deals_by_status": {},
            "conversion_pct": None,
            "gci_ytd": 0.0,
    ...
    # All deals for this provider — small enough to roll up in Python.
    deals_res = await (
        db.table("Deal")
    ...
    return {
        ...
        "provider": {
            "id": user["id"],
            "name": user.get("name") or user.get("email") or "Provider",
...
@function_tool(strict_mode=False)
async def read_provider_morning_story(
    ctx: RunContextWrapper[AgentContext],
    provider_id: str,
) -> dict[str, Any]:
    """Mirror the provider's /koala morning briefing; counts + named subjects only."""
    ...
    # Cross-agency guard — same pattern as provider_performance.
    space_res = await (
        db.table("Space")
    ...
    user = user_res.data or {"id": provider_id, "name": "Provider"}
    provider_name = user.get("name") or user.get("email") or "Provider"

    now = datetime.now(timezone.utc)
    week_out = now + timedelta(days=7)

    # ── Counts (mirroring /api/agent/morning shape) ─────────────────────
    # The provider-side route excludes agency-routed leads. We mirror
    # that filter so the agency sees what's actually on the provider's
    # personal desk, not aggregate agency flow.
    ...
    # updatedAt > 30 days ago counts as stuck. The provider route uses the
...
# Module-level list — `__init__.py` extends AGENCY_TOOLS from this.
TEAM_TOOLS: list = [team_health, provider_performance, read_provider_morning_story]
