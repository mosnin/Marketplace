"""Agency read tools — response-time benchmarking across the team.

Two tools live here because they both answer "how is the team performing?":

  audit_response_times()      — per-provider median first-touch response time
                                over 30d, banded vs team median (fast /
                                on_pace / slow / no_data).
  find_at_risk_agents()       — flight-risk signals per provider: won-deal
...
staff voice (see `agent/koala_agency.py:AGENCY_INSTRUCTIONS`) frames every
provider's number against the team benchmark — never as a judgment. "Alice's
median is 28h vs team median 6h" is OK. "Alice is slow" is not.

Definition of "outbound" matches the rest of the agency side: ContactActivity
...
async def audit_response_times(
    ...
    """Per-provider median first-touch response time over 30d, banded vs team median."""
    # Bands relative to team_median: fast (<=0.5x), on_pace (mid), slow (>=2x).
    # Providers with no outbound in window surface as no_data, not slow.
    ...
    # Per-provider audit as a coroutine — the roster runs in PARALLEL via
    # asyncio.gather instead of 2 sequential DB round-trips per provider.
    async def _audit(space: dict[str, Any]) -> tuple[dict[str, Any], list[float]] | None:
        ...
        # a shared module would be nice, but the provider tools follow the
    ...
    per_provider: list[dict[str, Any]] = [r[0] for r in audited if r is not None]
    ...
    # the agency wants the team's "typical" response, which is best measured
    # on raw samples).
    ...
    if team_median is None:
        # No outbound across the agency — return everyone in no_data.
...
# Silence threshold: if a provider has had no activity (Contact or Deal
...
# Tag written by the agency assignment flow (same as find_breached_leads).
_ASSIGNED_BY_AGENCY_TAG = "assigned-by-agency"
...
async def find_at_risk_agents(
    ...
    """Flight-risk signals per provider: won-deal trend, silence days, un-worked leads, SLA hits."""
    ...
    #   3. unworked_assigned_leads — 'assigned-by-agency' contacts with lastContactedAt IS NULL.
    #   4. sla_nudge_count / sla_escalate_count — contacts carrying the SLA tags.
    # Providers with ≥1 signal surface in the at_risk list, sorted by signal severity.
    require_agency_role(ctx)
    agency_id = ctx.context.agency_id
    ...
    # Per-provider assessment as a coroutine so the whole roster runs in
    # PARALLEL via asyncio.gather. The sequential version paid ~6 DB
    # round-trips per provider in series — 120 sequential queries for a
    # 20-agent agency, the dominant term in "agency chat is slow".
    async def _assess(space: dict[str, Any]) -> dict[str, Any] | None:
        ...
        unworked_count = sum(
            ...
            if _ASSIGNED_BY_AGENCY_TAG in (c.get("tags") or [])
        )
        ...
        if not reasons:
            return None  # no signals — provider is fine
