"""Agency read tools — pipeline pressure points.

Two tools live here because they both answer "where's the team losing
momentum?":

  find_stuck_deals(min_days)         — active deals across the agency that
                                       haven't been touched in `min_days`+
                                       days, ranked by age × value so the
                                       biggest stuck deals surface first.
  find_unassigned_leads(min_hours)   — agency-intake leads sitting in the
                                       queue without a provider on them, with
                                       a proposed routing target picked by
                                       lightest current load.
  find_breached_leads()              — routed leads (tag: assigned-by-agency)
                                       that have never been contacted
                                       (lastContactedAt IS NULL) and are past
                                       the agency's slaFirstResponseMinutes
                                       clock; flags any already past
                                       slaEscalateMinutes too.

Both are agency-scoped through `Space WHERE agencyId = ctx.agency_id`
and gated by `require_agency_role(ctx)`. Phase 3 will add the matching write
tools (assign_lead, ping_provider_on_stuck_deal); for now these are read-only
surface — the agency still does the actual move themselves.
"""
...
# Cap on returned rows — the agency chat needs the top N, not the full list.
_STUCK_DEAL_LIMIT = 20
_UNASSIGNED_LEAD_LIMIT = 50
_BREACHED_LEAD_LIMIT = 50

# Default SLA window when the agency row has no value set.
...
async def find_stuck_deals(
    ...
    """Find active deals across the agency stalled min_days+, ranked by days*value."""
    ...
    # Resolve the agency's spaces in one query — we filter Deal on
    ...
    # Resolve provider names for the assigned_to field.
    ...
    for d in deals:
        ...
        scored.append({
            ...
            "assigned_to": {
                "id": owner_id or "",
                "name": (owner or {}).get("name") or (owner or {}).get("email") or "Provider",
...
async def find_unassigned_leads(
    ...
    """Find unassigned agency-intake leads, with the lightest-load provider proposed."""
    # Unassigned = Contact in this agency tagged 'agency-lead', not 'assigned', older than min_hours.
    ...
    # Unassigned leads — same predicates as app/agency/leads/page.tsx.
    leads_res = await (
        db.table("Contact")
    ...
    unassigned = [
        ...
        if "agency-lead" in (c.get("tags") or [])
    ...
    # Compute open_load per provider — duplicate of team_health's logic so
    ...
    provider_loads: list[dict[str, Any]] = []
    for space in spaces:
        space_id = space["id"]
    ...
    # Sort ascending by open_load; the lightest provider is at the top.
    provider_loads.sort(key=lambda r: r["open_load"])
    lightest = provider_loads[0]

    # Team median for the "vs benchmark" framing in the reason string.
    loads = [r["open_load"] for r in provider_loads]
...
async def find_breached_leads(
    ...
    """Find routed leads past the agency's first-response SLA, oldest first."""
    # Breached = Contact in a member Space tagged 'assigned-by-agency',
    ...
    sla_first_minutes: int = int(
        agency.get("slaFirstResponseMinutes") or _DEFAULT_SLA_FIRST_RESPONSE_MINUTES
    )
    sla_escalate_minutes: int | None = (
        int(agency["slaEscalateMinutes"])
        if agency.get("slaEscalateMinutes") is not None
        else None
    )
    ...
    if not spaces:
        return {
            "ok": True,
            "breached_leads": [],
            "summary": "No member spaces found for this agency.",
        }
    ...
    #   - carry the 'assigned-by-agency' tag  (filtered in Python — same
    ...
    breached = [
        c for c in raw
        if "assigned-by-agency" in (c.get("tags") or [])
    ...
    for c in breached:
        ...
        owner = users_by_id.get(owner_id) if owner_id else None
        provider_name = (
            (owner or {}).get("name") or (owner or {}).get("email") or "Unknown Provider"
        )
