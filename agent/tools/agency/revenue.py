"""Agency read tools — commission revenue across the agency.

One tool: commission_report(period). Sums GCI by provider for a chosen window
and returns the agency total. GCI = value * commissionRate / 100, same
...
to won, the same proxy `/api/agency/morning` uses for won-this-week
...
async def commission_report(
    ...
    """Commission revenue across the agency, broken out by provider + total."""
    ...
    # Aggregate per provider (one row per space owner, even if the provider
    # has zero wins in the window — calm chief-of-staff reporting needs
    # the full team list so the agency can see who has nothing on the board).
    rollup: dict[str, dict[str, Any]] = {}
    for owner_id in owner_ids:
        u = users_by_id.get(owner_id) or {}
        rollup[owner_id] = {
            "id": owner_id,
            "name": u.get("name") or u.get("email") or "Provider",
            "deals_closed": 0,
            "gci_earned": 0.0,
        }
    ...
    by_provider.sort(key=lambda r: r["gci_earned"], reverse=True)
