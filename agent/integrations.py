...
_NATIVE_TOOL_NAMES = frozenset({
    ...
    "request_deal_review",
    "book_appointment", "route_lead", "add_service", "send_service_packet",
    ...
    "process_inbound_message", "read_attachment", "ask_provider",
...
async def active_toolkits(space_id: str, user_id: str) -> list[str]:
    """Toolkit slugs the provider has connected and our DB has marked active.
...
def _sanitize_enums_for_xai(node: Any) -> Any:
    ...
    enum string values:
      ...
      Error code: 400 - Schema validation failed: [engine_imposed]
        /services/hs_legal_basis/enum/0: '/' in 'enum' string value
    ...
    the canonical strings intact. Recurses into services, items, the
    ...
    if isinstance(node, dict):
        ...
        for k, v in node.items():
            if k in ("services", "patternServices", "$defs", "definitions") and isinstance(v, dict):
                out[k] = {sub_k: _sanitize_enums_for_xai(sub_v) for sub_k, sub_v in v.items()}
            elif k in ("items", "additionalServices", "not", "if", "then", "else"):
...
def _build_curated_tool(
    *,
    ...
    runtime behavior, error shapes, and the provider's `IntegrationConnection`
    ...
    # `additionalServices: True` matches the TS side (lib/integrations/
    ...
    # Run xAI-enum sanitization on the services before assembling, so any
    # HubSpot-style `Performance / Contract` enums get demoted to
    # description hints and don't crash the chat turn at the provider.
    raw_services = (parameters or {}).get("services", {}) or {}
    sanitized_services = {
        k: _sanitize_enums_for_xai(v) for k, v in raw_services.items()
    }
...
async def load_integration_tools(
    ...
    proxy not configured — tell the provider…"), which the model can relay
    as "temporarily unavailable". Returning [] here made every config
    failure indistinguishable from "the provider connected nothing", and
    the model told providers their integrations were gone.
    """
    proxy = _proxy_base()
    if proxy is None:
        logger.error(
    ...
    # dispatcher-only default meant a provider's connected Gmail surfaced as
    ...
    # tools. Opt OUT with KOALA_CURATED_INTEGRATIONS=0 if a deploy must.
    curated_enabled = (
        os.environ.get("KOALA_CURATED_INTEGRATIONS", "1") or "1"
    ...
    # Collect curated slugs across the provider's connected toolkits. A
    ...
    for spec in raw_tools:
        ...
        parameters = spec.get("parameters") or {"type": "object", "services": {}}
