#!/usr/bin/env python3
"""Generate a layered system-architecture diagram for Koala as an SVG.
...
draw_band(c, "Surfaces", "browser + MCP clients", [
    "Marketing site (logged-out)", "Clerk auth · provider/agency login",
    "Provider workspace  /s/[slug]", "Agency dashboard  /agency",
    "Platform admin  /admin", "Public intake  /apply (form + AI chat)",
    "Appointment booking  /book", "Link-in-bio  /p/[slug]",
    "Tokenized  /cma /packet /appointment", "Applicant portal  /clients (own session)",
...
draw_band(c, "Edge", "middleware.ts", [
    "Clerk session", "Route protection (/s /agency /admin /setup …)",
...
draw_band(c, "Next.js API", "312 endpoints (App Router)", [
    ...
    "pipelines", "appointments", "services", "calendar", "studio", "agency (44)",
    "agencies", "billing", "integrations", "clients", "applications", "calls",
...
draw_band(c, "Core services", "lib/*", [
    "lead-scoring (deterministic engine · gpt-4.1-mini summaries)", "notify · email · sms · appointment-emails",
    "permissions · api-auth  (offboarding gate)", "embeddings · vectorize · zilliz (pgvector)",
    "agency-routing", "commissions", "agency-seats", "billing (Stripe)",
...
draw_band(c, "Koala runtime", "the product · lib/ai-tools (TS, default)", [
    "runtime-flag  KOALA_CHAT_RUNTIME=ts", "@openai/agents in-process loop",
...
draw_band(c, "Modal sandbox", "agent/* (Python, autonomous + swarm)", [
    ...
    "koala.py + modules · 53 tools", "koala_agency.py  (chief-of-staff)",
...
draw_band(c, "Data stores", "source of truth", [
    ...
    "User · Space · SpaceSetting · Agency · Membership",
    "Contact · Deal · DealStage · Appointment · Service",
    "Conversation · Message · AgencyConversation · AgencyMessage",
    "Invitation · AuditLog · AgencyNotification · Attachment",
...
svg.append(f'<text x="{PAD+2}" y="{PAD+30}" font-family="{FONT}" font-size="26" '
           f'font-weight="800" fill="#11151c">Koala — System Architecture</text>')
svg.append(f'<text x="{PAD+2}" y="{PAD+52}" font-family="{FONT}" font-size="13.5" '
           f'fill="#6b7280">Agentic OS for providers &amp; agencies · Next.js 15 / React 19 · '
