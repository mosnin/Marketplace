...
A scoring rubric for any Koala surface, derived from Norman's *The Design of
Everyday Things*.

## What this is

A checklist. Pick a surface (FocusCard, the Koala bar composer, an
...
would, because every check names what passing looks like, in Koala's
...
| 2 | Fail — a provider will hit this in a normal session and feel it. Fix before merge. |
...
**Question scored.** A provider opens this surface for the first time. Without
reading docs, hovering tooltips, or asking Koala, can they name every
...
discovery. The koala-bar shows its placeholder text *and* the send arrow
...
|---|---|
| 5 | Every action is visible AND labeled with a verb. A new provider in a usability test names them all. |
...
**Question scored.** Once the provider sees the controls, does the surface
...
Koala's internal terms?

**Passes when.** Labels are the provider's words, not the product team's.
...
have 3 leads to follow up with." A provider who has never used a CRM can
read this and act.

**Fails when.** The screen uses table names, API status codes, or product
neologisms. A provider sees "intake activation" without context, or
...
| 4 | Copy is mostly the provider's vocabulary. One label could be tightened in a polish pass. |
| 3 | One control uses product-internal language a new provider would have to map. |
...
Replace every noun with the provider's noun. Replace every enum with a verb.
...
that look reinforced by a signifier (label, icon, position) the provider can
read at a glance?
...
border. The KOALA_PILL reads as "this calls Koala" because the warm
...
because their border + radius + h-9 match the Koala input vocabulary —
...
| 2 | A primary action visually equals a secondary action; the provider has to read to choose. |
...
thing it controls correct? When the provider moves their eye from the control
...
controls a toggle at the top — the provider has to remember the change to
...
| 2 | Multiple controls map ambiguously; the provider experiments to find what changed. |
...
confirmation or a toast (Koala voice: lowercase verb, period).

---

## 5. Feedback

**Question scored.** Every action the provider takes — does it get
...
the loader the moment it's pressed. The koala-bar shows the
"Koala is thinking" thinking state before the first token streams. Errors
...
lifecycle. Show a one-sentence Koala-voiced toast on success. On error,
...
"Reading the thread…") — the provider reads "Koala is working" as
...
Or does the surface let the provider walk into a known mistake and then
clean up after?
...
is selected. A form lets the provider enter a phone number with letters in
...
| 2 | Multiple wrong actions are allowed; the provider learns by submitting. |
...
the provider types, not on submit. Constrain the input shape (numeric
...
the provider isn't ready. Approval is the gate, not the undo. Toasts on
failed sends say "Couldn't send that. Try again." in Koala voice and
keep the draft in place — the provider doesn't lose work. The Koala bar's
"I lost the connection. Try again." is one sentence and recoverable.
Where an action is irreversible (disable Koala, delete a workspace), the
...
**Fails when.** A failed action wipes the provider's input. An error
...
predictable. The surface treats the wrong action as the provider's fault
...
| 3 | An error blames the provider instead of guiding them, or the recovery action requires extra clicks. |
| 2 | Multiple errors leak engineering terms or wipe input on failure. |
| 1 | An irreversible action lacks confirmation, or a failed send loses the provider's work. |

**Fix pattern.** Preserve input on every failure path. Replace error codes
with one-sentence Koala voice. Add an undo or a hold for any predictable
mistake. Add an AlertDialog for irreversible work. Treat the provider's
action as an approximation to a goal — guide them to the goal instead of
rejecting the input.

---

## Koala-specific checks

Three checks specific to an autonomous-agent product. The surface isn't a
passive form — the agent does work on the provider's behalf, so the bar
shifts.

## 8. Gulf bridging (execution + evaluation)

**Question scored.** Norman's two gulfs. **Execution**: how does the
provider make Koala do what they want? **Evaluation**: how does the
provider verify Koala did the right thing? A surface that bridges only
one is half-built.

**Passes when.** The Koala bar bridges execution with the composer (one
...
bridges both — the provider literally watches Koala work in real time, no
ambiguity about what just happened.

**Fails when.** Execution is easy but the provider can't verify the result
("did it actually send?"). Or the result is visible but the path to *do
anything* is unclear ("I see Koala drafted this, now what?"). Or the
...
| 1 | Neither gulf is bridged — the provider types and hopes. |
...
matches expectation), reflective (would a provider tell three friends)?

**Passes when.** **Visceral**: the surface reads calm, paper-flat, with
one focal element. Serif Times h1 + status sentence. No competing
elements. The brand orange shows up only where Koala spoke. **Behavioral**:
...
**Reflective**: after using the surface, the provider could explain to a
peer what Koala just did for them and why it was worth it. The morning
replay, the typed onboarding reveal, the FocusCard's "one decision at a
time" — these are the surfaces a provider screenshots and sends to another
provider.

**Fails when.** Visceral: the surface is loud, busy, or has two focal
elements competing. Behavioral: a control's effect surprises the provider
(the "Save" button discards changes; the X closes the page instead of the
modal). Reflective: the provider finishes the task but couldn't explain
...
| Score | What it looks like |
|---|---|
| 5 | First glance is calm and focal. Every interaction confirms expectation. The surface is the one a provider screenshots. |
...
sentence is "Koala did the work overnight," show the work, not the
diagnostics.
...
forcing the provider to carry it in their head?

**Passes when.** The provider doesn't have to remember the lead's
context — the FocusCard surfaces the contact name, the draft, the
reasoning, the channel. The provider doesn't have to remember how to draft
in their own voice — the onboarding reveal does it. The provider doesn't
have to remember which leads need follow-up — Koala opens the day with
the status sentence. Slugs auto-derive from business names; the provider
edits only if they want to. The intake link's URL is constructed; the
provider doesn't type it.

**Fails when.** The surface asks the provider to remember context Koala
already has. The provider is asked to retype the lead's name when Koala
knows it. The provider has to maintain a mental model of which leads are
"hot" instead of seeing the lead-warm pill. A settings page forces the
provider to pick from twelve toggles instead of one decided default. The
provider is asked to do a task the agent could have done.

**Score scale.**

| Score | What it looks like |
|---|---|
| 5 | The agent carries the knowledge; the surface shows the result. The provider's only job is to approve, edit, or redirect. |
| 4 | Mostly carried by the agent; one piece of context the provider has to fetch from another tab. |
| 3 | The provider has to remember one piece of context the agent could have provided. |
| 2 | The surface is a form that asks the provider to do the agent's job. |
| 1 | The surface is configuration that should have been a decision. |

**Fix pattern.** For every field, ask: does Koala already know this? If
yes, show it as a default the provider can override. For every toggle,
...
**Passes when.** One idea is statable in one sentence. The Koala bar's
sentence: "Koala is one keystroke away from anywhere in the workspace."
The FocusCard's sentence: "One decision at a time — Koala's queue,
distilled." The onboarding reveal's sentence: "Koala works for you
...
- **In an audit.** Score every primary surface (Koala home, FocusCard,
...
When the agent (Koala or the coding agent) runs this rubric, the dual
persona from CLAUDE.md still applies: the audit is design work, so it is
scored from the Jobs lens — the standard is "would this make a provider
...
gold-plating; no rearchitecting the surface to chase a 5 the provider will
never feel.

### What the rubric does not do

- It does not replace usability testing. It catches the principles a
  provider will feel; testing catches the things only a provider will
  notice.
...
- It does not score code quality. Good design and clean code overlap but
  are not the same thing; the rubric scores what a provider feels, not what
  a reviewer reads.
...
the only honest test for that is whether a provider uses it twice and tells
another provider about it.
