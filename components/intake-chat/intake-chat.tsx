...
/**
 * IntakeChat — the provider's intake form rendered as a chat.
 *
 ...
 * presentation changed. Each question becomes a Koala turn; the matching
 ...
 *     surface. Providers edit their questions in the existing form
...
function buildApplicationPayload(args: {
  ...
  return {
    ...
    serviceAddress: at('serviceAddress') ?? at('location'),
...
export function IntakeChat({
  ...
  // Flow resolution. When no provider-customized config is supplied, fall
  // back to the library defaults so the chat always has questions to ask
  // (this is what makes the agency variant work without configuration).
  const hasAnyConfig =
  ...
  //
  // The Koala intro shows ONCE, before the first question is answered.
  ...
  // from the provider's hero or the question itself.
  const showKoalaIntro = phase === 'asking' && answeredCount === 0;

  // End-of-flow transition: when the application is submitted, the chat
  // history, progress bar, and Koala intro all disappear and the success
  ...
  return (
    <div className="space-y-8">
      ...
      {showKoalaIntro && (
        <KoalaIntro agentName={agentName} />
      )}
...
/**
 * Slim segmented progress bar that lives between the provider hero and the
 * conversation. One segment per visible question; segments before the
 * cursor fill with the provider's accent color, the live segment animates
...
/**
 * Koala's calm intro — shown once, before the lead has answered any
 ...
 * Koala and the provider in one breath so the applicant knows who's
 ...
 */
function KoalaIntro({ agentName }: { agentName: string }) {
  return (
    <motion.p
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      Hi — I&rsquo;m Koala. I work with {agentName}. A few quick questions
...
function UserTurn({ text, accentColor }: { text: string; accentColor: string }) {
  // Accent-tinted bubble — the provider's brand color at low opacity with
...
function CurrentQuestion({
  ...
  // page-title pattern used everywhere else in Koala (STYLESHEET.md →
        ...
        return (
          <motion.button
            ...
            style={
              isPending
                ? { backgroundColor: accentColor }
                : ({ ['--tw-ring-color' as never]: accentColor } as React.CSSServices)
            }
...
function YesNoChoice({ onCommit, accentColor }: InputProps) {
  // Boolean checkbox → present as two tappable cards. The provider's
    ...
    <div className="flex flex-wrap gap-2.5">
      {[
        ...
        <button
          ...
          style={{ ['--tw-ring-color' as never]: accentColor } as React.CSSServices}
        >
          {option.label}
        </button>
      ))}
...
function ChatComposer({ question, value, onChange, onCommit, accentColor, error }: InputProps) {
  ...
  // send button. Mirrors the provider-facing Koala composer in shape.
  const v = typeof value === 'string' ? value : '';
 ...
 * so a malformed provider accent color never crashes the page.
 */
