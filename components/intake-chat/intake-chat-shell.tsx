...
/**
 * IntakeChatShell — the provider's storefront wrapped around the intake chat.
 *
 ...
 *      with the question.
 *   2. The provider's face (or a brand-orange-tinted monogram fallback —
...
export interface IntakeChatShellProps {
  /** The provider's brand-facing name — businessName from SpaceSetting, or
  ...
  agentName: string;
  /** Provider face. Already signed-and-resolved upstream; null if the
   *  provider hasn't uploaded one (we fall back to a serif monogram). */
  agentPhoto?: string | null;
  /** Optional cover photo (same field /p/[slug] uses). When present, it
   *  becomes a softened brand band behind the avatar. */
  coverPhotoUrl?: string | null;
  /** When the provider has uploaded a wordmark, it substitutes for the
   *  typed business name. Quiet, single-line — no doubled identity. */
  logoUrl?: string | null;
  /** Drives the blue-check next to the business name. */
  isVerified?: boolean;
  /** When set, the provider identity in the header becomes a Link to this
   *  href — typically the public profile at /p/[slug]. Lets applicants
   *  step over to learn more about the provider without abandoning the
  ...
  /** Provider/agency-setup-supplied trust signals. Koala never injects
...
/**
 * Provider identity hero — cover band + avatar + name.
 *
 ...
 *     monogram circle with the provider's initials in serif Times, brand-
 *     orange text. This is the ONLY place outside Koala proper where
 *     brand-orange appears in the intake — and it's the provider's
...
function ProviderIdentity({
   ...
   *  provider's public page. Gives applicants a way to learn more about
  ...
  return (
    <div className="text-center">
      {/* Cover band — only when the provider has uploaded a cover. The 16:9
      ...
      {/* Business name — the focal serif moment. Logo, if uploaded,
          ...
          provider's public page without breaking the intake flow. */}
...
export function IntakeChatShell({
  ...
  // The trust block is entirely optional. If the provider hasn't supplied
  ...
  return (
    ...
    <div className="relative min-h-dvh bg-background sm:bg-muted/40 text-foreground">
      ...
      {/* ── Sticky header — provider identity hero pinned at the top ──── */}
      <motion.header
      ...
      <footer className="flex-shrink-0 w-full bg-background/70 backdrop-blur-xl border-t border-border/40">
        <div className="max-w-2xl mx-auto px-5 sm:px-8 py-3 sm:py-4">
          {/* Trust signals — only renders when the provider supplies content.
