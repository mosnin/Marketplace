...
import { providerNavItems, providerMoreNavItems } from '@/lib/nav-items';
...
interface SidebarProps {
  ...
  agencyMemberships?: { id: string; name: string; role: string }[];
  isPlatformAdmin?: boolean;
}

// ═══════════════════════════════════════════════════════════════════════════════
// Agency nav definitions (unchanged structure)
// ═══════════════════════════════════════════════════════════════════════════════

// Phase 7 — agency admin sidebar collapses from 14 entries across 5 labeled
...
// earn daily prominence): Providers, Templates, Team Chat, Announcements,
...
// rules) live behind /agency/settings's own in-page tab strip.
// Agency nav items extend the shared NavItem with the agency-only
// `adminOnly` flag. Rendered through the shared SidebarNavItem accordion
// (base=""), so children/exact behave exactly like the provider nav.
type AgencyNavItem = NavItem & { adminOnly?: boolean };
type AgencyNavSection = { label: string; items: AgencyNavItem[] };

export const agencyAdminNavSections: AgencyNavSection[] = [
  {
    label: '',
    items: [
      // Koala-for-Agencies Phase 1 — pinned at the top, mirroring the
      // provider sidebar's top-pinned Koala entry. Same icon (chip avatar
      // is rendered by the agency nav via FlatNavItem; MessageCircle is the
      // fallback used elsewhere in the agency nav). The existing
      // /agency/agent-activity entry was previously also labelled "Koala"
      ...
      // to "Agent activity" below to avoid two nav rows with the same name.
      {
        ...
        adminOnly: false,
        isAI: true,
        // Mirrors the provider Koala dropdown (Brief / Inbox / History).
        // Brief and Reviews are real agency routes; "History" points at the
        // chat home (/agency), where the conversation-history drawer lives.
        children: [
          { href: '/agency/brief', label: 'Brief' },
          { href: '/agency/reviews', label: 'Inbox' },
          { href: '/agency', label: 'History', exact: true },
        ],
      },
      { href: '/agency/brief', label: 'Brief', icon: LayoutDashboard, exact: false, adminOnly: false },
      { href: '/agency/leads', label: 'Leads', icon: PhoneIncoming, exact: false, adminOnly: false },
      { href: '/agency/people', label: 'People', icon: Users, exact: false, adminOnly: false },
      { href: '/agency/deals', label: 'Deals', icon: Briefcase, exact: false, adminOnly: false },
      { href: '/agency/pipeline', label: 'Pipeline', icon: BarChart3, exact: false, adminOnly: false },
      { href: '/agency/forecast', label: 'Forecast', icon: TrendingUp, exact: false, adminOnly: false },
      { href: '/agency/services', label: 'Services', icon: Building2, exact: false, adminOnly: false },
      { href: '/agency/reviews', label: 'Reviews', icon: Flag, exact: false, adminOnly: false },
      { href: '/agency/agent-activity', label: 'Agent activity', icon: Activity, exact: false, adminOnly: false },
      { href: '/agency/integrations', label: 'Integrations', icon: Plug, exact: false, adminOnly: false },
      {
        ...
        // rules, MCP all exist under app/agency/settings/). Dropdown gives
        // agencies a direct door without first landing on the tab strip.
        children: [
          { href: '/agency/settings/form-builder', label: 'Form builder' },
          { href: '/agency/settings/auto-assignment', label: 'Auto-assignment' },
          { href: '/agency/settings/routing-rules', label: 'Routing rules' },
          { href: '/agency/settings/mcp', label: 'MCP' },
        ],
      },
    ],
  },
  {
    label: 'More',
    items: [
      { href: '/agency/members', label: 'Members', icon: Users, exact: false, adminOnly: false },
      { href: '/agency/providers', label: 'Providers', icon: UserCircle, exact: false, adminOnly: false },
      { href: '/agency/templates', label: 'Templates', icon: FileText, exact: false, adminOnly: false },
      { href: '/agency/leaderboard', label: 'Leaderboard', icon: Trophy, exact: false, adminOnly: false },
      { href: '/agency/analytics', label: 'Analytics', icon: BarChart3, exact: false, adminOnly: false },
      { href: '/agency/usage', label: 'Usage', icon: Gauge, exact: false, adminOnly: false },
      { href: '/agency/import-export', label: 'Import / export', icon: Upload, exact: false, adminOnly: true },
    ],
  },
];
...
export const agencyMemberNavSections: AgencyNavSection[] = [
  {
    label: '',
    items: [
      { href: '/agency', label: 'My day', icon: LayoutDashboard, exact: true, adminOnly: false },
      { href: '/agency/my-leads', label: 'My leads', icon: PhoneIncoming, exact: false, adminOnly: false },
    ],
  },
  {
    label: 'More',
...
const agencySettingsNavSections = [
  {
    label: 'Team',
    items: [
      { href: '/agency/settings', label: 'General', icon: Settings, exact: true },
      { href: '/agency/settings/profile', label: 'Profile', icon: UserCircle, exact: false },
      { href: '/agency/invitations', label: 'Invitations', icon: Mail, exact: false },
      { href: '/agency/settings/mcp', label: 'MCP', icon: Key, exact: false },
    ],
  },
  {
    label: 'Lead management',
    items: [
      { href: '/agency/settings/auto-assignment', label: 'Auto-assignment', icon: Shuffle, exact: false },
      { href: '/agency/settings/routing-rules', label: 'Routing rules', icon: GitBranch, exact: false },
    ],
  },
  {
    label: 'Account',
    items: [
      { href: '/agency/billing', label: 'Billing', icon: CreditCard, exact: false },
    ],
  },
];
...
function doesItemOwnPath(item: NavItem, pathname: string, base: string): boolean {
  ...
  // An `exact` parent only owns its own exact route — used by the agency
  // Koala entry (/agency), whose href is a prefix of every other agency
  // route. Without this, a prefix match would light up Koala on every
  // agency page. Provider parents don't set `exact`, so this is a no-op there.
  if (item.exact) {
...
// ═══════════════════════════════════════════════════════════════════════════════
// Section label (used in agency nav)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// Flat nav item (for agency nav, settings sub-pages)
// ═══════════════════════════════════════════════════════════════════════════════
...
function FlatNavItem({
   ...
   * the icon — matches the provider sidebar's top-pinned Koala treatment.
   * Use for the agency's Koala nav entry only.
   */
  isAI?: boolean;
}) {
  return (
    <Link
      href={href}
      className={cn(
        // h-9 row, same height as the provider nav rows and canonical button
    ...
    >
      {/* Active accent bar — 2px on the left, foreground tone, rounded
          corner on the inner edge. Matches the provider sidebar's active
      ...
      {isAI ? (
        /* Chip avatar — same 16×16 rounded-full as the provider Koala row */
        <img
          src="/chip-avatar.png"
          alt=""
...
// new contact, new deal, new service. Anything else lives behind a full menu
...
function QuickCreateMenu({ slug }: { slug: string }) {
  ...
  const items: { href: string; label: string }[] = [
    ...
    { href: `${base}/services/new`, label: 'New service' },
  ];
  return (
    <DropdownMenu>
...
export function WorkspaceSwitcher({
  ...
  // Build the workspace list. The provider's own workspace is always first;
  // agency memberships follow. Each gets a ⌘1/⌘2/⌘3… shortcut so the
  ...
  if (slug) {
    workspaces.push({
      ...
      href: base,
      icon: Briefcase,
      isCurrent: !isOnAgencyPage,
    });
  }
  for (const b of agencyMemberships) {
    workspaces.push({
      key: b.id,
      name: b.name,
      href: '/agency',
        ...
        {drawerExpanded && (
          <div className="mt-1 pt-1 border-t border-border/40 space-y-0.5">
            <WorkspaceSwitcherRows
              ...
              userEmail={userEmail}
              hasTeam={agencyMemberships.length > 0}
            />
          </div>
        )}
      </div>
      ...
      {workspaces.map((w, idx) => {
        ...
        return (
          <Link
            key={w.key}
            href={w.href}
            onClick={() => { if (!w.isCurrent && w.href === '/agency') triggerAccountSwitch(); }}
            className={cn(
...
// ═══════════════════════════════════════════════════════════════════════════════
// AgencySidebarConversations — slim agency conversation history rendered when
// on the agency Koala page (/agency). Mirrors the provider SidebarConversations
// structure (CHAT HISTORY label + New button + recent list + active rail) but
// calls the agency-scoped conversations API and links into /agency?conversationId=…
// instead of /s/[slug]/koala. Bounded to 6 rows + "See all →" link.
// ═══════════════════════════════════════════════════════════════════════════════

function AgencySidebarConversations() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const res = await fetch('/api/ai/agency-conversations');
  ...
  const handleNew = useCallback(async () => {
    ...
    router.push(`/agency?conversationId=${conv.id}`);
  }, [router]);
            ...
            {visible!.map((conv) => {
              ...
              return (
                <li key={conv.id} className="relative">
                  ...
                  >
                    <Link
                      href={`/agency?conversationId=${conv.id}`}
...
// ═══════════════════════════════════════════════════════════════════════════════
// Provider nav — 3 sections: AI at top, Workspace in middle, Settings at bottom
// ═══════════════════════════════════════════════════════════════════════════════

// ═══════════════════════════════════════════════════════════════════════════════
// ─────────────────────────────────────────────────────────────────────────────
// Section groupings — sit BETWEEN the existing nav items as small-caps
// headers. The provider nav is one flat list in `lib/nav-items.ts` (so any
...
//   • Koala   → no header (top-pinned, AI)
//   • WORKSPACE → daily work (People, Deals, Calendar, Mailbox, Services, Studio, Files)
...
// (ProviderNav, including conversation history on /koala) and the pinned
// user footer. The inspiration the provider shared shows a small
...
// divider, separated by a hairline. Reads as "leave the provider app to the
...
function ProviderNav({
  ...
  const settingsItem = providerNavItems.find((item) => item.href === '/settings')!;
  ...
  const otherItems = providerNavItems.filter(
    (item) =>
      !item.isAI &&
      item.href !== '/settings' &&
  ...
  //   • Calm count (leads, services) — muted pill, rounded-md, small.
  ...
  const getBadge = (item: NavItem): React.ReactNode => {
    ...
    if (item.badgeKey === 'services' && activeServiceCount > 0) {
      return (
        <span className={calmBadgeClasses}>
          <PulseNumber value={activeServiceCount > 99 ? '99+' : activeServiceCount} />
        </span>
      );
    }
    return undefined;
  ...
  const getBadgeText = (item: NavItem): string | undefined => {
    ...
    if (item.badgeKey === 'services' && activeServiceCount > 0) {
      return activeServiceCount > 99 ? '99+' : String(activeServiceCount);
    }
    return undefined;
  };
  ...
  return (
    ...
    >
      {/* Two-mode sidebar: on /koala the conversation history slides in
          ...
          Calendar…) stay visible the whole time — the provider must always
      ...
      <div className="space-y-0.5">
        {/* Always visible — top-pinned AI items (Koala + future AI rows).
            No header above this bucket: Koala is the brand mark; it
            doesn't need a category to belong to. */}
        {aiItems.map(renderItem)}

        {/* RECORDS — daily work surfaces (the provider's book of business:
            People, Deals, Services, etc.). Labeled to match the
      ...
      {/* Context-aware second section. Route IS the signal:
            - On /koala/* → CHAT HISTORY label + recent conversation list
              ...
              [0.22, 1, 0.36, 1] curve.
            - Elsewhere → render `providerMoreNavItems` if it has anything.
      ...
      {/* providerMoreNavItems — the secondary slot for non-Koala pages.
          ...
          Koala (the Koala state owns this region). */}
      {providerMoreNavItems.length > 0 && !onKoala && (
        <div>
          <div className="space-y-0.5">{providerMoreNavItems.map(renderItem)}</div>
        </div>
      )}
...
export function Sidebar({
  ...
  const base = `/s/${slug}`;
  const { user } = useUser();
  // Shared collapse state (provided by the layout). The agency branch below
  // uses it to rail like the provider sidebar; the header panel toggle drives it.
  const { collapsed: agencyCollapsed, toggle: agencyToggle } = useSidebarCollapsed();

  // Agency nav accordion — one parent expanded at a time, same contract as
  // ProviderNav. Declared here (not inside the agency branch) because that
  // branch returns early and hooks must not live after a conditional return.
  // base is "" for the agency nav (its hrefs are already absolute, e.g.
  // /agency/leads), so doesItemOwnPath is called with base "" below.
  const agencySections =
    agencyRole === 'provider_member'
  ...
  useEffect(() => {
    ...
    // Same as ProviderNav: only auto-open when the route maps to a parent.
  ...
  const isOnAgencySettings = pathname.startsWith('/agency/settings');

  // ── Agency settings sub-nav ──────────────────────────────────────────────
  if (isAgency && (isOnAgencyPage || isAgencyOnly) && isOnAgencySettings) {
    return (
      <aside data-dashboard-sidebar className={cn('hidden md:flex flex-col bg-sidebar border border-border/70 rounded-xl overflow-hidden shrink-0 m-3', SIDEBAR_WIDTH)}>
        ...
        <nav className="flex-1 px-3 pb-2 space-y-0.5 overflow-y-auto">
          {agencySettingsNavSections.map((section) => (
        ...
        {/* What's New + user menu chip — same footer slot as the provider and
            the main agency sidebar. Settings sub-nav is still an in-app
  ...
  // ── Agency sidebar ───────────────────────────────────────────────────────
  if (isAgency && (isOnAgencyPage || isAgencyOnly)) {
    return (
      <aside data-dashboard-sidebar className={cn('group/rail relative hidden md:flex flex-col bg-sidebar border border-border/70 rounded-xl shrink-0 overflow-hidden transition-[width] duration-200 ease-out m-3', agencyCollapsed ? SIDEBAR_COLLAPSED : SIDEBAR_WIDTH)}>
        {/* Same brand-warm tint as the provider sidebar so agencies see the
        ...
        <div className="relative z-10 flex flex-col h-full">
          {/* Brand mark — in collapsed rail mode it doubles as the expand
              affordance, mirroring the provider sidebar. */}
          <div className={cn('pt-5 pb-3', agencyCollapsed ? 'flex justify-center px-2' : 'flex items-center justify-between px-4')}>
            {agencyCollapsed ? (
              <button
                type="button"
                onClick={agencyToggle}
          ...
          <WorkspaceSwitcher
            currentName={agencyName ?? 'Team'}
            currentSubtitle="Team"
            currentIcon={Building2}
            slug={slug}
            spaceName={spaceName}
            agencyMemberships={agencyMemberships}
            isOnAgencyPage={isOnAgencyPage}
            collapsed={agencyCollapsed}
            userEmail={userEmail}
          />

          <div className="mt-3">
            <SearchPill collapsed={agencyCollapsed} />
          </div>

          {/* Agency primary nav — same structural vocabulary as ProviderNav:
          ...
          <nav className="flex-1 px-3 py-2 mt-1 space-y-3 overflow-y-auto">
            <div className="space-y-0.5">
              {agencySections.map((section) => {
                ...
                return (
                  <div key={section.label}>
                    {!agencyCollapsed && <SectionLabel>{section.label}</SectionLabel>}
            ...
            {/* Conversation history — mirrors the provider sidebar's contextual
                section. Slides in when on the agency Koala page (/agency exact),
                same AnimatePresence pattern. Shows agency conversations via the
                agency-scoped conversations API. Hidden in collapsed rail mode —
                there's no room for a conversation list at 56px. */}
            <AnimatePresence initial={false} mode="wait">
              {!agencyCollapsed && pathname === '/agency' && (
                <motion.div
                  key="agency-koala-history"
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 8 }}
                  transition={{ duration: 0.26, ease: [0.22, 1, 0.36, 1] }}
                >
                  {/* Hairline divider — same as provider sidebar (mx-3 to match) */}
                  <div className="mx-3 mt-1 mb-2 h-px bg-border/60" aria-hidden />
                  <AgencySidebarConversations />
                </motion.div>
              )}
            </AnimatePresence>
          </nav>

          {/* What's New card — same slot as the provider sidebar. Agency users
              see the same release notes. Collapses to an icon in rail mode,
              matching the provider shell. */}
          <SidebarWhatsNew collapsed={agencyCollapsed} />
          ...
          {/* User footer — matches the provider's SidebarUserMenu chip (avatar +
...
// ═══════════════════════════════════════════════════════════════════════════════
// Provider sidebar shell — consumes the collapse context so all subcomponents
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {/* Workspace identity (with switcher when there's somewhere to go).
            ...
            deal / new service" shortcuts. Hidden in collapsed rail mode
            because the row is the workspace identity at that width. */}
        <WorkspaceSwitcher
          currentName={spaceName}
          currentSubtitle="Solo provider"
          currentIcon={Briefcase}
          slug={slug}
          spaceName={spaceName}
          // The provider sidebar stays a provider surface — no agency rows
          // here. Switching to the agency lives on the header ("Switch to
          // {agency}") so the workspace identity here reads clean.
          agencyMemberships={[]}
          isOnAgencyPage={isOnAgencyPage}
          collapsed={collapsed}
          showQuickCreate
        ...
        {/* Notification card slot — reserved space the inspiration the
            provider sent shows as an "Update available →" card. We don't
        ...
        {/* Platform-admin console link — only rendered for isPlatformAdmin
            ...
            reads as "this exits the provider app." Calm; no CTA weight. */}
        {isPlatformAdmin && (
          <>
            <div className="border-t border-border/60" />
