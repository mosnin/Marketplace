...
/**
 * Opening moment. On every app/PWA open, Koala greets the provider by name
 ...
 * they were away, signs off with the small Koala mark, then the whole thing
...
// bundle: it survives client-side navigation (so switching between the provider
// and agency dashboards does NOT replay the heavy splash) but resets on a full
// page reload (so a first open or hard refresh plays it again). The lighter
// Koala-logo account-switch swipe is a separate component and is unaffected.
let splashPlayed = false;

export interface KoalaSnapshot {
  newLeads: number;
  followUpsDue: number;
  draftsReady: number;
}
...
function buildItems(s: KoalaSnapshot): { n: number; label: string }[] {
...
export function KoalaSplash({
  ...
  // hands the moment to the lighter Koala-logo swipe instead.
          ...
          <AnimatePresence mode="wait">
            {stage === "greeting" ? (
              ...
              >
                ...
                {/* Small Koala mark — the sign-off the overlay swipes up with. */}
                <motion.div
