...
const TYPE_ICONS: Record<string, typeof Bell> = {
  ...
  follow_up_due: Clock,
  waitlist: Users,
  appointment_needs_action: Briefcase,
};
...
export function NotificationCenter({ slug, spaceId }: { slug: string; spaceId?: string }) {
  ...
  // Realtime: refetch notifications when this space's Contact or Appointment
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes' as any,
        { event: '*', schema: 'public', table: 'Appointment', filter: `spaceId=eq.${spaceId}` },
        () => { loadNotifications(); },
      )
      .subscribe();
