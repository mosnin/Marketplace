...
/** A skill the provider can pick from the `/` menu. */
export interface SkillItem {
  slug: string;
  title: string;
  description: string;
  /** Composer text; may carry one {placeholder} for the provider to fill. */
  prompt: string;
}
 ...
 * the provider types straight over it; with no token the cursor lands at the
...
/**
 * Chat vs Agent — the per-message runtime the provider picks in the composer.
...
interface KoalaPromptBoxProps {
  ...
  skills?: SkillItem[];
  /**
   * Show the Chat ↔ Agent runtime switch. Default true (provider surface).
   * The agency chat is always its own agent, so the switch would be a lie
...
export const KoalaPromptBox = React.forwardRef<HTMLTextAreaElement, KoalaPromptBoxProps>(
  function KoalaPromptBox(
    ...
    // text-keyed so the provider can edit/clear without us re-stomping it.
    ...
    // textarea keep it open — the provider is still typing the query.
    useEffect(() => {
      if (!slashOpen) return;
      const onDown = (e: MouseEvent) => {
    ...
    function handleSubmit() {
      ...
      // next message doesn't silently run the tool loop the provider forgot
      // they'd left on.
      setChatMode('chat');
