...
import { koalaErrorMessage, classifyError } from '@/lib/ai-tools/koala-voice';
...
export interface UseAgentTaskOptions {
  ...
  /**
   * Backing API endpoints. Defaults route to the provider surface; the
   * agency variant (`/agency/koala`) overrides both to hit the agency-
   * gated routes (`resolveAgencyContext()` gates layer 2 of the
   ...
   *                             shape differs per variant (provider sends
   *                             `{ slug }`; agency sends nothing because
   *                             the agency route resolves agency from
   *                             the Clerk session).
   * - `resumeEndpoint`        — POST target for approve / deny resume.
   *                             Phase 1 doesn't ship agency approvals so
   *                             the agency variant inherits the provider
...
export function useAgentTask(options: UseAgentTaskOptions): UseAgentTaskResult {
   ...
   */
  const landKoalaError = useCallback((message: string) => {
    setError(message);
  ...
  const applyEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      ...
      case 'tool_call_start': {
        ...
        // tool_call block so the provider sees one clean card, not both.
      ...
      case 'permission_required': {
        ...
        // and no resolution when the provider came back. pendingApproval is
      ...
      case 'error': {
        ...
        const text =
          event.message && event.message.length < 400
            ? event.message
            : koalaErrorMessage(event.code ?? 'internal');
        landKoalaError(text);
        return;
      }
    }
  ...
  const consumeStream = useCallback(
    async (url: string, body: unknown) => {
      ...
      try {
            ...
            const code =
              res.status === 429
                ...
                : res.status === 401 || res.status === 403
                  ? 'auth'
                  : 'internal';
            message = koalaErrorMessage(code);
          }
          landKoalaError(message);
          return;
        }

        if (!res.body) {
          landKoalaError(koalaErrorMessage('network'));
          return;
        }
        ...
        if (!aborted) {
          ...
          landKoalaError(koalaErrorMessage(classifyError(raw)));
        } else {
    ...
    [abort, applyEvent, landKoalaError],
  );
   ...
   * so the agency variant can target /api/ai/agency-conversations (gated by
   * resolveAgencyContext) without a custom hook. Defaults preserve the
   * provider behaviour: POST /api/ai/conversations { slug }.
   */
  ...
  const send = useCallback(
    async (text: string, attachmentIds?: string[], mode: ChatMode = 'chat') => {
      ...
      try {
        ...
        // back so the provider doesn't see a hung user message + empty
        // assistant bubble. landKoalaError surfaces an error message in
    ...
    [isStreaming, spaceSlug, ensureConversationId, consumeStream, landKoalaError, taskEndpoint],
  );
