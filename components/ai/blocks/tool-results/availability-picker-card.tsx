 ...
 * `propose_appointment_times` tool as a tappable day-by-day grid. Click a slot
 * → the workspace fires a "Schedule the appointment at <slot>" prompt, which
 * Koala handles via the normal schedule_appointment approval flow. No new
...
export interface AvailabilityPickerCardProps {
   ...
   *  as the provider's next message. When omitted the card renders as
...
function SlotButton({
  ...
  const handleClick = () => {
    ...
    // (contact, service) that the model doesn't need to re-ask. The
    // ISO timestamp is the load-bearing detail — schedule_appointment parses it.
    const parts = ['Schedule the appointment at', slot.label];
    if (serviceAddress) parts.push(`for ${serviceAddress}`);
