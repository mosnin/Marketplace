...
export function ServiceCard({ service, slug, animDelay = 0 }: ServiceCardProps) {
  ...
  useEffect(() => {
    ...
    if (!slug) return;
    setLoading(true);
    fetch(`/api/cards/service/${service.id}?slug=${encodeURIComponent(slug)}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d: ServiceDetail | null) => {
        if (d) setDetail(d);
      })
      .catch(() => undefined)
      .finally(() => setLoading(false));
  }, [open, detail, loading, slug, service.id]);

  const compactPrice = formatPrice(service.price);
  const iconBgClass = iconBg(service.listingStatus);
  const chipClass = statusChipClass(service.listingStatus);

  // Sub-line: beds · baths · sqft
  const specs: string[] = [];
  if (service.beds != null) specs.push(`${service.beds} bd`);
  if (service.baths != null) specs.push(`${service.baths} ba`);
  if (service.sqft != null) specs.push(`${formatSqft(service.sqft)} sqft`);
            ...
            <div className="border-t border-border/40 bg-muted/20 px-4 py-4 space-y-4">
              ...
              {!loading && detail && (
                <>
                  ...
                  <ServiceHero photos={detail.photos ?? []} address={detail.address} />
                  ...
                  {/* Static map preview — only rendered when
                      ...
                      same intent. */}
                  <ServiceMap address={detail.address} />
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <a
                      href={`/s/${slug}/services/${service.id}`}
                      target="_blank"
 ...
 *  imagery the moment the provider opens a service card. */
function ServiceHero({ photos, address }: { photos: string[]; address: string }) {
...
function ServiceMap({ address }: { address: string }) {
  const [errored, setErrored] = useState(false);
