 ...
 * inline contacts / deals / appointments / services cards live, and we don't want
 ...
 * the provider still sees the answer without expanding. The group itself is
...
function argsSubtitle(args: Record<string, unknown> | undefined | null): string | undefined {
  ...
  const SKIP_KEYS = new Set(['contactId', 'dealId', 'appointmentId', 'serviceId', 'id']);
...
export function ToolGroupBlockView({ blocks, liveCallIds, onUserIntent }: ToolGroupBlockViewProps) {
  ...
  // truncated so the provider sees WHY without expanding. Empty result.error
