...
import { AppointmentsResult } from './tool-results/appointments-result';
import { ServicesResult } from './tool-results/services-result';
...
const TOOL_ICONS: Record<string, typeof Users> = {
  ...
  search_appointments: CalendarDays,
  get_note: FileText,
  send_email: Mail,
  ...
  draft_message: Mail,
  add_service: Building2,
  find_service: Building2,
  search_services: Building2,
};

/**
 * Tool-specific running verb. Providers don't think in developer words like
...
const TOOL_RUNNING_LABEL: Record<string, string> = {
  ...
  schedule_appointment: 'Checking calendar…',
  reschedule_appointment: 'Checking calendar…',
  check_availability: 'Checking calendar…',
  find_appointments: 'Checking calendar…',
  send_email: 'Drafting…',
  ...
  note_on_service: 'Saving note…',
  find_service: 'Looking up…',
  search_services: 'Searching…',
  add_service: 'Saving…',
...
/**
 * Produce a short prose hint from the tool args that a provider can read at
 * a glance. UUID fields (contactId, dealId) are meaningless to providers so
...
function argsProseHint(args: Record<string, unknown> | undefined | null): string | null {
  ...
  // UUIDs: skip entirely — they mean nothing to a provider.
  ...
  const UUID_KEYS = new Set(['contactId', 'dealId', 'appointmentId', 'serviceId', 'id']);
...
export function ToolCallBlockView({
  ...
  const richResult: React.ReactNode = (() => {
    ...
    if (block.display === 'appointments' && Array.isArray((data as { appointments?: unknown[] }).appointments)) {
      return <AppointmentsResult data={data as { appointments: never[] }} />;
    }
    if (block.display === 'services' && Array.isArray((data as { services?: unknown[] }).services)) {
      return <ServicesResult data={data as { services: never[] }} />;
    }
    if (
  ...
  // Inline error breadcrumb. On failure the provider needs to know WHY without
  ...
  return (
    ...
    >
      {/* Compact step row. Collapsed by default — args, summary, and full
          ...
          since it IS the provider's answer; (2) the rose-tone error
      ...
      >
        ...
        {/* Args hint — only when expanded. Collapsed view stays minimal
            (icon + label + status); the provider expands to see context. */}
        {argsHint && expanded && (
      ...
      {/* Inline error breadcrumb for failed tools. Always visible — this
          is Stream C's status-honesty pattern (commit 4859066): the provider
      ...
      {/* Rich inline result rendering — visible by default for known data
          shapes (contacts, deals, appointments) so the provider doesn't have to expand. */}
      {richResult}
