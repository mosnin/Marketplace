 ...
 *  (Composio trigger fired, etc.) rather than the provider's own chat
...
/** Slug → provider-readable phrase. Keep these short — they render
...
// draft, default it to auto-send after a short countdown unless the provider
...
// confidence "green dot" threshold in the row meta line. 30s gives a provider
...
function DraftRow({
  ...
  // A draft carrying a service packet — recognised by the secure
  // /packet/<token> path the agent's send_service_packet tool produces.
  // Subtle pill in the meta row so the provider knows what they're approving
  ...
  async function handleApprove() {
    ...
    // banner so the provider sees the actionable nudge instead of a win line.
    if (result?.sent) {
  ...
  // We start from the moment the row meets all conditions; if the provider
  ...
  // contact name still anchors the moment so the provider knows whose row
  ...
  return (
    <article className="group/row py-5 first:pt-0 last:pb-0">
      ...
      {/* Trigger provenance breadcrumb — Koala-voiced one-liner that
          ...
          quiet. This is the "Koala noticed something for me" trust
  ...
  async function batchApprove() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // rows and surface a single error. The provider can try again.
        setDrafts((prev) => {
      ...
      // Restore any drafts that failed so the provider can retry them in
      ...
      // Network blip — restore the snapshot and let the provider retry.
      setDrafts((prev) => {
        const have = new Set(prev.map((d) => d.id));
        return [...snapshot.filter((d) => !have.has(d.id)), ...prev];
      });
