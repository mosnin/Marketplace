...
import type { AgencyMorningResponse } from '@/app/api/agency/morning/route';

/**
 * The /agency home's chief-of-staff opening line.
 *
 * Replaces the old snapshot-counts pattern ("5 providers, 2 drafts, 3 hot
 * leads") with ONE prioritized action the agency should take RIGHT NOW —
 * composed server-side by /api/agency/morning. The single sentence is the
 ...
 * deep-links into /agency/koala with the prompt prefilled — making the
...
const STATS_LOADING: AgencyMorningResponse['stats'] = {
  providers: 0,
  activeDeals: 0,
  gciMtd: 0,
};
 ...
 * agency's eye should land on. Falls back to rendering the whole headline
...
export function AgencyMorningStory() {
  const router = useRouter();
  const [data, setData] = useState<AgencyMorningResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch('/api/agency/morning', { signal: controller.signal });
        if (res.ok) {
          const body = (await res.json()) as AgencyMorningResponse;
          setData(body);
        } else {
          setError(true);
        }
  ...
  function handleTap() {
    ...
    router.push(
      `/agency/koala?prefill=${encodeURIComponent(suggestedPrompt)}`,
    );
  }
...
/**
 * Hairline-divider context row. Three numbers — providers, active deals,
 * GCI MTD — rendered as small, muted facts beneath the headline. This is
 * intentionally NOT the snapshot strip from app/agency/page.tsx (that's
...
function StatsRow({
  ...
  stats: AgencyMorningResponse['stats'];
  loading?: boolean;
}) {
  return (
    <div className="flex items-center gap-4 text-[11px] tabular-nums text-muted-foreground">
      <Stat label="providers" value={loading ? '—' : `${stats.providers}`} />
      <Divider />
