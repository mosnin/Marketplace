...
export function OffboardMemberDialog({
  ...
  async function callOffboard<T>(dryRun: boolean): Promise<T> {
    const res = await fetch(`/api/agency/members/${member.id}/offboard`, {
      method: 'POST',
  ...
  const nothingToTransfer =
    ...
    preview.dealCount === 0 &&
    preview.openAppointmentCount === 0;
        ...
        {state === 'choose' && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Pick which active member should receive this agent&apos;s agency
                contacts, deals, and open appointments. This is reversible manually but not
          ...
          <>
            ...
            {nothingToTransfer ? (
              <div className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
                Nothing to transfer &mdash; this agent has no agency-owned data.
              </div>
            ) : (
              <div className="rounded-lg border border-border bg-card p-4 space-y-2">
                <p className="text-sm">
                  <span className="font-semibold tabular-nums">{preview.contactCount}</span>{' '}
                  agency {preview.contactCount === 1 ? 'contact' : 'contacts'} will move
                  to{' '}
                ...
                <p className="text-sm">
                  <span className="font-semibold tabular-nums">{preview.openAppointmentCount}</span>{' '}
                  open {preview.openAppointmentCount === 1 ? 'appointment' : 'appointments'} will be reassigned
                </p>
              </div>
            )}
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="tabular-nums font-medium text-foreground">
                  {result.appointmentsMoved}
                </span>{' '}
                {result.appointmentsMoved === 1 ? 'appointment' : 'appointments'} moved to{' '}
