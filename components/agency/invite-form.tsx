...
import {
  ...
  normalizeSeatUsage,
  type SeatPlan,
} from '@/components/agency/seat-usage-pill';
...
interface InviteFormProps {
  isOwner?: boolean;
  /**
   * Current seat usage for the agency. When present and well-formed, an
...
export function InviteForm({ isOwner = true, seatUsage }: InviteFormProps) {
  ...
  const [role, setRole] = useState<'provider_member' | 'agency_admin'>('provider_member');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        {isOwner ? (
          <div className="flex rounded-md border border-input overflow-hidden">
            <button
              type="button"
              onClick={() => setRole('provider_member')}
              disabled={loading}
          ...
          <input type="hidden" name="role" value="provider_member" />
        )}
        <Button
          type="submit"
          size="sm"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {overCap && (
            <span className="text-xs text-rose-600 dark:text-rose-400">
              ...
              <Link href="/agency/billing" className="underline underline-offset-2">
                Upgrade plan
              </Link>{' '}
              to invite more.
            </span>
      ...
      {role === 'agency_admin' && isOwner && (
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          <ShieldCheck size={12} />
          Admins can manage members, leads, and agency settings.
        </p>
      )}
      {message && (
        <div
