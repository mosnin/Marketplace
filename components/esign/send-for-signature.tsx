 ...
 * signature on the provider's OWN connected DocuSign account (via Composio).
 *
 * Two embeds share it:
...
interface SendForSignatureProps {
  ...
  slug: string;
  /** True when the provider has DocuSign connected (server-resolved). */
  connected: boolean;
  /** Fixed document to send (deal doc row). Omit to let the provider pick. */
  documentId?: string;
