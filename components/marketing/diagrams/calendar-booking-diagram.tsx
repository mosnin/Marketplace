 ...
 *
 * One beat: an empty calendar week view; a APPOINTMENT block fades into the
 * Wednesday 2pm slot; a small "Confirmed with M. Chen" row appears
 * below the grid, anchored by the Koala badge.
 *
 ...
 * six, so each row stays tall enough to hold the appointment block's three lines at
 * the shortest (`wide`) box. The confirmation row is shrink-0 and truncates.
 *
 * Motion contract:
 *   - 7.5s cycle.
 *   - 800ms hold on empty grid.
 *   - Appointment block: opacity 0 → 1 + 6px y-translate up, 280ms, EASE_APPLE.
 *   - 600ms after block lands, confirmation row fades in (220ms).
 *   - 2.2s hold. Then 200ms fade-out across both, 700ms pause, restart.
 *
 * Reduced-motion: render appointment block + confirmation row visible.
 *
 ...
 *   - Appointment block: hairline-bordered tile, `text-[10px] font-medium`.
...
// Four hours, not six: enough to stage a 2pm appointment with context above (12, 1)
// and below (3) while leaving each row tall enough to hold the appointment block's
// three lines at the shortest aspect (`wide`, ~56px/row at the md 2-col width).
const HOURS = ['12', '1', '2', '3'];

// Phase: 0 empty, 1 appointment visible, 2 appointment + confirm visible, 3 fade-out.
const APPOINTMENT_AT = 900;
...
function CalendarBookingContent() {
  ...
  useEffect(() => {
    ...
    function runCycle() {
      timers.push(setTimeout(() => !cancelled && setPhase(1), APPOINTMENT_AT));
  ...
  const appointmentVisible = phase === 1 || phase === 2;
  ...
  return (
    <div className="w-full h-full flex flex-col gap-2.5 min-h-0">
      {/* Calendar week grid — matches product calendar vocabulary. Fills all
          available height; rows distribute as equal fractions so the appointment
        ...
        <div className="flex-1 min-h-0 grid grid-cols-[36px_repeat(5,minmax(0,1fr))] grid-rows-4">
          {HOURS.map((h, rowIdx) => (
            <div key={h} className="contents">
              ...
              {DAYS.map((d) => {
                const isAppointmentCell = d === 'Wed' && h === '2';
                return (
                  <div
                    key={`${d}-${h}`}
                    className={cn(
      ...
      {/* Confirmation strip — quiet row below the grid; the Koala badge
          ...
          one thing — Koala booked it — and truncates rather than wrapping
