...
/**
 * `<KoalaDiagramShell>` — the paper-flat container every animated UI
...
interface KoalaDiagramMotionContextValue {
...
const KoalaDiagramMotionContext = createContext<KoalaDiagramMotionContextValue>({
  reduced: false,
});
 ...
 */
export function useDiagramMotion(): KoalaDiagramMotionContextValue {
  return useContext(KoalaDiagramMotionContext);
}

export interface KoalaDiagramShellProps {
  aspect?: Aspect;
...
export function KoalaDiagramShell({
  aspect = 'video',
  pad = 8,
  caption,
  className,
  children,
}: KoalaDiagramShellProps) {
  const reduced = useReducedMotion() ?? false;
  const padClass = pad === 6 ? 'p-6' : 'p-8';

  return (
    <KoalaDiagramMotionContext.Provider value={{ reduced }}>
      <div
        data-koala-diagram
        className={cn(
    ...
    </KoalaDiagramMotionContext.Provider>
  );
}

/**
 * The Koala authorship badge as it lives inside diagrams — same
 ...
 * names; this is one of them (AGENT_BADGE / KOALA_AVATAR).
 */
export function DiagramKoalaBadge({
  label = 'Koala',
  className,
}: {
  label?: string;
