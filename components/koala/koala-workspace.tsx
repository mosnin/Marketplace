...
interface KoalaWorkspaceProps {
  ...
  initialInput?: string;
  /** Pre-populate the composer on mount but do NOT auto-send — the provider
   ...
   *  line under the post-appointment affordance. Snapshot at page load: the provider
   ...
   *
   * - `provider` (default) — solo / agency-member chat at /s/<slug>/koala.
   *   Backed by `/api/ai/task` and the native provider tool catalog.
   * - `agency` — chief-of-staff chat at /agency/koala. Backed by the
   *   agency-gated `/api/ai/agency-task` (defense layer 2) and the
   *   `AGENCY_TOOLS` registry (empty in Phase 1; Phase 2/3 will populate).
   *
 ...
 *      sound is a delight the provider chooses to turn on, never one we
...
export function KoalaWorkspace({
  ...
  // The single sentence the provider sees when they approve a celebrate-able
  ...
  const {
    ...
    // Agency variant routes through `/api/ai/agency-task` — the agency
    // version re-checks `resolveAgencyContext()` (defense layer 2) before
    // forwarding the turn to Modal with `mode: 'agency'`.
    ...(isAgency
      ? {
          taskEndpoint: '/api/ai/agency-task',
          conversationsEndpoint: '/api/ai/agency-conversations',
          // The agency conversations route reads agency scope from the
    ...
    onConversationCreated: (id) => {
      ...
      // — provider sits at /s/<slug>/koala, agency at /agency/koala.
      const newConvBase = isAgency ? '/agency' : `/s/${slug}/koala`;
  ...
  // Wrap approve / alwaysAllow so the moment the provider approves a
  ...
  useEffect(() => {
    if (searchParams.get('view') === 'history') {
      ...
      const baseUrl = isAgency ? '/agency' : `/s/${slug}/koala`;
      router.replace(`${baseUrl}${qs ? `?${qs}` : ''}`, { scroll: false });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Agency history comes from the agency-gated `/api/ai/agency-messages`
  // (reads "AgencyMessage"); provider history from `/api/ai/messages` (reads
  // "Message"). Choosing by variant, mirroring taskEndpoint / conversations-
  // Endpoint above, is what fixes the agency "can't load history after reload"
  // 404 — the provider endpoint refuses agency conversations by design.
  const messagesEndpoint = isAgency ? '/api/ai/agency-messages' : '/api/ai/messages';

  // Per-conversation mutations (rename / delete) hit the agency-gated
  // `/api/ai/agency-conversations/<id>` for the agency variant — the provider
  // `/api/ai/conversations/<id>` route refuses agency rows (and the agency
  // rows live in a different table entirely). Returns the base path; callers
  // append the conversation id.
  const conversationItemBase = isAgency ? '/api/ai/agency-conversations' : '/api/ai/conversations';
  ...
  // Resolve the route base once. Same URL shape both variants — agency
  // sits at /agency/koala, provider at /s/<slug>/koala.
  const koalaBaseUrl = isAgency ? '/agency' : `/s/${slug}/koala`;
  ...
  async function handleNewConversation() {
    const endpoint = isAgency ? '/api/ai/agency-conversations' : '/api/ai/conversations';
    const body = isAgency ? {} : { slug };
    try {
  ...
  // call is delayed enough that the provider can walk it back without a
  ...
  useEffect(() => {
    return () => {
      ...
      // a page nav after the provider walked away from the toast.
    ...
    // conversationItemBase is constant per mount (isAgency never changes), so
  ...
  async function handleDeleteConversation(id: string) {
    ...
    // Optimistic: remove from the sidebar immediately so the provider sees
    ...
    // Agency conversations live in their own "AgencyConversation" table, so
    // the delete routes to the agency-gated /api/ai/agency-conversations/<id>
    // (which scopes the delete to the caller's agency). The provider route
    // is used for the provider variant.
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // provider. They've already moved on by now, so the toast carries
  ...
  const handleMentionSearch = useCallback(
    async (query: string): Promise<MentionItem[]> => {
      ...
      try {
        ...
        // id; once a search succeeds again the provider just dismisses it.
  ...
  const handleSend = useCallback(
    async (
      ...
      // clean — the helper no-ops when the provider hasn't enabled sound
  ...
  // (the server doesn't know the provider's local hour); we render an
  ...
  //
  // Agency variant doesn't surface drafts/questions on the chat home —
  // those endpoints are space-scoped (provider-only). Skip the fetch.
  ...
  // Day-one signal: zero of everything. The provider has truly never engaged.
  ...
  const TOOL_ACTION_MAP: Record<string, string> = {
    ...
    schedule_appointment: 'Checking the calendar…',
    reschedule_appointment: 'Checking the calendar…',
    find_appointments: 'Checking the calendar…',
  ...
  const TOOL_STEP_KEYWORDS: Record<string, string[]> = {
    ...
    schedule_appointment: ['appointment', 'calendar', 'schedule'],
    reschedule_appointment: ['appointment', 'calendar', 'schedule'],
    find_appointments: ['appointment', 'calendar'],
    set_followup: ['follow'],
  ...
  const currentAction = useMemo<string | null>(() => {
    ...
    if (liveCallIds && liveCallIds.size > 0) {
      for (const block of tailMessage.blocks) {
        if (
          ...
          // get a friendly verb derived from the toolkit name so providers
    ...
    // dead air with a single calm status so the provider knows Koala is on
    // it, not stuck.
    ...
    <div className="relative flex flex-col h-full min-h-0">
      ...
      <div className="absolute top-1.5 right-2 sm:top-2 sm:right-3 z-20 flex items-center gap-1.5">
        ...
        {/* One three-dots menu — folds New chat, History, Brief/Drafts, and
            (provider) Run now / Memory / Koala settings into a single animated
            ...
            motion-reduce aware). Routes resolve per variant — provider:
            /s/<slug>/koala/*, agency: /agency/* — never a broken /s//… href. */}
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" sideOffset={6} className="w-48">
            ...
            <DropdownMenuItem asChild>
              <Link
                href={isAgency ? '/agency/brief' : `/s/${slug}/koala/brief`}
            ...
            <DropdownMenuItem asChild>
              <Link
                href={isAgency ? '/agency/reviews' : `/s/${slug}/koala/inbox`}
                className="cursor-pointer"
              >
        ...
        {/* Split panel disabled on agency variant — RightPanel tabs
            (contacts/deals/etc.) are provider-scoped in Phase 1. */}
        {!isAgency && <SplitPanelToggle isSplit={isSplit} onToggle={toggleSplit} />}
      </div>
      ...
      {isLoadingConversation ? (
        ...
        /* Empty → active swap.
           ...
           through the exit animation, so the provider briefly had TWO
            ...
            >
              ...
              <div className="w-full max-w-2xl">
                <motion.h1
                  ...
                  {/* Day-one guidance — provider only, and only for users with
                      zero history. The agency chat goes straight from greeting
                      to composer (no subtext) per the agency design. */}
                  {!isAgency && isFresh && (
                    <motion.p
              ...
              <div className="w-full max-w-3xl mx-auto chat-content-wrap pt-12 sm:pt-14 pb-36">
                ...
                <div className="space-y-7">
                  {messages.map((msg, i) => {
                    ...
                    // — typed text reading "instantly there" is the provider's
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 pt-0.5 space-y-3">
                            ...
                            {/* Inline retry button — shown on the tail error
                                message so the provider doesn't have to retype. */}
