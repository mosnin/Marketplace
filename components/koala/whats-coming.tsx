...
interface TodayData {
  ...
  appointmentsUpcoming: UpcomingAppointment[];
}
 ...
 * Shows appointments scheduled forward and follow-ups due today or overdue.
 * Hides itself when both lists are empty so quiet days stay calm.
 */
export function WhatsComing({ slug }: { slug: string }) {
  const [data, setData] = useState<TodayData>({ followUpsDue: [], appointmentsUpcoming: [] });
      ...
      try {
        ...
        if (res.ok) {
          ...
          setData({
            ...
            appointmentsUpcoming: json.appointmentsUpcoming ?? [],
          });
        }
      } catch {
        // non-critical
  ...
  const total = data.followUpsDue.length + data.appointmentsUpcoming.length;
    ...
    <section>
      ...
      {!loading && total > 0 && (
        <motion.div
          ...
          animate="enter"
        >
          {data.appointmentsUpcoming.map((appointment) => (
            <motion.div key={appointment.id} variants={STAGGER_ITEM}>
              <Link
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-medium text-foreground tabular-nums">
                      {formatDay(appointment.startsAt)} · {formatTime(appointment.startsAt)}
                    </span>
                    {appointment.guestName && (
                      <span className="text-muted-foreground truncate">with {appointment.guestName}</span>
                    )}
                  </div>
                  {appointment.serviceAddress && (
                    <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                      <MapPin size={10} />
                      <span className="truncate">{appointment.serviceAddress}</span>
                    </p>
                  )}
                </div>
                <ChevronRight
