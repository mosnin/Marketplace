...
/**
 * PostAppointmentRecorder — the one client component on /koala/log.
 *
 * State machine:
 ...
 *     Tap is forgiving.
 *   - The transcript is invisible. The provider never sees it. They see
 *     the actions land, which is the substance.
 *   - "Approve all" is the primary path. Editing exists but is one click
 *     away — the brief says most providers approve all.
...
interface Props {
   ...
   *  toward this person. Sent through as `contextHint` on the post-appointment
   *  request body. The recorder UI is unchanged; the provider never sees
   *  the id. */
  personId?: string;
...
export function PostAppointmentRecorder({ slug, personId, dealId }: Props) {
  ...
  // The provider never sees these — they're a hint to the proposal model so
  ...
  const processAudio = useCallback(async (blob: Blob) => {
    ...
    try {
      ...
      fd.append('audio', blob, 'appointment.webm');
    ...
    setState('processing');
    try {
      const res = await fetch('/api/koala/post-appointment', {
        method: 'POST',
  ...
  const handleApproveAll = useCallback(async () => {
    ...
    setState('approving');
    try {
      const res = await fetch('/api/koala/post-appointment/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
/** Keys we never expose to the provider in the args editor — they're
...
const HIDDEN_ARG_KEYS = new Set(['personId', 'dealId', 'contactId', 'serviceId', 'appointmentId']);
...
function ArgsEditor({
  ...
  // synthesize new keys — the model already chose them; the provider edits
