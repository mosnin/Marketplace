 ...
 *
 * Sections:
 *   ON DECK   the provider's action cards (reuses brief.cards)
...
interface Props {
   ...
   *  by the dedicated /koala/brief page where the provider navigated
...
export function DailyBrief({ slug, initialBrief, alwaysLive = false }: Props) {
  ...
  // handling: the provider never stares at a blank surface wondering
  ...
  // so the "Yesterday →" toggle works whether the provider is in the live
...
/**
 * Day-one provider: no cards, no calendar events, no pipeline, no overnight
