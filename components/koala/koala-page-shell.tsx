 ...
 *
 * `variant` — 'provider' (default) or 'agency'. Does not change the shell's
 * visual structure today, but is forwarded so downstream consumers and the
 * agency-home renderer can pass `variant="agency"` when composing sub-pages
 * inside the agency surface. The KoalaWorkspace nav dropdown (Brief /
 * Drafts / History) reads the same prop to resolve agency-correct routes.
 */
...
interface KoalaPageShellProps {
   ...
   *
   * - `provider` (default) — /s/[slug]/koala/* sub-pages.
   * - `agency` — /agency/* sub-pages.
   *
   * The shell's visual output is identical for both variants today. The prop
   * is pinned on the interface so the agency-home renderer (and any future
   * agency-specific sub-page) can declare intent clearly. The KoalaWorkspace
