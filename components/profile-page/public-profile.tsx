/**
 * The public provider "link in bio" page rendered at /p/[slug].
 *
...
interface PublicProfileProps {
  ...
  videos: PublicVideo[];
  /** Provider-curated hero image. When set it becomes the full-bleed header
  ...
  /** Small blue checkmark next to the provider's name. Provider-controlled
 ...
 *  Instagram first (where providers live) → Facebook → X → LinkedIn →
...
/** A single service card inside the carousel. Image-led; fixed width so
 *  the carousel hints a peek of the next card at the right edge on mobile.
 *  When a service has no photo we render a tasteful muted placeholder
...
function ServiceCard({ service }: { service: PublicService }) {
  ...
  if (service.listingUrl) {
    return (
      <a
        href={safeHref(service.listingUrl)}
        target="_blank"
        rel="noopener noreferrer"
...
export function PublicProfile({
  ...
  // Pick the best image for the desktop blur fill: provider-curated cover
  // wins; otherwise the provider's face; otherwise null (we fall through to
      ...
      <>
        ...
        <div className="px-6 pb-10">
          ...
          {/* Services — horizontal carousel.
              ...
              card locks the card to the left edge as the provider swipes.
