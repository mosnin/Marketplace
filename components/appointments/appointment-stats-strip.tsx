...
interface AppointmentStatsStripProps {
  appointments: Array<{
    startsAt: string;
    status: string;
...
function computeStats(appointments: AppointmentStatsStripProps['appointments']): AppointmentStats {
  const now = new Date();
  ...
  const appointmentsThisWeek = appointments.filter((t) => {
    const d = new Date(t.startsAt);
    return d >= weekStart && d < weekEnd && t.status !== 'cancelled';
  }).length;

  const completed = appointments.filter((t) => t.status === 'completed');
  ...
  const past = appointments.filter((t) => new Date(t.startsAt) < now && t.status !== 'cancelled');
  ...
  const confirmed = appointments.filter((t) => ['confirmed', 'completed'].includes(t.status));
  let avgConfirmTime = '—';
  if (confirmed.length > 0) {
    // Use time until appointment as proxy
    const times = confirmed
      .map((t) => {
        const appointmentDate = new Date(t.startsAt);
        const created = t.createdAt ? new Date(t.createdAt) : null;
        if (!created) return null;
        return appointmentDate.getTime() - created.getTime();
      })
  ...
  return { appointmentsThisWeek, conversionRate, noShowRate, avgConfirmTime };
}

export function AppointmentStatsStrip({ appointments }: AppointmentStatsStripProps) {
  const stats = computeStats(appointments);

  const items: Array<{ label: string; value: React.ReactNode }> = [
    {
      label: 'This Week',
      value: <AnimatedNumber value={stats.appointmentsThisWeek} />,
    },
    {
      label: 'Conversion',
