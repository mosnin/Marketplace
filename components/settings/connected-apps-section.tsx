 ...
 *   - none     → "Connect" pill
 *
 * The provider sees one row per app. Connect → OAuth at the provider →
 ...
 * + connect, but the provider sees one tap.
 *
 ...
 * the provider comes back to the tab, not just when they next open a chat.
 */
...
interface ConnectionRow {
   ...
   *  'active' — at least one trigger registered and listening; Koala
   *             will draft something the next time the provider's app
   *             emits an event we care about
   *  'paused' — triggers exist but the provider turned them off
   *  'failed' — registration was attempted but Composio rejected. The
   *             provider sees this as "couldn't tune in" so silent
 ...
 * the provider can act on. Composio errors like `Auth_Config_NotFound` are
 * meaningless to a non-engineer — we map them to "set up the auth config in
 * the Composio dashboard" so the provider knows where to go.
 */
 ...
 * checklist). Those messages name infrastructure the provider can't act
 * on — they belong on the admin-flavored /settings surface, not on
 * /integrations where the provider lives. Default: off (provider view).
 * Pass `showSetupHealth` from /settings to keep the warnings where the
 * operator can fix them.
 */
/**
 * Endpoint set the panel talks to. Defaults to the provider routes so existing
 * callers are untouched. The agency integrations surface passes the
 * /api/agency/integrations routes instead — same component, same rendering,
 * different scope. `health` is optional: the agency surface has no health
...
const PROVIDER_ENDPOINTS: ConnectedAppsEndpoints = {
...
export function ConnectedAppsSection({
  ...
  // Auto-clear the callback banner when the provider dismisses it. Persisted
  ...
  const fetchHealth = useCallback(async () => {
    // No health endpoint (e.g. agency surface) → skip; the static status
  ...
  // This catches stale auth that expires while the provider is away.
  useEffect(() => {
    void fetchHealth();
  ...
  /**
   * Pause or resume Koala's watch on this connection. PATCHes the
  ...
  async function handleTogglePause(connectionId: string, currentlyPaused: boolean) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError("Couldn't change Koala's watch on that app.");
      }
    } catch {
      setConnections((prev) =>
        prev.map((c) =>
  ...
  if (configured === false) {
    ...
    // redeploys. The provider can't act on any of it. The admin-flavored
    // /settings view passes `showSetupHealth` to surface the checklist
    // where someone can fix it; the provider sees the canonical empty
    // state instead.
    if (!showSetupHealth) {
      return (
  ...
  // default URL, the provider never lands back in the app and the connection
  // never persists. Surface it loud — but only on the admin surface; the
  // provider can't act on a missing env var.
  ...
  return (
    <div className="space-y-8">
      {/* OAuth callback banner — green on success, amber on failure. The
          previous build silently dropped these results so the provider never
      ...
      {CATEGORY_ORDER.map((cat) => {
        ...
        // depth 501 list) but the provider never sees a row they can't act
        // on.
      ...
      <div className="flex-1 min-w-0">
        ...
        {showWatch && isFailed && (
          <p className="mt-1 text-[11px] text-amber-700 dark:text-amber-400 leading-relaxed">
            Koala couldn&apos;t tune in to this app. Try reconnecting.
          </p>
        )}
        {showWatch && !isFailed && (
          <p className="mt-1 text-[11px] text-muted-foreground leading-relaxed">
            {isPaused ? 'Quiet here.' : 'Koala is listening.'}{' '}
            <button
              type="button"
...
function Action({
  ...
  if (action.kind === 'coming-soon') {
    // Disabled, no click target. The provider can read the row, can see
