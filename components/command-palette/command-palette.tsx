 ...
 *   - Create actions route to the create-deal / add-contact / schedule-appointment
...
export function CommandPalette({ slug }: Props) {
  ...
  const staticActions: PaletteAction[] = useMemo(() => [
    ...
    { kind: 'route', id: 'nav-commissions', group: 'Navigate', label: 'Commissions', icon: BarChart2, href: `${base}/services/commissions` },
    ...
    { kind: 'route', id: 'new-appointment',    group: 'Create', label: 'Schedule appointment', icon: PhoneCall, href: `${base}/calendar` },
  ], [base]);
  ...
  useEffect(() => {
    ...
    searchRef.current = setTimeout(async () => {
      try {
        // /api/search returns { contacts, deals, appointments } in one call.
