...
import { ServiceShareDialog } from './service-share-dialog';
...
interface Props {
  ...
  linkedAppointments: { id: string; guestName: string; startsAt: string; status: string }[];
}

export function ServiceDetailClient({ slug, initial, linkedDeals, linkedAppointments }: Props) {
  ...
  async function remove() {
    if (!confirm('Delete this service? Linked deals and appointments stay intact.')) return;
    const res = await fetch(`/api/services/${service.id}`, { method: 'DELETE' });
    if (!res.ok) { toast.error("Couldn't delete that service."); return; }
    toast.success('Deleted.');
    router.push(`/s/${slug}/services`);
  }
  ...
  if (editing) {
    return (
      <div className="rounded-xl border border-border/70 bg-card p-5">
        <h1 className="text-lg font-semibold mb-4">Edit service</h1>
        <ServiceForm
          initial={service}
    ...
    <div className="space-y-8">
      ...
      <header className="space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          ...
          {service.serviceType && (
            <span>· {service.serviceType.replace('_', ' ')}</span>
          )}
      ...
      <div className="space-y-8 border-t border-border/60 pt-6">
        <LinkedSection
          title="Linked deals"
          icon={Briefcase}
          empty="No deals linked to this service yet."
        ...
        <LinkedSection
          title="Appointments"
          icon={CalendarDays}
          empty="No appointments have been scheduled here yet."
          items={linkedAppointments.map((t) => {
