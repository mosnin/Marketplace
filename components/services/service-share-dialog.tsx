...
import type { ServicePacket } from '@/lib/types';
import type { DealDocument } from '@/lib/deals/documents';
import { documentKindLabel } from '@/lib/deals/documents';

interface Props {
  serviceId: string;
  /** Deal ids linked to this service — used to list their documents. */
...
/**
 * Dialog for creating / managing listing-packet share links for a service.
 * Fetches existing packets + candidate documents (from deals linked to this
 * service), lets the provider curate which documents go in, and hands back
 * the shareable URL on create.
 */
export function ServiceShareDialog({ serviceId, linkedDealIds, origin, onClose }: Props) {
    ...
    async function load() {
      ...
      try {
        const [packetsRes, ...docResults] = await Promise.all([
          fetch(`/api/services/${serviceId}/packets`),
  ...
  async function create() {
    ...
    setCreating(true);
    try {
      const res = await fetch(`/api/services/${serviceId}/packets`, {
        method: 'POST',
      ...
      const packet: ServicePacket = await res.json();
  ...
  async function revoke(p: ServicePacket) {
    if (!confirm("Revoke this link? Anyone with the URL loses access right away.")) return;
    const res = await fetch(`/api/services/${serviceId}/packets/${p.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function copyLink(p: ServicePacket) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm pt-[8vh] px-4" onClick={onClose}>
      <div className="w-full max-w-[720px] rounded-xl border border-border/70 bg-card shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-3.5 border-b border-border flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Share this service</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="max-h-[70vh] overflow-y-auto p-5 space-y-5">
          ...
          <section className="space-y-3">
            ...
            <input
              ...
              placeholder="e.g. Appointment packet for Jane"
