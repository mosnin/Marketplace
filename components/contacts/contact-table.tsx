 ...
 * piece of "who's hot" signal a provider wants without sorting for it.
 */
...
export function ContactTable({ slug }: ContactTableProps) {
  ...
  // state — which would tell a provider with 200 contacts they have none.
  ...
  // provider's screenshot showed a permanent checkbox column crowding every
  ...
  // ("Qualifying 5 → Appointment 0 → Applied 0 → 5 total" pipeline strip) is gone;
  ...
  return (
    <div className="space-y-6">
      {/* Header — canonical three-line pattern: muted greeting, serif Times
          h1, one-sentence status. The "Tell Koala → / or fill out the form"
      ...
      {/* Empty state — context-aware. The fresh-workspace case is where the
          "Tell Koala → / or fill out the form" pair lives now: when the
        ...
        defaultValues={
          editContact
            ? {
                ...
                services: editContact.services.join(', '),
...
function ContactRow({
  ...
  const body = (
    <>
      ...
      {/* Right metadata — follow-up pill stays visible (the provider needs
