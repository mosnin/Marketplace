...
/**
 * Provider onboarding - V2, the conversational flow.
 *
 * The provider doesn't fill out a form; they have a conversation with Koala,
 ...
 *                     the Koala mark, then a blur-out into the chat.
 *   Act II (chat)   - Koala types each question; the provider answers through
 *                     real inline inputs rendered as their own chat bubbles.
 *                     Same surface, same motion as the live intake chat.
 *   Act III (ready) - Koala types a real first-touch draft (the payoff), then
 ...
 * call (check_slug, save_profile, create_space, save_provider_profile,
 * complete) and the agency-create/space-patch sequence are byte-for-byte what
 ...
 * (`onboarding-provider.tsx`) stays the rollback path - don't refactor it.
 */
...
import { KOALA_PILL } from '@/lib/typography';
...
import { KoalaSays, UserSays, AnswerAffordance, Thread } from './onboarding-chat';
...
} from './onboarding-provider-shared';
...
const ROLE_LABEL: Record<Role, string> = {
  ...
  agency_owner: 'I own the agency',
};

export function OnboardingProviderV2({ defaultName }: Props) {
  ...
  // Greet has no input - once Koala finishes saying hello, glide to the
  ...
  // Auto-derive slug from business name until the provider edits it.
  useEffect(() => {
  ...
  const handleCreateWorkspace = useCallback(async () => {
    ...
    try {
      ...
      await fetch('/api/onboarding', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'save_provider_profile', role, zipCode, yearsExperience: TENURE_TO_YEARS[tenure] }),
      });
  ...
  // which redirects when it finishes. (agency branch for owners.)
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'agency_owner') {
        ...
        if (!agencyRes.ok) throw new Error('agency_create');
        const agencyData = (await agencyRes.json().catch(() => ({}))) as { agency?: { id?: string } };
        const newAgencyId = agencyData.agency?.id;
        if (newAgencyId) {
          await fetch('/api/spaces', {
            method: 'PATCH', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ slug, agencyId: newAgencyId }),
          }).catch(() => undefined);
        }
  ...
  const promptFor = useCallback((s: Step): string => {
    switch (s) {
      case 'greet': return "I'm Koala, your new companion. From here on, I'm working right alongside you.";
  ...
  // The provider's answer bubble for a completed step. null = no bubble.
    ...
    <>
      ...
      >
        ...
        <div className="pointer-events-none fixed inset-x-0 top-0 z-10 flex justify-center bg-gradient-to-b from-background to-transparent pt-6 pb-8">
          <BrandLogo className="h-5 opacity-80" alt="Koala" />
        </div>
        ...
        <div className="relative z-[1]">
          <Thread signal={`${stepIdx}:${typedDone}`}>
            ...
            {ORDER.slice(0, stepIdx).map((id) => {
              ...
              return (
                <Fragment key={id}>
                  <KoalaSays text={promptFor(id)} />
                  {answer && <UserSays>{answer}</UserSays>}
                </Fragment>
              );
            })}

            {/* Active turn - Koala types, then the answer affordance fades in. */}
            <KoalaSays
              key={`active-${current}`}
      ...
      case 'promise':
        return (
          <div className="flex justify-end">
            <button type="button" onClick={advance} className={KOALA_PILL}>
...
function VoiceAffordance({
  ...
  const direct = `Hi, yes, 1422 Pine is available. Asking $625K, 3 bed / 2 bath, last open house had 11 groups through. I can hold a private appointment Tue 5–7p or Wed 6–7p. Which works? ${firstName}, ${business}`;
  return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={cn('flex justify-end transition-opacity duration-500', typed ? 'opacity-100' : 'pointer-events-none opacity-0')}>
        <button type="button" onClick={onFinish} disabled={submitting} className={KOALA_PILL}>
