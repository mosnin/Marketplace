 ...
 *                               holds, swaps to the Koala mark, then the
 ...
 * (`components/dashboard/koala-splash.tsx`): `motion/react`, the Apple-ish
...
export function OnboardingIntro({
  ...
  onDone: () => void;
  /** The cold-open title line. Defaults to the provider copy; the agency
   *  flow passes its own. */
  line?: string;
}) {
          ...
          <AnimatePresence mode="wait">
            {stage === 'line' ? (
              ...
              <motion.div
                ...
                {...rise}
              >
                <BrandLogo className="h-8 sm:h-9" alt="Koala" />
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
...
export function OnboardingReady({
  ...
  onDone: () => void;
  /** The working-words that flash one at a time. Defaults to the provider set. */
  words?: readonly string[];
  /** The line that lands before the dissolve. Defaults to the provider copy. */
  finalLine?: string;
}) {
