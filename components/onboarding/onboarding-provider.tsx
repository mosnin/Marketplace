...
/**
 * Provider onboarding — six stages, ~4 minutes.
 *
 * Replaces the one-screen `OnboardingQuick`. The point isn't to "collect more
 * fields" — it's that by the end, Koala already knows the provider's role,
 ...
 *   6. Plan                — Koala's first-week plan, derived from the above.
 *
 * Persistence:
 *   - End of stage 2: `save_profile` + `create_space`. Slug is committed.
 *   - End of stages 3, 4, 5: `save_provider_profile` upsert (cumulative).
 *   - End of stage 6: `complete` → redirect to /s/{slug}/koala.
 *
 * Refresh recovery is intentionally minimal in Phase 1 — if the provider
...
type Role = 'solo' | 'team_lead' | 'agency_owner';
...
export function OnboardingProvider({ defaultName }: Props) {
  ...
  // /agency. Mirrors the agency-with-workspace branch in onboarding-flow.tsx.
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'agency_owner') {
        ...
        const agencyData = (await agencyRes.json().catch(() => ({}))) as {
          agency?: { id?: string };
        };
        ...
        toast.success("Agency created. Koala is ready.");
        router.push('/agency');
        return;
      }
    ...
    <div className="space-y-8">
      ...
      <Section label="What are you running?">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <PickerButton selected={props.role === 'agency_owner'} onClick={() => props.onChangeRole('agency_owner')}>
            <span className="font-medium">I own the agency</span>
          </PickerButton>
        </div>
      </Section>
...
function StageVoice({
  ...
  // Same scenario for both — only tone differs. Provider picks the one that
  // sounds like them; Koala pins that as the default tone going forward.
