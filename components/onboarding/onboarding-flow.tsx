...
// ── Agency cinematics copy ───────────────────────────────────────────────
// The agency flow gets its OWN bookend preloaders (separate from the
// provider cold open), built on the same OnboardingIntro / OnboardingReady
// engine. Module-level so their array identity is stable across renders.
const AGENCY_INTRO_LINE = 'Introducing the future of agency software.';
const AGENCY_READY_WORDS = ['Building.', 'Wiring.', 'Done.'] as const;
...
const AGENCY_TYPE: TileOption<string>[] = [
...
interface FormValues {
  name: string;
  role: Role | null;
  // Provider / agency-with-workspace
  businessName: string;
  slug: string;
 ...
 * friction steps into a single pill-pair. Pain-point survey was removed; Koala
...
type StepId =
  ...
  | 'hear'
  | 'agency-name'
  | 'agency-contact'  // grouped: office address + phone
  | 'agency-logo'
  | 'agency-agent-count'
  | 'agency-type'
  | 'agency-market'
...
function stepsFor(role: Role | null): StepId[] {
  ...
  if (role === 'agency' || role === 'agency_only') {
    base.push(
      'agency-name',
      'agency-contact',
      'agency-logo',
...
interface OnboardingFlowProps {
   ...
   * business-logo step - a provider's profile photo should not double as their
...
export function OnboardingFlow({ defaultName, userImageUrl: _userImageUrl }: OnboardingFlowProps) {
  ...
  async function finalize() {
    ...
    try {
      if (role === 'agency_only') {
        // Agency-only: skip workspace creation.
        ...
        const profileRes = await fetch('/api/onboarding', {
          ...
          body: JSON.stringify({
            ...
            phone: '',
            businessName: values.agencyName.trim(),
          }),
        });
        if (!profileRes.ok) throw await errorFrom(profileRes);

        const agencyRes = await fetch('/api/agency/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(agencyCreateBody(values)),
        });
        if (!agencyRes.ok) throw await errorFrom(agencyRes);

        const completeRes = await fetch('/api/onboarding', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'complete', accountType: 'agency_only' }),
        });
        if (!completeRes.ok) throw await errorFrom(completeRes);

        redirectRef.current = '/agency';
        setPhase('ready');
        return;
      }

      // Provider or agency-with-workspace. Personal phone is collected on the
      ...
      const profileRes = await fetch('/api/onboarding', {
        ...
        body: JSON.stringify({
          action: 'save_profile',
          name: values.name.trim(),
          phone: values.providerPhone.trim(),
      ...
      // null: the survey step was removed; Koala infers usage from behavior.
      const spaceRes = await fetch('/api/onboarding', {
        ...
        body: JSON.stringify({
          ...
          phone: values.providerPhone.trim() || undefined,
      ...
      if (role === 'agency') {
        ...
        if (!agencyRes.ok) throw new Error(agencyData?.error || 'Failed to create agency.');

        const newAgencyId = agencyData.agency?.id;
        if (newAgencyId && spaceData?.slug) {
          await fetch('/api/spaces', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ slug: spaceData.slug, agencyId: newAgencyId }),
          }).catch(() => undefined);
        }
      }

      const accountType = role === 'agency' ? 'both' : 'provider';
      const completeRes = await fetch('/api/onboarding', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      redirectRef.current = role === 'agency'
        ? '/agency'
  ...
  // Cold open — the agency cinematic preloader, before the form.
  if (phase === 'intro') {
    return <OnboardingIntro line={AGENCY_INTRO_LINE} onDone={() => setPhase('flow')} />;
  }
  ...
  return (
    ...
    >
          ...
          fields={[
            {
              ...
              onChange: (v) => set('providerPhone', v),
              maxLength: 40,
            },
            {
              key: 'bio',
      ...
      {stepId === 'agency-logo' && (
        <PhotoStep
          ...
          value={values.agencyLogoUrl || null}
          onChange={(url) => set('agencyLogoUrl', url ?? '')}
          onNext={goNext}
          onSkip={goNext}
          uploadKind="agency_logo"
        />
      )}
      ...
      {stepId === 'agency-type' && (
        <TilesStep
          ...
          options={AGENCY_TYPE}
          value={values.agencyType || null}
          onSelect={(v) => set('agencyType', v)}
          onNext={goNext}
          columns={3}
          advanceOnSelect
        />
      ...
      {stepId === 'hear' && (
        <TilesStep
          title="How did you hear about Koala?"
          options={HEAR_ABOUT}
    ...
    {phase === 'ready' && (
      <OnboardingReady
        words={AGENCY_READY_WORDS}
        finalLine={values.role === 'provider' ? 'Your account is ready.' : 'Your agency is ready.'}
...
function agencyCreateBody(v: FormValues) {
  return {
    name: v.agencyName.trim(),
    logoUrl: v.agencyLogoUrl.trim() || undefined,
    websiteUrl: v.agencyWebsiteUrl.trim() || undefined,
    officeAddress: v.officeAddress.trim() || undefined,
    officePhone: v.officePhone.trim() || undefined,
    agentCount: v.agentCount || undefined,
    agencyType: v.agencyType || undefined,
