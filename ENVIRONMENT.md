...
Configuration and external service reference for Koala. Based on actual repository code.
...
| `RESEND_API_KEY` | `lib/email.ts`, `lib/appointment-emails.ts` | Resend API key for sending all transactional emails (leads, appointments, invitations) | **Medium** | Email notifications silently skipped; leads still saved normally |
| `RESEND_FROM_EMAIL` | `lib/email.ts`, `lib/appointment-emails.ts` | Sender address for notification emails (must be verified in Resend) | **Medium** | Falls back to `notifications@updates.yourdomain.com`; must be set to a verified domain |
...
| `STRIPE_PRICE_STARTER` | `app/api/billing/checkout/route.ts` | Stripe price ID for the Starter agency plan | **Medium** | Starter plan checkout cannot be created |
| `STRIPE_PRICE_TEAM` | `app/api/billing/checkout/route.ts` | Stripe price ID for the Team agency plan | **Medium** | Team plan checkout cannot be created |
| `STRIPE_PRICE_ENTERPRISE` | `app/api/billing/checkout/route.ts` | Stripe price ID for the Enterprise agency plan | **Medium** | Enterprise plan checkout cannot be created |
| `COMPOSIO_API_KEY` | `lib/integrations/composio.ts`, `agent/integrations.py` | Composio API key for loading the provider's connected toolkits (Gmail, Slack, HubSpot, etc.) into the chat agent AND autonomous runs | **High (when integrations used)** | Connections show "Connected" in /settings but Koala can't see them as tools. **Must be set in BOTH Vercel env AND the Modal `koala-secrets` secret** so chat (Modal) and the Next.js callback both work. |
| `COMPOSIO_WEBHOOK_SECRET` | `app/api/webhooks/composio/route.ts` | Signing secret for inbound Composio trigger webhooks (HMAC-SHA256). Grab it from Composio dashboard → Project Settings → Webhook. Required for the receiver to authenticate trigger deliveries — without it, the receiver returns 500 and Koala never notices anything happening in the provider's connected apps. | **High (when triggers used)** | Inbound trigger deliveries rejected; Koala cannot react to email replies, calendar accepts, deal stage changes, etc. Outbound tool execution still works. |
...
| Service | Role in Koala | Key integration files |
|---|---|---|
...
| **Resend** | Transactional email — sends lead notifications, appointment confirmations/reminders/follow-ups, agency invitations, follow-up digests, and CRM emails | `lib/email.ts`, `lib/appointment-emails.ts`, `app/api/public/apply/route.ts` |
| **Telnyx** | SMS notifications — sends text messages to workspace owners for new leads, appointment bookings, and deals (opt-in per workspace via settings) | `lib/sms.ts`, `lib/notify.ts` |
...
| **Stripe** | Agency seat-based billing checkout/portal/webhook | `lib/stripe.ts`, `app/api/billing/checkout/route.ts`, `app/api/billing/portal/route.ts`, `app/api/billing/cancel/route.ts` |
...
| `STRIPE_PRICE_STARTER` / `STRIPE_PRICE_TEAM` / `STRIPE_PRICE_ENTERPRISE` | Required to initiate agency checkout sessions at each tier. Billing flow fails without the tier's price ID. |
...
| Resend | Yes | `resend@^4.8.0` | All transactional emails (leads, appointments, invitations, digests), fully integrated |
| Telnyx | Yes | `telnyx@^6.26.0` | SMS notifications for leads, appointments, and deals, plus the `send_sms` AI tool |
| Stripe | Yes | `stripe@^20.4.1` | Agency seat-based billing (checkout, portal, cancel). Requires `STRIPE_PRICE_STARTER` / `STRIPE_PRICE_TEAM` / `STRIPE_PRICE_ENTERPRISE` <!-- TODO: verify full list of Stripe secrets (webhook secret, API key) — not inspected in this pass --> |
...
| `phoneNumber` | Provider's phone number |
