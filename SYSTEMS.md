...
- **Fragile seams:** (1) A agency route must verify the caller belongs to the **specific** agency that owns the resource, not just "is a agency." (2) `getAgencyContext` multi-membership tie-break is order-dependent. (3) Seat-cap + pending-invite count has no DB-level serialization (double-spend race); invitation accept re-activates offboarded users by design.
