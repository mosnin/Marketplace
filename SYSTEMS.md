...
| Appointment booking, double-booking, availability, calendar sync | **4. Appointments & Calendar** |
| Koala chat in-app: replies, tool calls, approval prompts, streaming | **5. Koala Chat Runtime (TS)** |
| Koala acting on its own, didn't fire on an event, cron, swarm, runaway spend | **6. Autonomous Agent** |
| Agency dashboard, team roster, lead routing, commissions, reviews, invites, seats | **7. Agency Tier** |
| Applicant/client portal login, messaging a provider, client documents | **8. Client Portal** |
...
- **Entry:** `middleware.ts` (Clerk gate, ban/offboarding fast-path, admin pre-check); `lib/api-auth.ts` (`requireAuth`, `requireSpaceOwner`, `requireContactAccess` — the canonical API choke-point); `lib/permissions.ts` (`requireAgency`/`requirePlatformAdmin`/`getAgencyContext`); `app/auth/redirect/page.tsx` (post-login routing).
- **Owns:** `User` (`clerkId`, `status`, `platformRole`), `AgencyMembership`, `Space.ownerId`.
...
- **Entry:** `app/api/public/apply/route.ts`, `app/api/public/apply/agency/route.ts`, `app/api/public/intake-chat/route.ts`, `app/apply/[slug]/**`; scoring: `lib/lead-scoring.ts` → `lib/scoring/engine.ts` (deterministic) + `lib/scoring/enhance.ts` (optional LLM summary, `gpt-4.1-mini`).
- **Owns:** `Contact`, `ApplicationStatusUpdate`, `FormDraft`; reads `Space`, `SpaceSetting`, `Agency`.
...
- **Fragile seams:** (1) Parent rows enforce the space boundary, but several **child-table queries** (`DealActivity`, `Appointment` in timeline, rescore updates) rely on FK correctness instead of their own `.eq('spaceId')`. (2) Reorder is atomic via the `reorder_deal` RPC; the checklist-shift loop is not. (3) Pipeline bootstrap is read-then-multi-write with no transaction (concurrent first-load can duplicate).

## 4. Appointments & Calendar

Public appointment booking, guest self-management by token, agent appointment CRUD, Google Calendar sync.

- **Entry:** `app/api/appointments/book/route.ts`, `app/api/appointments/available/route.ts`, `app/api/appointments/[id]/route.ts`, `app/api/appointments/gcal/route.ts`, `app/api/calendar/events/route.ts`, `app/appointment/[token]/page.tsx`, `app/book/[slug]/**`; `lib/gcal-helpers.ts`, `lib/appointment-emails.ts`.
- **Owns:** `Appointment`, `AppointmentFeedback`, `AppointmentWaitlist`, `AppointmentAvailabilityOverride`, `AppointmentServiceProfile`, `GoogleCalendarToken`, `CalendarEventMirror`.
- **External:** Google Calendar OAuth2 + freeBusy, Composio (agent calendar path), Resend/SMS.
- **Fragile seams:** (1) The `book_appointment_atomic` RPC is the *sole* double-booking guard — any path bypassing it reintroduces the race. (2) `GoogleCalendarToken.accessToken` encryption is inconsistent across three refresh paths (one reads/writes plaintext). (3) Availability conflict can scope to one service, missing same-provider cross-service overlaps.

## 5. Koala Chat Runtime (TS, on-demand)

The default in-app chat backend: `@openai/agents` loop, 55 domain tools, SSE streaming, approval-gated mutations. `KOALA_CHAT_RUNTIME=ts` (default); `=modal` proxies to the sandbox.
...
Scheduled + event-driven Koala runs per workspace, plus multi-agent swarms, in a Python/Modal sandbox.
...
Multi-agent org oversight: membership, lead routing, commissions, deal reviews, templates, invitations, seats. Agency↔provider isolation is the recurring bug class (PRs #294–#296).

- **Entry:** `lib/permissions.ts` (`requireAgency`/`getAgencyContext`/`getAgencyMemberContext`), `lib/agency-routing.ts`, `lib/agency-seats.ts`, `app/api/agency/**`, `app/api/invitations/[token]/route.ts`.
- **Owns:** `Agency`, `AgencyMembership`, `Invitation`, `AgencyRemoval`, `CommissionLedger`, `DealRoutingRule`, `AgencyTemplate`, `DealReviewRequest/Comment`, `AgencyNotification`.
- **Fragile seams:** (1) A agency route must verify the caller belongs to the **specific** agency that owns the resource, not just "is an agency." (2) `getAgencyContext` multi-membership tie-break is order-dependent. (3) Seat-cap + pending-invite count has no DB-level serialization (double-spend race); invitation accept re-activates offboarded users by design.
...
AI content generation, file/document storage, service packets, e-sign, third-party toolkits.
...
- **Owns:** `File`, `StudioGeneration/Brand/Post`, `ContactDocument`, `DealDocument`, `ServicePacket`, `SignatureRequest`, `IntegrationConnection`.
...
- **Owns:** `Space.stripe*`/`trialUsedAt`, `Agency.stripe*`/`plan`/`seatLimit`, `AuditLog`, `DeadLetterEvent`, `PushSubscription`, `CallLog`.
...
1. **Isolation-by-title-prefix is not isolation.** Agency-Koala / team-chat conversations once lived in the provider `Conversation`/`Message` tables distinguished only by a `[AGENCY_KOALA]`/`[AGENCY_CHAT]` title prefix. Because an agency_owner also owns their personal provider space, *space ownership alone is not a boundary*. Any provider read of `Conversation`/`Message` MUST route through `lib/chat/conversation-access` (`isProviderConversation` / reserved-prefix exclusion) — list, single-conversation, messages, **and the write path** (`resolveConversation`). Agency chat now has its own `AgencyConversation`/`AgencyMessage` tables (keyed by `agencyId`); prefer structural separation over string matches.
2. **`Space.agencyId` is NOT "which agency the user belongs to."** Membership lives in `AgencyMembership` (a user can be in several). `Space.agencyId` is only the intake-config owner and must never be overwritten on a second join. Don't read it as a membership signal.
3. **Parallel provider↔agency routes drift.** `task`/`agency-task`, `conversations`/`agency-conversations`, `messages`/`agency-messages` are near-duplicates; a fix applied to one is easily missed on the other (the isolation guard gap lived in exactly this seam). When you touch one, check its twin.
...
5. **Service-role bypasses RLS.** All server queries use the service-role key, so RLS is *not* the tenant boundary — app-layer `.eq('spaceId'/'agencyId')` scoping is. Add the scope filter; don't rely on a policy.
