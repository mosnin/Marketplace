...
This document defines clear boundaries between each major workflow in Koala. AI agents and contributors must respect these boundaries when making changes.
...
| Attribute | Detail |
|---|---|
| **Purpose** | Triage and follow-up operations for the authenticated provider |
...
|---|---|---|---|
| Onboarding completion | User/workspace activation | `User.onboardingCompletedAt` | The provider has set up their workspace and is ready to use the CRM |
...
These two events:
- Happen to different actors (provider vs prospect)
