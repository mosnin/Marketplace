 ...
 * across ALL spaces: applications (Contact rows) and appointments (Appointment rows). This is
 * the "one page by email" surface. Read-only against provider data; the client's
...
/**
 * Pull the client's applications + appointments by email. `email` MUST be the verified
...
export async function getClientPortalData(email: string): Promise<ClientPortalData> {
  ...
  const [{ data: contacts }, { data: appointments }] = await Promise.all([
    supabase
      .from('Contact')
      .select(
    ...
    supabase
      .from('Appointment')
      .select('id, serviceAddress, startsAt, status, spaceId, contactId, guestEmail, Space(name, slug)')
  ...
  const portalAppointments: PortalAppointment[] = (appointments ?? []).map((t) => {
    const space = t.Space as SpaceRel;
    return {
      id: t.id as string,
      serviceAddress: (t.serviceAddress as string | null) ?? null,
  ...
  const contactIds = Array.from(
    new Set([
      ...
      ...portalAppointments.map((t) => t.contactId).filter((id): id is string => Boolean(id)),
    ]),
  );

  return { applications, appointments: portalAppointments, contactIds };
}
