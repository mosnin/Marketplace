 ...
 * specific gap in THIS provider's data named in one Koala-voiced
 ...
 * `spaceHasStabilityHistory` so a brand-new provider can't fire
 ...
 *   appointment_conversion_drop  Share of completed appointments converting to
 *                         application/offer dropped 20+pt vs. the
 *                         22-42-day baseline, ≥4 recent appointments.
...
// ── 5. Unworked tag — a segment the provider hasn't touched ───────────────────
...
export async function tipUnworkedTag(spaceId: string): Promise<Signal[]> {
  ...
  // Pull all non-agency contacts and bucket by tag.
  const { data, error } = await supabase
    .from('Contact')
  ...
  for (const row of data as Row[]) {
    ...
    for (const tag of tags) {
      // Skip system tags — they're not segments the provider curated.
  ...
  return [
    {
      ...
      urgency: 2,
      confidence,
      // Trend tips have no named subject — the subject is the provider's
...
/** 0.78 at 4 appointments, 0.88 at 6+. */
export function appointmentConversionConfidence(recentAppointments: number): number {
  return recentAppointments >= 6 ? 0.88 : 0.78;
}
...
// ── 9. Appointment conversion drop — fewer post-appointment applications/offers ───────────

export async function tipAppointmentConversionDrop(spaceId: string): Promise<Signal[]> {
  if (!(await spaceHasStabilityHistory(spaceId))) return [];
  ...
  type AppointmentRow = { id: string; contactId: string | null; endsAt: string };
  const fetchAppointments = async (from: Date, to: Date) => {
    ...
    return ((data ?? []) as AppointmentRow[]).filter((t) => t.contactId);
  };
  const recent = await fetchAppointments(recentStart, today);
  const baseline = await fetchAppointments(baselineStart, baselineEnd);
  ...
  // OR a Deal stage move within 7 days. Both because some providers stage
  ...
  const converted = (appointment: AppointmentRow): boolean => {
    ...
    const within7 = (whenIso: string | null): boolean => {
      ...
      return t > appointmentEnd && t - appointmentEnd <= 7 * MS_PER_DAY;
    };
    return (
      draftRows.some((d) => d.contactId === appointment.contactId && within7(d.createdAt)) ||
      dealRows.some((d) => d.contactId === appointment.contactId && within7(d.stageChangedAt))
    );
  };
  ...
  const confidence = appointmentConversionConfidence(recent.length);
  ...
  return [
    {
      ...
      confidence,
      subject: {
        id: 'appointment_conversion_drop',
        name: 'Appointment conversion dropped',
        href: '/deals?status=appointment_completed',
      },
      evidence: `${recent.length} appointments last three weeks. ${recentMovedWord} moved to application. Last quarter the rate was ${baselineFraction}.`,
      tipCategory: 'appointment_conversion_drop',
    },
  ];
}
