 ...
 *
 * Two signals only. Slack is thin for providers — it's the agency
...
/**
 * Is the message addressed to the provider? True only when the provider's
...
export function mentionsProvider(
  ...
  providerSlackId: string | null,
): boolean {
  if (!providerSlackId) return false;
  return extractMentions(text).includes(providerSlackId);
}

/**
 * Does the DM sender belong to the provider's known team? In Slack, every
 * user inside the provider's workspace shows up in `users.list` — that's
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    ...
    for (const ch of channels) {
      ...
      for (const msg of messages) {
        ...
        if (providerSlackId && msg.user === providerSlackId) continue;
        ...
        } else if (mentionsProvider(msg.text, providerSlackId)) {
          const sender = userById.get(msg.user);
          const name = teammateDisplayName(sender);
