 ...
 * provider "what Koala did overnight, ready for your approve" — the
 * single highest-value card type for a working provider whose agent
 * actually ran.
 *
...
function evidenceFor(channel: DraftRow['channel']): string {
  switch (channel) {
    case 'sms':
      return 'Koala drafted a text. Approve or edit.';
    case 'email':
      return 'Koala drafted an email. Approve or edit.';
    case 'note':
      return 'Koala flagged a note for your review.';
  }
}
...
export const draftsSource: SignalGatherer = {
  ...
  // shows the provider "Koala did this overnight, ready for your approve."
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    ...
    for (const draft of data as unknown as DraftRow[]) {
      ...
      // working state for the agent, not actionable for the provider's
      ...
      signals.push({
        ...
        // the FocusCard's job, where the provider has the editing UI ready.
        draftedAction: {
          kind: 'open',
