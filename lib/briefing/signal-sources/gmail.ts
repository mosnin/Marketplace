 ...
 *     are the "Koala did this overnight, approve or edit" cards.
 *   - Gmail (this source) covers the gaps the trigger feed never sees:
 *       1. Sent threads gone QUIET — the provider wrote, the lead didn't
 *          come back. Trigger fires on inbound only.
 *       2. Inbound that landed but the agent DECIDED not to draft for
 *          (low-signal judgment). The provider still wants to see them.
 ...
 * Unknown senders are dropped — this source is about people the provider
...
const QUIET_MIN_PROVIDER_CHARS = 100; // a "hi" doesn't deserve a follow-up nag
...
type GmailMessage = {
  ...
  to?: string | string[]; // the provider's address for inbound; recipient for sent
  subject?: string;
 ...
 *   - Any tier: 4+ days of silence + last provider msg >100 chars → urgency 2, 0.85
 *
...
export function classifyQuietThread(args: {
  leadScore: number | null;
  daysSinceLastProviderMessage: number;
  lastProviderMessageChars: number;
}): { urgency: 1 | 2; confidence: number } | null {
  const isHot = (args.leadScore ?? 0) >= HOT_LEAD_THRESHOLD;
  if (isHot && args.daysSinceLastProviderMessage >= QUIET_HOT_DAYS) {
    return { urgency: 1, confidence: 0.9 };
  }
  if (
    args.daysSinceLastProviderMessage >= QUIET_ANY_DAYS &&
    args.lastProviderMessageChars > QUIET_MIN_PROVIDER_CHARS
  ) {
...
export function evidenceForQuiet(args: {
  ...
  const dayPhrase =
    args.daysSinceLastProviderMessage === 1
      ? 'yesterday'
      : `${args.daysSinceLastProviderMessage} days ago`;
...
export function evidenceForSkippedInbound(args: { contactName: string }): string {
  return `${args.contactName} emailed recently. Koala didn't draft — worth a look.`;
}
...
/** Look up the provider's contacts so we can cross-walk participant emails. */
...
function isSentByProvider(m: GmailMessage): boolean {
...
export const gmailSource: SignalGatherer = {
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    ...
    for (const thread of buckets.values()) {
      ...
      // The "other side" of the thread — the participant who isn't the provider.
      ...
      // ── Signals 1 + 2: provider sent the most recent message, lead quiet.
      if (isSentByProvider(last)) {
        ...
        const classification = classifyQuietThread({
          ...
          daysSinceLastProviderMessage: daysQuiet,
          lastProviderMessageChars: bodyChars(last),
        });
