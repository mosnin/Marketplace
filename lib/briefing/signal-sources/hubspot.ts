 ...
 * only names people the provider recognises. The new-contact tip is the
 ...
 *                      The provider's pipeline is out of sync; surface it
 ...
 *                      somewhere else; the provider should pull them in.
 *
 * Data source: hybrid.
 *
 ...
 * (correct behaviour — we won't name a deal the provider won't recognise).
 */
 ...
 * signal only fires when Koala is BEHIND HubSpot — if the provider has
 ...
 * we normalise to a flat HubspotDeal[] tolerant of nested `services`.
 */
  ...
  const result = await withTimeout(
    () =>
      executeToolForEntity({
        ...
        arguments: {
          services: ['dealname', 'dealstage', 'closedate', 'hs_lastmodifieddate'],
          limit: 100,
          ...
          filters: [{ serviceName: 'closedate', operator: 'LTE', value: sevenDaysOut }],
        },
      }),
    COMPOSIO_TIMEOUT_MS,
  );
  ...
  const result = await withTimeout(
    () =>
      executeToolForEntity({
        ...
        arguments: {
          services: ['email', 'firstname', 'lastname', 'createdate'],
          limit: 100,
        },
      }),
    COMPOSIO_TIMEOUT_MS,
...
function normalizeHubspotDeals(data: unknown): HubspotDeal[] {
  ...
  return results
    .map((r) => {
      const props = (r.services ?? r) as Record<string, unknown>;
      return {
...
export const hubspotSource: SignalGatherer = {
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    // 1. Skip entirely without an active HubSpot connection. The provider
    ...
    for (const hsDeal of hubspotDeals) {
      ...
      // but the provider knows their own pipeline. Surface the mismatch
