/**
 * Google Calendar signal source — reads what the provider scheduled
 * OUTSIDE Koala (listing presentations, lender meetings, coffee with
 * referral sources). The internal `calendar` source covers Koala-native
 * Appointment rows; this one covers everything else.
 *
 * The brief's rule: only name people the provider recognizes. We drop
 ...
 *
 * `self`-flagged attendees (the provider's own account) are skipped: the
 * provider isn't the subject of their own brief card.
 */
...
/** Did the provider's attendee-response trigger fire within the last 24h? */
...
export const calendarGoogleSource: SignalGatherer = {
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    ...
    // 2. Pull events (Composio) and the provider's contacts (Supabase) in
    ...
    for (const event of events) {
      ...
      // anything later today the provider will see in their calendar.
    }
