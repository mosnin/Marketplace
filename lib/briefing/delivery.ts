 ...
 * the per-provider 7am local IS the delivery moment). Per channel:
 *
...
async function deliverEmail(ctx: DeliverContext, isEmpty: boolean): Promise<DeliveryResult['email']> {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: `Koala <brief@${getBriefDomain()}>`,
...
function getBriefDomain(): string {
  ...
  return process.env.BRIEF_EMAIL_DOMAIN ?? 'alerts.usekoala.com';
}
...
async function deliverSms(ctx: DeliverContext, isEmpty: boolean): Promise<DeliveryResult['sms']> {
  ...
  try {
    // Check if this is the provider's FIRST-EVER brief SMS. If so, send
