 ...
 *
 * Seat limits are driven off the Agency.plan / Agency.seatLimit columns.
...
export type AgencyPlan = 'team' | 'team_plus';
...
const DEFAULT_PLAN: AgencyPlan = 'team';
const DEFAULT_SEAT_LIMIT = PLANS.team.includedUsers;

function isValidPlan(value: unknown): value is AgencyPlan {
...
async function loadPlan(
  agencyId: string
): Promise<{ plan: AgencyPlan; seatLimit: number | null }> {
  try {
    ...
    const plan: AgencyPlan = isValidPlan(row.plan) ? row.plan : DEFAULT_PLAN;
    ...
    if (row.seatLimit === null || row.seatLimit === undefined) {
      ...
      // agency tier, so we never fall back to null here.
 ...
 */
export async function getSeatUsage(agencyId: string): Promise<SeatUsage> {
  const [{ plan, seatLimit }, members, pendingInvites] = await Promise.all([
    loadPlan(agencyId),
    countMembers(agencyId),
    countPendingInvites(agencyId),
  ]);
 ...
 * (pre-migration) doesn't unlock the agency.
 */
