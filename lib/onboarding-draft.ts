 ...
 * real first-touch message - in the provider's chosen voice, naming
 * their business, tuned to their primary lead source - so the provider
 ...
 * template keyed off the provider's own inputs is instant, free, and
 ...
 * what makes the reveal identical every time the provider sees it.
 */

export type DraftTone = 'warm' | 'direct';

export interface OnboardingDraftInput {
  /** Provider's full name; we use the first token. */
  name: string;
...
/** Pick the provider's first name, or a friendly fallback. */
