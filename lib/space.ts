...
export async function getSpaceFromSlug(inputSlug: string): Promise<Space | null> {
  ...
  const { data, error } = await supabase
    .from('Space')
    .select('id, slug, name, emoji, ownerId, agencyId, createdAt, stripeSubscriptionStatus')
    .eq('slug', slug)
    .limit(1)
    .maybeSingle();
...
export async function getSpaceByOwnerId(ownerId: string): Promise<Space | null> {
  ...
  // space.agencyId is the intake-config owner, NOT a membership signal:
  // membership lives in AgencyMembership. Don't read agencyId as "which
  // agency this user belongs to."
  const { data, error } = await supabase
    .from('Space')
    .select('*')
    .eq('ownerId', ownerId)
...
export async function getSpaceForUser(clerkUserId: string): Promise<Space | null> {
  ...
  // came back undefined → coerced to 'inactive' → every paying provider was
  // blocked from any route that combined getSpaceForUser + requireActiveSubscription
  // (Studio generate/edit are the live callers). Active+trialing providers saw
