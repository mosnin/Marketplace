/**
 * Bind a provider's connected Composio toolkits as callable tools on the
 * Koala chat agent.
 *
 * The story this closes — Musk lens: the provider can already OAuth into
 ...
 * to a client with zero provider confirmation. Unacceptable. So we fetch
 ...
 *                   the post-appointment executor uses.
 ...
 * or `messaging` toolkit reaching out on the provider's behalf is always
 * something they should see before it happens.
 */
const ALWAYS_APPROVE_CATEGORIES = new Set(['social', 'messaging']);

/**
 * Decide whether a Composio action needs provider approval before it
 * fires. Conservative by design: when in doubt, gate it. A false
 * positive costs one tap; a false negative posts to the provider's
 ...
 * the charset; we lowercase them so they read like the rest of Koala's
 ...
 * provider actually asks for). When a toolkit exceeds the per-toolkit cap,
 ...
 * happens at wrap time), so one fetch serves every provider. Without this,
...
function buildOneTool(raw: ComposioRawTool, toolkitSlug: string, userId: string): SdkTool {
  ...
  const fullDescription =
    ...
    `${raw.name ?? actionSlug} — ${toolkitSlug} action available via the provider's connected account.`;
  const description =
...
/**
 * Build the full Composio tool list for a provider's chat turn.
 *
...
export async function buildComposioAgentTools(
  ...
  for (const toolkit of toolkits) {
    try {
      ...
      // single sick toolkit doesn't deny the provider every other tool.
