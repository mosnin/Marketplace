 ...
 *
 * The provider pastes their API key once (Follow Up Boss → Admin → API). We
...
export interface FubVerifyResult {
  ...
  label: string | null;
  /** Set when ok=false — a provider-friendly reason. */
  reason?: string;
}
...
/**
 * Pull the provider's People list, most-recently-updated first, mapped to the
