 ...
 * a provider-friendly message, NOT show a stack trace).
 */
 ...
 *
 * `entityId` should be the provider's Clerk userId — Composio scopes
 * connections per "entity" so a future agency dashboard can list a
 * provider's connected accounts cleanly.
 *
 ...
 * the provider's Auth Configs were correctly set up in the Composio dashboard.
 * Fix: look up the provider's Auth Config for the toolkit, then pass its
...
export async function initiateConnection(args: {
  ...
  try {
    ...
    // Take the first (or first composio-managed) auth config. Most providers
...
/**
 * List the provider's connected accounts on Composio's side. Used by the
 * /settings reconcile pass to discover connections that Composio knows
 * about but our DB doesn't (e.g. providers who completed OAuth before
...
export async function loadToolsForEntity(args: {
  ...
  // actions even after the provider enabled 100+ slugs on the dashboard.
 ...
 * post-appointment execute path, which fires approved proposals imperatively
...
// ─── Triggers ────────────────────────────────────────────────────────────────
//
// Triggers are how Composio tells US when something happens in the provider's
 ...
 *
 * `entityId` is the provider's Clerk userId (Composio "user"). `slug` is the
 * trigger slug (e.g. 'GMAIL_NEW_GMAIL_MESSAGE'). `connectedAccountId` ties the
 * subscription to a specific connection — required when the provider has more
