 ...
 * the provider's connected app" so Koala can react without being asked.
 *
 * This module owns four things:
 *
 *   1. CURATED_TRIGGERS — the slugs we auto-register per toolkit. Hard-
 *      coded, not provider-configurable. The provider's choice surface is
 ...
 *                    so the agent drafts a response for the provider to
 ...
 * the provider would never see Koala "noticing" what the catalog implies.
 *
 ...
 * map to actionable provider moments. See the per-entry comments for the
 * thinking.
 */
export const CURATED_TRIGGERS: Record<string, string[]> = {
  // ── Comms: the provider's inbox surfaces ───────────────────────────
  ...
  // ── Calendar: appointments, accepts, cancellations ───────────────────────
  googlecalendar: [
  ...
  youtube: [
    // Subscription / new-activity triggers exist but a provider's YouTube
    // is a content channel, not a real-time signal source. Skip until
    // proven providers want it.
  ],
  google_ads: [], // no trigger surface; the catalog ships read tools only
  notion: [], // page/database triggers exist, but Notion isn't load-bearing in the provider workflow
  googledocs: [], // doc-content triggers ship but rarely actionable; skip until asked
  googlesheets: [], // metadata-changed triggers are noisy; the provider wants outcomes, not cell edits
  googledrive: [], // file-created could be useful (new disclosure?) but the signal is too generic
  onedrive: [], // same shape as googledrive
  dropbox: [], // same shape
  zoho: [], // CRM mirror; left empty until a provider asks
 ...
 * a response, ask the provider a question, or do nothing. The kind enum
 ...
 * `data`, `message`, or `services`. We try both flat and nested so
 ...
 * `services` — and the SDK's normalised IncomingTriggerPayload itself
 ...
 * rulebook to follow. The model knows the provider's CRM and history;
...
const TEMPLATES: Record<string, (p: Record<string, unknown>) => string | null> = {
  ...
  HUBSPOT_CONTACT_CREATED_TRIGGER: (p) => {
    const props = (pickNested(p, ['services']) as Record<string, unknown>) ?? p;
 ...
 * status='failed' and does NOT block the rest — a provider with three
 ...
 * completion. The provider sees the connection succeed; missing triggers
 ...
 * is torn down, so a disconnected provider doesn't keep paying for
 ...
 * and 'paused'. Provider-facing — the integrations panel offers ONE
 * toggle per connected app, not one per trigger. Per-trigger granularity
 * is a settings rabbit hole the provider does not need.
 *
 ...
 * "Koala is watching" / "Paused" affordance. Returns false when the
 ...
 *   - "paused"  → triggers registered but provider paused them
 *   - "failed"  → registration attempted but Composio rejected; provider
 *                 sees Koala promised to watch and silently doesn't
 *                 unless we expose this state.
 *
 * Precedence: active > paused > failed > off. If a connection has both
 * a working trigger and a broken one, we show active (the provider's
...
export async function dispatchTrigger(args: {
  ...
  if (kind === 'DRAFT') {
    ...
    // reads from that column to render the "Koala noticed because
