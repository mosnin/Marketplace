 ...
 *
 * The last bit deserves a note: by convention we *don't* require the provider
 * to enter their own "me" row. If there are no rows, `netToMe === gci`. If
 * the provider adds an explicit "me" row with a percentage, that's treated
 * as their slice and everything not assigned is considered overhead /
 * agency. This mirrors how most providers actually model it: they enter
...
export const COMMISSION_PARTIES: { value: CommissionParty; label: string; description: string }[] = [
  ...
  { value: 'agency',    label: 'Agency',     description: 'Your office / agency split.' },
