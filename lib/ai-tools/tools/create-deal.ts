 ...
 *   - no milestones / commissionRate / probability (the provider can set
...
const parameters = z
  .object({
    ...
    stageId: z
      ...
      .optional()
      .describe(
        'Target DealStage.id. Optional — if omitted or null the deal lands in the first stage of the appropriate pipeline (buyer pipeline when a buyer contact is attached, otherwise the seller pipeline). Only set this when the provider explicitly named the stage.',
      ),
    description: z.string().max(5000).optional(),
    value: z.number().nonnegative().nullable().optional().describe('Deal value in dollars.'),
    address: z.string().max(500).optional().describe('Service address, if relevant.'),
...
export const createDealTool = defineTool<typeof parameters, CreateDealResult>({
  ...
  async handler(args, ctx) {
    ...
    // pipeline so the provider doesn't have to know stage ids.
    ...
    // POST /api/deals. A provider who says "create a deal for Jane (a buyer)"
