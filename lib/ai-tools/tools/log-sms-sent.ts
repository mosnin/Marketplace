/**
 * `log_sms_sent` — record an SMS the provider sent OUTSIDE Koala.
 *
...
const parameters = z
  ...
  .describe('Log an SMS sent outside Koala to a contact. Audit trail only.');
...
export const logSmsSentTool = defineTool<typeof parameters, LogSmsResult>({
  ...
  riskLevel: 'low',
  description:
    'Record an SMS the provider sent OUTSIDE Koala against a contact\'s timeline. Does NOT send anything.',
  parameters,
