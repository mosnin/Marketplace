/**
 * `find_comparable_services` — search local Service rows in this workspace.
 *
 * Read-only. Honest about scope: this is NOT MLS. It searches Service rows
 * the provider has saved (their own listings, off-market notes, owned). If
...
const parameters = z
  .object({
    ...
    status: z
      ...
      .optional()
      .describe("Service.listingStatus filter."),
  })
  .describe('Find up to 6 saved services matching the criteria. Local CRM only — no MLS.');
...
export const findComparableServicesTool = defineTool<typeof parameters, FindCompsResult>({
  name: 'find_comparable_services',
  riskLevel: 'safe',
  description:
    'Search saved Service rows in this workspace by location, beds/baths, price range, status. Returns up to 6. Does NOT query MLS.',
  parameters,
  ...
  async handler(args, ctx) {
    ...
    if (error) {
      return { summary: `Service lookup failed: ${error.message}`, display: 'error' };
    }

    let rows = (data ?? []) as Array<ServiceMatch & { updatedAt: string }>;
    if (rows.length === 0) {
      return {
        summary: 'No comparable services on file.',
        data: { services: [], note: 'No comparable services on file.' },
        display: 'plain',
      };
    }
    ...
    const services: ServiceMatch[] = rows.map((r) => ({
      id: r.id,
      address: r.address,
      city: r.city,
      beds: r.beds,
    ...
    return {
      summary: `${services.length} comparable propert${services.length === 1 ? 'y' : 'ies'} on file.`,
      data: { services },
      display: 'plain',
    };
  },
