/**
 * `delegate_task` — the orchestration tool. Koala calls this on its own when
 ...
 * chat turn. It is Koala's version of Claude Code's Task tool.
 *
 * What it does:
 *   1. Creates a SwarmRun row (status 'queued') for the provider's space.
 ...
 * job the provider watches inline. The orchestrator stays in the chat with the
 * provider and continues to gate any DIRECT mutation it makes through the normal
 ...
 * Wiring the full Koala tool set + approvals into the Modal sub-agents is a
 ...
 * The pump parses + strips this before the provider ever sees the summary.
 * Format: `⟦koala:subagent runId=<id>⟧`
 */
const RUN_ID_MARKER = /⟦koala:subagent runId=([A-Za-z0-9-]+)⟧/;

export function encodeSubagentRunId(runId: string): string {
  return `⟦koala:subagent runId=${runId}⟧`;
}
...
/** Remove the machine marker so the provider-facing summary stays clean. */
...
export function buildDelegateTaskTool() {
  return defineTool<typeof parameters, DelegateTaskData>({
    ...
    description:
      ...
      'provider you have kicked it off; the live task card shows the rest.',
    parameters,
    riskLevel: 'safe',
