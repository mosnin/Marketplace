/**
 * `find_service` — read-only lookup over the Service table.
 *
...
const parameters = z
  ...
  .describe('Find a service by address, id, MLS number, or status.');
...
function toHit(row: Record<string, unknown>): ServiceHit {
  return {
    id: row.id as string,
...
function summariseOne(p: ServiceHit): string {
...
export const findServiceTool = defineTool<typeof parameters, FindServiceResult>({
  name: 'find_service',
  riskLevel: 'safe',
  description:
    "Find a service by address, MLS number, id, or status. Returns rich detail for a single match or a shortlist (≤8) when ambiguous.",
  parameters,
  ...
  async handler(args, ctx) {
    ...
    if (args.query) {
      ...
      if (byId) {
        ...
        return {
          ...
          data: { match: 'single' as const, service: hit },
          display: 'plain',
        };
      }
    }
    ...
    if (error) {
      return {
        summary: `Service lookup failed: ${error.message}`,
    ...
    const services = rows.map(toHit);
    const lines = services.map((p) => `• ${summariseOne(p)}`).join('\n');
    return {
      summary: `Found ${services.length} services:\n${lines}`,
      data: { match: 'shortlist' as const, services },
      display: 'plain',
    };
  },
});
