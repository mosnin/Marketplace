/**
 * `list_files` — list files the provider uploaded to the Files page.
 *
 ...
 * matching rows; the provider can refine with a more specific query when
...
const parameters = z
  ...
  .describe('List files in the provider\'s Files surface, optionally filtered.');
...
export const listFilesTool = defineTool<typeof parameters, ListFilesResult>({
  ...
  riskLevel: 'safe',
  description:
    'List the provider\'s uploaded files (images, documents, videos, audio). Use to find a file by name before passing it to send_email, attach_file_to_service, or read_file.',
  parameters,
