/**
 * `propose_appointment_times` — suggest 4-6 open 60-minute slots on the provider's
 * calendar for the next 7 days during business hours. The chat renders
 * these as a tappable picker; the provider picks one → the workspace sends
 * a "Schedule the appointment at <slot>" prompt → Koala fires `schedule_appointment`
...
const parameters = z
  .object({
    contactId: z
      ...
      .min(1)
      .optional()
      .describe('Saved Contact.id this appointment is for. Surface their name on the picker.'),
    serviceAddress: z
      .string()
      .min(1)
      .optional()
      .describe('Service address the appointment is for. Surface on the picker.'),
    durationMinutes: z
      .number()
      .int()
      .min(15)
      .max(240)
      .optional()
      .describe('Appointment length. Defaults to 60.'),
    daysAhead: z
      .number()
      .int()
      .min(1)
  ...
  .describe(
    'Propose 4-6 open slots in business hours over the next week. Renders as a tappable picker — the provider picks a slot to fire schedule_appointment.',
  );
...
interface ProposeAppointmentTimesResult {
  slots: ProposedSlot[];
  contactId?: string;
...
/** Business hours window in the provider's local time. Lean toward
 *  showings: skip pre-9am, skip after 6pm, skip weekends. */
const BUSINESS_START_HOUR = 9;
const BUSINESS_END_HOUR = 18;

export const proposeAppointmentTimesTool = defineTool<typeof parameters, ProposeAppointmentTimesResult>({
  name: 'propose_appointment_times',
  riskLevel: 'safe',
  description:
    'Suggest 4-6 open appointment times for the provider over the next week. Returns slots the provider can tap to schedule.',
  parameters,
  ...
  async handler(args, ctx) {
    ...
    if (appointmentRes.error || eventRes.error) {
      return {
    ...
    for (const t of (appointmentRes.data ?? []) as Array<{ startsAt: string; endsAt: string }>) {
      conflicts.push({
