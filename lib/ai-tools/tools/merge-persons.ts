 ...
 * ContactActivity, Appointment, and DealContact rows are re-pointed at keepId.
 *
 * Approval-gated with an explicit summariseCall — the provider sees
...
const parameters = z
  ...
  .describe('Merge two contacts: move all activity/appointments/deal links onto keepId, then delete mergeId.');
...
export const mergePersonsTool = defineTool<typeof parameters, MergePersonsResult>({
  ...
  async handler(args, ctx) {
    // Both contacts must exist in this space and not be agency rows.
    ...
    const [actCountRes, appointmentCountRes, dcCountRes] = await Promise.all([
      supabase
    ...
    const appointmentsCount = (appointmentCountRes as unknown as { count: number | null }).count ?? 0;
    ...
    const { error: appointmentErr } = await supabase
      ...
      .eq('spaceId', ctx.space.id);
    if (appointmentErr) {
      logger.error('[tools.merge_persons] appointment move failed (PARTIAL MERGE)', { keep: args.keepId, merge: args.mergeId }, appointmentErr);
      return {
        summary: `Partial merge — activities moved, appointments failed: ${appointmentErr.message}. Please reconcile manually.`,
        display: 'error',
      };
    }
    ...
    if (dcReadErr) {
      ...
      return {
        summary: `Partial merge — activity/appointments moved, deal links failed: ${dcReadErr.message}.`,
        display: 'error',
      };
    }
    if (dcRows && dcRows.length > 0) {
      ...
      if (toInsert.length > 0) {
        ...
        if (dcInsertErr) {
          ...
          return {
            summary: `Partial merge — activity/appointments moved, deal-link insert failed: ${dcInsertErr.message}.`,
            display: 'error',
          };
        }
      }
    ...
    const { error: activityErr } = await supabase.from('ContactActivity').insert({
      ...
      metadata: {
        ...
        movedAppointments: appointmentsCount,
    ...
    return {
      summary: `Merged "${mergeName}" into "${keepName}". Moved ${activitiesCount} activit${activitiesCount === 1 ? 'y' : 'ies'}, ${appointmentsCount} appointment${appointmentsCount === 1 ? '' : 's'}, ${dealLinksCount} deal link${dealLinksCount === 1 ? '' : 's'}.`,
      data: {
        keepId: args.keepId,
