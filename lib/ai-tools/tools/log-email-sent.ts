/**
 * `log_email_sent` — record an email the provider sent OUTSIDE Koala.
 *
...
const parameters = z
  ...
  .describe('Log an email sent outside Koala to a contact. Audit trail only.');
...
export const logEmailSentTool = defineTool<typeof parameters, LogEmailResult>({
  ...
  riskLevel: 'low',
  description:
    'Record an email the provider sent OUTSIDE Koala against a contact\'s timeline. Does NOT send anything.',
  parameters,
