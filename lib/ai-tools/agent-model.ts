 ...
 * an OpenAI key, the provider's chosen model (`x-ai/grok-4.3`) was ignored.
 *
 * Now:
 *   - One client, the app-wide `getLLMClient()` (OpenRouter or OpenAI).
 *   - The provider's workspace model, resolved through `resolveChatModel()` so
...
/**
 * The SDK `Model` the agent runs on: the provider's workspace model (resolved
