 ...
 *   The provider's UI POSTs the decision to
...
function buildSseStream(input: BuildStreamInput): ReadableStream<Uint8Array> {
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // call's telemetry — the sentence the provider sees before the
      ...
      const pushEvent = (event: PushableEvent) => {
        ...
        if (event.type === 'tool_call_result') {
          ...
          // marker, strip the marker so the provider sees clean text, push a
      ...
      try {
        ...
        if (!aborted) {
          ...
          pushEvent({
            type: 'error',
            message: koalaErrorMessage('internal'),
            code: 'internal',
          });
        }
        controller.close();
      ...
      try {
        ...
        // to the browser, so the provider SAW the reply. If we just log and
        ...
        // failure surfaces as an error event so the provider knows the
        ...
        if (textBuffer.trim() || subagentBlocks.length > 0) {
          ...
          if (!saved) {
            ...
            pushEvent({
              type: 'error',
              message: koalaErrorMessage('persistence'),
              code: 'persistence',
            });
          }
        }
 ...
 * the provider-facing approval prompts. Returns the new row id, or null
