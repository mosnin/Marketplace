 ...
 *
 * The provider-facing `summariseCall` is OUR concern, not the SDK's. The
...
export function toSdkTool<TArgs, TData>(def: ToolDefinition<TArgs, TData>, ctx: ToolContext) {
  ...
  return tool({
    ...
    // accepts it. Strict mode requires every key in `services` to
    ...
    // supplied and to be an array including every key in services."
    //
    ...
    async execute(input) {
      try {
        ...
        // provider's chat. Reformat to the same `Error: ` prefix the
        // success/error paths use so the model continues normally and
        // can paraphrase to the provider in Koala voice.
        //
 ...
 *   "Invalid schema for function 'add_service': In context=(...,
 ...
 * The SDK gives us the tool name + raw args; the provider sees the
...
// provider decides → reload → apply decision → resume. The helpers below
...
/**
 * Provider-facing approval prompt extracted from one SDK interruption.
...
export interface ApprovalPrompt {
  ...
  arguments: unknown;
  /** What the provider reads in the prompt — domain-specific via summariseCall. */
  summary: string;
}
...
export function extractApprovals(
  ...
  return interruptions.map((item) => {
    ...
    try {
      ...
      // raw string so the provider at least sees what was attempted.
 ...
 * (or AgentQuestion) and rehydrated when the provider decides.
 */
...
/** What the provider's PATCH /api/agent/drafts/[id] resolves to. */
export type ApprovalDecision = { approved: true } | { approved: false; message?: string };

/**
 * Apply the provider's approve/reject decision to a rehydrated state.
