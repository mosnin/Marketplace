/**
 * Koala-voiced helpers for the chat path.
 *
 ...
 *
 *   - koalaErrorMessage: maps an error code to a first-person Koala line so
 *     a tool failure or backend hiccup reads like Koala talking, not like a
 *     stack trace leaking through.
 *
 * `koalaErrorMessage` / `classifyError` are sync, side-effect free, and safe
 ...
 *
 * Strategy:
 *   1. Strip HTML, control chars, mention prefix from koala-workspace.
...
/**
 * The single source of truth for what Koala says when something goes wrong.
 * First-person, in-character, no system-warning vocabulary.
 */
export function koalaErrorMessage(code: KoalaErrorCode): string {
  switch (code) {
    case 'cold_start':
...
/**
 * Best-effort classification of a raw error message into a KoalaErrorCode.
 * Used at points where we only have a string (e.g. a fetch reject reason)
 * and need to pick the right Koala line.
 */
export function classifyError(raw: string | undefined | null): KoalaErrorCode {
