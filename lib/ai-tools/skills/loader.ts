 ...
 *     one `{placeholder}` for the provider to fill in). A skill appears in
 ...
 * Koala's current scale (3 skills). Add if/when the count crosses ~20.
 *
...
/** A skill the provider can invoke from the chat `/` menu. */
export interface UserSkill {
  slug: string;
  title: string;
  description: string;
  /** Composer text; may contain one `{placeholder}` for the provider to fill. */
  prompt: string;
}
