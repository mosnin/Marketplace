 ...
 * via chat completions, with the provider's workspace model, the full tool set,
 ...
 *
 * `'modal'` — opt-in (KOALA_CHAT_RUNTIME=modal). Proxies the WHOLE chat turn
...
export function chatRuntime(): ChatRuntime {
  const v = process.env.KOALA_CHAT_RUNTIME;
