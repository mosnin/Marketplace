 ...
 * (matching what the Modal route does today) so the provider sees the
 ...
 * the per-step tool-search churn would only confuse the provider — the
 ...
 *
 * The `registry` is the provider-facing tool list — we use it ONLY to render
...
function mapRunItemEvent(
  ...
  switch (event.name) {
    ...
    case 'tool_approval_requested': {
      // Reuse the bridge's extractor so the provider-facing summary is the
    ...
    case 'handoff_occurred': {
      ...
      // provider informed without forcing a client-side change.
