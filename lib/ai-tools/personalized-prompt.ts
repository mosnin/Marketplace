/**
 * Per-provider context that gets folded into the agent's system prompt at
 ...
 * gives the model the provider's name + the loudest pipeline facts before
 * it picks up a tool. It saves tool calls AND sounds like the agent knows
 * the provider.
 *
 * What it pulls:
 *   - Provider's first name (Clerk → User table)
 ...
 *     them by provider verb without naming SDK tool slugs)
 *
 * What it does NOT pull:
 *   - Full timelines or activity dumps. The agent has tools for that.
 *   - PII beyond first name. The model shouldn't be reciting the provider's
...
export interface PersonalizedSnapshot {
  /** Provider's first name. Null if we couldn't resolve. */
  ...
  /** Pending AgentDraft rows the provider needs to decide on. */
...
export function renderSnapshot(s: PersonalizedSnapshot): string {
  const lines: string[] = [];
  if (s.firstName) {
    lines.push(`Provider: ${s.firstName}.`);
  }
  const facts: string[] = [];
