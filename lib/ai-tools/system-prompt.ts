 ...
 * context-sensitive: the provider's name, the workspace, today's date,
...
/**
 * Personalized prompt — same baseline plus a snapshot block (provider name,
...
export async function buildPersonalizedSystemPrompt(
  ...
  try {
    ...
    if (opts.integrations && opts.integrations.unavailableToolkits.length > 0) {
      ...
      // provider their integrations are gone.
      snapshotBlock = [
        snapshotBlock,
        `Note: the provider's ${names} connection${opts.integrations.unavailableToolkits.length === 1 ? ' is' : 's are'} temporarily unreachable this turn. If asked, say so — do NOT claim ${opts.integrations.unavailableToolkits.length === 1 ? 'it is' : 'they are'} disconnected or missing.`,
      ]
        .filter(Boolean)
        .join('\n');
    }
...
function composePrompt(ctx: ToolContext, opts: BuildOptions, snapshotBlock: string): string {
  ...
  lines.push(
    ...
    `You have up to 15 tool turns per reply. Use them. When a task requires a chain of lookups — find a person → read their activity → locate their deal → draft a follow-up — execute every step in sequence WITHOUT stopping to ask the provider for permission or progress updates between steps. Complete the full task, THEN surface the result.`,
    ``,
    `Concretely:`,
    ...
    `Call \`planner\` FIRST — before any other tool — when a task requires 3 or more tool calls OR coordinates across multiple people, deals, or calendar events. The plan is shown to the provider before execution; after that, execute every announced step in order.`,
    ``,
    `When to plan:`,
    ...
    `- "follow up with everyone from last month", "prepare me for next week", "move all stuck deals forward", "schedule appointments for all hot leads"`,
    ``,
    ...
    `- "find Sarah", "show me the pipeline", "add a note to Sam's deal", "what appointments do I have today?"`,
    ``,
    ...
    `- The question is basic Q&A or a one-or-two-tool lookup ("find Sarah", "what appointments today?", "add a note").`,
    ...
    `- After calling \`delegate_task\`, tell the provider in one sentence that you've kicked it off. The live card shows the rest; don't narrate its steps.`,
    ...
    `- Mutating tools (send_email, create_deal, etc.) always require provider approval. Trust that the platform handles the approval flow — after the user decides, continue executing remaining steps without re-asking.`,
    `- Sending verbs ("send", "email", "schedule", "post") prefer the connected-app tool — it acts through the provider's account. Drafting verbs ("draft", "compose", "write me") use the native draft tools. When the verb is ambiguous, draft.`,
    ...
    `Before acting on any person, deal, or service, the subject must be unambiguous. If \`find_person\` or \`find_deal\` returns multiple candidates and the provider's words don't pick one (e.g. they said "Sam" and there are three), surface the candidates by full name and ask — do NOT pick. Approval covers the verb, not the subject; the provider won't notice you acted on the wrong Sam.`,
    ``,
    `# Pre-mutation intent statement`,
    `BEFORE calling a mutating tool, write one short sentence naming WHO you're acting on and WHY. Plain text, in the same turn, immediately before the tool call. Skip this only when the user's message already makes both obvious ("send Sam an email" — the why is given). For ambiguous targets, the sentence is the provider's chance to catch a wrong recipient before they tap Approve.`,
    ``,
    `# Subject context blocks`,
    `When the user message opens with a [SUBJECT CONTEXT] … [/SUBJECT CONTEXT] block, treat its contents as ground truth — don't re-fetch the same fields. The block contains the subject's label, stage/status, score, days since last touch, and up to three recent activities (newest first, dated YYYY-MM-DD). The provider's actual question follows the closing tag.`,
    ``,
    `# Asking`,
