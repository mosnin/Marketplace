 ...
 *
 * Runs the provider-facing turn in-process on `@openai/agents` against the
 ...
 * `KOALA_CHAT_RUNTIME=modal` proxies the whole turn to the sandbox. See
 * `runtime-flag.ts`.
 *
 * Two entry points:
 *
 ...
 *   - `resumeChatTurn` — load a paused run, apply the provider's approval
 ...
 * modern models). A provider with modest credit can't make a single call
 * because the pre-charge alone exceeds it — exact symptom: HTTP 402
 * "requested up to 65536 tokens, but can only afford X."
 *
 * 4096 is ~4× the typical Koala turn (~1k tokens) — headroom for
 * long-form drafts (offer letters, post-appointment packets) while keeping the
 ...
 * whichever third-party apps the provider has connected. Loaded dynamically
...
export function buildChatAgent(
  ...
  // (tests / A-B), or the provider's workspace model resolved to the active
  ...
  // Personalized prompt is async — it loads a snapshot of the provider's
...
/**
 * Resolve the Composio tools the provider's chat should see this turn.
 ...
 * provider revoked our OAuth grant on the provider's side and our row is
 ...
 * Next time the provider opens /integrations, they see amber + Reconnect
...
export interface IntegrationLoadResult {
  ...
  liveToolkits: string[];
  /** Toolkits the provider has connected but whose tools could not be
   ...
   *  "I don't have your Gmail" to a provider who connected Gmail is the
...
export async function loadIntegrationToolsDetailed(
  ...
  // The provider HAS connections but the server can't reach Composio at
  // all (key unset). Silent-empty here is what made a misconfigured
  // deploy read as "Koala lost my integrations" — degrade loudly instead.
...
export interface RunChatTurnInput {
   ...
   * provider just said — every turn becomes a fresh start. Passing history
   * here is the difference between "Sam who?" and "right, Sam who you
   * mentioned two messages ago."
   */
  history?: ChatHistoryRow[];
  /**
   * The provider's workspace chat model slug (e.g. `x-ai/grok-4.3`). Resolved
   ...
   * the card") no longer has to go through Modal.
   */
...
export interface ResumeChatTurnInput {
  ...
  /** The provider's decision for the pending approval. */
