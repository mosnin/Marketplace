 ...
 *     inline draft engine, the provider's "Send a check-in" tap.
...
const SYSTEM_PROMPT =
  "You are Koala, an AI assistant for a real-estate CRM. Compose ONE short outbound message the provider can send right now. " +
  ...
  "Email body: 2-4 sentences, plain text, no markdown, no signature (the provider's name is appended downstream). " +
...
function buildVoiceMessage(samples: VoiceSample[]): string {
  ...
  const lines: string[] = [
    "The provider's voice — recent emails they sent to OTHER people. Match cadence, sentence length, and word choice only.",
    "Do NOT address the new recipient by any name that appears in these samples. Do NOT mention any deal, service, address, date, or detail from these samples — those belong to other recipients and would be a privacy breach.",
