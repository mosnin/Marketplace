/**
 * Client-portal auth — fully separate from the provider Clerk auth.
 *
 * Email + password accounts for end users (applicants / appointment-bookers). Sessions
 * are stateless signed JWTs in an httpOnly cookie scoped to the portal; never
 * touches Clerk, Clerk cookies, or the provider session. Passwords are scrypt
...
export const CLIENT_SESSION_COOKIE = 'koala_client_session';
