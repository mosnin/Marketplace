 ...
 *   service-photos/{spaceId}/{serviceId}/{uuid}-{filename}
 *   onboarding/{userId}/{uuid}-{filename}
 *   studio/{spaceId}/{uuid}-{filename}          — Studio-generated media
 *   profile-cover/{spaceId}/{uuid}-{filename}   — provider's public-page cover photo
 *
 * Public vs signed: feature attachments stay PRIVATE — we serve them via
 * `getSignedUrl()` with a short TTL. Service photos and avatars can be
 * PUBLIC (the provider wants them on a public-facing intake form anyway);
...
/** Default signed URL lifetime (seconds). 1 hour — enough for the provider
 ...
 * promote a private file to a public prefix (e.g. files/ → service-photos/)
 ...
 * object key (e.g. `service-photos/sp_x/prop_y/uuid-name.jpg`). Returns
 * null if the URL doesn't match our bucket's public shape, which makes the
 * caller skip the delete instead of guessing.
 *
 * Used by Service DELETE: the `photos` column is a JSONB array of public
