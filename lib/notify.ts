 ...
 *   - notifyAppointmentBookings (per-event: new appointment bookings)
...
import { sendAgentNotification, type AppointmentEmailData } from '@/lib/appointment-emails';
import { sendSMS, newLeadSMS, newAppointmentSMS, newDealSMS } from '@/lib/sms';
...
interface SpaceOwnerInfo {
  ...
  notifyAppointmentBookings: boolean;
...
async function getSpaceOwnerInfo(spaceId: string): Promise<SpaceOwnerInfo | null> {
  try {
    const [{ data: space }, { data: settings }] = await Promise.all([
      ...
      supabase
        .from('SpaceSetting')
        .select('notifications, smsNotifications, phoneNumber, notifyNewLeads, notifyAppointmentBookings, notifyNewDeals, notifyFollowUps, notifyPush')
        .eq('spaceId', spaceId)
        .maybeSingle(),
    ]);
...
export interface NotifyNewAppointmentParams {
  spaceId: string;
  appointmentData: AppointmentEmailData;
}

/**
 * Notify space owner about a new appointment booking via email + SMS.
 * Respects both the channel toggles AND the notifyAppointmentBookings event toggle.
 */
export async function notifyNewAppointment(params: NotifyNewAppointmentParams): Promise<void> {
  const info = await getSpaceOwnerInfo(params.spaceId);
  if (!info || !info.notifyAppointmentBookings) return;
  ...
  if (info.emailEnabled) {
    promises.push(
      sendAgentNotification(info.ownerEmail, params.appointmentData)
        .catch((err) => logger.error('[notify] appointment email failed', { spaceId: params.spaceId }, err))
    );
  }

  // SMS notification to agent
  if (info.smsEnabled && info.ownerPhone) {
    const d = new Date(params.appointmentData.startsAt);
    promises.push(
      sendSMS(
        newAppointmentSMS({
          spaceName: info.spaceName,
          guestName: params.appointmentData.guestName,
          date: d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
          time: d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }),
          service: params.appointmentData.serviceAddress,
          phone: info.ownerPhone,
        })
      ).catch((err) => logger.error('[notify] appointment SMS failed', { spaceId: params.spaceId }, err))
    );
  }
  ...
  if (info.pushEnabled) {
    ...
    const prop = params.appointmentData.serviceAddress ? ` at ${params.appointmentData.serviceAddress}` : '';
    promises.push(
      sendPushToSpace(params.spaceId, {
        title: `New appointment: ${params.appointmentData.guestName}`,
        body: `${when}${prop}.`,
        url: `/s/${info.spaceSlug}/calendar`,
      }).catch((err) => logger.error('[notify] appointment push failed', { spaceId: params.spaceId }, err))
    );
  }
