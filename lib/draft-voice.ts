/**
 * Voice samples — what the provider actually wrote, post-edit.
 *
 * The compose path used to read like a template because the model only ever
 * saw the SYSTEM_PROMPT and the subject's facts. After a provider edits a
 ...
 *
 * What we're NOT doing: storing a per-provider "voice profile," running a
 * fine-tune, or building a vector index. The provider's last 3 edited drafts
 ...
 *      samples and not to reuse deals/services/dates from them. That's
...
// samples does. Require two before we ship any. If the provider has only
...
//
// LOOKBACK_DAYS = 60. Any older than that and the provider's voice has
...
// provider edits maybe a handful per day), and the compose route fires
