 ...
 *   - Any Service whose `address` appears verbatim in the message
 *
 ...
 * If the same provider re-sends the same query in five minutes (refresh, retry
 ...
 * book.
 *
 * Trade-off: we accept some over-matching (e.g. "send" matching a service
...
async function matchServicesByAddress(
  spaceId: string,
  tokens: string[],
): Promise<ContextEntity[]> {
  if (tokens.length === 0) return [];
  ...
  const { data, error } = await supabase
    ...
    .limit(MAX_NAME_MATCHES);
  if (error) {
    logger.warn('[vector-context] service address match failed', { spaceId }, error);
    return [];
  }
  type Row = {
    id: string;
  ...
  return ((data ?? []) as Row[]).map((r) => {
    ...
    return { id: r.id, label: r.address, hint: parts.join(' · ') || 'service' };
  });
}
...
function formatBlock(result: Omit<RetrieveContextResult, 'block'>): string {
  ...
  if (result.services.length > 0) {
    lines.push('### Mentioned services');
    for (const p of result.services) lines.push(`- ${p.label} (${p.hint})`);
  }
...
export async function retrieveContext(
  ...
  const [memories, contacts, deals, services] = await Promise.all([
    ...
    matchServicesByAddress(input.spaceId, tokens).catch((err) => {
      logger.warn('[vector-context] service match threw', { spaceId: input.spaceId }, err);
      return [] as ContextEntity[];
    }),
  ]);
