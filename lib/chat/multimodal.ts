 ...
 *       same block shape handles provider contract scans + MLS PDFs.
 *
 ...
 *       v1 we tell the provider in the response to ask via Claude.
 *
 *   google (Gemini)
 ...
 *       the attachments and emit a calm Koala-voice line in the text
 *       block so the provider knows to switch models. The router can then
...
export interface BuildResult {
  ...
  /** A one-line Koala-voice note when we dropped attachments; empty
 ...
 *  provider. PNG / JPEG / WebP / GIF cover what providers actually paste —
...
export interface SdkBuildResult {
  ...
  unsupported: MultimodalAttachment[];
  /** One calm Koala-voice line when attachments were dropped; '' otherwise. */
  fallbackNote: string;
}
...
/**
 * One calm sentence telling the provider an attachment didn't make it to
 * the model — Koala voice, no apology, no emoji. Sonner toast or inline
