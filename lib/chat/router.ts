 ...
 * send email, schedule appointment, etc.) or call a connected integration.
 *
...
/**
 * Imperative action verbs that signal the provider wants Koala to DO
...
/**
 * Workspace-DATA queries that READ the provider's book of business. These are
 * not mutations, but they still REQUIRE tools (find_person, find_service,
 ...
 * deflect. So any message that references the provider's data (a read verb
 * paired with, or just a mention of, a workspace noun) routes to the agent.
 *
 * This is the fix for the "Koala has no access to anything" failure: read
 ...
 * which providers with Gmail connected experience as Koala losing their
...
/**
 * Agency-domain reads. The shared WORKSPACE_QUERY_RE was curated from provider
 * traffic; agency questions ("team health", "at-risk agents", "audit response
 * times", "who's unassigned") matched nothing and fell through to the agency
 * direct path — whose snapshot prompt is INSTRUCTED to say it doesn't have
 * the answer. The "Koala says it has no tools" complaint on the agency
 * surface was manufactured by that fallthrough. These nouns cover the agency
 * tool catalog's natural-language surface (team/roster/performance/revenue/
 * routing/reviews/briefing).
 */
const AGENCY_QUERY_RE =
  /\b(team|roster|members?|providers?|agents?|agency|floor|unassigned|reassignments?|routing|response times?|at[\s-]?risk|flight[\s-]?risk|performance|production|leaderboard|rankings?|revenue|volume|gci|splits?|review queue|reviews?|briefing|brief|health|coverage|sla)\b/i;
...
export function decideRoute(
  ...
  try {
    ...
    // Text reads of the provider's own data still need tools — route them to
...
/**
 * Agency-surface routing: everything decideRoute knows, plus the agency
 * noun set. The agency direct path serves only a 5-field aggregate snapshot
 * — any question about specific providers, routing, performance, or reviews
 * needs the agency tool catalog on the agent path.
 */
export function decideAgencyRoute(
  userMessage: string,
  attachments: RouteAttachment[] = [],
): RouteDecision {
  try {
    ...
    const text = (userMessage ?? '').trim();
    if (text && attachments.length === 0 && AGENCY_QUERY_RE.test(text)) {
      return 'agent';
    }
    return base;
  } catch {
...
const ESCALATION_PHRASES = [
  ...
  // Hand-off language
  /\blet me (?:hand|pass) (?:this|that) (?:to|over to) (?:koala'?s? tools|the agent|koala)\b/i,
