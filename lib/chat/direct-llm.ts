 ...
 *
 *   system prompt (KOALA_INSTRUCTIONS_LITE, ~300 tokens)
...
/**
 * The direct path's system prompt. Slimmer than the full KOALA_INSTRUCTIONS
...
(images, PDFs), explain real estate concepts, and reason about the provider's
workspace using the context you're given. You do NOT take action here — no
sending email, no creating contacts, no scheduling. If the provider asks for
...
about the provider's CRM. Name contacts/deals/services verbatim from the
block when relevant. Don't fabricate details — if the block doesn't
mention something, say you don't know and suggest the provider look it up.
`.trim();
