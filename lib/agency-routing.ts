 ...
 *   - Pre-migration safe: if the new Agency columns don't exist yet,
 *     the autoAssign flag is treated as false, so the caller falls back
 *     to the agency-owner space (current behaviour). The DealRoutingRule
...
/**
 * Load the three routing-config columns off the Agency row. If the
 * columns are missing (migration hasn't landed yet), returns a safe
 * default that disables auto-assignment.
 */
async function loadAgencyRoutingConfig(
  agencyId: string,
): Promise<AgencyRoutingConfig | null> {
  try {
    ...
    if (error) {
      ...
      if (code === '42703') {
        logger.debug('[agency-routing] routing columns not present yet; treating as disabled', {
          agencyId,
        });
        return { autoAssignEnabled: false, assignmentMethod: 'manual', lastAssignedUserId: null };
      }
      logger.warn('[agency-routing] failed to load agency routing config', { agencyId }, error);
      return null;
    }

    if (!data) {
      logger.warn('[agency-routing] agency row not found', { agencyId });
      return null;
    }
    ...
    logger.error('[agency-routing] unexpected error loading routing config', { agencyId }, err);
    return null;
  }
}

/**
 * Load enabled DealRoutingRule rows for this agency, ordered by
 ...
 */
async function loadEnabledRules(agencyId: string): Promise<DealRoutingRuleRow[]> {
  try {
    const { data, error } = await supabase
      .from('DealRoutingRule')
      .select(
        'id, agencyId, name, priority, enabled, leadType, minBudget, maxBudget, matchTag, destinationUserId, destinationPoolMethod, destinationPoolTag, createdAt',
      )
    ...
    if (error) {
      ...
      logger.warn('[agency-routing] failed to load routing rules', { agencyId }, error);
      return [];
    }

    return (data ?? []) as DealRoutingRuleRow[];
  } catch (err) {
    logger.warn('[agency-routing] routing-rule load threw', { agencyId }, err);
    return [];
  }
}
 ...
 *   - A Space exists with ownerId = user.id AND agencyId = this agency
 *
 ...
 */
async function getEligibleAgents(agencyId: string): Promise<EligibleAgent[]> {
  try {
    ...
    if (memberError) {
      logger.warn('[agency-routing] failed to enumerate memberships', { agencyId }, memberError);
      return [];
    }
    ...
    // Fetch user status + agency-scoped spaces in parallel.
    ...
    if (usersResult.error) {
      ...
      if (code === '42703') {
        const { data: usersNoStatus, error: retryErr } = await supabase
          ...
          .in('id', userIds);
        if (retryErr) {
          logger.warn('[agency-routing] failed to fetch users (fallback)', { agencyId }, retryErr);
          return [];
        }
        ...
        logger.warn('[agency-routing] failed to fetch users', { agencyId }, usersResult.error);
        return [];
      }
    } else {
    ...
    if (spacesResult.error) {
      logger.warn('[agency-routing] failed to fetch spaces', { agencyId }, spacesResult.error);
      return [];
    }
    ...
    for (const s of spacesResult.data ?? []) {
      ...
      // If a user somehow has multiple spaces in this agency, the
    ...
    logger.error('[agency-routing] unexpected error enumerating agents', { agencyId }, err);
    return [];
  }
}
 ...
 */
async function updateCursor(agencyId: string, newCursorUserId: string): Promise<void> {
  try {
    ...
    if (error) {
      ...
      logger.warn('[agency-routing] cursor update failed', { agencyId, newCursorUserId }, error);
    }
  } catch (err) {
    logger.warn('[agency-routing] cursor update threw', { agencyId, newCursorUserId }, err);
  }
}
 ...
 *   - type IN ('QUALIFICATION','APPOINTMENT','APPLICATION')
...
async function countActiveLeadsBySpace(spaceIds: string[]): Promise<Map<string, number>> {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .in('spaceId', spaceIds)
      .in('type', ['QUALIFICATION', 'APPOINTMENT', 'APPLICATION']);

    if (error) {
      logger.warn('[agency-routing] contact count query failed; treating all counts as 0', {}, error);
      return counts;
    }
    ...
    logger.warn('[agency-routing] contact count threw; treating all counts as 0', {}, err);
  }
 ...
 * eligible agents), honouring the Agency round-robin cursor for stability.
 */
 ...
 * longer in this agency / no space; or the pool is empty) — the
 * caller should treat null as "skip this rule, move to the next".
 *
 * eligible: pre-computed active provider pool for this agency, passed
...
async function resolveRuleDestination(
  ...
  if (rule.destinationUserId) {
    ...
    if (!match) {
      logger.info('[agency-routing] rule destination agent ineligible; skipping rule', {
        ruleId: rule.id,
  ...
  // schema but IGNORED at the engine level until AgencyMembership
  // grows a tags column — see migration header for the TODO.
  if (rule.destinationPoolMethod) {
    if (rule.destinationPoolTag) {
      logger.debug('[agency-routing] destinationPoolTag present but ignored (no tags column yet)', {
        ruleId: rule.id,
  ...
  logger.warn('[agency-routing] rule has neither destinationUserId nor destinationPoolMethod', {
    ruleId: rule.id,
  });
  return null;
}
...
/**
 * Pick which provider_member should receive a new lead for this agency.
 * Returns null when:
 *   - autoAssignEnabled is false
 *   - assignmentMethod is 'manual' AND no rule with criteria matches
 *   - no eligible agent (zero active provider_members OR none has a Space
 *     inside this agency)
 * Callers should fall back to the agency owner's space when null.
 *
 ...
 */
export async function routeAgencyLead(
  agencyId: string,
  lead?: RoutingLeadInput,
): Promise<RoutingResult | null> {
  try {
    const config = await loadAgencyRoutingConfig(agencyId);
    ...
    // so compute once.
    const agents = await getEligibleAgents(agencyId);
    if (agents.length === 0) {
      logger.info('[agency-routing] no eligible agents; caller falls back to owner space', {
        agencyId,
    ...
    // no-arg call.
    if (lead) {
      const rules = await loadEnabledRules(agencyId);
      for (const rule of rules) {
        ...
        void updateCursor(agencyId, picked.userId);
        logger.info('[agency-routing] rule match', {
          agencyId,
          ruleId: rule.id,
          ruleName: rule.name,
    ...
    if (config.assignmentMethod === 'round_robin') {
      ...
      void updateCursor(agencyId, picked.userId);
      logger.info('[agency-routing] round-robin pick', {
        agencyId,
        pickedUserId: picked.userId,
        pool: agents.length,
    ...
    void updateCursor(agencyId, picked.userId);
    logger.info('[agency-routing] score-based pick', {
      agencyId,
      pickedUserId: picked.userId,
      pool: agents.length,
    });
    ...
    logger.error('[agency-routing] unexpected failure; returning null', { agencyId }, err);
    return null;
  }
}
