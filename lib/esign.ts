/**
 * E-signature via Composio — the provider's OWN connected DocuSign account.
 *
 ...
 * provider connects it through the same OAuth flow as Gmail/Slack; Composio
 * owns the OAuth app and holds the tokens. We send and read envelopes on
 * the provider's behalf by executing Composio DocuSign actions, scoped to
 * the provider's Clerk userId (the Composio "entity").
 *
...
/** True when the provider has an active DocuSign connection on Composio. */
...
/**
 * Send a stored document out for signature on the provider's DocuSign account.
...
export async function sendForSignature(
  ...
  // 4. Fire the Composio CREATE_ENVELOPE action on the provider's account.
