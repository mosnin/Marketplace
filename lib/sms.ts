...
export function newAppointmentSMS(p: { spaceName: string; guestName: string; date: string; time: string; service?: string | null; phone: string }): SendSMSParams {
  const prop = p.service ? ` at ${p.service}` : '';
  return {
    to: p.phone,
    body: `[${p.spaceName}] New appointment booked: ${p.guestName}${prop} on ${p.date} at ${p.time}. Check your dashboard for details.`,
  };
}
...
export function appointmentConfirmationSMS(p: { guestName: string; guestPhone: string; businessName: string; date: string; time: string; service?: string | null }): SendSMSParams {
  ...
  return {
    to: p.guestPhone,
    body: `Hi ${p.guestName}! Your appointment with ${p.businessName}${prop} is confirmed for ${p.date} at ${p.time}. Contact your agent if you need to reschedule.`,
  };
}
...
export function appointmentReminderSMS(p: { guestName: string; guestPhone: string; businessName: string; time: string; service?: string | null }): SendSMSParams {
  ...
  return {
    to: p.guestPhone,
    body: `Hi ${p.guestName}, reminder: your appointment with ${p.businessName}${prop} is tomorrow at ${p.time}. See you there!`,
  };
}
