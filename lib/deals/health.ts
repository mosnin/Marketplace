 ...
 *
 * Health is deliberately simple — three states only — because providers don't
...
// (agency pipeline, morning routes, old tests) keep compiling. Internally we
...
export function dealHealth(deal: DealHealthInput): DealHealthMeta {
  ...
  // Expected close date — overdue means something the provider expected to close
  ...
  // Follow-up overdue means the provider committed to doing something and hasn't.
  ...
  // Provider-authored next action that's past its due date — a strong
 ...
 *
 * Priority:
 *   1. Explicit `nextAction` — provider typed it, respect it.
...
export function inferNextAction(
  ...
  // 1. Provider-authored next action wins.
 ...
 * board: closing this week, at-risk/stuck, and waiting on the provider.
 *
