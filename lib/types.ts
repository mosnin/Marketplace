...
export type AccountType = 'provider' | 'agency_only' | 'both';
...
export type Agency = {
  ...
  agencyFormConfig: IntakeFormConfig | null;
  agencyRentalFormConfig: IntakeFormConfig | null;
  agencyBuyerFormConfig: IntakeFormConfig | null;
  agencyRentalScoringModel: import('@/lib/scoring/scoring-model-types').ScoringModel | null;
  agencyBuyerScoringModel: import('@/lib/scoring/scoring-model-types').ScoringModel | null;
  /**
   * Plan tier (V2 — lib/plans.ts). 'team' → 5 seats, 'team_plus' → 10. A
   * agency that hasn't subscribed may still carry a legacy/unset value in
  ...
  createdAt: Date;
  // Intake trust signals — agency-level compliance text inherited by
  // /apply/b/[agencyId]. Per-space SpaceSetting values take a back seat
  // when the intake is served via the agency variant.
...
export type AgencyMembership = {
  ...
  // own profile within the agency. Null until the member fills it in.
...
export type ClientType = 'QUALIFICATION' | 'APPOINTMENT' | 'APPLICATION';
...
export type Deal = {
  ...
  /** Provider-authored "what's next" — shown prominently on the card and in the Today inbox. */
  ...
  /** Nullable FK to Service. Deals without a linked service still work. */
...
export type FormConfigSource = 'custom' | 'agency' | 'legacy';
...
export type ApplicationMessageSenderType = 'applicant' | 'provider';
