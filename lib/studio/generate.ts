/**
 * Studio generation — the shared core used by both the provider-facing route
 * (`/api/studio/generate`) and the internal route the Koala agent calls
...
export async function runStudioGeneration(args: {
  ...
  // Brand kit — fold the provider's palette into the prompt so output comes
  ...
  if (genErr) {
    ...
    // Surface the underlying error so the provider sees WHAT's wrong instead
