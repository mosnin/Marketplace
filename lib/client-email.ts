...
function portalUrl(): string {
  return (
    ...
    `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://my.usekoala.com'}/clients`
  );
}
...
function shell(heading: string, bodyHtml: string): string {
  return `<!doctype html><html><body style="margin:0;background:#fff;color:#111;font-family:ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif">
    <div style="max-width:480px;margin:0 auto;padding:40px 24px">
      ...
      ${bodyHtml}
      <p style="color:#999;font-size:12px;margin-top:32px">Koala · client portal</p>
    </div>
  </body></html>`;
}
...
/** Generic portal notification (new message from provider, info request, status change). */
