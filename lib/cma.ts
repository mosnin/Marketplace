 ...
 *
 * In-house only. Comps come from the provider's own Service rows (the same
 * source `find_comparable_services` uses) — never MLS, never an external API.
 *
...
/** Snapshot of the subject service frozen into the report. */
...
/** Free-typed subject fields (when the provider isn't picking a saved row). */
...
/**
 * Resolve the comp `price` + basis for a Service row: prefer a sold listing's
...
const COMP_SELECT =
  'id, address, city, stateRegion, beds, baths, squareFeet, serviceType, listPrice, listingStatus, updatedAt';

const MAX_COMPS = 6;

/**
 * Score a candidate against the subject. Lower is closer. Mirrors the
 * intent of `find_comparable_services` (beds/baths/price similarity) but
 * adds sqft + a sold-comp preference, since a CMA wants the most relevant
 * recent sales near the top.
 */
function scoreComp(row: ServiceRow, subject: CmaSubject): number {
  let score = 0;
  ...
  // Prefer sold comps and same service type as light tie-breakers.
  if (row.listingStatus === 'sold') score -= 1;
  if (subject.serviceType && row.serviceType === subject.serviceType) score -= 0.5;
  return score;
}

function toComp(row: ServiceRow): CmaComp {
...
export interface BuildCmaArgs {
  spaceId: string;
  /** Pick a saved Service as the subject. */
 ...
 * own Service rows, scores them by similarity to the subject, and computes the
...
export async function buildCma(args: BuildCmaArgs): Promise<CmaPayload> {
  const { spaceId, subjectServiceId, subjectFields } = args;
  ...
  if (subjectServiceId) {
    const { data, error } = await supabase
      .from('Service')
      .select(COMP_SELECT)
      .eq('id', subjectServiceId)
      .eq('spaceId', spaceId)
      .maybeSingle();
    if (error) throw new Error(`Subject lookup failed: ${error.message}`);
    if (!data) throw new Error('Subject service not found.');
    ...
    subject = {
      ...
      serviceType: subjectFields.serviceType ?? null,
      listPrice: subjectFields.listPrice ?? null,
    };
  } else {
    throw new Error('Provide a subjectServiceId or subject fields with an address.');
  }
  ...
  if (subject.serviceId) rows = rows.filter((r) => r.id !== subject.serviceId);
 ...
 * crypto-strong randomness — same posture as ServicePacket tokens.
 */
