 ...
 *
 * When Koala creates an event (appointment, callback, follow-up), it does two
 * things:
 *   1. Writes the event to the provider's connected external calendar
 *      via Composio (`GOOGLECALENDAR_CREATE_EVENT` etc.).
 *   2. Logs the same event to `CalendarEventMirror` as a backup row.
 *
 * The external calendar is the source of truth — Koala reads from it
 * on-demand. The mirror row is forensics: if the provider swaps
 ...
 *     whether to surface the failure to the provider.
 ...
 *     truth to back up.
 *
 * Why a thin helper instead of inlining: appointment booking, post-appointment
...
export const PROVIDER_TOOL_SLUGS = {
  ...
  outlook_calendar: {
    // Composio's Outlook slugs. We don't write to Outlook yet — appointment
...
export interface CalendarConnection {
  ...
  id: string;
  /** Composio entityId — the provider's Clerk userId. */
  userId: string;
  /** Provider slug. */
...
/**
 * Find the provider's active calendar connection for this space. Returns
...
export interface WriteThroughInput {
  ...
  /** Display title for the event ("Appointment: Sam Lee", "Callback: Jordan"). */
  title: string;
...
/**
 * Write an event to the provider's external calendar AND log it to the
 ...
 * appointment) doesn't fail because Google was slow.
 */
export async function writeEventThrough(
  ...
  if (mirrorErr || !mirrorRow) {
    ...
    // any) still landed — the provider's calendar is the truth.
