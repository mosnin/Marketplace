 ...
 *
 * All helpers operate in the provider's LOCAL timezone — the calendar grid
...
/**
 * Build an ISO 8601 timestamp WITH the provider's local timezone offset.
 * Used when posting to Google Calendar — Google needs the offset so the
 * event lands on the wall-clock the provider picked, not UTC midnight.
 *
