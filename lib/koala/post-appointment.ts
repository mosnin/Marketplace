/**
 * Post-appointment orchestration helpers.
 *
 ...
 * ask the model to propose 2-5 actions the provider would have taken
 * manually. The model does NOT execute the tools — it returns intent
 * only. The provider approves the batch in one tap; the route then runs
 ...
 *    provider who wants to review before anything happens.
 ...
 * truncate to 5. Either failure mode is the model's fault; the provider
...
export type PostAppointmentToolName = (typeof POST_APPOINTMENT_TOOL_ALLOWLIST)[number];

/**
 * Composio slug allowlist for integration tools the post-appointment proposer is
 ...
 * `/api/koala/post-appointment/execute` with `tool: "GMAIL_TRASH_EMAIL"` (or any
 * other Composio Gmail/Calendar slug) is rejected — the endpoint is a
 * post-appointment orchestrator, not a generic Composio passthrough. Tightening
 * here also limits blast radius if the model drifts.
 */
export const POST_APPOINTMENT_INTEGRATION_SLUG_ALLOWLIST = [
  'GMAIL_SEND_EMAIL',
  'GOOGLECALENDAR_CREATE_EVENT',
] as const;

export type PostAppointmentIntegrationSlug =
  (typeof POST_APPOINTMENT_INTEGRATION_SLUG_ALLOWLIST)[number];

/**
 * A Composio tool the provider's connected accounts expose. We pull this
 ...
 * for the done-verb resolver, NOT shown to the provider in the proposal
...
export interface IntegrationToolSpec {
  ...
  slug: string;
  /** Provider-readable description. May be Composio's text, may be pre-processed. */
  description: string;
...
export interface ProposedAction {
  ...
  tool: PostAppointmentToolName | string;
  ...
  humanSummary?: string;
  /** Whether this verb mutates state (i.e. needs the provider's yes). */
  mutates: boolean;
  /**
 ...
 *
 *  When the provider has connected apps (Gmail, Google Calendar, ...),
...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    'Turn a provider\'s post-appointment debrief into 2-5 proposed actions. Return intent only — never execute.',
    '',
    'Allowed tools:',
    native,
  ];
  ...
  if (integrationTools.length > 0) {
    ...
    lines.push(
      '',
      'Connected-app tools (the provider has authorized these):',
      integ,
      '',
      'When the provider\'s intent is to actually send, schedule, or post — not draft — prefer the connected-app tool.',
      'Examples: "send Sam a follow-up" → GMAIL_SEND_EMAIL (if Gmail connected). "put the appointment on my calendar" → GOOGLECALENDAR_CREATE_EVENT.',
      'If the provider only wants to see what they\'d say (a draft), use the draft_* tools instead.',
    );
  }
 ...
 *  the in-process registry; integration tools come from the provider's
...
export function shapeProposals(
  ...
  const nativeAllow = new Set(opts.allowlist ?? POST_APPOINTMENT_TOOL_ALLOWLIST);
  ...
  for (const item of parsed.data.proposals) {
    ...
    if (nativeAllow.has(item.tool)) {
      ...
      out.push({
        tool: item.tool as PostAppointmentToolName,
        args: item.args,
        summary,
...
/** Provider-voice summary for an integration proposal. The toolkit slug
...
function integrationProposalSummary(
  ...
  const verb = providerVerbForToolkit(toolkit);
...
export async function proposeActions(
  ...
  const allowed = input.tools.filter((t) =>
    (POST_APPOINTMENT_TOOL_ALLOWLIST as readonly string[]).includes(t.name),
  );
...
/**
 * Resolve the Composio tools the provider's post-appointment orchestrator should
 ...
 * native catalog and the provider never sees the seam.
 *
 ...
 */
export async function loadPostAppointmentIntegrationTools(args: {
  spaceId: string;
  userId: string;
  ...
  try {
    ...
    logger.warn(
      '[post-appointment] integration tools load failed — proceeding without them',
 ...
 *  `GMAIL_SEND_EMAIL` → `gmail`. Cross-checks against the provider's
 ...
 *
 * The post-appointment ceremony is the bid-for-10 surface; chat-side approval
...
export function formatHumanSummary(
  ...
  // Integration proposals come from connected apps. The provider doesn't
  ...
  if (proposal.integrationToolkit) {
    const verb = providerVerbForToolkit(proposal.integrationToolkit);
...
/**
 * Provider-voice verb for the proposal stack. The provider doesn't know
 ...
 */
export function providerVerbForToolkit(toolkit: string): string {
  switch (toolkit) {
 ...
 * the provider reads this on the way back to their car. Falls back to a
