 ...
 *
 * Email routing precedence (when the provider has connected an inbox):
 *   1. Provider's connected Gmail/Outlook via Composio (sends as the provider —
 ...
 *      for providers who haven't connected their own inbox yet).
 *
 ...
 * the wrong identity is worse than not sending; the provider should see the
 * failure and reconnect/retry.
 *
 * SMS routing:
 *   1. Telnyx REST (only path for now; per-provider SMS-as-themselves is
 *      a follow-up that requires per-provider 10DLC approval).
 *
 ...
 *                unconfigured); the provider sees the error in the UI.
 */
...
export interface DeliveryResult {
  sent: boolean;
  /** 'gmail' / 'outlook' = via provider's inbox; 'email' = via shared Resend. */
...
export interface SendDraftOptions {
  ...
  spaceId?: string;
  /** Clerk userId of the provider who approved the draft. Becomes the
   *  Composio entity id for the OAuth lookup. */
  userId?: string;
}

// ─── Inbox-connect: send as the provider via Composio ──────────────────────────
 ...
 * or null. Gmail wins ties because it's the dominant provider inbox; if a
 * provider has both, the deliberate fallback to Outlook would need an
 ...
 * @param options  Per-provider context that enables inbox-connect routing.
...
export async function sendDraft(
  ...
  switch (draft.channel) {
    case 'email': {
      // Try the provider's connected inbox first. The point of inbox-connect
      // is "every email looks like it came from the provider, on their
      ...
      if (options.spaceId && options.userId) {
        try {
          ...
          // We log but don't bail because the provider still needs to be
