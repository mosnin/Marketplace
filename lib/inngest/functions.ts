...
// Above the cap → log + drop. The provider noticing "Koala got quiet"
 ...
 *      provider's space/user. If either is missing or non-active, drop.
 ...
 * provider can dismiss. We accept that risk over the alternative of
...
export const handleComposioTrigger = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    //    provider turned it off; the receiver doesn't know that, the
    //    handler does.
    ...
    //    hourly cap is finer-grained but lets a provider with five
