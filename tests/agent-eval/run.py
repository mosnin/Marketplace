#!/usr/bin/env python3
"""Koala agent eval runner — tool-routing regression net.

Runs each case in cases.json against a real Koala agent, but with every
...
started bouncing through ask_provider. Every fix to the system prompt or the
...
# ---------------------------------------------------------------------------
# Path + env setup. Must happen BEFORE we import any koala/* modules so the
# `from koala import …` style imports inside agent/ resolve.
# ---------------------------------------------------------------------------
...
_STUB_RETURNS: dict[str, Any] = {
    ...
    "book_appointment": {"action": "booked", "appointmentId": "t_eval_stub", "when": "2026-06-01T14:00:00Z"},
    "route_lead": {"action": "previewed", "destinationProvider": "Eval Provider"},
    "add_service": {"action": "added", "serviceId": "p_eval_stub"},
    "send_service_packet": {
        "action": "drafted",
    ...
    "ask_provider": {"action": "asked", "questionId": "q_eval_stub"},
...
def _build_synthetic_curated_tools(connected_toolkits: list[str]) -> list:
    """Build the curated FunctionTools the provider would see in prod.
...
async def run_case(case: dict[str, Any], model: str | None) -> tuple[bool, str, list[CallRecord]]:
    ...
    koala.input_guardrails = []

    captured: list[CallRecord] = []
    _patch_tools_to_capture(koala, captured)
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Koala tool-routing eval runner")
    parser.add_argument(
        "--model",
        default=None,
