/**
 * Route-level test for POST /api/koala/post-appointment.
 *
...
const {
  ...
  loadPostAppointmentIntegrationToolsMock,
} = vi.hoisted(() => ({
  proposeActionsMock: vi.fn(),
  attachHumanSummariesMock: vi.fn(),
  loadPostAppointmentIntegrationToolsMock: vi.fn(),
}));
vi.mock('@/lib/koala/post-appointment', async () => {
  const actual =
    await vi.importActual<typeof import('@/lib/koala/post-appointment')>('@/lib/koala/post-appointment');
  return {
    ...actual,
    proposeActions: proposeActionsMock,
    attachHumanSummaries: attachHumanSummariesMock,
    loadPostAppointmentIntegrationTools: loadPostAppointmentIntegrationToolsMock,
  };
});
...
import { POST } from '@/app/api/koala/post-appointment/route';
...
function makeReq(body: unknown): NextRequest {
  return new NextRequest('http://localhost/api/koala/post-appointment', {
    method: 'POST',
...
describe('POST /api/koala/post-appointment', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.OPENAI_API_KEY = 'sk-test';
    // Default to no connected apps — the graceful-degradation baseline.
    loadPostAppointmentIntegrationToolsMock.mockResolvedValue([]);
  });
  ...
  it('returns proposals on happy path', async () => {
    ...
    proposeActionsMock.mockResolvedValue([
      { tool: 'log_call', args: { personId: 'p1', summary: 'Appointment' }, summary: 'Log call', mutates: true },
  ...
  it('forwards a contextHint with personId/dealId to the orchestrator', async () => {
    // Surfaces the URL-driven pre-fill — when the provider opens the recorder
  ...
  it('forwards integration tools to the orchestrator when the provider has connected apps', async () => {
    // The Gmail-connected case: the orchestrator must be told the
    // GMAIL_SEND_EMAIL tool exists, otherwise the post-appointment stack can
    ...
    proposeActionsMock.mockResolvedValue([
      {
        ...
        args: { to: 'sam@chen.com', subject: 'Appointment follow-up', body: '...' },
    ...
    expect(loadPostAppointmentIntegrationToolsMock).toHaveBeenCalledOnce();
    expect(loadPostAppointmentIntegrationToolsMock).toHaveBeenCalledWith({
      spaceId: 'space_1',
      userId: 'user_1',
    });
