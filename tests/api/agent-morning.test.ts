 ...
 * tests can assert e.g. `.is('agencyId', null)` was applied.
 */
...
describe('GET /api/agent/morning — public contract', () => {
  ...
  it('agency-routed contacts excluded: each Contact query chains .is("agencyId", null)', async () => {
    queueSupabase({});
    ...
    for (const call of contactCalls) {
      ...
      const hasAgencyFilter = isCalls.some(
        ([, args]) => args[0] === 'agencyId' && args[1] === null,
      );
      expect(hasAgencyFilter, 'every Contact query must filter agencyId IS NULL').toBe(true);
    }
