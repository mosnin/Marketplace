 ...
 *     KOALA_CHAT_RUNTIME is unset/empty/anything-other-than-modal.
 *   - Only KOALA_CHAT_RUNTIME=modal proxies the agent turn to Modal.
...
// reserved agency/team title — to exercise the #303 write-path isolation guard.
...
describe('POST /api/ai/task — reserved-title conversationId is not reused (isolation guard, #303)', () => {
  // A agency_owner also owns their personal provider space, and the pre-#295
  ...
  it('mints a fresh conversation instead of reusing a [AGENCY_KOALA] one', async () => {
    ...
    const res = await POST(
      makeRequest({ message: 'add Preston as a contact', conversationId: 'agency_conv_1' }),
    );
    expect(res.status).toBe(200);
    expect(tsStreamMock).toHaveBeenCalledTimes(1);
    const call = tsStreamMock.mock.calls[0][0] as { conversationId: string };
    expect(call.conversationId).not.toBe('agency_conv_1');
  });

  it('mints a fresh conversation instead of reusing a [AGENCY_CHAT] one', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    convLookup.row = { id: 'team_conv_1', spaceId: 's_1', title: '[AGENCY_CHAT] standup' };
  ...
  it('reuses a plain provider conversation in the same space (guard is not over-broad)', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    convLookup.row = { id: 'provider_conv_1', spaceId: 's_1', title: 'Follow up with the Garcias' };
    await POST(makeRequest({ message: 'add Preston as a contact', conversationId: 'provider_conv_1' }));
    const call = tsStreamMock.mock.calls[0][0] as { conversationId: string };
    expect(call.conversationId).toBe('provider_conv_1');
  });
});
...
describe('POST /api/ai/task — agent branch (in-process TS is the default)', () => {
  ...
  it('treats any value other than the exact string "modal" as the TS runtime', async () => {
    process.env.KOALA_CHAT_RUNTIME = 'MODAL'; // wrong case → TS default
    await POST(makeRequest());
    expect(tsStreamMock).toHaveBeenCalledTimes(1);
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('proxies action verbs to the Modal agent only when KOALA_CHAT_RUNTIME=modal', async () => {
  ...
  it('returns 503 when KOALA_CHAT_RUNTIME=modal but MODAL_CHAT_URL is not configured', async () => {
    process.env.KOALA_CHAT_RUNTIME = 'modal';
...
describe('POST /api/ai/task — dual-path router', () => {
  ...
  it('explicit mode:chat forces the direct path even for an action verb', async () => {
    ...
    // provider asked for a fast answer, not the tool loop.
  ...
  it('explicit mode:agent forces Modal even for a generic question', async () => {
    ...
    // acting runtime) so the provider's chosen runtime is honored.
  ...
  it('mode:agent degrades to the in-process TS agent when MODAL_CHAT_URL is unset', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    delete process.env.MODAL_CHAT_URL;
    // A missing Modal URL must not fail the one turn the provider asked to run;
