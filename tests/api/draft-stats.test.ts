 ...
 * shape downstream callers (the agent itself, future agency dashboards) will
 * bind to.
 *
...
// agency dashboard's "Draft impact" card bind to. Test it directly so the
// agency-wide consumer doesn't need the full request mock.
...
describe('aggregateDraftStats (helper)', () => {
  ...
  it('mixed input rolls up identically across provider and agency scopes', () => {
    // The card is agency-wide — it concatenates rows from every space in
    // the agency and feeds them in. Same shape, same numbers.
