 ...
 * silent rename here breaks every provider's integrations screen.
 *
 * What we guard:
 *   - Auth + space gate (401, 403)
 *   - Revoked rows are filtered out — the audit trail is not provider-facing
...
describe('GET /api/integrations', () => {
  ...
  it('filters revoked connections out of the response', async () => {
    ...
    // Revoked row should NOT appear; expired row SHOULD appear (the provider
  ...
  it('projects exactly the provider-facing fields — no DB internals leak', async () => {
