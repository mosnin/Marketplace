/**
 * Threat-model test suite for the provider / agency conversation isolation
 * boundary.
 *
 * The threat: agency-Koala and agency team chats share the
 * `Conversation` table with provider conversations, keyed by `spaceId` and
 * distinguished only by a reserved title prefix. A agency_owner also owns
 * their personal provider space, so space ownership ALONE does not isolate the
 * two surfaces. Without an explicit reserved-title guard, a provider (or a
 * agency hitting the provider endpoints) could read or mutate agency-side
 * conversations through the provider routes.
 *
 * Every test here CROSSES the boundary on purpose and asserts denial:
 *   - GET /api/ai/messages on a [AGENCY_KOALA] conv  -> 404, no message rows
 *   - GET /api/ai/messages on a [AGENCY_CHAT] conv -> 404, no message rows
 *   - GET /api/ai/conversations (provider list) excludes BOTH reserved prefixes
 *   - PATCH /api/ai/conversations/[id] on a [AGENCY_KOALA] conv -> 404
 *   - DELETE /api/ai/conversations/[id] on a [AGENCY_KOALA] conv -> 404
 *
...
vi.mock('@/lib/space', () => ({
  getSpaceFromSlug: vi.fn(async () => ({ id: 's_provider_1', slug: 'jane', ownerId: 'u_1' })),
}));
...
// call so we can assert the provider list applies BOTH reserved-prefix filters.
...
// The routes read `req.nextUrl.searchParams`, a NextRequest service a plain
...
const idParams = { params: Promise.resolve({ id: 'c_agency_1' }) };

// ── GET /api/ai/messages ────────────────────────────────────────────────────

describe('GET /api/ai/messages — agency/team conversations are denied', () => {
  it('404s a [AGENCY_KOALA] conversation and returns NO message rows', async () => {
    // The caller legitimately owns the space (agency_owner owns their provider
    // space). Ownership passes; the reserved-title guard is what denies.
    seed('Conversation', { data: { id: 'c_agency_1', spaceId: 's_provider_1', title: '[AGENCY_KOALA] private notes' } });
    seed('User', { data: { id: 'u_1' } });
    seed('Space', { data: { id: 's_provider_1', ownerId: 'u_1' } });
    // If the guard were missing, this is the row set that would leak.
    seed('Message', { data: [{ id: 'm_1', role: 'assistant', content: 'agency secret', blocks: null, createdAt: '2026-01-01' }] });

    const res = await getMessages(messagesRequest('c_agency_1'));
  ...
  it('404s a [AGENCY_CHAT] conversation and returns NO message rows', async () => {
    seed('Conversation', { data: { id: 'c_team_1', spaceId: 's_provider_1', title: '[AGENCY_CHAT] team room' } });
    seed('User', { data: { id: 'u_1' } });
    seed('Space', { data: { id: 's_provider_1', ownerId: 'u_1' } });
  ...
  it('serves a plain provider conversation (control: the guard is not over-broad)', async () => {
    seed('Conversation', { data: { id: 'c_provider_1', spaceId: 's_provider_1', title: 'Follow up with the Garcias' } });
    seed('User', { data: { id: 'u_1' } });
    seed('Space', { data: { id: 's_provider_1', ownerId: 'u_1' } });
    seed('Message', { data: [{ id: 'm_1', role: 'user', content: 'hi', blocks: null, createdAt: '2026-01-01' }] });

    const res = await getMessages(messagesRequest('c_provider_1'));
...
// ── GET /api/ai/conversations (provider list) ────────────────────────────────
...
describe('GET /api/ai/conversations — list excludes BOTH reserved prefixes', () => {
  it('applies a .not(title,like) filter for each reserved prefix', async () => {
    ...
    const titleLikeValues = notFilterCalls
      ...
      .map((c) => c.value);
    expect(titleLikeValues).toContain('[AGENCY_KOALA]%');
    expect(titleLikeValues).toContain('[AGENCY_CHAT]%');
  });

  it('does not return a seeded agency row in the provider list', async () => {
    ...
    seed('User', { data: { id: 'u_1' } });
    seed('Conversation', { data: [{ id: 'c_provider_1', spaceId: 's_provider_1', title: 'Garcias' }] });
    ...
    expect(titles).not.toContain('[AGENCY_KOALA] private notes');
    expect(titles).not.toContain('[AGENCY_CHAT] team room');
  });
});

// ── PATCH / DELETE /api/ai/conversations/[id] ───────────────────────────────

describe('PATCH /api/ai/conversations/[id] — agency conversation denied', () => {
  it('404s renaming a [AGENCY_KOALA] conversation even when ownership matches', async () => {
    ...
    seed('Conversation', { data: { id: 'c_agency_1', spaceId: 's_provider_1', title: '[AGENCY_KOALA] private', Space: { ownerId: 'u_1' } } });
    seed('User', { data: { id: 'u_1' } });
...
describe('DELETE /api/ai/conversations/[id] — agency conversation denied', () => {
  it('404s deleting a [AGENCY_KOALA] conversation even when ownership matches', async () => {
    seed('Conversation', { data: { id: 'c_agency_1', spaceId: 's_provider_1', title: '[AGENCY_KOALA] private', Space: { ownerId: 'u_1' } } });
    seed('User', { data: { id: 'u_1' } });
  ...
  it('404s deleting a [AGENCY_CHAT] conversation even when ownership matches', async () => {
    seed('Conversation', { data: { id: 'c_team_1', spaceId: 's_provider_1', title: '[AGENCY_CHAT] team', Space: { ownerId: 'u_1' } } });
    seed('User', { data: { id: 'u_1' } });
