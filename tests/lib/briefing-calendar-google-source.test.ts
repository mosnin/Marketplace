 ...
 *     self-flagged attendees (the provider's own account). Without these,
 *     the brief would either name nobody (cards drop) or name the provider
...
describe('calendar-google source — attendee crosswalk', () => {
  ...
  it('skips the self-flagged attendee (the provider themselves)', () => {
    const match = matchAttendeeToContact(
      [
        { email: 'provider@agency.com', self: true, responseStatus: 'accepted' },
        { email: 'dani@example.com', responseStatus: 'accepted' },
      ],
      contactsByEmail,
    );
...
describe('calendar-google source — decline detection', () => {
  ...
  it('skips a self-flagged declining attendee (the provider declined their own meeting)', () => {
    const declined = findDeclinedContact(
      [{ email: 'provider@agency.com', self: true, responseStatus: 'declined' }],
      contactsByEmail,
    );
    expect(declined).toBeNull();
  });
