...
// ── Supabase mock ─────────────────────────────────────────────────────────
// Mirrors the table-keyed FIFO chain mock used in agency-offboard.test.ts:
...
// rpc queue (unused here but kept for structural parity with agency-offboard).
...
import { PATCH } from '@/app/api/agency/commissions/ledger/[id]/route';
import { GET } from '@/app/api/agency/commissions/export/route';
...
function setCaller(
  role: 'agency_owner' | 'agency_admin' | 'provider_member' = 'agency_owner',
  dbUserId = 'u_caller',
): void {
...
function buildPatchRequest(id: string, body: PatchBody | string | undefined): Request {
  ...
  return new Request(`http://localhost/api/agency/commissions/ledger/${id}`, init);
}
...
function buildExportRequest(query: string): Request {
  return new Request(`http://localhost/api/agency/commissions/export${query}`);
}
...
describe('PATCH /api/agency/commissions/ledger/[id]', () => {
  describe('authorization', () => {
    it('returns 403 when requireAgency throws', async () => {
      requireAgencyImpl = async () => {
    ...
    it('returns 403 when the caller is a provider_member', async () => {
      setCaller('provider_member');
      const { status, json } = await invokePatch('led_1', { status: 'paid' });
      expect(status).toBe(403);
  ...
  describe('lookups', () => {
    it('returns 404 when the ledger row belongs to a different agency', async () => {
  ...
  describe('happy paths', () => {
    it('status-change only: returns 200, fires UPDATE audit, leaves amounts unchanged', async () => {
      // Lookup: existing ledger row in this agency.
      queueSingle('CommissionLedger', {
        id: 'led_1',
        agencyId: 'brk_1',
        dealValue: 500000,
      ...
      if ('agencyAmount' in lastPayload) {
        expect(lastPayload.agencyAmount).toBe(2500);
      }
    });
...
describe('GET /api/agency/commissions/export', () => {
