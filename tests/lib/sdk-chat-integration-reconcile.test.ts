 ...
 * a single connected account is dead (the provider revoked our OAuth grant
...
describe('loadIntegrationTools — reconcile-on-error', () => {
  it('degrades LOUDLY (not silently) when the SDK is not configured but toolkits are connected', async () => {
    ...
    // model then told providers their integrations were gone. New contract:
  ...
  it('returns [] when the provider has no active toolkits', async () => {
    activeToolkitsMock.mockResolvedValue([]);
