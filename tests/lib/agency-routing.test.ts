/**
 * BP7d — vitest coverage for the agency lead-routing engine (BP7b).
 *
 * The engine under test is `routeAgencyLead(agencyId)` in
 * lib/agency-routing.ts. Three sibling agents (BP7a schema, BP7b
 * engine + integration, BP7c settings) are wiring this feature end-to-
 * end in parallel; this file owns unit coverage only.
 *
 * Mock shape mirrors tests/lib/agency-templates.test.ts:
 ...
 *
 * One deviation from agency-templates.test.ts: the engine calls
 * `.maybeSingle()` on the Agency row for config load. Our `singleThen`
...
// `updateCalls.Agency` in particular.
...
type AgencyConfigRow = {
  autoAssignEnabled: boolean;
  assignmentMethod: 'manual' | 'round_robin' | 'score_based';
  lastAssignedUserId: string | null;
};

function makeAgency(overrides: Partial<AgencyConfigRow> = {}): AgencyConfigRow {
  return {
...
/**
 * Seed a canonical "three eligible providers" fixture:
 *   agents u_a, u_b, u_c, each with a Space inside agency b_1.
 * Tests that care about a different topology override `mockByTable`
 * entries directly.
 */
function seedThreeAgents(
  config: Partial<AgencyConfigRow> = {},
  opts: { extraUsers?: Array<Record<string, unknown>> } = {},
): void {
  mockByTable.Agency = { single: makeAgency(config) };
  mockByTable.AgencyMembership = {
    rows: [
      { userId: 'u_a', role: 'provider_member' },
      { userId: 'u_b', role: 'provider_member' },
      { userId: 'u_c', role: 'provider_member' },
    ],
  };
  mockByTable.User = {
    rows: [
  ...
  mockByTable.Space = {
    rows: [
      { id: 's_a', ownerId: 'u_a', agencyId: 'b_1' },
      { id: 's_b', ownerId: 'u_b', agencyId: 'b_1' },
      { id: 's_c', ownerId: 'u_c', agencyId: 'b_1' },
    ],
  };
}

async function callRouter(): Promise<unknown> {
  const mod = await import('@/lib/agency-routing');
  return mod.routeAgencyLead('b_1');
}
...
// Gating
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead — gating', () => {
  it('returns null when autoAssignEnabled is false', async () => {
    mockByTable.Agency = {
      single: makeAgency({ autoAssignEnabled: false, assignmentMethod: 'round_robin' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    // No side-effect write to Agency.
    expect(updateCalls.Agency ?? []).toHaveLength(0);
  });

  it("returns null when assignmentMethod is 'manual'", async () => {
    mockByTable.Agency = {
      single: makeAgency({ autoAssignEnabled: true, assignmentMethod: 'manual' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Agency ?? []).toHaveLength(0);
  ...
  it('returns null when no provider_members exist', async () => {
    mockByTable.Agency = {
      single: makeAgency({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.AgencyMembership = { rows: [] };
  ...
  it('returns null when the only provider has no Space in this agency', async () => {
    mockByTable.Agency = {
      single: makeAgency({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.AgencyMembership = {
      rows: [{ userId: 'u_orphan', role: 'provider_member' }],
    };
    mockByTable.User = { rows: [{ id: 'u_orphan', status: 'active' }] };
    // Space exists, but the engine filters by `agencyId=this` — returning an
    // empty `rows` for Space is the simplest way to simulate a cross-tenant
    // miss because the chain mock doesn't actually filter by `eq('agencyId')`.
    mockByTable.Space = { rows: [] };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Agency ?? []).toHaveLength(0);
  ...
  it('fails closed on infra errors (Agency select errors → null, no update)', async () => {
    mockByTable.Agency = {
...
// Round-robin
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead — round_robin', () => {
  ...
  it('updates lastAssignedUserId after picking (fire-and-forget)', async () => {
    ...
    await Promise.resolve();
    const writes = updateCalls.Agency ?? [];
  ...
  it('ignores offboarded agents (eligibility filter)', async () => {
    ...
    mockByTable.Agency = {
      single: makeAgency({ assignmentMethod: 'round_robin', lastAssignedUserId: null }),
    };
    mockByTable.AgencyMembership = {
      rows: [
        { userId: 'u_a', role: 'provider_member' },
...
// Score-based
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead — score_based', () => {
  /**
   * Build a Contact rows array that yields `count` active leads per space.
   * Active = type IN QUALIFICATION/APPOINTMENT/APPLICATION, snoozedUntil IS NULL.
   */
