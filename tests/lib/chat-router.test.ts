...
describe('decideRoute', () => {
  ...
  it('routes reads of the provider data to agent — they need tools', () => {
  ...
  it('routes action verbs to agent', () => {
    ...
    expect(decideRoute('Cancel the 5pm appointment')).toBe('agent');
  ...
  it('still routes to agent when both attachments AND action verbs present', () => {
    ...
    expect(decideRoute('Add this service to the deal', att)).toBe('agent');
  });
  ...
  it('routes integration reads to agent — only the agent has integration tools', () => {
    ...
    // isn't an action verb), so a Gmail-connected provider's question landed
...
describe('decideAgencyRoute', () => {
  it('routes agency-domain reads to agent — the snapshot path has no agency tools', () => {
    // The shared provider router sent all of these to the agency direct path,
    // whose prompt is INSTRUCTED to say it doesn't have the answer — agencies
    // read that as "Koala has no tools".
    expect(decideAgencyRoute("how's team health?")).toBe('agent');
    expect(decideAgencyRoute('audit response times')).toBe('agent');
    expect(decideAgencyRoute('which agents are at risk?')).toBe('agent');
    expect(decideAgencyRoute('show provider performance')).toBe('agent');
    expect(decideAgencyRoute("who's unassigned?")).toBe('agent');
    expect(decideAgencyRoute('reassign Maria to John')).toBe('agent');
    expect(decideAgencyRoute('what does the leaderboard look like?')).toBe('agent');
    expect(decideAgencyRoute('agency revenue this quarter')).toBe('agent');
  });

  it('keeps generic Q&A on direct', () => {
    expect(decideAgencyRoute("what's a CMA?")).toBe('direct');
    expect(decideAgencyRoute('Explain the 1031 exchange rules to me.')).toBe('direct');
  });

  it('inherits the provider routing for shared phrasings', () => {
    expect(decideAgencyRoute('Send Preston the follow-up email')).toBe('agent');
    expect(decideAgencyRoute('do I have any new emails?')).toBe('agent');
  });
});
