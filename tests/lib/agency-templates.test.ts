/**
 * BP6d — tests for the agency-template library + publish fan-out.
 *
 ...
 *
 * Mock shape mirrors tests/lib/agency-reviews.test.ts (table-keyed chain
 * mock + per-test override of the auth state). The only new wrinkle is
 * the publish fan-out: the route reads N AgencyMembership rows then
...
type AgencyRole = 'agency_owner' | 'agency_admin' | 'provider_member' | null;
interface AuthState {
  clerkId: string | null;
  dbUserId: string;
...
vi.mock('@/lib/permissions', () => ({
  requireAgency: vi.fn(async () => {
    if (!authState.agencyRole || authState.agencyRole === 'provider_member') {
      throw new Error('Forbidden: agency access required');
    }
    return {
      membership: { id: 'm_1', role: authState.agencyRole, userId: authState.dbUserId, agencyId: authState.agencyId },
      agency: { id: authState.agencyId, name: 'Test Agency', ownerId: authState.dbUserId },
      dbUserId: authState.dbUserId,
    };
  }),
  getAgencyMemberContext: vi.fn(async () => {
    if (!authState.agencyRole) return null;
    return {
      membership: { id: 'm_1', role: authState.agencyRole, userId: authState.dbUserId, agencyId: authState.agencyId },
      agency: { id: authState.agencyId, name: 'Test Agency', ownerId: authState.dbUserId },
      dbUserId: authState.dbUserId,
...
// A fully-formed AgencyTemplate row — used as the `single` payload the
// supabase mock returns from `.single()`/`.maybeSingle()` calls on the
// AgencyTemplate table. Version/publish counters default to an initial
...
// ──────────────────────────────────────────────────────────────────────────
describe('POST /api/agency/templates', () => {
  async function invoke(body: unknown): Promise<Response> {
    const mod = await import('@/app/api/agency/templates/route');
    return mod.POST(postReq('http://x/api/agency/templates', body) as never);
  }

  it('403 when caller role is provider_member', async () => {
    authState.agencyRole = 'provider_member';
  ...
  it('201 on happy path — inserted row has version=1', async () => {
    // The route inserts and selects the row back. Steer the AgencyTemplate
    // `single` payload so the returned body matches what we'd expect the DB
    // to have written.
    mockByTable.AgencyTemplate = {
    ...
    const inserted = insertCalls.AgencyTemplate ?? [];
...
// ──────────────────────────────────────────────────────────────────────────
describe('PATCH /api/agency/templates/[id]', () => {
  async function invoke(id: string, body: unknown): Promise<Response> {
    ...
    const mod = await import('@/app/api/agency/templates/[id]/route');
    return mod.PATCH(patchReq(`http://x/api/agency/templates/${id}`, body) as never, {
      params: Promise.resolve({ id }),
    });
  }

  it('404 when row not in caller agency', async () => {
    mockByTable.AgencyTemplate = { single: null };
  ...
  it('version increments when body changes (1 → 2)', async () => {
    ...
    const updates = updateCalls.AgencyTemplate ?? [];
  ...
  it('empty patch body → 400 rather than bumping version', async () => {
    ...
    // not-found branch.
    mockByTable.AgencyTemplate = {
      single: makeTemplateRow({ id: 't_1', agencyId: 'b_1', version: 1 }),
    };
...
// ──────────────────────────────────────────────────────────────────────────
describe('DELETE /api/agency/templates/[id]', () => {
  async function invoke(id: string): Promise<Response> {
    const mod = await import('@/app/api/agency/templates/[id]/route');
    return mod.DELETE(deleteReq(`http://x/api/agency/templates/${id}`) as never, {
      params: Promise.resolve({ id }),
    });
  }
  ...
  it('204 on happy path', async () => {
    mockByTable.AgencyTemplate = {
      single: makeTemplateRow({ id: 't_1', agencyId: 'b_1' }),
    };
    const res = await invoke('t_1');
    expect(res.status).toBe(204);
    expect(deleteCalls.AgencyTemplate ?? 0).toBeGreaterThanOrEqual(1);
  });

  it('404 when row not in agency', async () => {
    mockByTable.AgencyTemplate = { single: null };
...
// ──────────────────────────────────────────────────────────────────────────
describe('POST /api/agency/templates/[id]/publish', () => {
  async function invoke(id: string): Promise<Response> {
    const mod = await import('@/app/api/agency/templates/[id]/publish/route');
    return mod.POST(postReq(`http://x/api/agency/templates/${id}/publish`, {}) as never, {
      params: Promise.resolve({ id }),
    });
  }
  ...
  it('happy path: pushes to 2 agents, skips 1 with sourceVersion=null; publishedCount=2', async () => {
    ...
    // AgencyMembership list — three provider_member rows.
    mockByTable.AgencyMembership = {
      rows: [
        { id: 'm_a', userId: 'u_a', agencyId: 'b_1', role: 'provider_member' },
        { id: 'm_b', userId: 'u_b', agencyId: 'b_1', role: 'provider_member' },
        { id: 'm_c', userId: 'u_c', agencyId: 'b_1', role: 'provider_member' },
      ],
    };

    // Space rows — each agent has one space in agency b_1.
    mockByTable.Space = {
      rows: [
        { id: 'space_a', ownerId: 'u_a', agencyId: 'b_1' },
        { id: 'space_b', ownerId: 'u_b', agencyId: 'b_1' },
        { id: 'space_c', ownerId: 'u_c', agencyId: 'b_1' },
      ],
    };
    ...
    // AgencyTemplate should have been updated with publishedCount=2 (and
    // publishedAt set).
    const tmplUpdates = updateCalls.AgencyTemplate ?? [];
  ...
  it('pushes into a fresh agent (no prior MessageTemplate) via INSERT', async () => {
    ...
    // One agent in the agency.
    mockByTable.AgencyMembership = {
      rows: [
        { id: 'm_fresh', userId: 'u_fresh', agencyId: 'b_1', role: 'provider_member' },
      ],
    };
    // Agent's space.
    mockByTable.Space = {
      rows: [{ id: 'space_fresh', ownerId: 'u_fresh', agencyId: 'b_1' }],
    };
