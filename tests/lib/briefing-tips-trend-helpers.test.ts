...
import {
  ...
  appointmentConversionConfidence,
  median,
  interArrivalDays,
...
describe('trend tips — confidence ladders match the spec', () => {
  ...
  it('appointmentConversionConfidence steps up at 6 appointments', () => {
    expect(appointmentConversionConfidence(4)).toBe(0.78);
    expect(appointmentConversionConfidence(5)).toBe(0.78);
    expect(appointmentConversionConfidence(6)).toBe(0.88);
    expect(appointmentConversionConfidence(12)).toBe(0.88);
  });
});
...
describe('trend tips — cool-down policy', () => {
  it('all four C2 trend categories get a 14-day cool-down', () => {
    ...
    expect(coolDownDaysFor('appointment_conversion_drop', 'shown')).toBe(14);
  ...
  it('C2 trend categories still get 60 days when dismissed', () => {
    ...
    expect(coolDownDaysFor('appointment_conversion_drop', 'dismissed')).toBe(60);
  ...
  it('acted resets all trend cool-downs to 0 — fresh trigger fires again', () => {
    ...
    expect(coolDownDaysFor('appointment_conversion_drop', 'acted')).toBe(0);
