...
describe('enrichContext — deal subject', () => {
  it('returns label, stage, status, days-quiet and 3 activities', async () => {
    ...
    setActivities([
      { type: 'email', content: 'appointment confirmation', createdAt: '2026-04-29T15:00:00Z' },
    ...
    expect(ctx!.lastActivities[0]).toBe('2026-04-29 — email: appointment confirmation');
...
describe('renderEnrichedContextBlock', () => {
  it('wraps the body in [SUBJECT CONTEXT] / [/SUBJECT CONTEXT] tags', () => {
    const text = renderEnrichedContextBlock({
      ...
      scoreLabel: 'hot',
      leadScore: 88,
      lastActivities: ['2026-04-29 — email: appointment confirmation'],
    ...
    expect(text).toContain('- 2026-04-29 — email: appointment confirmation');
  });
});
