  ...
  describe('classifyQuietThread', () => {
    it('surfaces a hot-tier thread quiet for 2+ days at urgency 1, 0.90', () => {
      const result = classifyQuietThread({
        ...
        daysSinceLastProviderMessage: 2,
        lastProviderMessageChars: 30, // hot tier doesn't need the >100 floor
      });
    ...
    it('still surfaces a hot-tier thread on day 4 at the hot calibration', () => {
      ...
      const result = classifyQuietThread({
        ...
        daysSinceLastProviderMessage: 4,
        lastProviderMessageChars: 200,
      });
      expect(result).toEqual({ urgency: 1, confidence: 0.9 });
    });
    ...
    it('skips a hot-tier thread quiet only 1 day (under the 2-day threshold)', () => {
      expect(
        classifyQuietThread({
          leadScore: HOT_LEAD_THRESHOLD,
          daysSinceLastProviderMessage: 1,
          lastProviderMessageChars: 250,
        }),
      ).toBeNull();
    });

    it('surfaces an any-tier thread quiet 4+ days with a >100-char provider message at urgency 2, 0.85', () => {
      const result = classifyQuietThread({
        leadScore: HOT_LEAD_THRESHOLD - 30,
        daysSinceLastProviderMessage: 4,
        lastProviderMessageChars: 101,
      });
      expect(result).toEqual({ urgency: 2, confidence: 0.85 });
    });

    it('skips an any-tier thread when the last provider message was a 99-char drive-by', () => {
      // The 100-char floor is the cheap proxy for "real touch vs trivia."
      // A "got it, thanks" doesn't earn a follow-up nag.
      expect(
        classifyQuietThread({
          leadScore: 20,
          daysSinceLastProviderMessage: 5,
          lastProviderMessageChars: 99,
        }),
      ).toBeNull();
    });
    ...
    it('skips an any-tier thread quiet only 3 days (under the 4-day threshold)', () => {
      expect(
        classifyQuietThread({
          leadScore: 20,
          daysSinceLastProviderMessage: 3,
          lastProviderMessageChars: 500,
        }),
      ).toBeNull();
    });
