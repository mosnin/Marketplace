 ...
 * Python sandbox (opt-in via `KOALA_CHAT_RUNTIME=modal`).
 *
...
const ORIGINAL = process.env.KOALA_CHAT_RUNTIME;

describe('chatRuntime', () => {
  afterEach(() => {
    if (ORIGINAL === undefined) delete process.env.KOALA_CHAT_RUNTIME;
    else process.env.KOALA_CHAT_RUNTIME = ORIGINAL;
  });
  ...
  it('returns "ts" when the env var is empty', () => {
    process.env.KOALA_CHAT_RUNTIME = '';
    expect(chatRuntime()).toBe('ts');
  });
  ...
  it('returns "ts" for any value other than the exact string "modal"', () => {
    ...
    process.env.KOALA_CHAT_RUNTIME = ' modal ';
    expect(chatRuntime()).toBe('ts');
    process.env.KOALA_CHAT_RUNTIME = 'true';
    expect(chatRuntime()).toBe('ts');
    process.env.KOALA_CHAT_RUNTIME = '1';
    expect(chatRuntime()).toBe('ts');
