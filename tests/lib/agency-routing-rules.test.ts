 ...
 * agency lead-routing engine.
 *
 * The engine under test is `routeAgencyLead(agencyId, lead?)` in
 * lib/agency-routing.ts. With the BP7d layer added, enabled
 ...
 *
 * Mock shape mirrors tests/lib/agency-routing.test.ts — a
 * table-keyed chain mock on `@/lib/supabase` with `mockByTable[name] =
 * { rows, single, error }` overrides.
 *
 * NB on pool-tag narrowing: AgencyMembership has no `tags` column
...
/**
 * Seed a canonical "three eligible providers" fixture: u_a, u_b, u_c each
 * with a Space inside agency b_1. Additional overrides can swap in
...
async function callRouter(lead?: {
  ...
  return mod.routeAgencyLead('b_1', lead);
}
...
// Criteria matching
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead + rules — criteria', () => {
...
// Priority & enabled
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead + rules — ordering & flags', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead + rules — destination fallback', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeAgencyLead + rules — pool method', () => {
  it('pool round-robin over the full provider set (no tag narrowing yet)', async () => {
  ...
  it.skip('pool method + destinationPoolTag narrows pool to tagged agents (TODO: requires AgencyMembership.tags column)', async () => {
    // The schema accepts destinationPoolTag but the v1 engine ignores it
    // because there is no tags column on AgencyMembership or User yet.
