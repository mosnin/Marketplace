...
describe('catalog entry shape', () => {
  ...
  it('includes the load-bearing provider apps', () => {
...
describe('catalog cuts (audit 7→8)', () => {
  it('Linear is gone — developer issue tracker, not a provider tool', () => {
    expect(findIntegration('linear')).toBeUndefined();
  });

  it('Monday is gone — enterprise PM, not a provider tool', () => {
...
describe('COMING_SOON_TOOLKITS', () => {
  it('matches the set of catalog entries flagged comingSoon: true (no drift)', () => {
    ...
    // the provider taps Connect and Composio returns a confusing error.
