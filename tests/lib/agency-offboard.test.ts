...
// `AgencyMembership` table (target, then destination), so we let each table
...
// ── requireAgency mock ─────────────────────────────────────────────────────
let requireAgencyImpl: () => Promise<{ agency: Agency; membership: AgencyMembership; dbUserId: string }>;

vi.mock('@/lib/permissions', () => ({
  requireAgency: vi.fn(() => requireAgencyImpl()),
}));
...
// ── Route under test ───────────────────────────────────────────────────────
import { POST } from '@/app/api/agency/members/[id]/offboard/route';
...
function makeAgency(overrides: Partial<Agency> = {}): Agency {
  return {
    ...
    agencyFormConfig: null,
    agencyRentalFormConfig: null,
    agencyBuyerFormConfig: null,
    agencyRentalScoringModel: null,
    agencyBuyerScoringModel: null,
    plan: 'team',
    seatLimit: 5,
    ...
    agencyFairHousingNotice: null,
    agencyShowEqualHousingMark: false,
    ...overrides,
  };
}
...
function setCaller(role: 'agency_owner' | 'agency_admin' | 'provider_member' = 'agency_owner', dbUserId = 'u_caller'): void {
...
function buildRequest(id: string, body: PostBody | string | undefined): Request {
  ...
  return new Request(`http://localhost/api/agency/members/${id}/offboard`, init);
}
...
describe('POST /api/agency/members/[id]/offboard', () => {
  describe('authorization', () => {
    ...
    it('returns 403 when the caller is agency_admin (not owner)', async () => {
      setCaller('agency_admin');
      const { status, json } = await invoke('mem_target', { destinationMembershipId: 'mem_dest' });
      expect(status).toBe(403);
  ...
  describe('lookups', () => {
    it('returns 404 when the target membership is not in this agency', async () => {
      queueSingle('AgencyMembership', null); // target not found
    ...
    it('returns 404 when the destination membership is not found', async () => {
      ...
      queueSingle('AgencyMembership', null); // destination missing
    ...
    it('returns 403 when the target is the agency_owner', async () => {
      queueSingle('AgencyMembership', {
        id: 'mem_target',
        userId: 'u_owner',
        role: 'agency_owner',
  ...
  describe('happy paths', () => {
    ...
    it('dry-run returns counts plus leaving/destination names', async () => {
      ...
      expect(json.openAppointmentCount).toBe(1);
      ...
      expect((rpcMock.mock.calls as unknown[][])[0][0]).toBe('offboard_agency_member');
    ...
    it('real-run returns moved counts and fires an OFFBOARD audit event', async () => {
      ...
      expect(json.appointmentsMoved).toBe(1);
      ...
      expect(auditArgs.resource).toBe('AgencyMembership');
