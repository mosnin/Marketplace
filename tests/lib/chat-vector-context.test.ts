...
const { supabaseMock, rpcResp, contactResp, dealResp, serviceResp } = vi.hoisted(() => {
  ...
  const serviceResp = { data: [] as Array<Record<string, unknown>>, error: null as { message: string } | null };
  ...
  const tableChain = () => ({
    ...
    limit() {
      ...
      if (activeTable === 'Service') return Promise.resolve(serviceResp);
  ...
  return { supabaseMock, rpcResp, contactResp, dealResp, serviceResp };
});
...
describe('retrieveContext — happy path', () => {
  it('embeds the message and queries match_agent_memory', async () => {
    rpcResp.data = [
      ...
      { content: 'Sarah\'s appointment was canceled', similarity: 0.71 },
    ];
  ...
  it('includes literal-name matches for contacts/deals/services', async () => {
    contactResp.data = [
    ...
    expect(r.services[0]).toMatchObject({ id: 'p1', label: '456 Oak Ave' });
