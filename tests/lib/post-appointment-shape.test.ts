...
describe('shapeProposals', () => {
  ...
  it('drops tools outside the post-appointment allowlist', () => {
    const out = shapeProposals({
      proposals: [
        { tool: 'find_service', args: {} }, // not on allowlist
        { tool: 'pipeline_summary', args: {} }, // not on allowlist
        { tool: 'log_call', args: { personId: 'abc12345', summary: 'Appointment' } },
      ],
    });
...
describe('shapeProposals — integration tools', () => {
  ...
  it('keeps integration slugs that match the provider\'s connected apps', () => {
    const out = shapeProposals(
      {
        proposals: [
          { tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com', subject: 'Appointment follow-up', body: 'Loved meeting you' } },
        ],
      },
  ...
  it('drops integration slugs the provider has NOT connected', () => {
  ...
  it('keeps native and integration tools side by side', () => {
    const out = shapeProposals(
      {
        proposals: [
          { tool: 'log_call', args: { personId: 'p1', summary: 'appointment' } },
          { tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com' } },
          { tool: 'GOOGLECALENDAR_CREATE_EVENT', args: { summary: 'follow-up appointment' } },
        ],
      },
  ...
  it('produces a clean provider-voice summary without "via Gmail"', () => {
    const out = shapeProposals(
      { proposals: [{ tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com' } }] },
      { integrationTools: [GMAIL] },
    );
    expect(out[0].summary).not.toMatch(/via gmail/i);
    // Verb is the truth: "Email" tells the provider what's happening.
...
describe('providerVerbForToolkit / doneVerbForToolkit', () => {
  it('returns concise verbs for known toolkits', () => {
    expect(providerVerbForToolkit('gmail')).toBe('Email');
    expect(providerVerbForToolkit('googlecalendar')).toBe('Add to calendar');
  ...
  it('falls back gracefully for unknown toolkits', () => {
    expect(providerVerbForToolkit('weird-app')).toBe('Run connected action');
