...
import { extractMentions, mentionsProvider, isTeammate } from '@/lib/briefing/signal-sources/slack';
...
describe('slack signal source — pure helpers', () => {
  ...
  describe('mentionsProvider', () => {
    it('returns true when the provider id appears in the text', () => {
      expect(mentionsProvider('hey <@U0ME>, this one', 'U0ME')).toBe(true);
    });

    it('returns false when a different teammate is mentioned', () => {
      expect(mentionsProvider('<@U0JESS> can you check this?', 'U0ME')).toBe(false);
    });

    it('returns false when the provider id is null (unknown self)', () => {
      expect(mentionsProvider('<@U0ME> ping', null)).toBe(false);
    });

    it('returns false on null/empty text', () => {
      expect(mentionsProvider(null, 'U0ME')).toBe(false);
      expect(mentionsProvider('', 'U0ME')).toBe(false);
    });
  });
