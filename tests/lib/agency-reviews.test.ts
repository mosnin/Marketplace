...
interface AuthState {
  clerkId: string | null;
  dbUserId: string;
  agencyRole: 'agency_owner' | 'agency_admin' | 'provider_member' | null;
  agencyId: string;
...
vi.mock('@/lib/permissions', () => ({
  requireAgency: vi.fn(async () => {
    ...
    return {
      ...
      agency: { id: auth.agencyId, name: 'Test Agency' },
      dbUserId: auth.dbUserId,
    };
  }),
  getAgencyMemberContext: vi.fn(async () => {
    if (!auth.agencyRole) return null;
    return {
      membership: { id: 'm_1', role: auth.agencyRole, userId: auth.dbUserId },
      agency: { id: auth.agencyId, name: 'Test Agency' },
      dbUserId: auth.dbUserId,
...
const notifyAgencyMock = vi.fn(async () => undefined);
vi.mock('@/lib/agency-notify', () => ({ notifyAgency: notifyAgencyMock }));
...
describe('POST /api/deals/[id]/review-request', () => {
  ...
  it('400 when reason is missing', async () => {
    mockByTable.Deal = {
      single: { id: 'd_1', title: 'X', spaceId: 's_1', Space: { id: 's_1', slug: 'jane', agencyId: 'b_1' } },
    };
  ...
  it('409 when Space has no agency', async () => {
    mockByTable.Deal = {
      single: { id: 'd_1', title: 'X', spaceId: 's_1', Space: { id: 's_1', slug: 'jane', agencyId: null } },
    };
    const res = await invoke('d_1', { reason: 'r' });
    expect(res.status).toBe(409);
    const body = (await res.json()) as { error: string };
    expect(body.error).toMatch(/non-agency/i);
  });
  ...
  it('201 on happy path + fires notifyAgency with review_requested', async () => {
    mockByTable.Deal = {
      single: { id: 'd_1', title: 'X', spaceId: 's_1', Space: { id: 's_1', slug: 'jane', agencyId: 'b_1' } },
    };
    mockByTable.User = { single: { id: 'u_1' } };
    ...
    expect(notifyAgencyMock).toHaveBeenCalledTimes(1);
    expect((notifyAgencyMock.mock.calls as unknown[][])[0][0]).toMatchObject({ type: 'review_requested' });
  });
});

// ──────────────────────────────────────────────────────────────────────────
// PATCH /api/agency/reviews/[id]
// ──────────────────────────────────────────────────────────────────────────
describe('PATCH /api/agency/reviews/[id]', () => {
  async function invoke(id: string, body: unknown): Promise<Response> {
    const mod = await import('@/app/api/agency/reviews/[id]/route');
    return mod.PATCH(patchReq(`http://x/api/agency/reviews/${id}`, body) as never, {
      params: Promise.resolve({ id }),
    });
  }

  it('403 when caller is a provider_member', async () => {
    auth.agencyRole = 'provider_member';
    const res = await invoke('r_1', { status: 'approved' });
    expect(res.status).toBe(403);
  });

  it('404 when review does not belong to caller agency', async () => {
  ...
  it('409 when review already resolved', async () => {
    mockByTable.DealReviewRequest = {
      single: { id: 'r_1', agencyId: 'b_1', status: 'approved', resolvedAt: new Date().toISOString() },
    };
  ...
  it('400 when status value is invalid', async () => {
    mockByTable.DealReviewRequest = {
      single: { id: 'r_1', agencyId: 'b_1', status: 'open' },
    };
...
// ──────────────────────────────────────────────────────────────────────────
// POST /api/agency/reviews/[id]/comments
// ──────────────────────────────────────────────────────────────────────────
describe('POST /api/agency/reviews/[id]/comments', () => {
  async function invoke(id: string, body: unknown): Promise<Response> {
    const mod = await import('@/app/api/agency/reviews/[id]/comments/route');
    return mod.POST(jsonReq(`http://x/api/agency/reviews/${id}/comments`, body) as never, {
      params: Promise.resolve({ id }),
    });
  }
  ...
  it('400 when body is missing', async () => {
    mockByTable.DealReviewRequest = {
      single: { id: 'r_1', agencyId: 'b_1', requestingUserId: 'u_2' },
    };
  ...
  it('201 when caller IS the requesting agent (non-agency)', async () => {
    auth.agencyRole = null;
    mockByTable.DealReviewRequest = {
      single: { id: 'r_1', agencyId: 'b_1', requestingUserId: 'u_1', status: 'open' },
    };
    mockByTable.User = { single: { id: 'u_1', name: 'Jane', clerkId: 'clerk_1' } };
